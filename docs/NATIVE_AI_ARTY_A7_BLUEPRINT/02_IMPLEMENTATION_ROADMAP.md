...
## C.1 Memory / IO dependency chain (Blueprint V2 — `17_` §6)

**Authority:** [`17_MASTERPLAN_BLUEPRINT_V2_IO_AWARE.md`](17_MASTERPLAN_BLUEPRINT_V2_IO_AWARE.md).  
Live queue: `LOOP_STATE.json`. Status words below are dependency shape (2026-08-23).

### Phase A — IO correctness

```text
MIG measurement + silicon grid           CLOSED   (mig_metric_00, mig_board_r2)
DDR wavefront characterization           CLOSED   (ddr_wavefront_00 PASS_NARROW)
WF-GLOBAL-TOPK + integrated               CLOSED   (carried_risk_r1 CLOSED)
DESCRIPTOR-CONTRACT-00                   CLOSED   (104b frozen)
ddr_cue_soa_00                           BLOCKED  (transport FAIL — SOA not falsified)
ddr_cue_soa_00r_axi_liveness             OPEN     (P0 repair — AXI liveness)
```

### Phase B — Prove IO value

```text
ddr_cue_soa_bench_01                     QUEUED   (after 00R PASS; iron-law BENCH)
```

### Phase C — IO-aware graph

```text
graph_late_materialize_00                QUEUED   (score early, fetch expensive late)
```

### Phase D — LM memory physics (parallel)

```text
LM-06 WM bit-exact                       CLOSED   (lm06_wm_00)
lm06_wm_trace_00                         QUEUED   (u_a lifetime / M_peak — human re-open ladder)
lm06_wm_ladder                           BLOCKED  (ceilings 96/64/48/32; Pareto stop)
```

### Phase E — Global memory

```text
bram_owner_00                            BLOCKED  (phase FSM; SPEC §28 report)
full_integration                         BLOCKED
```

**Iron-law rule:** no gate may claim wall-clock speedup from byte reduction without Phase B measured table.

Cutting LM-06 BRAM before MRC traces remains forbidden. Ladder rungs are **reporting ceilings**, not blind cuts.

## C.2 Semantic / claim chain (Phases F–G)

```text
teacher-off framing (UART)           PASS_NARROW
        ↓
HS22-LM06-ACTIVE-00                  PLANNED  (Phase F — FPGA token from frozen LM-06)
        ↓
teacher-off SEMANTIC retrieval       OPEN     (Phase G — HS-02 silicon)
        ↓
...
## C.5 Blueprint V2 gate registry (Phases A–G + research)

Full doctrine: [`17_MASTERPLAN_BLUEPRINT_V2_IO_AWARE.md`](17_MASTERPLAN_BLUEPRINT_V2_IO_AWARE.md).  
Iron-law template: `results/A7-NATIVE-GRAPH/STATUS/GATE_IRON_LAW_TEMPLATE.md`.

| Gate ID | Phase | ONE UNKNOWN (summary) | LOOP_STATE |
|---------|-------|----------------------|------------|
| `ddr_cue_soa_00r_axi_liveness` | A | 832 B / 64 cand lawful delivery | OPEN |
| `ddr_cue_soa_bench_01` | B | SOA vs AOS bytes/query + memory_wait at same law | QUEUED |
| `graph_late_materialize_00` | C | Expensive payload after global Top-K | QUEUED |
| `lm06_wm_trace_00` | D | `M_peak` / `u_a` lifetime | QUEUED |
| `lm06_wm_ladder` | D | Pareto BRAM ceiling | BLOCKED |
| `bram_owner_00` | E | Phase ownership FSM | BLOCKED |
| `hs22_lm06_active_00` | F | FPGA token from LM-06 | not queued |
| `hs02_semantic` | G | Teacher-off retrieval | DONE_ENG LIMIT |
| `npu_v1_law_00` | research | Pairwise margin φ scope | not queued |
| `enc_geom_diag_00` | research | Encoder geometry | encoder lane only |
