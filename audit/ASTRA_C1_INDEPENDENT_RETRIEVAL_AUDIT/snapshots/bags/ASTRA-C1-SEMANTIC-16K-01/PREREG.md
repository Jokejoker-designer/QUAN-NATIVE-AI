# PREREG — ASTRA-C1-SEMANTIC-16K-01
...
bags CONTEXT-02 / PAGE-SKIP / STREAM-02 / DIR-FULL16 / N4096-STREAM-02 /
KEY-INTERSECT / AXI-BEAT. Does not write
`D:\FPGA\ASTRA_C1_INDEPENDENT_RETRIEVAL_AUDIT`.

C0 59-word lexicon **cannot** name 100 entities. This bag registers **NEW**
lexicon law `qse-v2-lex-semantic-16k-01`:
...
- xvlog `-i $bag -i $incq -i $incc` — bag **FIRST** so runtime table is the
  NEW law. This is **documented**, not a silent C0 59-word claim.
- C0 FILE `rtl/native_graph/query/qse_role_lexicon.svh` remains unedited
  (hash-gate MATCH `38189974…`).

## Primary unknown (Master V1.1 §7, N=16384 semantic)

At N=16384, law `qse-v2-intersect-context-02` (instantiate ctx keys +
stream-02 walker via context wrapper `8255a798…`, do not edit `14f75db7…` /
`124be808…` / `8255a798…` unless hash-gate MATCH instantiate) on a
**semantic** corpus with ≥100 distinct subject ids and ≥8 relation ids
(independent gold, **not** cloned “pump supplies chiller” × N): does recall
of labeled gold hold on required retrieve classes without
`SEARCH_INCOMPLETE` hiding misses?

If XSim cannot host 16384, FAIL honestly with `FIRST_DIVERGENCE MEM`. Do
**not** silently drop to N=256 clones.

## Claim this bag may close

N=16384 semantic-rung gate only (PLAN bag 7 first rung, not 800k close).
`RESULT=PASS_THIS_GATE_ONLY` if XSim FAIL=0 **and** marker
`ASTRA_C1_SEMANTIC_16K_XSIM_PASS` **and** retrieve classes with
`gold_n>=1` hit labeled gold **and** `SEARCH_INCOMPLETE` is not used to
hide a miss **and** gold was not rewritten after FAIL **and** subject mass
≥100 and rel mass ≥8.
`RESULT=FAIL` if any retrieve class with `gold_n>=1` misses declared gold
(incomp does not convert that to PASS), or N is dropped, or C0 is
silent-shadowed without a named lexicon law, or XSim cannot host 16384
(MEM). Does **not** close Master evidence-recall ≥95% or
...
context DUT `8255a798…`. Editing KEEP bags. nid-derived keys.
...
Write `GOLDEN.json` + `query_gold.svh` + `corpus.json` + named lexicon,
hash them to `GOLD_HASH_PRE_XVLOG.txt` **and** `SHA256.txt`, **then** xvlog.
...
```text
direct            → gold hits >0; independent labeled SRO (not {110,144,145} clone)
paraphrase
role_reversal
wrong_relation
wrong_context     → CONTEXT_SELECTIVE; emit ≠ direct
...
PASS_THIS_GATE_ONLY  hash-gate MATCH; gold hashed before first xvlog and
                     ...
                     ASTRA_C1_SEMANTIC_16K_XSIM_PASS; N=16384;
                     subject mass >=100; rel mass >=8; not 12-entity clone;
                     retrieve gold hits; SEARCH_INCOMPLETE absent on
                     gold_n>=1; poke_v=0; leftover A09 not compiled;
                     CAND_CAP=16<16384; NEW lexicon law documented;
...
FAIL                 retrieve gold miss hidden as SEARCH_INCOMPLETE;
