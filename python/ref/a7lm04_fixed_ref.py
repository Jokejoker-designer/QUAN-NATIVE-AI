"""A7-LM-04 fixed-point 2-layer 4-head GPT. law_id = lm05-signsgd-v1 (unchanged).

Same integer law as a7lm03_fixed_ref. Only V/C/d/H/d_ff change.
Persistent W is DDR-resident on the FPGA; this module is the host oracle only.
"""
...
assert WMEM_WORDS == 100352
assert OFF_POS == 16384
assert OFF_L0 == 18432
assert LAYER_WORDS == 32768
assert OFF_HEAD == 83968
...
def corpus_loss(model: TinyGPT100k, pairs: list[tuple[list[int], int]]) -> int:
...
def heldout_ce(model: TinyGPT100k, pairs: list[tuple[list[int], int]] | None = None) -> int:
    return corpus_loss(model, HELDOUT if pairs is None else pairs)


def heldout_margin(model: TinyGPT100k, pairs: list[tuple[list[int], int]] | None = None) -> int:
