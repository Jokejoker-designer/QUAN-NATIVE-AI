"""A7-LM-05 fixed-point 4-layer 4-head GPT. law_id = lm05-signsgd-v1 (unchanged).

Same integer law as a7lm04_fixed_ref. Geometry only: V=512 C=64 d=96 L=4 H=4 d_ff=192.
...
assert WMEM_WORDS == 399360
assert OFF_POS == 49152
assert OFF_L0 == 55296
assert LAYER_WORDS == 73728
assert OFF_HEAD == 350208
...
def corpus_loss(model: TinyGPT399k, pairs: list[tuple[list[int], int]]) -> int:
...
def heldout_ce(model: TinyGPT399k, pairs: list[tuple[list[int], int]] | None = None) -> int:
    return corpus_loss(model, HELDOUT if pairs is None else pairs)


def heldout_margin(model: TinyGPT399k, pairs: list[tuple[list[int], int]] | None = None) -> int:
