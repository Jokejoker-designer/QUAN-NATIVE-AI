...
$wrapSv    = Join-Path $bag "a7ng_astra_09_r7_uart_query_rew_wrap.sv"
$tbSv      = Join-Path $bag "tb_astra_09_r7_uart_query_rew.sv"
$plantSv   = Join-Path $bag "a7ng_astra_09_r7_axi_plant.sv"
$svhBag    = Join-Path $bag "a7ng_astra_09_r7_uart_query_rew.svh"
...
if ($liveA09 -ne $expectA09) { throw "A09R7_FROZEN_A09_HASH_CHANGED $liveA09" }
if ($liveR2  -ne $expectR2)  { throw "A09R7_FROZEN_R2_HASH_CHANGED $liveR2" }
if ($liveSgd -ne $expectSgd) { throw "A09R7_FROZEN_SGD_HASH_CHANGED $liveSgd" }
$wrapTxt  = Get-Content -LiteralPath $wrapSv -Raw
$tbTxt    = Get-Content -LiteralPath $tbSv -Raw
$plantTxt = Get-Content -LiteralPath $plantSv -Raw
$svhTxt   = Get-Content -LiteralPath $svhBag -Raw
$ovPat    = '(?i)\bforce\b|\brelease\b|\bdeposit\b|\$force|\$deposit'
if ($wrapTxt  -match $ovPat) { throw "A09R7_SIM_OVERRIDE_IN_WRAP" }
if ($tbTxt    -match $ovPat) { throw "A09R7_SIM_OVERRIDE_IN_TB" }
if ($plantTxt -match $ovPat) { throw "A09R7_SIM_OVERRIDE_IN_PLANT" }
if ($svhTxt   -match $ovPat) { throw "A09R7_SIM_OVERRIDE_IN_SVH" }
Write-Host "FORCE_PRESENT=NO"
if ($wrapTxt -notmatch 'a7ng_astra_09_r2_cand_ovf') {
  throw "A09R7_WRAP_MISSING_A09R2"
}
if ($wrapTxt -match 'a7ng_astra_09_r6_iso_pub') {
  throw "A09R7_ISO_PUB_DUT_IN_WRAP"
}
if ($tbTxt -match 'a7ng_astra_09_r6_iso_pub') {
  throw "A09R7_ISO_PUB_DUT_IN_TB"
}
if ($wrapTxt -match '(?m)^\s*a7ng_shared_rank_sgd_q8_sym_f2r2\s+\w+') {
  throw "A09R7_SIBLING_SGD_IN_WRAP"
}
if ($tbTxt -match '(?m)^\s*a7ng_shared_rank_sgd_q8_sym_f2r2\s+\w+') {
  throw "A09R7_SIBLING_SGD_IN_TB"
}
if ($plantTxt -match '(?m)^\s*a7ng_shared_rank_sgd_q8_sym_f2r2\s+\w+') {
  throw "A09R7_SIBLING_SGD_IN_PLANT"
}
...
$keep = @(
  ...
  (Join-Path $root "results\A7-NATIVE-GRAPH\ASTRA-09-R6-UART-ISO-PUBLIC-01\a7ng_astra_09_r6_uart_iso_public_wrap.sv"),
  (Join-Path $root "results\A7-NATIVE-GRAPH\ASTRA-09-R6-UART-ISO-PUBLIC-01\a7ng_astra_09_r6_iso_pub.sv")
)
$r2InFiles = $false
foreach ($f in $files) {
  $bn = [IO.Path]::GetFileName($f)
  if ($bn -eq "a7ng_astra_09_r2_cand_ovf.sv") { $r2InFiles = $true }
  if ($bn -match "astra_09_integ_path|astra_rtp_soc_top|astra09_pipe|astra_09_r3_uart|astra_09_r4_uart|astra_09_r5_uart|astra_09_r6") {
    throw "FORBIDDEN_DUT_IN_COMPILE_LIST $f"
  }
}
if (-not $r2InFiles) { throw "A09R7_A09R2_NOT_IN_COMPILE_LIST" }
function Rel([string]$p) {
  return $p.Substring($root.Length).TrimStart('\', '/').Replace('\', '/')
}
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "A09R7_FILE_MISSING $p" }
...
$pre += (ShaLine $stub)
$pre += "# KEEP_NOT_COMPILED (frozen leftover A09 + prior UART wraps; not this DUT)"
foreach ($f in $keep) { $pre += (ShaLine $f) }
$pre += "# PROVENANCE frozen A09/SGD/R2 (R2 compiled this bag; not edited)"
...
Write-Host "A09R7_SHA_FROZEN $shaPre PROGRAM=NO PRODUCTION_TOP=UNKNOWN FORCE_PRESENT=NO"
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "A09R7_XVLOG_NOT_FOUND" }
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "A09R7_XVLOG_FAIL"
}
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
if (-not (Test-Path -LiteralPath $glbl)) { throw "A09R7_GLBL_MISSING $glbl" }
& "$bin\xvlog.bat" $glbl
if ($LASTEXITCODE -ne 0) { throw "A09R7_GLBL_XVLOG_FAIL" }

$mmcmMode = "UNISIM_MMCME2_BASE"
& "$bin\xelab.bat" tb_astra_09_r7_uart_query_rew glbl -s a09r7qr -timescale 1ns/1ps -L unisims_ver --debug typical
if ($LASTEXITCODE -ne 0) {
  if (Test-Path (Join-Path $work "xelab.log")) {
    Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
    $r0 = Join-Path $bag "xelab_fail_r0.log"
    if (-not (Test-Path -LiteralPath $r0)) {
      Copy-Item (Join-Path $work "xelab.log") $r0 -Force
    }
  }
  Write-Host "UNISIM_XELAB_FAIL trying MMCM_STUB"
  $mmcmMode = "MMCM_STUB"
  & "$bin\xvlog.bat" --sv $stub
  if ($LASTEXITCODE -ne 0) { throw "A09R7_STUB_XVLOG_FAIL" }
  & "$bin\xelab.bat" tb_astra_09_r7_uart_query_rew glbl -s a09r7qr -timescale 1ns/1ps --debug typical
  if ($LASTEXITCODE -ne 0) {
    if (Test-Path (Join-Path $work "xelab.log")) {
      Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
    }
    throw "A09R7_XELAB_FAIL"
  }
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
...
Write-Host "MMCM_MODE $mmcmMode FORCE_PRESENT=NO FROZEN_A09R2=INSTANTIATED"

$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" a09r7qr -R --onerror quit -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_09_R7_UART_QUERY_REW_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "A09R7_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_09_R7_UART_QUERY_REW_RUN_OK PROGRAM=NO PRODUCTION_TOP=UNKNOWN FORCE_PRESENT=NO"
