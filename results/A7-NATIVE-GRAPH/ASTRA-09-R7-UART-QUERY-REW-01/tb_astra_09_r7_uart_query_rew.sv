// ASTRA-09-R7-UART-QUERY-REW-01. PROGRAM=NO. Bag-local TB.
// Instantiates a7ng_astra_09_r7_uart_query_rew_wrap (UART around frozen A09-R2).
// Drives only uart_txd_in / sw / btn. Hierarchical reads of w_o are observe-only.
`timescale 1ns / 1ps
`include "a7ng_astra_09_r7_uart_query_rew.svh"

module tb_astra_09_r7_uart_query_rew;
  ...
  localparam int TX_N = A7NG_A09R7_TX_N;
  ...
  localparam logic [3:0] CST_QRUN  = 4'd0;
  localparam logic [3:0] CST_HOLD  = 4'd2;
  ...
  a7ng_astra_09_r7_uart_query_rew_wrap dut (
    .CLK100MHZ(CLK100MHZ),
    .sw(sw),
    .btn(btn),
    .led(led),
  ...
  always @(posedge dut.clk) begin
    ...
    if (dut.result_v && !dut.sent)
      $display("HIER result_v ans=%0d p0=%0d p1=%0d st=%0d tbl=%0d npath=%0d ntrunc=%0d rov=%0d wov=%0d acc=%0d phi0=%0d w0=%0d txn=%0d gen=%0d ep=%0d t=%0t",
        dut.ans, dut.p0, dut.p1, dut.status, dut.tbl, dut.npath, dut.ntrunc,
        dut.r_ovf, dut.w_ovf, dut.pend_acc, dut.phi0, dut.inner_w0,
        dut.txn_id, dut.gen, dut.epoch, $time);
    if (dut.rew_v)
      $display("REW_PULSE rew=%0d txn=%0d gen=%0d ep=%0d acc=%0d t=%0t",
        $signed(dut.rew_i), dut.rew_txn, dut.rew_gen, dut.rew_epoch,
        dut.pend_acc, $time);
    if (dut.u_a09r2.sgd_upd)
      $display("INNER_UPD rew=%0d phi0=%0d t=%0t",
        $signed(dut.u_a09r2.rew_lat), dut.u_a09r2.phi[0], $time);
    if (dut.u_a09r2.sgd_done && dut.pend_cmt)
      $display("INNER_DONE w0=%0d hier_sgd_w0=%0d pub_w0=%0d nupd=%0d nbad=%0d nstale=%0d tbl=%0d t=%0t",
        dut.inner_w0, dut.u_a09r2.u_sgd.w_o[0], dut.w[0],
        dut.n_upd, dut.n_bad, dut.n_stale, dut.tbl, $time);
    if (dut.retire)
      $display("RETIRE t=%0t", $time);
  end
  ...
  task automatic wait_hold;
    integer g;
    begin
      g = 0;
      while (dut.tx_active || dut.tx_busy || (dut.cst != CST_HOLD)) begin
        @(posedge CLK100MHZ);
        g = g + 1;
        if (g > 2_000_000) begin
          $display("FAIL HOLD_TIMEOUT tx=%0b busy=%0b cst=%0d t=%0t",
            dut.tx_active, dut.tx_busy, dut.cst, $time);
          fail = fail + 1;
          if (first_div == "") first_div = "HOLD_TIMEOUT";
          return;
        end
      end
  ...
  task automatic wait_idle;
    ...
    begin
      g = 0;
      while (dut.busy || dut.tx_active || dut.tx_busy || (dut.cst != CST_QRUN)) begin
        @(posedge CLK100MHZ);
        g = g + 1;
        if (g > 2_000_000) begin
          $display("FAIL IDLE_TIMEOUT busy=%0b tx=%0b cst=%0d rv=%0b t=%0t",
            dut.busy, dut.tx_active, dut.cst, dut.result_v, $time);
          fail = fail + 1;
  ...
  task automatic decode_and_check(
    ...
    begin
      ...
      $display("DECODE %s magic=%02h tbl=%0d st=%0d ans=%0d p0=%0d p1=%0d npath=%0d ntrunc=%0d rov=%0d wov=%0d acc=%0d eol=%02h hier_tbl=%0d w0=%0d phi0=%0d txn=%0d",
        tag, fr[0], tbl_u, st_u, ans_u, p0_u, p1_u, npath_u, ntrunc_u,
        rov_u, wov_u, acc_u, fr[15], dut.tbl, dut.inner_w0, dut.phi0, dut.txn_id);
  ...
  task automatic uart_send_rew(
      input logic [7:0] rew8,
      input logic [7:0] txn,
      input logic [7:0] g,
      input logic [15:0] ep
  );
    begin
      $display("REW_CMD_SENT rew8=%02h txn=%0d gen=%0d ep=%0d t=%0t",
        rew8, txn, g, ep, $time);
      uart_send_byte(CMD_REW);
      uart_send_byte(rew8);
      uart_send_byte(txn);
      uart_send_byte(g);
      uart_send_byte(ep[7:0]);
      uart_send_byte(ep[15:8]);
      uart_send_byte(EOL);
    end
  endtask

  task automatic uart_send_ret;
    begin
      $display("RET_CMD_SENT t=%0t", $time);
      uart_send_byte(CMD_RET);
      uart_send_byte(EOL);
    end
  endtask
  ...
  initial begin
    ...
    btn = 4'd0;
    uart_txd_in = 1'b1;
    $display("TB ASTRA-09-R7-UART-QUERY-REW DUT=a7ng_astra_09_r7_uart_query_rew_wrap INNER=a7ng_astra_09_r2_cand_ovf.u_sgd PROGRAM=NO");
    ...
    if (dut.rst_n !== 1'b1) begin
      $display("ASTRA_09_R7_UART_QUERY_REW_FAIL n=%0d first=%s", fail, first_div);
      $finish;
    end
    ...
    chk("SMOKE_UART_ACC", dut.pend_acc === 1'b1);
    chk("SMOKE_UART_PHI0", dut.phi0 === 8'sd50);
    chk("SMOKE_PRE_W0", (dut.inner_w0 === 16'sd0)
        && (dut.u_a09r2.u_sgd.w_o[0] === 16'sd0)
        && (dut.w[0] === 16'sd0));
    chk("SMOKE_UART_TBL0", dut.tbl === 1'b0);
    wait_hold();

    uart_send_rew(8'hFD, dut.txn_id, dut.gen, dut.epoch);
    begin : obs_blk
      logic [7:0] ofr [0:TX_N-1];
      logic signed [15:0] w0_u;
      logic signed [7:0] phi0_u;
      logic [7:0] nupd_u, nbad_u, nstale_u, ndup_u;
      uart_get_frame(ofr);
      $display("FRAME REW_OBS %02h %02h %02h %02h %02h %02h %02h %02h %02h %02h %02h %02h %02h %02h %02h %02h",
      ...
      nstale_u = ofr[7];
      ndup_u   = ofr[13];
      $display("DECODE REW_OBS magic=%02h w0=%0d phi0=%0d nupd=%0d nbad=%0d nstale=%0d ndup=%0d txn=%0d gen=%0d tag=%02h hier_w0=%0d sgd_w0=%0d pub_w0=%0d tbl=%0d cmt=%0d",
        ofr[0], w0_u, phi0_u, nupd_u, nbad_u, nstale_u, ndup_u,
        ofr[8], ofr[9], ofr[14], dut.inner_w0, dut.u_a09r2.u_sgd.w_o[0],
        dut.w[0], dut.tbl, dut.pend_cmt);
      chk("REW_M3_MAGIC", ofr[0] === MAGIC);
      chk("REW_M3_EOL", ofr[15] === EOL);
      chk("REW_M3_TAG", ofr[14] === OBS_TAG);
      chk("REW_M3_W0", w0_u === -16'sd5);
      chk("REW_M3_PHI0", phi0_u === 8'sd50);
      chk("REW_M3_NUPD", nupd_u === 8'd1);
      chk("REW_M3_NBAD0", nbad_u === 8'd0);
      chk("REW_M3_NSTALE0", nstale_u === 8'd0);
      chk("REW_M3_NDUP0", ndup_u === 8'd0);
      chk("REW_M3_TBL0", dut.tbl === 1'b0);
      chk("REW_M3_INNER_W0", dut.inner_w0 === -16'sd5);
      chk("REW_M3_HIER_SGD_W0", dut.u_a09r2.u_sgd.w_o[0] === -16'sd5);
      chk("REW_M3_PUB_W0", dut.w[0] === -16'sd5);
      chk("REW_M3_CMT", dut.pend_cmt === 1'b1);
    end
    wait_hold();
    uart_send_ret();
    wait_idle();
    ...
    if (fail == 0)
      $display("ASTRA_09_R7_UART_QUERY_REW_PASS");
    else
      $display("ASTRA_09_R7_UART_QUERY_REW_FAIL n=%0d first=%s", fail, first_div);
    $finish;
  end
