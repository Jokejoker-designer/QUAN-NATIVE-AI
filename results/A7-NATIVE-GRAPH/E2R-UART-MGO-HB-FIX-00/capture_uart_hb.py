#!/usr/bin/env python3
"""Capture COM12 for E2R-UART-MGO-HB-FIX-00 — F1w MGO after RPATH_IDLE, no BOOT flood."""
import argparse
import re
import sys
import time
...
def main():
    ...
    while time.time() - t0 < args.seconds and not stop:
        ...
        if all(any(x.startswith(k) for x in lines) for k in f1v_keys):
            # Bound capture: stop shortly after MGO (+ a few trailing lines)
            ...
            if (time.time() - t0) > 90.0:
                break
        # Early abort on BOOT flood after RPATH_IDLE
