`timescale 1ns / 1ps
// ASTRA-C3-HELD-OUT-800K-01. PROGRAM=NO. Bag-local TB.
// 5-seed 4-arm on 800k cartesian dir + C3 fact_pack. Not planted 96-slot. Not MIG.
`include "a7ng_astra_c3_held_out.svh"
module tb_astra_c3_held_out_800k;
  ...
  task automatic dump(input string tag);
    $display("%s st=%0d npath=%0d ans=%0d p0=%0d gold=%0d subj=%0d obj=%0d ctx=%0d nupd=%0d tbl=%0d",
      tag, st, npath, ans, p0, c3k_gold_nid(qsubj, 4), qsubj, qobj, qctx, nupd, tbl);
  endtask
  ...
  initial begin
    int seed, gold, acc_a, acc_b, acc_c, acc_d, gain_pp, pair_pos, host_bad, dj_fail;
    ...
    $display("C3_800K N=800000 FACT_BASE=0E000000 TO_CYC=65535 TR={13,15,16} HO={19,20} PROGRAM=NO BOARD_PASS=REJECT");
    ...
    for (seed=0; seed<C3_N_SEED; seed=seed+1) begin
      ...
      for (q=0;q<8;q=q+1) begin
        q_run(HO_S[q]);
        gold = c3k_gold_nid(HO_S[q], 4);
        dump($sformatf("B_S%0d_Q%0d", seed, q));
        if ((st===ST_ANSWER) && (p0===gold[19:0]) && (gold!=0)) acc_b = acc_b + 1;
      ...
      for (q=0;q<8;q=q+1) begin
        ...
        if ((st===ST_ANSWER) && (p0===gold[19:0]) && (gold!=0)) acc_a = acc_a + 1;
        if (nhwin!==16'd0 || nhaddr!==16'd0 || tbl) host_bad = host_bad + 1;
        retire_q();
      end
      ...
      for (q=0;q<8;q=q+1) begin
        ...
        if ((st===ST_ANSWER) && (p0===gold[19:0]) && (gold!=0)) acc_c = acc_c + 1;
        if (nhwin!==16'd0 || nhaddr!==16'd0 || tbl) host_bad = host_bad + 1;
        retire_q();
      end
      ...
      for (q=0;q<8;q=q+1) begin
        ...
        if ((st===ST_ANSWER) && (p0===gold[19:0]) && (gold!=0)) acc_d = acc_d + 1;
        if (nhwin!==16'd0 || nhaddr!==16'd0 || tbl) host_bad = host_bad + 1;
        retire_q();
      end
    ...
    $display("C3_SUM A=%0d/%0d B=%0d C=%0d D=%0d gain_pp=%0d pair_pos=%0d/%0d host_bad=%0d",
    ...
    if (fail==0) $display("ASTRA_C3_HELD_OUT_800K_XSIM_PASS");
    else $display("ASTRA_C3_HELD_OUT_800K_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C3_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO quality=800K_CARTESIAN_DIR_C3_FACTPACK_NOT_MIG");
    $finish;
  end
endmodule
