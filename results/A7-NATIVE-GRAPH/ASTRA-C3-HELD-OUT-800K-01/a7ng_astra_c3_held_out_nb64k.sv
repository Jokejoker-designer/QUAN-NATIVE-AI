// a7ng_astra_c3_held_out_nb64k.sv — ASTRA-C3-HELD-OUT-800K-01. PROGRAM=NO.
// Bag-local copy of C3 wrap. KEEP a7ng_astra_c3_held_out.sv is not edited.
// Only delta vs KEEP: N_BUCKETS=65536 so dir keys match gen_800k (16-bit).
// Instantiates C1 synonym KEEP + frozen a7ng_shared_rank_sgd_q8_sym_f2r2.
`timescale 1ns / 1ps
`include "a7ng_astra_c3_held_out.svh"

module a7ng_astra_c3_held_out_nb64k #(
  parameter int unsigned ID_W       = 20,
  parameter int unsigned CAND_CAP   = 16,
  parameter int unsigned TO_CYC     = A7NG_C3_TO_CYC,
  parameter int unsigned MAX_PATH   = 16,
  ...
  logic [19:0] pp0 [0:15], pp1 [0:15], pans [0:15];
  logic [7:0]  pc1 [0:15], pc2 [0:15], px1 [0:15], px2 [0:15];
  logic        pt1 [0:15], pt2 [0:15], pp1v [0:15], pp2v [0:15], ph2 [0:15];
  logic signed [15:0] pv [0:15];
  logic signed [7:0]  phis [0:15][0:31];
  ...
  assign m_axi_arid = fetch_mode ? A7NG_C3_FACT_RID : sp_arid;
  ...
  assign n_host_winner_o = h6;
  assign n_host_addr_o = h7;
  ...
  always_comb begin
    ...
    if (st==S_SC || st==S_SW) begin
      for (k = 0; k < 32; k = k + 1)
        phi[k] = fphi(k, pc1[pi[3:0]], pc2[pi[3:0]], px1[pi[3:0]], px2[pi[3:0]],
                      r_ctx, pt1[pi[3:0]], pt2[pi[3:0]],
                      pp1v[pi[3:0]], pp2v[pi[3:0]], ph2[pi[3:0]]);
    end else begin
  ...
  a7ng_query_axi_sparse_intersect_synonym #(
    .N_TABLES(4), .N_BUCKETS(65536), .CAND_CAP(CAND_CAP),
    .ID_W(20), .INDEX_BASE(INDEX_BASE)
  ) u_sp (
  ...
  a7ng_shared_rank_sgd_q8_sym_f2r2 u_sgd (
    .clk(clk), .rst_n(rst_n), .freeze_i(freeze_q),
    ...
    if (!rst_n) begin
      ...
      unique case (st)
        ...
        S_WALK: begin
          ...
          if (w_done) begin
            // 800k dir[48] marks overflow-list present (occ>4), not a fail.
            // KEEP C3 maps q_overflow to INCOMP; this bag wrap must not.
            r_ovf <= 1'b0; qse_retire <= 1'b1; fi <= '0;
        ...
        S_R: begin
          tocnt <= tocnt + 16'd1;
          if (f_rvalid) begin
            if ((m_axi_rid==A7NG_C3_FACT_RID) && (m_axi_rresp==A7NG_C3_RRESP_OK)
                && m_axi_rlast && (m_axi_rdata[75:72]==A7NG_C3_VER) && m_axi_rdata[70]
        ...
        S_EI: begin
          if (r_ovf) begin r_st <= A7NG_C3_ST_INCOMP; st <= S_HOLD; end
            ...
            if (np == 0) begin r_st <= A7NG_C3_ST_UNKNOWN; st <= S_HOLD; end
        ...
        S_ED: begin
          if (fv[ei[3:0]]
            ...
            if (np < MAX_PATH[4:0]) begin
              pp0[np[3:0]] <= fe[ei[3:0]];
              pp1[np[3:0]] <= 20'd0;
              pans[np[3:0]] <= fo[ei[3:0]];
              pc1[np[3:0]] <= fcf[ei[3:0]];
              pc2[np[3:0]] <= fcf[ei[3:0]];
              px1[np[3:0]] <= fcx[ei[3:0]];
              px2[np[3:0]] <= fcx[ei[3:0]];
              pt1[np[3:0]] <= ft[ei[3:0]];
              pt2[np[3:0]] <= 1'b0;
              pp1v[np[3:0]] <= fpv[ei[3:0]];
              pp2v[np[3:0]] <= fpv[ei[3:0]];
              ph2[np[3:0]] <= 1'b0;
              np <= np + 1'b1;
        ...
        S_EJ: begin
          if (ej >= nf) begin ei <= ei + 1'b1; st <= S_EI; end
            ...
            if (fv[ei[3:0]] && fv[ej[3:0]] && ft[ei[3:0]] && ft[ej[3:0]]
              ...
              if (np < MAX_PATH[4:0]) begin
                pp0[np[3:0]] <= fe[ei[3:0]];
                pp1[np[3:0]] <= fe[ej[3:0]];
                pans[np[3:0]] <= fo[ej[3:0]];
                pc1[np[3:0]] <= fcf[ei[3:0]];
                pc2[np[3:0]] <= fcf[ej[3:0]];
                px1[np[3:0]] <= fcx[ei[3:0]];
                px2[np[3:0]] <= fcx[ej[3:0]];
                pt1[np[3:0]] <= ft[ei[3:0]];
                pt2[np[3:0]] <= ft[ej[3:0]];
                pp1v[np[3:0]] <= fpv[ei[3:0]];
                pp2v[np[3:0]] <= fpv[ej[3:0]];
                ph2[np[3:0]] <= 1'b1;
                np <= np + 1'b1;
              end else path_ovf <= 1'b1;
            end
        ...
        S_SW: if (sgd_done) begin
          pv[pi[3:0]] <= sgd_v;
          for (kf = 0; kf < 32; kf = kf + 1) phis[pi[3:0]][kf] <= phi[kf];
