`timescale 1ns / 1ps
// ASTRA-09-R2-CAND-OVF-01. PROGRAM=NO. Bag-local TB.
// Instantiates new named A09 revision (not frozen a7ng_astra_09_integ_path) + frozen SGD ISO.
// Overflow plant > CAND_CAP; DUT itself INCOMP ans=0. Hierarchical peek must not show ans=4.
module tb_astra_09_r2_cand_ovf;
  ...
  initial begin
    ...
    chk("OVF_MAX_PATH", (st===ST_INCOMP) && (ans===20'd0) && (p0===20'd0)
        && !pend_acc && !tbl && (npath>MAX_PATH[4:0]) && !r_ovf);
    ...
    if (fail==0) $display("ASTRA_09_R2_CAND_OVF_XSIM_PASS");
    else $display("ASTRA_09_R2_CAND_OVF_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $finish;
  end
endmodule
