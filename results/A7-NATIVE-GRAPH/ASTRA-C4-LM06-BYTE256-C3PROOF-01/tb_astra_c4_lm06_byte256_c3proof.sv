`timescale 1ns / 1ps
// ASTRA-C4-LM06-BYTE256-C3PROOF-01. PROGRAM=NO. Bag-local TB.
// Plants graph facts only. Does not drive PROOF ASCII into the DUT.
...
module tb_astra_c4_lm06_byte256_c3proof;
  ...
  a7ng_astra_c4_lm06_byte256_c3proof u_dut (
    ...
    .c3_status_o(c3st), .c3_ans_o(c3ans), .c3_p0_o(c3p0), .c3_result_v_o(c3res),
  ...
  task automatic load_normal;
    begin
      ld(A7NG_C4X_LD_COPY, 16'sd50);
      ld(A7NG_C4X_LD_SAFE,  16'sd80);
      ld(A7NG_C4X_LD_EOS,   16'sd200);
    end
  endtask
    ...
    begin
      ...
      while (!done && guard<80000) begin
        ...
        if (tv) begin
          ...
          ntok_o=ntok_o+1;
          if (eos || (gtok==A7NG_C4P_EOS)) eos_seen=1;
          $display("TOK n=%0d v=%0d eos=%0d head10=%0d n_host=%0d ans=%0d st=%0d",
            ntok_o, gtok, eos, head10, nhost, c3ans, c3st);
        end
      end
      chk("NO_TIMEOUT", done && guard<80000);
      if (nhost!==16'd0 || nhwin!==16'd0 || tbl) host_bad=host_bad+1;
      t0 = a7ng_c4p_ch(dst[7:0], 0);
      t1 = a7ng_c4p_ch(dst[7:0], 1);
      t2 = a7ng_c4p_ch(dst[7:0], 2);
      if (plant) begin
        acc_n=acc_n+1;
        if ((c3st===A7NG_C3_ST_ANSWER) && (c3ans===dst[19:0])) ans_ok=ans_ok+1;
        if ((c3st===A7NG_C3_ST_ANSWER) && (c3ans===dst[19:0])
            && (seq[0]===t0) && (seq[1]===t1) && (seq[2]===t2) && eos_seen)
          acc_ok=acc_ok+1;
        $display("HOLD dst=%0d ans=%0d st=%0d t0=%0d t1=%0d t2=%0d ntok=%0d eos=%0d",
          dst, c3ans, c3st, seq[0], seq[1], seq[2], ntok_o, eos_seen);
      end else begin
        un_n=un_n+1;
  ...
  initial begin
    ...
    $display("C4_C3PROOF_HELDOUT VOCAB_VER=3 N_HOLD=20 TRAIN_DST=20..39 HOLD_DST=40..59 PROGRAM=NO BOARD_PASS=REJECT LM06_BYTE256=NOT_FROZEN");
    $display("CLASS_not_compose_renderer HIT dut=a7ng_astra_c4_lm06_byte256_c3proof");
    $display("CLASS_c3_reasoner_instantiated HIT wrap=a7ng_astra_c3_held_out");
    $display("CLASS_fpga_dict_from_c3_ans HIT vocab_ver=3");
    chk("VOCAB_VER", vver===A7NG_C4P_VOCAB_VER);
    ...
    ld(A7NG_C4X_LD_COPY, 16'sd0);
    run_q(HOLD_DST[0], 1'b1, ntok, eoss);
    chk("ZERO_SAFE", eoss && (seq[0]==0 || ntok==1));
    $display("CLASS_w_zero_differs HIT tok0=%0d", seq[0]);

    load_normal();
    run_q(40, 1'b1, ntok, eoss); t0a=seq[0];
    run_q(50, 1'b1, ntok, eoss); t0b=seq[0];
    chk("EVID_REPLACED", t0a!==t0b);
    $display("CLASS_evid_replaced_changes HIT t0a=%0d t0b=%0d", t0a, t0b);
    $display("CLASS_host_next_token_zero HIT");
    $display("CLASS_eos_or_max HIT saw_eos=%0d", saw_eos);
    $display("CLASS_path_c3_dict_ctxcopy_eos HIT n_hold=20 acc=%0d", acc_ok);
    chk("HOST0", host_bad==0);
    chk("EOS40", saw_eos>=40);

    if (fail==0) begin
      $display("ASTRA_C4_LM06_BYTE256_C3PROOF_XSIM_PASS");
      $display("C4_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO LM06_BYTE256=NOT_FROZEN quality=C3_DEST_FPGA_DICT_CTXCOPY_NOT_802K");
    end else $display("ASTRA_C4_LM06_BYTE256_C3PROOF_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $finish;
  end
endmodule
