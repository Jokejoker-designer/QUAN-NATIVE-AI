# ASTRA-02 overflow-page XSim. PROGRAM=NO. COM12=UNTOUCHED.
...
set tb   [file join $bag tb_astra02_overflow.sv]
...
if {[catch {exec $xvlog_bin -sv $pkg $qse $vg $mem $walk $dut $tb -i $incq -i $incc -i $bag > $xvlog_log 2>@1}]} {
  puts [read [open $xvlog_log r]]
  puts FIRST_DIVERGENCE
  puts ASTRA02_XVLOG_FAIL
  exit 2
}
set xelab_log [file join $bag xelab.log]
if {[catch {exec $xelab_bin tb_astra02_overflow -s astra02 -timescale 1ns/1ps > $xelab_log 2>@1}]} {
  puts [read [open $xelab_log r]]
  puts FIRST_DIVERGENCE
  puts ASTRA02_XELAB_FAIL
  exit 3
}
set xsim_log [file join $bag xsim.log]
catch {exec $xsim_bin astra02 -R -log $xsim_log}
set body [read [open $xsim_log r]]
puts $body
if {[string match *FIRST_DIVERGENCE* $body]} {
  puts ASTRA02_FIRST_DIVERGENCE
  exit 6
}
if {![string match *ASTRA02_OVERFLOW_XSIM_PASS* $body]} {
  puts ASTRA02_NOT_PASS
  exit 5
}
puts ASTRA02_XSIM_OK
exit 0
