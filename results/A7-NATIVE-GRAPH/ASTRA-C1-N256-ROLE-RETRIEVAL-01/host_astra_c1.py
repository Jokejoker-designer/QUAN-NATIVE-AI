#!/usr/bin/env python3
"""ASTRA-C1 N=256 independent host gold for frozen qse-v2-role-00 + sparse dir.
...
def metrics_of(emit: list[int], relevant: list[int]) -> dict:
    rset = set(relevant)
    eset = set(emit)
    hit = len(rset & eset)
    prec = (hit / len(emit)) if emit else (1.0 if not relevant else 0.0)
    rec = (hit / len(relevant)) if relevant else (1.0 if not emit else 0.0)
    missed = [i for i in relevant if i not in eset]
...
def write_svh(path: Path, queries: list[dict], writes: dict[int, int]) -> None:
    n = len(queries)
    nwr = len(writes)
    wr_i = sorted(writes)
    lines = [
        "// generated by host_astra_c1.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
...
def main() -> int:
    ...
    psc = [d for d in docs if d["evidence"] == 1 and d["subj_id"] == 10 and d["rel_id"] == 1 and d["obj_id"] == 1]
    ...
    queries_spec = [
        ("direct", "pump supplies chiller", lambda d: d["subj_id"] == 10 and d["rel_id"] == 1 and d["obj_id"] == 1),
        ("paraphrase", "pump supply chiller", lambda d: d["subj_id"] == 10 and d["rel_id"] == 1 and d["obj_id"] == 1),
        ...
        ("wrong_context", "pump supplies chiller water", lambda d: d["subj_id"] == 10 and d["rel_id"] == 1 and d["obj_id"] == 1 and d["ctx_id"] == 1),
        ("distractor", "supply duct", lambda d: False),
    ...
    # Independent gold FIRST (labels only; no router).
    gold_map = {}
    ...
    for name, text, pred in queries_spec:
        ...
        q.update(rt)
        met = metrics_of(q["emit"], q["relevant"])
        missed = met["missed"]
        budget = (q["n_trunc"] > 0) or (q["overflow_flag"] and missed)
        if missed and not budget and name not in ("unrelated",):
            # relevant beyond walked lists with no trunc/ovf is a host-index bug
            if name in ("overflow_page", "high_id_sentinel", "direct", "paraphrase", "role_reversal", "wrong_relation"):
                fails.append(f"false miss {name} missed={missed} emit={q['emit']}")
        incomp = 1 if (missed and (q["n_trunc"] > 0 or q["overflow_flag"])) else 0
        q["precision"] = met["precision"]
        q["recall"] = met["recall"]
        q["hit"] = met["hit"]
        q["search_incomplete"] = incomp
        q["reduction"] = 1.0 - (q["n_emit"] / N)
    ...
    psc_nids = set(gold_map["direct"])
    chk(psc_nids.isdisjoint(set(qd["distractor"]["emit"])), "distractor pulled PSC")
    chk(qd["distractor"]["subj_id"] == 7 and qd["distractor"]["rel_id"] == 0, "distractor parse")
    chk(qd["distractor"]["ctx_id"] == 1, "distractor ctx supply")
    chk(qd["direct"]["n_emit"] <= CAND_CAP, "direct cap")

    if fails:
        print("ASTRA_C1_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        return 2
    ...
    write_svh(BAG / "query_gold.svh", queries, writes)
    ...
    (BAG / "GOLD_HASH_PRE_XVLOG.txt").write_text(
        "\n".join(
            [
                "# independent gold hashed BEFORE xvlog",
                f"{gsha}  GOLDEN.json",
    ...
    for q in queries:
        print(
            ...
            f"P={q['precision']:.3f} R={q['recall']:.3f} occ={q['occupancies']} "
            f"ovf={q['overflow_flag']} trunc={q['n_trunc']} incomp={q['search_incomplete']}"
        )
    return 0
