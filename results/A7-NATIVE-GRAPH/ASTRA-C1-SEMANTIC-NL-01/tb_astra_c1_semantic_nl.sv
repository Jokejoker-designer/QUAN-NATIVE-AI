// tb_astra_c1_semantic_nl.sv — C1 SEMANTIC-NL. PROGRAM=NO.
...
// ONE UNKNOWN: NL query keys after QSE MUST differ from fill-template
// "boiler feeds header" (k0=3332 k1=3588), yet labeled fill-meaning gold
// {120,121,122} still HIT. HELDOUT paraphrase (same keys) is NOT this unknown.
// Marker ASTRA_C1_SEMANTIC_NL_XSIM_PASS only if keys_match=0 AND gold HIT
// AND fill-template control hits AND FAIL=0. If frozen extract cannot,
// print NOT_SELECTIVE_NL and do not emit the PASS marker (do not fake keys).
// Do not drop N from 16384. Headline precision_all=TP/emit_n.
// MEM_DEPTH TB-only; FIRST_DIVERGENCE MEM if directory does not fit.
`timescale 1ns / 1ps

module tb_astra_c1_semantic_nl;
  import a7ng_pkg::*;
  ...
  integer incomp_retrieve, keys_match, nl_hit, nl_tp;
  integer arlen_bad;
  ...
  initial begin
    ...
    if (G_N != 16384)
      diverge("N_DROP", "G_N!=16384 do_not_silently_drop_to_N256");
    if (CAND_CAP >= G_N)
    ...
    if (MEM_DEPTH < (6144 + 4 * G_N_BUCKETS))
      diverge("MEM", "MEM_DEPTH too small for 4*65536 directory");
    if (G_N_SUBJECTS < 100)
      diverge("SEMANTIC_MASS", "subj<100");
    if (G_N_RELS < 8)
      diverge("SEMANTIC_MASS", "rel<8");
    if (G_NQ != 3)
      diverge("NQ", "G_NQ!=3 (fill+nl_synonym+unrelated)");
    if (G_FILL_Q != 0)
      diverge("FILL_IDX", "G_FILL_Q!=0");
    if (G_NL_Q != 1)
      diverge("NL_IDX", "G_NL_Q!=1");
    if (G_UNRELATED_Q != 2)
      diverge("UNRELATED_IDX", "G_UNRELATED_Q!=2");
    ...
    $display("C1_SEMANTIC_NL_N=%0d N_BUCKETS=%0d CAND_CAP=%0d INDEX_HEAD=%0d LAW=qse-v2-intersect-context-02 LEX=qse-v2-lex-semantic-16k-01 POKE_V=0 PAGE_BUFFER_BEATS=1 RARE_LIST_FIRST=1 CAND_CAP_AFTER_EMIT=1 REDUCTION_X1000=NOT_EMITTED CAND_CAP_FINAL=NOT_FROZEN DDR_QUERY_BOUND_FINAL=NOT_FROZEN MEM_DEPTH=%0d N_SUBJECTS=%0d N_RELS=%0d FILL_GRID=120,121,122 FILL_K0=%0d FILL_K1=%0d NL_TEXT=what_does_the_boiler_supply_to_the_header",
      G_N, G_N_BUCKETS, CAND_CAP, G_INDEX_HEAD, MEM_DEPTH, G_N_SUBJECTS, G_N_RELS, G_FILL_K0, G_FILL_K1);
    ...
    for (qi = 0; qi < G_NQ; qi = qi + 1) begin
      ...
      if (qi == G_FILL_Q) begin
        ...
        if (k0_cx !== G_FILL_K0 || k1_cx !== G_FILL_K1)
          diverge("KEY_MISMATCH", $sformatf("fill keys %0d %0d want 3332 3588", k0_cx, k1_cx));
      end
      if (qi == G_NL_Q) begin
        keys_match = (k0_cx === G_K0[G_FILL_Q] && k1_cx === G_K1[G_FILL_Q]);
        nl_tp = tp;
        nl_hit = (tp > 0);
        $display("KEYS_VS_FILL class=nl_synonym k0=%0d k1=%0d vs_fill_template k0=%0d k1=%0d keys_match=%0d law=qse-v2-intersect-context-02",
          k0_cx, k1_cx, G_K0[G_FILL_Q], G_K1[G_FILL_Q], keys_match);
        if (keys_match) begin
          $display("FAIL NL_KEYS_MATCH_FILL HELDOUT_PARAPHRASE_NOT_THIS_UNKNOWN k0=%0d k1=%0d", k0_cx, k1_cx);
          fail = fail + 1;
        end
        if (nl_hit) begin
          $display("NL_GOLD_HIT tp=%0d emit_n=%0d gold_n=%0d keys_match=%0d", nl_tp, n_got, nrel, keys_match);
        end else begin
          $display("NL_GOLD_MISS tp=%0d emit_n=%0d gold_n=%0d keys_match=%0d", nl_tp, n_got, nrel, keys_match);
        end
      end
    ...
    $display("NL_SUMMARY keys_match=%0d nl_hit=%0d nl_tp=%0d fill_tp=%0d fail=%0d",
      keys_match, nl_hit, nl_tp, fill_tp, fail);
    if ((fail == 0) && unrelated_empty && (fill_tp > 0) && fill_ids_ok && (keys_match == 0) && nl_hit && (incomp_retrieve == 0) && (G_N == 16384)) begin
      $display("ASTRA_C1_SEMANTIC_NL_XSIM_PASS");
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,PAGE_SKIP_MIX,N_65536,MASTER_95,ACCEPT_BOARD");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end else begin
      if ((keys_match == 0) && (nl_hit == 0) && (fill_tp > 0) && fill_ids_ok && (G_N == 16384))
        $display("NOT_SELECTIVE_NL keys_match=0 gold_miss=1 frozen_extract_cannot_bind_non_alias_synonym");
      $display("ASTRA_C1_SEMANTIC_NL_XSIM_NO_MARKER fail=%0d unrelated_empty=%0d fill_tp=%0d fill_ids_ok=%0d keys_match=%0d nl_hit=%0d incomp_retrieve=%0d G_N=%0d",
        fail, unrelated_empty, fill_tp, fill_ids_ok, keys_match, nl_hit, incomp_retrieve, G_N);
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,PAGE_SKIP_MIX,N_65536,MASTER_95,ACCEPT_BOARD");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end
