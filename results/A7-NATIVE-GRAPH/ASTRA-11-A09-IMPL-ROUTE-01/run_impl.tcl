# ASTRA-11-A09-IMPL-ROUTE-01. PROGRAM=NO. BIT=NO. write_bitstream forbidden.
# Instantiates a7ng_astra_09_integ_path as u_a09. No copy-paste graph.
# synth (in-context) + opt + place + route. Clock-network included. No bitstream.
set bag  [file normalize [file dirname [info script]]]
set root [file normalize [file join $bag ../../..]]
set env(XILINXD_LICENSE_FILE) {D:\Xilinx\licenses\vivado_basic.lic}
set part xc7a100tcsg324-1
set top  a7ng_astra_11_a09_impl_wrap
set first_div ""
...
set files [list \
  ...
  [file join $bag a7ng_astra_11_a09_impl_wrap.sv] \
]
...
proc astra_forbid_bit {args} {
  ...
  puts ASTRA_11_A09_IMPL_ROUTE_ABORTED
  exit 1
}
# write_bitstream is never invoked. PROGRAM=NO. BIT=NOT_BUILT.
...
proc astra_copy_fail_r0 {} {
  ...
  puts "ASTRA_11_A09_FAIL_R0_PRESERVED $dst"
}
...
if {[catch {synth_design -top $top -part $part} err]} {
  ...
  puts ASTRA_11_A09_IMPL_ROUTE_SYNTH_FAIL
  exit 1
}
...
astra_util_extract [file join $bag util_synth.rpt] [file join $bag UTIL_EXTRACT_SYNTH.txt] Synthesized
astra_timing_extract [file join $bag timing_synth.rpt] [file join $bag TIMING_EXTRACT_SYNTH.txt] Synthesized
puts ASTRA_11_A09_IMPL_ROUTE_SYNTH_DONE
...
if {[catch {opt_design} err]} {
  astra_fail OPT $err
  puts ASTRA_11_A09_IMPL_ROUTE_OPT_FAIL
  exit 1
}
puts ASTRA_11_A09_IMPL_ROUTE_OPT_DONE
...
if {[catch {place_design} err]} {
  ...
  puts ASTRA_11_A09_IMPL_ROUTE_PLACE_FAIL
  exit 1
}
if {[catch {phys_opt_design} err]} {
  puts "PHYS_OPT_WARN $err"
}
puts ASTRA_11_A09_IMPL_ROUTE_PLACE_DONE
...
if {[catch {route_design} err]} {
  ...
  puts ASTRA_11_A09_IMPL_ROUTE_ROUTE_FAIL
  exit 1
}
...
puts "ASTRA_11_A09_IMPL_ROUTE_ROUTE_DONE WNS=$wns TNS=$tns WHS=$whs THS=$ths PROGRAM=NO BIT=NOT_BUILT"
...
if {$wns_num ne "" && $wns_num < 0.0} {
  ...
  if {[catch {phys_opt_design} e2]} {
    puts "ASTRA_11_A09_EXP_PHYS_OPT_WARN $e2"
  } else {
    puts ASTRA_11_A09_EXP_PHYS_OPT_DONE
  }
  ...
  puts ASTRA_11_A09_EXP_DONE
  puts "ASTRA_11_A09_IMPL_ROUTE_DONE WNS=$wns WHS=$whs BIT=NOT_BUILT PROGRAM=NO RESULT=FAIL_WNS"
  exit 0
}

puts "ASTRA_11_A09_IMPL_ROUTE_DONE WNS=$wns TNS=$tns WHS=$whs BIT=NOT_BUILT PROGRAM=NO"
exit 0
