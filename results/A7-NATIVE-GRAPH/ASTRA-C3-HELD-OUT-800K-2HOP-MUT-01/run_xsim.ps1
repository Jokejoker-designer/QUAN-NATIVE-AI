...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C3K2HM_FILE_MISSING $p" }
...
if ($hashFail) { throw "C3K2HM_HASH_GATE_FAIL do_not_edit_keep_rtl" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c3_held_out_800k_2hop_mut.sv"
$mem = Join-Path $bag "a7ng_axi_mem_c3_800k_2hop_mut.sv"
...
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C3K2HM_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C3K2HM_GOLD_HASH_DRIFT GOLDEN.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C3K2HM_LEFTOVER_A09" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C3K2HM_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C3K2HM_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") { throw "C3K2HM_CTX_8255a798_AS_DUT" }
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C3K2HM_TINYGPT_AS_DUT" }
}
...
$pre = @(
  ...
  "# LAW=800k cartesian facts + ctx-folded k0 overlay 2-hop mutation FACT_BASE=0x0E000000 CAND_CAP=16 POKE_V=0"
) + $hashLines
$pre += "# COMPILED"
...
Write-Host "C3K2HM_SHA_FROZEN"

if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null
Set-Location $work
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C3K2HM_XVLOG_NOT_FOUND" }
& "$bin\xvlog.bat" --sv -i $bag -i $incq -i $incc -i $inci $files
if ($LASTEXITCODE -ne 0) { throw "C3K2HM_XVLOG_FAIL" }
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
& "$bin\xelab.bat" tb_astra_c3_held_out_800k_2hop_mut -s c3k2hm -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) {
  if (Test-Path (Join-Path $work "xelab.log")) {
    Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
  }
  throw "C3K2HM_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c3k2hm -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C3_HELD_OUT_800K_2HOP_MUT_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C3K2HM_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C3_HELD_OUT_800K_2HOP_MUT_RUN_OK"
