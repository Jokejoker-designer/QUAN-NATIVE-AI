...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5U_FILE_MISSING $p" }
...
if ($hashFail) { throw "C5U_HASH_GATE_FAIL do_not_edit_keep_rtl" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_unified.sv"
$dut = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_prod_top.sv"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5U_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5U_GOLD_HASH_DRIFT GOLDEN.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5U_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C5U_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5U_SYNTH_MIG_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C5U_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C5U_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") { throw "C5U_CTX_8255a798_AS_DUT" }
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C5U_LEFTOVER_A09" }
}
...
$pre = @(
  ...
  "# LAW=unified regression on one production hierarchy; modeled AXI not MIG"
) + $hashLines
$pre += "# COMPILED"
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C5_UNIFIED_REGRESSION_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C5U_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C5_UNIFIED_REGRESSION_RUN_OK"
