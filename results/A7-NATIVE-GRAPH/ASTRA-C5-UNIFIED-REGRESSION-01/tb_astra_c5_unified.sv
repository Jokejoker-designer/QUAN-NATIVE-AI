`timescale 1ns / 1ps
// ASTRA-C5-UNIFIED-REGRESSION-01. PROGRAM=NO. Bag-local TB (test-only plant).
...
module tb_astra_c5_unified;
  ...
  function automatic logic [127:0] dir_pack_ovf(input int count);
    dir_pack_ovf = {48'd0, 16'd7, 16'd1, count[15:0], 4'd0, POST_HEAP};
  endfunction
  ...
  task automatic wait_done(input string tag);
    begin
      guard = 0; cap_v = 0; cap_st = 4'hF; cap_obj = 8'd0;
      while (!done && guard < 400000) begin
        @(posedge clk);
        guard = guard + 1;
        if (c3res && !cap_v) begin
          cap_st = c3st; cap_obj = c3obj; cap_v = 1'b1;
        end
      end
  ...
  task automatic plant_index_ovf(input int nfact, input int k0, input int k1, input int k2);
    begin
      mem_wr(dir_addr(0, k0), dir_pack_ovf(nfact));
      mem_wr(dir_addr(1, k1), dir_pack_ovf(nfact));
      mem_wr(dir_addr(2, k2), dir_pack_ovf(nfact));
    end
  endtask
  ...
  initial begin
    fail = 0; first_div = ""; nslot = 0; first_obj = 8'd0; saved_w0 = 16'sd0;
    urx = 1'b1; rst_n = 0;
    for (i = 0; i < SLOTS; i = i + 1) mvld[i] = 0;
    plant_train0();
    repeat (8) @(posedge clk); rst_n = 1; repeat (8) @(posedge clk);
    $display("C5_UNIFIED PROGRAM=NO BOARD_PASS=REJECT DDR_QUERY_BOUND_FINAL=NOT_FROZEN MIG=NO A09=NO");
    guard = 0;
    ...
    first_obj = cap_obj;
    saved_w0 = pw0;
    $display("MEAS Q1 st=%0d obj=%0d pvalid=%0d w0=%0d pcmt=%0d cap=%0d", c3st, c3obj, pvalid, pw0, pcmt, cap_st);
    if (cap_st == A7NG_C5P_ST_ANSWER) $display("CLASS_two_hop_answer HIT st=%0d obj=%0d", cap_st, cap_obj);
    ...
    guard = 0;
    while (pvalid && guard < 20000) begin @(posedge clk); guard = guard + 1; end
    ...
    guard = 0;
    while (!(pvalid && (pw0 === saved_w0)) && guard < 200000) begin @(posedge clk); guard = guard + 1; end
    ...
    if (fail == 0) $display("ASTRA_C5_UNIFIED_REGRESSION_XSIM_PASS");
    else $display("ASTRA_C5_UNIFIED_REGRESSION_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO DDR_QUERY_BOUND_FINAL=NOT_FROZEN quality=MODELED_AXI_NOT_MIG_NO_CONFLICT_OPCODE_COMPACT_LM");
    $finish;
  end
endmodule
