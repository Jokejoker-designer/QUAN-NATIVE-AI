`timescale 1ns / 1ps
// ASTRA-F3-R3-SAMPLED-WORLDS-01. PROGRAM=NO. Bag-local TB.
// Sampled 5 subject-relation worlds, 8 hold queries, shared+private facts.
// Overlapping dest IDs. Shuffle mislabels mixed_ctx. 32-phi dumped on log.
`include "tb_oracles.svh"
module tb_astra_f3r3_sampled_worlds;
  ...
  a7ng_astra_f3r3_sampled_worlds u_dut (
  ...
  task automatic plant_q(input int qi, input int kind);
    ...
    int g0, g1, d0, d1, s1e, s2e, pf0, pf1;
    ...
    begin
      hold = (kind==1) || (kind==3);
      shuf = (kind==2) || (kind==3);
      w = hold ? F3R3_HO_W[qi] : F3R3_TR_W[qi];
      subj = F3R3_W_SUBJ[w]; rel = F3R3_W_REL[w];
      k0 = F3R3_W_K0[w]; k2 = F3R3_W_K2[w];
      gold_has_f = !shuf;
      dist_has_f = shuf;
      if (kind==0) begin
        g0=F3R3_TR_G0[qi]; g1=g0+1; d0=g0+2; d1=g0+3;
      end else if (kind==1) begin
        d0=F3R3_HO_D0[qi]; d1=d0+1; g0=F3R3_HO_G0[qi]; g1=g0+1;
      end else if (kind==2) begin
        d0=F3R3_SH_D0[qi]; d1=d0+1; g0=d0+2; g1=d0+3;
      end else begin
        g0=F3R3_SHH_G0[qi]; g1=g0+1; d0=F3R3_SHH_D0[qi]; d1=d0+1;
      end
      gmid=32'h30; dmid=32'h38+qi;
      if (kind==0) gdst = F3R3_SHARED_DST;
      else if (kind==1) gdst = F3R3_HO_GDST[qi];
      else gdst = F3R3_SHARED_DST;
      ddst = 32'h90+qi;
      if (gold_has_f) begin
        if (hold) begin cx1g=3; cx2g=1; end
        else begin cx1g=1; cx2g=0; end
        cfg=200;
      end else begin cx1g=0; cx2g=0; cfg=200; end
      if (dist_has_f) begin
        if (hold) begin cx1d=3; cx2d=1; end
        else begin cx1d=1; cx2d=0; end
        cfd=200;
      end else begin cx1d=0; cx2d=0; cfd=200; end
      s1e=32'h200+w*2; s2e=32'h201+w*2;
      pf0=32'h300+w*2; pf1=32'h301+w*2;
      reset_mem();
      plant_post8(g0,g1,d0,d1,s1e,s2e,pf0,pf1);
      plant_index(8, k0, k2);
      ...
      wr_f(s1e, subj, 32'h50, rel, 200, 1, 1, 2);
      wr_f(s2e, subj, 32'h51, rel, 200, 1, 1, 2);
      wr_f(pf0, subj, 32'hA0+w*2, rel, 80, 1, 1, 0);
      wr_f(pf1, subj, 32'hA1+w*2, rel, 80, 1, 1, 0);
    end
  endtask
  ...
  function automatic bit pid_picks_hold(input int qi);
    int pg, pd; begin
      ...
      pid_picks_hold = (pg > pd) || ((pg==pd) && (F3R3_HO_G0[qi] < F3R3_HO_D0[qi]));
    end
  endfunction
  ...
  initial begin
    ...
    $display("GEN seed=%08h N_WORLDS=5 N_SHARED=3 N_PRIVATE=2 N_TRAIN=8 N_HOLD=8 N_EPOCH=2",
      F3R3_GEN_SEED);
    for (q=0;q<8;q=q+1)
      $display("GEN_TR q=%0d w=%0d qry=%s", q, F3R3_TR_W[q], qstr(F3R3_TR_W[q]));
    for (q=0;q<8;q=q+1)
      $display("GEN_HO q=%0d w=%0d ov=%0d dst=%0d qry=%s",
        q, F3R3_HO_W[q], F3R3_HO_OV[q], F3R3_HO_GDST[q], qstr(F3R3_HO_W[q]));
    ...
    for (q=0;q<8;q=q+1) begin
      q_s(q, 1);
      dump($sformatf("FR_Q%0d_W%0d", q, F3R3_HO_W[q]));
      chk($sformatf("FR_Q%0d_ANSWER", q), (st===ST_ANSWER) && !tbl && (npath>=5'd1));
      chk($sformatf("FR_Q%0d_DIST", q), p0===F3R3_HO_D0[q][19:0]);
      if (p0===F3R3_HO_G0[q][19:0]) fr_c = fr_c + 1;
      retire_q();
    end
    ...
    for (q=0;q<8;q=q+1) begin
      q_s(q, 0);
      dump($sformatf("EN_TR_Q%0d_W%0d", q, F3R3_TR_W[q]));
      chk($sformatf("EN_TR_Q%0d_GOLD", q), (st===ST_ANSWER) && !tbl && (p0===F3R3_TR_G0[q][19:0]));
    ...
    for (q=0;q<8;q=q+1) begin
      q_s(q, 1);
      dump($sformatf("EN_HO_E1_Q%0d_W%0d", q, F3R3_HO_W[q]));
      chk($sformatf("EN_HO_E1_Q%0d_GOLD", q), (st===ST_ANSWER) && !tbl && (p0===F3R3_HO_G0[q][19:0]));
      ...
      if (p0===F3R3_HO_G0[q][19:0]) en_e1 = en_e1 + 1;
      $display("PID_Q%0d ov=%0d pg=%0d pd=%0d pick_gold=%0d gdst=%0d ddst=%0d",
        q, F3R3_HO_OV[q], pidmap[F3R3_HO_GDST[q][7:0]], pidmap[F3R3_HO_DDST[q][7:0]],
        pid_picks_hold(q), F3R3_HO_GDST[q], F3R3_HO_DDST[q]);
      if (pid_picks_hold(q)) begin
        if (F3R3_HO_OV[q]) pid_ov = pid_ov + 1;
    ...
    for (q=0;q<8;q=q+1) begin
      ...
      chk($sformatf("EN_TR_E2_Q%0d_GOLD", q), p0===F3R3_TR_G0[q][19:0]);
      pulse_rew(3); wait_upd(8+q+1); retire_q();
    end
    for (q=0;q<8;q=q+1) begin
      q_s(q, 1);
      dump($sformatf("EN_HO_E2_Q%0d", q));
      chk($sformatf("EN_HO_E2_Q%0d_GOLD", q), (st===ST_ANSWER) && (p0===F3R3_HO_G0[q][19:0]));
      if (p0===F3R3_HO_G0[q][19:0]) en_e2 = en_e2 + 1;
      if (q==7) snap_w();
      retire_q();
    end
    ...
    for (q=0;q<8;q=q+1) begin
      ...
      chk($sformatf("EN_HO_RL_Q%0d_GOLD", q), (st===ST_ANSWER) && !tbl && (p0===F3R3_HO_G0[q][19:0]));
      if (p0===F3R3_HO_G0[q][19:0]) en_rl = en_rl + 1;
      retire_q();
    end
    ...
    for (q=0;q<8;q=q+1) begin
      ...
      chk($sformatf("SH_TR_Q%0d_DIST", q), (st===ST_ANSWER) && (p0===F3R3_SH_D0[q][19:0]));
    ...
    for (q=0;q<8;q=q+1) begin
      ...
      chk($sformatf("SH_HO_Q%0d_NOT_GOLD", q),
        (st===ST_ANSWER) && (p0!==F3R3_SHH_G0[q][19:0]));
      chk($sformatf("SH_HO_Q%0d_DIST", q), p0===F3R3_SHH_D0[q][19:0]);
      dump_phi($sformatf("SH_HO_Q%0d", q));
      if (p0===F3R3_SHH_G0[q][19:0]) sh_c = sh_c + 1;
      retire_q();
    end
    ...
    $display("METRICS en_e1=%0d/8 en_e2=%0d/8 en_rl=%0d/8 fr=%0d/8 sh=%0d/8 pid_ov=%0d/2 pid_dj=%0d/6 phi_neq=%0d/8 n_rank=%0d n_ans=%0d",
      en_e1, en_e2, en_rl, fr_c, sh_c, pid_ov, pid_dj, phi_neq_c, n_rank, n_ans);
    chk("EN_E1_8", en_e1==8);
    ...
    if (fail==0) $display("ASTRA_F3R3_SAMPLED_WORLDS_XSIM_PASS");
    else $display("ASTRA_F3R3_SAMPLED_WORLDS_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $finish;
  end
endmodule
