...
$files = @(
  ...
  (Join-Path $bag "tb_astra_c5_final_v1_pend_replay.sv")
)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5PEND_LIVE_PROD_TOP" }
  if ($leaf -eq "a7ng_astra_c4_lm06_grounded_gen.sv") { throw "C5PEND_GROUNDED_GEN" }
}
...
if ($LASTEXITCODE -ne 0) { throw "C5PEND_XVLOG_FAIL" }
& "$bin\xelab.bat" tb_astra_c5_final_v1_pend_replay -s c5pendv1 -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) { throw "C5PEND_XELAB_FAIL" }
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c5pendv1 -R -log $xsimLog
if ($LASTEXITCODE -ne 0) { throw "C5PEND_XSIM_FAIL" }
$ok = (Select-String -Path $xsimLog -Pattern "ASTRA_C5_FINAL_V1_PEND_REPLAY_XSIM_PASS" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_pend_replay HIT" -Quiet)
if (-not $ok) { throw "C5PEND_MARKER_MISSING" }
Write-Host "ASTRA_C5_FINAL_V1_PEND_REPLAY_RUN_OK"
