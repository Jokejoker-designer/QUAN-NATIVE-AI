#!/usr/bin/env python3
"""Finish E3h OOC (S_H-only). Same 6-number + Case A/B/C readout as E3f.

Baseline = E3f: WNS -22.550 ns, hv 20/20.
Does not overwrite E3a/E3d/E3e/E3g/E3f n20/dcp.
Does not stamp C4_MASTER / BOARD_PASS. Does not program.

Usage:
  python finish_e3h_ooc.py
  python finish_e3h_ooc.py --selftest   # classify E3f n20 as if E3h; write nothing
"""
...
def six_numbers(n20: dict, setup: dict) -> dict:
    ...
    return {
        ...
        "hv_in_top20": n_tgt,
        "hv_left_top20": n_paths > 0 and n_tgt == 0,
        "hv_still_20_of_20": n_paths == 20 and n_tgt == 20,
...
def classify(six: dict) -> dict:
    wns = six.get("wns_ns")
    delta = None if wns is None else (wns - E3F_WNS)
    left = bool(six.get("hv_left_top20"))
    still20 = bool(six.get("hv_still_20_of_20"))
    strong = delta is not None and delta >= STRONG_WNS_NS
    fam = six.get("worst_dest_family")
    if left and strong:
        case, label = "A", "path_cut_and_wns_moved"
        note = "hv left top-20 and WNS improved strongly. E3h S_H physical hypothesis confirmed."
    elif left:
        case, label = "B", "path_cut_new_family"
        note = (
            "hv left top-20; WNS only slight/flat. E3h still PASS as a path cut. "
        ...
        note = (
            "hv still 20/20 and WNS nearly unchanged. "
            "Check whether the S_H register boundary survived synthesis."
        )
    else:
        case, label = "MIXED", "read_topn"
        note = "Neither clean A/B/C. Read timing_n20_e3h.rpt before RTL."
    ...
    if case in ("A", "B"):
        ...
        next_gate = "E3h_HV_NOT_CUT"

    return {
        "case": case,
        "label": label,
        "note": note,
        "wns_delta_vs_e3f_ns": delta,
        "wns_strong": strong,
        "next_gate": next_gate,
...
def selftest() -> int:
    rpt = BAG / "timing_n20_e3f.rpt"
    summ = BAG / "timing_summary_e3f.rpt"
    fail = 0
    ...
    cls = classify(six)
    if not six["hv_still_20_of_20"]:
        print("FAIL e3f hv 20/20", six)
        fail += 1
    if six["worst_logic_levels"] != E3F_LEVELS:
        print("FAIL levels", six["worst_logic_levels"])
        fail += 1
    if cls["case"] != "C":
        print("FAIL classify E3f-as-E3h should be C", cls)
        fail += 1
    if fail == 0:
        print("E3H_FINISH_SELFTEST_PASS PROGRAM=NO (E3f n20 classified C as baseline; no E3h write)")
    return fail


def write_md(six: dict, cls: dict, util: dict, shas: dict) -> str:
    return f"""# E3h OOC readout — S_H-only (6-number + Case A/B/C)

**PROGRAM=NO.** `C4_MASTER` OPEN. Not `BOARD_PASS`. `S_Q` was not touched.

| Item | E3f | E3h |
|------|-----|-----|
| WNS | −22.550 ns | **{six.get("wns_ns")} ns** |
| TNS | −63996.180 ns | {six.get("tns_ns")} ns |
| Failing endpoints | 4406 | {six.get("failing_endpoints")} |
| Top-20 family | hv 20/20 | {six.get("top20_family_counts")} |
| Worst logic levels | 46 | {six.get("worst_logic_levels")} |
| Worst CARRY4 / DSP48E1 | 28 / 3 | {six.get("worst_carry4")} / {six.get("worst_dsp48e1")} |
...
WNS Δ vs E3f: **{cls.get("wns_delta_vs_e3f_ns")} ns**. Next gate: `{cls.get("next_gate")}`.

`timing_n20_e3h.rpt` SHA256 `{shas.get("n20")}`. Does **not** overwrite E3a/E3d/E3e/E3g/E3f.

| Resource | E3f | E3h |
|----------|-----|-----|
| Slice LUTs | 30815 | {util.get("slice_luts")} |
| Slice Registers | 42817 | {util.get("slice_registers")} |
| DSP | 74 | {util.get("dsps")} |
...
def finish() -> int:
    ...
    util_p = BAG / "util_e3h.rpt"
    ram_p = BAG / "ram_e3h.rpt"
    if not rpt.is_file():
        print("WAITING", rpt)
        return 2
    ...
    payload = {
        ...
        "e3h": "TOPN_PARSED",
        "scope": "S_H_ONLY",
        "e3f_case_b_stands": True,
        "e3h_functional": "PASS_n242",
        "six": six,
        "classification": cls,
        "report_sha256": raw.get("report_sha256"),
        "n_paths": raw.get("n_paths"),
        "util": util,
        "note": "Does not overwrite E3A/E3D/E3E/E3G/E3F n20. Top-N points; no mass c4_rq refactor.",
        "paths": raw.get("paths"),
    }
    (BAG / "E3H_N20.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    (BAG / "E3H_OOC.md").write_text(write_md(six, cls, util, shas), encoding="utf-8")
    ...
    if live.is_file():
        e3e_fin.patch_json(
            live,
            {
                ...
                "e3h_ooc": "DONE",
                "e3h_wns_ns": six.get("wns_ns"),
                "e3h_case": cls.get("case"),
                "e3h_next_gate": cls.get("next_gate"),
                "e3h_hv_left_top20": six.get("hv_left_top20"),
                "updated": now,
            },
        )
    ...
    if g14.is_file():
        ...
        data["smallest_remaining_experiment"] = (
            f"E3h Case {cls.get('case')} next={cls.get('next_gate')}. Do not program."
        )
        data.setdefault("e3a", {})["e3h"] = (
        ...
        for item in data.get("section_22", []):
            if item.get("id") == "FINAL PHYSICAL CLOSURE":
                item["note"] = f"E3h Case {cls.get('case')} WNS {six.get('wns_ns')} ns; not routed C6"
        g14.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")

    print(
        "E3H_OOC_PARSED PROGRAM=NO",
        "case",
        cls.get("case"),
        "wns",
        six.get("wns_ns"),
        "delta",
        cls.get("wns_delta_vs_e3f_ns"),
        "hv_top20",
        six.get("hv_in_top20"),
        "levels",
