#!/usr/bin/env python3
"""Finish E3s OOC (dmax keep). Baseline = E3r: WNS -3.669 ns, smres/elut 20/20.
...
def classify(six: dict) -> dict:
    wns = six.get("wns_ns")
    delta = None if wns is None else (wns - E3R_WNS)
    ...
    if left and strong:
        ...
        note = "elut still 20/20 from dots. dmax keep may not have cut S_SMLUT flatten."
    else:
        case, label = "MIXED", "read_topn"
        note = "Neither clean A/B/C. Read timing_n20_e3s.rpt before RTL."
    next_gate = "READ_TOPN"
    if case in ("A", "B"):
        if fam and fam != TARGET:
            next_gate = f"E3t_{fam}"
    elif case == "C":
        next_gate = "E3T_SMLUT_PIPE"
    return {
        "case": case,
        "label": label,
        "note": note,
        "wns_delta_vs_e3r_ns": delta,
        "wns_strong": strong,
        "next_gate": next_gate,
    }
...
def selftest() -> int:
    ...
    if six["smres_in_top20"] != 20:
        ...
        fail += 1
    if cls["case"] != "C":
        print("FAIL classify E3r-as-E3s should be C", cls)
        fail += 1
    if fail == 0:
        print("E3S_FINISH_SELFTEST_PASS PROGRAM=NO")
    return fail


def write_md(six: dict, cls: dict, util: dict, shas: dict) -> str:
    return f"""# E3s OOC readout — dmax keep (6-number + Case A/B/C)

**PROGRAM=NO.** `C4_MASTER` OPEN. Not `BOARD_PASS`.

| Item | E3r | E3s |
|------|-----|-----|
| WNS | −3.669 ns | **{six.get("wns_ns")} ns** |
| TNS | −1427.389 ns | {six.get("tns_ns")} ns |
| Failing endpoints | 879 | {six.get("failing_endpoints")} |
| Top-20 family | smres 20 | {six.get("top20_family_counts")} |
| Worst logic levels | 22 | {six.get("worst_logic_levels")} |
...
WNS Δ vs E3r: **{cls.get("wns_delta_vs_e3r_ns")} ns**. Next gate: `{cls.get("next_gate")}`.

`timing_n20_e3s.rpt` SHA256 `{shas.get("n20")}`. Does **not** overwrite prior n20.
"""
...
def finish() -> int:
    ...
    (BAG / "E3S_N20.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    (BAG / "E3S_OOC.md").write_text(write_md(six, cls, util, shas), encoding="utf-8")
    ...
    if live.is_file():
        e3e_fin.patch_json(
            live,
            {
                ...
                "e3s_ooc": "DONE",
                "e3s_wns_ns": six.get("wns_ns"),
                "e3s_case": cls.get("case"),
                "e3s_next_gate": cls.get("next_gate"),
                "last_closed_ooc": "E3s",
