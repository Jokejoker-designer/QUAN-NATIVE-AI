#!/usr/bin/env python3
"""Finish E3q OOC (S_H hacc product pipeline). Baseline = E3p: WNS -5.220 ns, hacc 20/20.
...
def six_numbers(n20: dict, setup: dict) -> dict:
    ...
    return {
        ...
        "hacc_in_top20": n_tgt,
        "hacc_left_top20": n_paths > 0 and n_tgt == 0,
        "hacc_still_20_of_20": n_paths == 20 and n_tgt == 20,
        ...
        "source_is_vv": "vv_reg" in src.lower(),
    }


def classify(six: dict) -> dict:
    wns = six.get("wns_ns")
    delta = None if wns is None else (wns - E3P_WNS)
    left = bool(six.get("hacc_left_top20"))
    still20 = bool(six.get("hacc_still_20_of_20"))
    ...
    if left and strong:
        ...
        note = "hacc left top-20 and WNS improved strongly."
    elif left:
        case, label = "B", "path_cut_new_family"
        note = "hacc left top-20; WNS slight/flat. Path-cut PASS. Target the new named family only."
    elif still20 and vv_src and dsp and not strong:
        case, label = "C", "register_boundary_may_not_have_survived"
        note = "hacc still 20/20 from vv through DSP. S_H_MAC/prod_r may have collapsed."
    else:
        case, label = "MIXED", "read_topn"
        note = "Neither clean A/B/C. Dest may still be hacc with source off vv. Read timing_n20_e3q.rpt."
    next_gate = "READ_TOPN"
    if case in ("A", "B"):
        if fam and fam != TARGET:
            next_gate = f"E3r_{fam}"
    elif case == "C":
        next_gate = "E3Q_PIPE_NOT_CUT"
    return {
        "case": case,
        "label": label,
        "note": note,
        "wns_delta_vs_e3p_ns": delta,
        "wns_strong": strong,
        "next_gate": next_gate,
    }
...
def selftest() -> int:
    rpt = BAG / "timing_n20_e3p.rpt"
    summ = BAG / "timing_summary_e3p.rpt"
    ...
    cls = classify(six)
    fail = 0
    if six["hacc_in_top20"] != 20:
        print("FAIL hacc count", six)
        fail += 1
    if not six["source_is_vv"]:
        print("FAIL E3p source should be vv", six)
        fail += 1
    if cls["case"] != "C":
        print("FAIL classify E3p-as-E3q should be C", cls)
        fail += 1
    if fail == 0:
        print("E3Q_FINISH_SELFTEST_PASS PROGRAM=NO")
    return fail


def write_md(six: dict, cls: dict, util: dict, shas: dict) -> str:
    return f"""# E3q OOC readout — S_H hacc product pipeline (6-number + Case A/B/C)

**PROGRAM=NO.** `C4_MASTER` OPEN. Not `BOARD_PASS`. S_F2 was not pipelined.

| Item | E3p | E3q |
|------|-----|-----|
| WNS | −5.220 ns | **{six.get("wns_ns")} ns** |
| TNS | −1900.469 ns | {six.get("tns_ns")} ns |
| Failing endpoints | 1024 | {six.get("failing_endpoints")} |
| Top-20 family | hacc 20 | {six.get("top20_family_counts")} |
| Worst logic levels | 23 | {six.get("worst_logic_levels")} |
| Worst CARRY4 / DSP48E1 | 12 / 2 | {six.get("worst_carry4")} / {six.get("worst_dsp48e1")} |
...
WNS Δ vs E3p: **{cls.get("wns_delta_vs_e3p_ns")} ns**. Next gate: `{cls.get("next_gate")}`.

`timing_n20_e3q.rpt` SHA256 `{shas.get("n20")}`. Does **not** overwrite prior n20.
"""
...
def finish() -> int:
    ...
    payload = {
        ...
        "scope": "HACC_PROD_PIPE",
        "six": six,
        "classification": cls,
        "report_sha256": raw.get("report_sha256"),
        "util": util,
    }
    (BAG / "E3Q_N20.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    (BAG / "E3Q_OOC.md").write_text(write_md(six, cls, util, shas), encoding="utf-8")
    ...
    if live.is_file():
        e3e_fin.patch_json(
            live,
            {
                ...
                "e3q_ooc": "DONE",
                "e3q_wns_ns": six.get("wns_ns"),
                "e3q_case": cls.get("case"),
                "e3q_next_gate": cls.get("next_gate"),
                "last_closed_ooc": "E3q",
    ...
    print(
        "E3Q_OOC_PARSED PROGRAM=NO",
        "case",
        cls.get("case"),
        "wns",
        six.get("wns_ns"),
        "hacc_top20",
        six.get("hacc_in_top20"),
        "vv_src",
