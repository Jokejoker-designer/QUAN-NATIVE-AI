#!/usr/bin/env python3
"""Finish E3w OOC (SMRES num/abs pipe). Baseline = E3v: WNS -2.598 ns, smres_div 3 / prod_r 17.
...
def six_numbers(n20: dict, setup: dict) -> dict:
    ...
    return {
        ...
        "smres_div_in_top20": n_tgt,
        "smres_div_left_top20": n_paths > 0 and n_tgt == 0,
        "smres_div_still_20_of_20": n_paths == 20 and n_tgt == 20,
        ...
        "source_is_elut": "elut" in src_l,
    }


def classify(six: dict) -> dict:
    wns = six.get("wns_ns")
    delta = None if wns is None else (wns - E3V_WNS)
    left = bool(six.get("smres_div_left_top20"))
    still20 = bool(six.get("smres_div_still_20_of_20"))
    strong = delta is not None and delta >= STRONG_WNS_NS
    elut_src = bool(six.get("source_is_elut"))
    fam = six.get("worst_dest_family")
    if left and strong:
        case, label = "A", "path_cut_and_wns_moved"
        note = "smres_div left top-20 and WNS improved strongly."
    elif left:
        case, label = "B", "path_cut_new_family"
        note = "smres_div left top-20; WNS slight/flat. Path-cut PASS. Target the new named family only."
    elif still20 and elut_src and not strong:
        case, label = "C", "register_boundary_may_not_have_survived"
        note = "un still 20/20 from elut. ST_NUM/ST_ABS pipe may have collapsed."
    else:
        case, label = "MIXED", "read_topn"
        note = "Neither clean A/B/C. Read timing_n20_e3w.rpt before RTL."
    next_gate = "READ_TOPN"
    if case in ("A", "B"):
        if fam and fam != TARGET:
            next_gate = f"E3x_{fam}"
    elif case == "C":
        next_gate = "E3W_PIPE_NOT_CUT"
    return {
        "case": case,
        "label": label,
        "note": note,
        "wns_delta_vs_e3v_ns": delta,
        "wns_strong": strong,
        "next_gate": next_gate,
    }
...
def selftest() -> int:
    ...
    cls = classify(six)
    fail = 0
    if six["smres_div_in_top20"] != 3:
        print("FAIL smres_div count", six)
        fail += 1
    if not six["source_is_elut"]:
        print("FAIL E3v source should be elut", six)
        fail += 1
    if cls["case"] != "MIXED":
        print("FAIL classify E3v-as-E3w should be MIXED", cls)
        fail += 1
    if fail == 0:
        print("E3W_FINISH_SELFTEST_PASS PROGRAM=NO")
    return fail


def write_md(six: dict, cls: dict, util: dict, shas: dict) -> str:
    return f"""# E3w OOC readout — SMRES num/abs pipe (6-number + Case A/B/C)

**PROGRAM=NO.** `C4_MASTER` OPEN. Not `BOARD_PASS`.

| Item | E3v | E3w |
|------|-----|-----|
| WNS | −2.598 ns | **{six.get("wns_ns")} ns** |
| TNS | −192.428 ns | {six.get("tns_ns")} ns |
| Failing endpoints | 96 | {six.get("failing_endpoints")} |
| Top-20 family | smres_div 3, prod_r 17 | {six.get("top20_family_counts")} |
| Worst logic levels | 21 | {six.get("worst_logic_levels")} |
| Worst CARRY4 / DSP48E1 | 10 / 0 | {six.get("worst_carry4")} / {six.get("worst_dsp48e1")} |
...
WNS Δ vs E3v: **{cls.get("wns_delta_vs_e3v_ns")} ns**. Next gate: `{cls.get("next_gate")}`.

`timing_n20_e3w.rpt` SHA256 `{shas.get("n20")}`. Does **not** overwrite prior n20.
"""
...
def finish() -> int:
    ...
    payload = {
        ...
        "scope": "SMRES_NUM_PIPE",
        "six": six,
        "classification": cls,
        "report_sha256": raw.get("report_sha256"),
        "util": util,
    }
    (BAG / "E3W_N20.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    (BAG / "E3W_OOC.md").write_text(write_md(six, cls, util, shas), encoding="utf-8")
    ...
    if live.is_file():
        e3e_fin.patch_json(
            live,
            {
                ...
                "e3w_ooc": "DONE",
                "e3w_wns_ns": six.get("wns_ns"),
                "e3w_case": cls.get("case"),
                "e3w_next_gate": cls.get("next_gate"),
                "last_closed_ooc": "E3w",
    ...
    print(
        "E3W_OOC_PARSED PROGRAM=NO",
        "case",
        cls.get("case"),
        "wns",
        six.get("wns_ns"),
        "smres_div_top20",
        six.get("smres_div_in_top20"),
        "elut_src",
        six.get("source_is_elut"),
        "next",
        cls.get("next_gate"),
    )
    return 0
