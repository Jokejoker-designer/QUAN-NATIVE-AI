#!/usr/bin/env python3
"""Finish E3u OOC (di-indexed wgt8 operand pipe into prod_r). Baseline = E3t: WNS -2.766 ns, prod_r 20/20.
...
def classify(six: dict) -> dict:
    wns = six.get("wns_ns")
    delta = None if wns is None else (wns - E3T_WNS)
    ...
    if left and strong:
        ...
        note = "prod_r left top-20; WNS slight/flat. Path-cut PASS. Target the new named family only."
    elif still20 and di_src and dsp and not strong:
        case, label = "C", "register_boundary_may_not_have_survived"
        note = "prod_r still 20/20 from di through DSP. S_PROD/w8_r may have collapsed."
    else:
        case, label = "MIXED", "read_topn"
        note = "Neither clean A/B/C. Read timing_n20_e3u.rpt before RTL."
    next_gate = "READ_TOPN"
    if case in ("A", "B"):
        if fam and fam != TARGET:
            next_gate = f"E3v_{fam}"
    elif case == "C":
        next_gate = "E3U_PIPE_NOT_CUT"
    return {
        "case": case,
        "label": label,
        "note": note,
        "wns_delta_vs_e3t_ns": delta,
        "wns_strong": strong,
        "next_gate": next_gate,
    }
...
def selftest() -> int:
    ...
    if not six["source_is_di"]:
        print("FAIL E3t source should be di", six)
        fail += 1
    if cls["case"] != "C":
        print("FAIL classify E3t-as-E3u should be C", cls)
        fail += 1
    if fail == 0:
        print("E3U_FINISH_SELFTEST_PASS PROGRAM=NO")
    return fail


def write_md(six: dict, cls: dict, util: dict, shas: dict) -> str:
    return f"""# E3u OOC readout — di-indexed wgt8 operand pipe into prod_r (6-number + Case A/B/C)

**PROGRAM=NO.** `C4_MASTER` OPEN. Not `BOARD_PASS`.

| Item | E3t | E3u |
|------|-----|-----|
| WNS | −2.766 ns | **{six.get("wns_ns")} ns** |
| TNS | −369.748 ns | {six.get("tns_ns")} ns |
| Failing endpoints | 351 | {six.get("failing_endpoints")} |
| Top-20 family | prod_r 20 | {six.get("top20_family_counts")} |
| Worst logic levels | 11 | {six.get("worst_logic_levels")} |
...
WNS Δ vs E3t: **{cls.get("wns_delta_vs_e3t_ns")} ns**. Next gate: `{cls.get("next_gate")}`.

`timing_n20_e3u.rpt` SHA256 `{shas.get("n20")}`. Does **not** overwrite prior n20.
"""
...
def finish() -> int:
    ...
    (BAG / "E3U_N20.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    (BAG / "E3U_OOC.md").write_text(write_md(six, cls, util, shas), encoding="utf-8")
    ...
    if live.is_file():
        e3e_fin.patch_json(
            live,
            {
                ...
                "e3u_ooc": "DONE",
                "e3u_wns_ns": six.get("wns_ns"),
                "e3u_case": cls.get("case"),
                "e3u_next_gate": cls.get("next_gate"),
                "last_closed_ooc": "E3u",
