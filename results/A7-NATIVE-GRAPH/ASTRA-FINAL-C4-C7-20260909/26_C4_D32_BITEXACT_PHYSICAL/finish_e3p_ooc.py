#!/usr/bin/env python3
"""Finish E3p OOC (di-indexed acc product pipeline). Baseline = E3o: WNS -5.957 ns, acc 20/20.
...
def classify(six: dict) -> dict:
    ...
    elif still20 and di_src and dsp and not strong:
        case, label = "C", "register_boundary_may_not_have_survived"
        note = "acc still 20/20 from di through DSP. S_ACC_MAC/prod_r may have collapsed."
    else:
        case, label = "MIXED", "read_topn"
        note = "Neither clean A/B/C. Dest may still be acc with source off di. Read timing_n20_e3p.rpt."
    next_gate = "READ_TOPN"
    if case in ("A", "B"):
        if fam and fam != TARGET:
            next_gate = f"E3q_{fam}"
    elif case == "C":
        next_gate = "E3P_PIPE_NOT_CUT"
    return {
        "case": case,
        "label": label,
        "note": note,
        "wns_delta_vs_e3o_ns": delta,
        "wns_strong": strong,
        "next_gate": next_gate,
    }
...
def selftest() -> int:
    ...
    cls = classify(six)
    fail = 0
    if six["acc_in_top20"] != 20:
        print("FAIL acc count", six)
        fail += 1
    if not six["source_is_di"]:
        print("FAIL E3o source should be di", six)
        fail += 1
    if cls["case"] != "C":
        print("FAIL classify E3o-as-E3p should be C", cls)
        fail += 1
    if fail == 0:
        print("E3P_FINISH_SELFTEST_PASS PROGRAM=NO")
    return fail


def write_md(six: dict, cls: dict, util: dict, shas: dict) -> str:
    return f"""# E3p OOC readout — di-indexed acc product pipeline (6-number + Case A/B/C)

**PROGRAM=NO.** `C4_MASTER` OPEN. Not `BOARD_PASS`. S_F2 was not pipelined.

| Item | E3o | E3p |
|------|-----|-----|
| WNS | −5.957 ns | **{six.get("wns_ns")} ns** |
| TNS | −1884.503 ns | {six.get("tns_ns")} ns |
| Failing endpoints | 620 | {six.get("failing_endpoints")} |
| Top-20 family | acc 20 | {six.get("top20_family_counts")} |
...
WNS Δ vs E3o: **{cls.get("wns_delta_vs_e3o_ns")} ns**. Next gate: `{cls.get("next_gate")}`.

`timing_n20_e3p.rpt` SHA256 `{shas.get("n20")}`. Does **not** overwrite prior n20.

| Resource | E3o | E3p |
|----------|-----|-----|
| Slice LUTs | 28453 | {util.get("slice_luts")} |
| Slice Registers | 42940 | {util.get("slice_registers")} |
| DSP | 19 | {util.get("dsps")} |
| Block RAM Tile | 0 | {util.get("block_ram_tile")} |
| LUT as Memory | 200 | {util.get("lut_as_memory")} |
"""
...
def finish() -> int:
    ...
    payload = {
        ...
        "scope": "DI_ACC_PROD_PIPE",
        "six": six,
        "classification": cls,
        "report_sha256": raw.get("report_sha256"),
        "util": util,
    }
    (BAG / "E3P_N20.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    (BAG / "E3P_OOC.md").write_text(write_md(six, cls, util, shas), encoding="utf-8")
    ...
    if live.is_file():
        e3e_fin.patch_json(
            live,
            {
                ...
                "e3p_ooc": "DONE",
                "e3p_wns_ns": six.get("wns_ns"),
                "e3p_case": cls.get("case"),
                "e3p_next_gate": cls.get("next_gate"),
                "last_closed_ooc": "E3p",
