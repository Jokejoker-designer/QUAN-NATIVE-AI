#!/usr/bin/env python3
"""Finish E3k OOC (S_EMB-only). Baseline = E3j: WNS -16.734 ns, x_wdata worst.
...
def six_numbers(n20: dict, setup: dict) -> dict:
    ...
    return {
        ...
        "x_wdata_in_top20": n_tgt,
        "x_left_top20": n_paths > 0 and n_tgt == 0,
        "x_still_20_of_20": n_paths == 20 and n_tgt == 20,
...
def classify(six: dict) -> dict:
    wns = six.get("wns_ns")
    delta = None if wns is None else (wns - E3J_WNS)
    left = bool(six.get("x_left_top20"))
    still20 = bool(six.get("x_still_20_of_20"))
    strong = delta is not None and delta >= STRONG_WNS_NS
    fam = six.get("worst_dest_family")
    if left and strong:
        case, label = "A", "path_cut_and_wns_moved"
        note = "x_wdata left top-20 and WNS improved strongly."
    elif left:
        case, label = "B", "path_cut_new_family"
        note = "x_wdata left top-20; WNS slight/flat. Path-cut PASS. Target the new named family only."
    elif still20 and (delta is None or delta < STRONG_WNS_NS):
        case, label = "C", "register_boundary_may_not_have_survived"
        note = "x_wdata still 20/20. Check whether the S_EMB register boundary survived synthesis."
    else:
        case, label = "MIXED", "read_topn"
        note = "Neither clean A/B/C. Read timing_n20_e3k.rpt before RTL."
    next_gate = "READ_TOPN"
    if case in ("A", "B"):
        if fam == "yv":
            next_gate = "E3l_S_Y"
        elif fam and fam != TARGET:
            next_gate = f"E3l_{fam}"
    elif case == "C":
        next_gate = "E3K_EMB_NOT_CUT"
    return {
        "case": case,
        "label": label,
        "note": note,
        "wns_delta_vs_e3j_ns": delta,
        "wns_strong": strong,
        "next_gate": next_gate,
    }
...
def selftest() -> int:
    ...
    if six["worst_dest_family"] != TARGET:
        ...
        fail += 1
    if six["x_wdata_in_top20"] != 20:
        print("FAIL x_wdata count", six)
        fail += 1
    if cls["case"] != "C":
        print("FAIL classify E3j-as-E3k should be C", cls)
        fail += 1
    if fail == 0:
        print("E3K_FINISH_SELFTEST_PASS PROGRAM=NO")
    return fail


def write_md(six: dict, cls: dict, util: dict, shas: dict) -> str:
    return f"""# E3k OOC readout — S_EMB-only (6-number + Case A/B/C)

**PROGRAM=NO.** `C4_MASTER` OPEN. Not `BOARD_PASS`. `S_Y` was not touched.

| Item | E3j | E3k |
|------|-----|-----|
| WNS | −16.734 ns | **{six.get("wns_ns")} ns** |
| TNS | −19812.107 ns | {six.get("tns_ns")} ns |
| Failing endpoints | 1735 | {six.get("failing_endpoints")} |
| Top-20 family | x_wdata 20 | {six.get("top20_family_counts")} |
| Worst logic levels | 41 | {six.get("worst_logic_levels")} |
| Worst CARRY4 / DSP48E1 | 24 / 2 | {six.get("worst_carry4")} / {six.get("worst_dsp48e1")} |
...
WNS Δ vs E3j: **{cls.get("wns_delta_vs_e3j_ns")} ns**. Next gate: `{cls.get("next_gate")}`.

`timing_n20_e3k.rpt` SHA256 `{shas.get("n20")}`. Does **not** overwrite prior n20.

| Resource | E3j | E3k |
|----------|-----|-----|
| Slice LUTs | 29528 | {util.get("slice_luts")} |
| Slice Registers | 42869 | {util.get("slice_registers")} |
| DSP | 33 | {util.get("dsps")} |
...
def finish() -> int:
    ...
    (BAG / "E3K_N20.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    (BAG / "E3K_OOC.md").write_text(write_md(six, cls, util, shas), encoding="utf-8")
    ...
    if live.is_file():
        e3e_fin.patch_json(
            live,
            {
                ...
                "e3k_ooc": "DONE",
                "e3k_wns_ns": six.get("wns_ns"),
                "e3k_case": cls.get("case"),
                "e3k_next_gate": cls.get("next_gate"),
                "updated": now,
            },
        )
    print(
        "E3K_OOC_PARSED PROGRAM=NO",
        "case",
        cls.get("case"),
        "wns",
        six.get("wns_ns"),
        "x_top20",
        six.get("x_wdata_in_top20"),
        "next",
        cls.get("next_gate"),
    )
    return 0
