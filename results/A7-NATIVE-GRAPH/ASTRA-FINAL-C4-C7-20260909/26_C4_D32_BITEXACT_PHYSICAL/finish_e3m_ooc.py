#!/usr/bin/env python3
"""Finish E3m OOC (S_H acc-only). Baseline = E3l: WNS -6.543 ns, acc/rq from vv.
...
def classify(six: dict) -> dict:
    wns = six.get("wns_ns")
    delta = None if wns is None else (wns - E3L_WNS)
    ...
    if left and strong:
        ...
        note = "acc still 20/20. Check whether hacc keep survived synthesis."
    else:
        case, label = "MIXED", "read_topn"
        note = "Neither clean A/B/C. Read timing_n20_e3m.rpt before RTL."
    next_gate = "READ_TOPN"
    if case in ("A", "B"):
        if fam and fam != TARGET:
            next_gate = f"E3n_{fam}"
    elif case == "C":
        next_gate = "E3M_HACC_NOT_CUT"
    return {
        "case": case,
        "label": label,
        "note": note,
        "wns_delta_vs_e3l_ns": delta,
        "wns_strong": strong,
        "next_gate": next_gate,
    }
...
def selftest() -> int:
    ...
    if six["worst_dest_family"] != TARGET:
        ...
        fail += 1
    if six["acc_in_top20"] != 15:
        print("FAIL acc count", six)
        fail += 1
    if cls["case"] != "MIXED":
        print("FAIL classify E3l-as-E3m should be MIXED", cls)
        fail += 1
    if fail == 0:
        print("E3M_FINISH_SELFTEST_PASS PROGRAM=NO")
    return fail


def write_md(six: dict, cls: dict, util: dict, shas: dict) -> str:
    return f"""# E3m OOC readout — S_H acc-only (6-number + Case A/B/C)

**PROGRAM=NO.** `C4_MASTER` OPEN. Not `BOARD_PASS`. S_Y was not retouched.

| Item | E3l | E3m |
|------|-----|-----|
| WNS | −6.543 ns | **{six.get("wns_ns")} ns** |
| TNS | −1971.193 ns | {six.get("tns_ns")} ns |
| Failing endpoints | 583 | {six.get("failing_endpoints")} |
| Top-20 family | acc 15, rq_snap 5 | {six.get("top20_family_counts")} |
| Worst logic levels | 26 | {six.get("worst_logic_levels")} |
...
WNS Δ vs E3l: **{cls.get("wns_delta_vs_e3l_ns")} ns**. Next gate: `{cls.get("next_gate")}`.

`timing_n20_e3m.rpt` SHA256 `{shas.get("n20")}`. Does **not** overwrite prior n20.

| Resource | E3l | E3m |
|----------|-----|-----|
| Slice LUTs | 28475 | {util.get("slice_luts")} |
| Slice Registers | 42880 | {util.get("slice_registers")} |
...
def finish() -> int:
    ...
    (BAG / "E3M_N20.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    (BAG / "E3M_OOC.md").write_text(write_md(six, cls, util, shas), encoding="utf-8")
    ...
    if live.is_file():
        e3e_fin.patch_json(
            live,
            {
                ...
                "e3m_ooc": "DONE",
                "e3m_wns_ns": six.get("wns_ns"),
                "e3m_case": cls.get("case"),
                "e3m_next_gate": cls.get("next_gate"),
                "updated": now,
            },
        )
    print(
        "E3M_OOC_PARSED PROGRAM=NO",
        "case",
        cls.get("case"),
        "wns",
        six.get("wns_ns"),
