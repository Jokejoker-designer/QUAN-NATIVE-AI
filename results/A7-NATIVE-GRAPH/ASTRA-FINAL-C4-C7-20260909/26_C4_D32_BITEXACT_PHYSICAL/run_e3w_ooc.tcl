# E3w OOC of SMRES num/abs pipe D32.
# Do not overwrite E3a..E3v n20/dcp.
# PROGRAM=NO. BIT=NO.
...
read_verilog -sv $rq
read_verilog -sv $dut
puts "E3W_OOC_START part=$part PROGRAM=NO C4_MASTER=OPEN BOARD_PASS=OPEN SMRES_NUM_PIPE"
if {[catch {synth_design -mode out_of_context -top a7ng_astra_c4_lm06_d32_fr_v2 -part $part} err]} {
  puts "E3W_OOC_SYNTH_FAIL $err"
  exit 1
}
if {[llength [get_ports -quiet clk]] > 0} {
  create_clock -period 10.000 -name clk [get_ports clk]
}
write_checkpoint -force [file join $bag d32_ooc_e3w.dcp]
report_timing -delay_type max -max_paths 20 -nworst 1 -sort_by slack \
  -file [file join $bag timing_n20_e3w.rpt]
report_timing_summary -file [file join $bag timing_summary_e3w.rpt]
report_utilization -file [file join $bag util_e3w.rpt]
report_ram_utilization -file [file join $bag ram_e3w.rpt]
puts "E3W_OOC_DONE PROGRAM=NO C4_MASTER=OPEN"
exit 0
