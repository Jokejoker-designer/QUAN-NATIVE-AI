#!/usr/bin/env python3
"""Finish E3l OOC (S_Y-only). Baseline = E3k: WNS -15.041 ns, rq_val/yv cone.
...
def six_numbers(n20: dict, setup: dict) -> dict:
    ...
    n_tgt = int(counts.get("rq_snap") or 0) + int(counts.get("yv") or 0)
    ...
    return {
        ...
        "y_cone_in_top20": n_tgt,
        "y_cone_left_top20": n_paths > 0 and n_tgt == 0,
        "y_cone_still_20_of_20": n_paths == 20 and n_tgt == 20,
...
def classify(six: dict) -> dict:
    wns = six.get("wns_ns")
    delta = None if wns is None else (wns - E3K_WNS)
    left = bool(six.get("y_cone_left_top20"))
    still20 = bool(six.get("y_cone_still_20_of_20"))
    strong = delta is not None and delta >= STRONG_WNS_NS
    fam = six.get("worst_dest_family")
    if left and strong:
        case, label = "A", "path_cut_and_wns_moved"
        note = "S_Y cone (yv/rq_snap) left top-20 and WNS improved strongly."
    elif left:
        case, label = "B", "path_cut_new_family"
        note = "S_Y cone left top-20; WNS slight/flat. Path-cut PASS. Target the new named family only."
    elif still20 and (delta is None or delta < STRONG_WNS_NS):
        case, label = "C", "register_boundary_may_not_have_survived"
        note = "S_Y cone still 20/20. Check whether the S_Y register boundary survived synthesis."
    else:
        case, label = "MIXED", "read_topn"
        note = "Neither clean A/B/C. Read timing_n20_e3l.rpt before RTL."
    next_gate = "READ_TOPN"
    if case in ("A", "B"):
        if fam and fam not in TARGET_FAMS:
            next_gate = f"E3m_{fam}"
        elif fam in TARGET_FAMS:
            next_gate = "READ_TOPN"
    elif case == "C":
        next_gate = "E3L_Y_NOT_CUT"
    return {
        "case": case,
        "label": label,
        "note": note,
        "wns_delta_vs_e3k_ns": delta,
        "wns_strong": strong,
        "next_gate": next_gate,
    }
...
def selftest() -> int:
    rpt = BAG / "timing_n20_e3k.rpt"
    summ = BAG / "timing_summary_e3k.rpt"
    ...
    cls = classify(six)
    fail = 0
    if six["worst_dest_family"] != "rq_snap":
        print("FAIL family", six)
        fail += 1
    if six["y_cone_in_top20"] != 20:
        print("FAIL y cone count", six)
        fail += 1
    if cls["case"] != "C":
        print("FAIL classify E3k-as-E3l should be C", cls)
        fail += 1
    if fail == 0:
        print("E3L_FINISH_SELFTEST_PASS PROGRAM=NO")
    return fail


def write_md(six: dict, cls: dict, util: dict, shas: dict) -> str:
    return f"""# E3l OOC readout — S_Y-only (6-number + Case A/B/C)

**PROGRAM=NO.** `C4_MASTER` OPEN. Not `BOARD_PASS`. S_F1 was not retouched.

| Item | E3k | E3l |
|------|-----|-----|
| WNS | −15.041 ns | **{six.get("wns_ns")} ns** |
| TNS | −9485.734 ns | {six.get("tns_ns")} ns |
| Failing endpoints | 1125 | {six.get("failing_endpoints")} |
| Top-20 family | rq_snap 20 (yv cone) | {six.get("top20_family_counts")} |
| Worst logic levels | 38 | {six.get("worst_logic_levels")} |
| Worst CARRY4 / DSP48E1 | 23 / 5 | {six.get("worst_carry4")} / {six.get("worst_dsp48e1")} |
...
WNS Δ vs E3k: **{cls.get("wns_delta_vs_e3k_ns")} ns**. Next gate: `{cls.get("next_gate")}`.

`timing_n20_e3l.rpt` SHA256 `{shas.get("n20")}`. Does **not** overwrite prior n20.

| Resource | E3k | E3l |
|----------|-----|-----|
| Slice LUTs | 28501 | {util.get("slice_luts")} |
| Slice Registers | 42831 | {util.get("slice_registers")} |
| DSP | 29 | {util.get("dsps")} |
...
def finish() -> int:
    rpt = BAG / "timing_n20_e3l.rpt"
    summ = BAG / "timing_summary_e3l.rpt"
    util_p = BAG / "util_e3l.rpt"
    if not rpt.is_file():
        print("WAITING", rpt)
        return 2
    ...
    (BAG / "E3L_N20.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    (BAG / "E3L_OOC.md").write_text(write_md(six, cls, util, shas), encoding="utf-8")
    ...
    if live.is_file():
        e3e_fin.patch_json(
            live,
            {
                ...
                "e3l_ooc": "DONE",
                "e3l_wns_ns": six.get("wns_ns"),
                "e3l_case": cls.get("case"),
                "e3l_next_gate": cls.get("next_gate"),
                "updated": now,
            },
        )
    print(
        "E3L_OOC_PARSED PROGRAM=NO",
        "case",
        cls.get("case"),
        "wns",
        six.get("wns_ns"),
        "y_cone_top20",
        six.get("y_cone_in_top20"),
        "next",
        cls.get("next_gate"),
    )
    return 0
