`timescale 1ns / 1ps
// ASTRA-C5-FINAL-V1 WO §15-17: zero/corrupt C4 control on C5 top. PROGRAM=NO.
// Plusarg ZERO_WE / CORRUPT_WQ via hex in xsim_work. Not C5_MASTER.
...
module tb_astra_c5_final_v1_c4_causal;
  ...
  initial begin
    ...
    $display("C5_FINAL_V1_C4_CAUSAL PROGRAM=NO C5_MASTER=OPEN BOARD_PASS=OPEN");
    $display("PROD_DICT=OPEN TB_ALIAS=SYNTHETIC_CONFIRM_V3_ON_C3_ENDPOINTS");
    if ($test$plusargs("ZERO_WE")) $display("MODE=ZERO_WE");
    else if ($test$plusargs("CORRUPT_WQ")) $display("MODE=CORRUPT_WQ");
    else $display("MODE=BASE");
    guard = 0;
    ...
    chk("C3_ANSWER", (cap_st == A7NG_C3_ST_ANSWER) && (cap_ans == VALVE) && cap_pok);
    chk("ALLOWED", cap_allow && cap_aok);
    $display("MEAS_GOLD t0=%0d t1=%0d t2=%0d t3=%0d t4=%0d npin=%0d nht=%0d nhw=%0d",
             pinb[0], pinb[1], pinb[2], pinb[3], pinb[4], npin, nht, nhw);

    if ($test$plusargs("ZERO_WE") || $test$plusargs("CORRUPT_WQ")) begin
      if ((npin == 5) && (pinb[0] == 8'd116) && (pinb[1] == 8'd105) &&
          (pinb[2] == 8'd102) && (pinb[3] == 8'd104) && (pinb[4] == 8'd0)) begin
        $display("CLASS_c4_ablate MISS still_gold_tifh");
        chk("NOT_GOLD", 1'b0);
      end else begin
        $display("CLASS_c4_ablate HIT npin=%0d t0=%0d t1=%0d t2=%0d t3=%0d",
                 npin, pinb[0], pinb[1], pinb[2], pinb[3]);
        chk("NOT_GOLD", 1'b1);
      end
      if ((npin == 3) && (pinb[0] == 8'h6E) && (pinb[1] == 8'h6F) && (pinb[2] == 8'd0) &&
          cap_allow)
        $display("CLASS_safe_hide MISS n,o,EOS while allow=1");
      else $display("CLASS_safe_hide HIT");
      if ($test$plusargs("ZERO_WE")) $display("CLASS_zero_we_done");
      else $display("CLASS_corrupt_wq_done");
    end else begin
      chk_pin("F_LEARNED", 5, 8'd116, 8'd105, 8'd102, 8'd104, 8'd0);
      if ((npin == 5) && (pinb[0] == 8'd116) && (pinb[1] == 8'd105))
        $display("CLASS_c4_live HIT");
      else $display("CLASS_c4_live MISS");
    end

    chk("HOST_TOK0", nht == 16'd0);
    chk("HOST_WIN0", nhw == 16'd0);
    chk("NOQID", !qid);
    chk("NOA09", !a09);
    chk("NOPLANT", !plant);
    chk("DUAL0", !dual);
    $display("MEAS_HOST_TOK_RTL nht=%0d d32_assign_const0=1 no_host_tok_input", nht);
    $display("CLASS_host_tok_input MISS no D32 host-token input path; n_host_tok_o=16'd0");
    if (nhw == 16'd0) $display("CLASS_host_winner0 HIT");
    else $display("CLASS_host_winner0 MISS nhw=%0d", nhw);

    if (fail == 0) $display("ASTRA_C5_FINAL_V1_C4_CAUSAL_XSIM_PASS");
    else $display("ASTRA_C5_FINAL_V1_C4_CAUSAL_XSIM_FAIL n=%0d first=%s", fail, first_div);
