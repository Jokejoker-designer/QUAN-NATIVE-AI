`timescale 1ns / 1ps
// G04 WO360 D32 FR vs IntegerModel. PROGRAM=NO. Not C4_MASTER.
`include "a7ng_astra_c4_lm06_d32_fr_v1.svh"
`include "GOLDEN.svh"

module tb_astra_c4_lm06_d32_fr_v1_wo360;
  logic clk, rst_n, go, retire, allowed, zero_w, busy, done, tok_v, eos, bank_r;
  logic [7:0] tok, n_out;
  logic [7:0] ctx [0:15];
  logic [15:0] n_host;
  logic [3:0] vver;
  int fail, guard, nt, i, c, t;
  int got [0:7];
  string first_div;
  ...
  task automatic run_one(input int idx);
    begin
      ...
      if ((idx % 30) == 0) $display("PROGRESS %0d n=%0d t0=%0d guard=%0d fail=%0d", idx, nt, got[0], guard, fail);
      @(posedge clk);
      retire <= 1'b1;
      @(posedge clk);
      retire <= 1'b0;
  ...
  initial begin
    ...
    $display("C4_D32_FR_V1_WO360 PROGRAM=NO BOARD_PASS=OPEN C4_MASTER=OPEN");
    ...
    if (fail == 0) begin
      ...
      $display("ASTRA_C4_D32_FR_V1_WO360_XSIM_PASS n=%0d", A7NG_C4D32_NCASE);
    end else begin
      $display("CLASS_ref_match MISS");
      $display("ASTRA_C4_D32_FR_V1_WO360_XSIM_FAIL n=%0d first=%s", fail, first_div);
