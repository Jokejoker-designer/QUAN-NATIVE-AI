...
$tb = Join-Path $bag "tb_astra_c4_lm06_d32_fr_v1_wo360.sv"
...
$pre = @(
  ...
  "# PROGRAM=NO BOARD_PASS=OPEN C4_MASTER=OPEN D32_FR_V1_WO360"
)
...
$pre += "$(Sha256 $tb)  tb_astra_c4_lm06_d32_fr_v1_wo360.sv"
...
Write-Host "C4D32_WO360_SHA_FROZEN"
...
& "$bin\xelab.bat" tb_astra_c4_lm06_d32_fr_v1_wo360 -s c4d32fr360 -timescale 1ns/1ps
...
& "$bin\xsim.bat" c4d32fr360 -R -log $xsimLog
$xsimExit = $LASTEXITCODE
$hasPass = $false
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = (Select-String -Path $xsimLog -Pattern "ASTRA_C4_D32_FR_V1_WO360_XSIM_PASS" -Quiet) -and
...
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C4D32_WO360_XSIM_FAIL_OR_MARKER_MISSING pass=$hasPass"
}
Write-Host "ASTRA_C4_D32_FR_V1_WO360_RUN_OK"
