...
$files = @(
  ...
  (Join-Path $bag "tb_astra_c5_final_v1_ctx_unk.sv")
)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5CTX_LIVE_PROD_TOP" }
  if ($leaf -eq "a7ng_astra_c4_lm06_grounded_gen.sv") { throw "C5CTX_GROUNDED_GEN" }
}
...
if ($LASTEXITCODE -ne 0) { throw "C5CTX_XVLOG_FAIL" }
& "$bin\xelab.bat" tb_astra_c5_final_v1_ctx_unk -s c5ctxv1 -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) { throw "C5CTX_XELAB_FAIL" }
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c5ctxv1 -R -log $xsimLog
if ($LASTEXITCODE -ne 0) { throw "C5CTX_XSIM_FAIL" }
$ok = (Select-String -Path $xsimLog -Pattern "ASTRA_C5_FINAL_V1_CTX_UNK_XSIM_PASS" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_unrelated HIT" -Quiet)
if (-not $ok) { throw "C5CTX_MARKER_MISSING" }
Write-Host "ASTRA_C5_FINAL_V1_CTX_UNK_RUN_OK"
