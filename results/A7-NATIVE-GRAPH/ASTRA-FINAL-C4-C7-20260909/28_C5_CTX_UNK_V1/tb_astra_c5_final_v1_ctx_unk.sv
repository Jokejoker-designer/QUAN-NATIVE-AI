`timescale 1ns / 1ps
// ASTRA-C5-FINAL-V1 WO §15-05/06: wrong context + unrelated → UNKNOWN.
...
module tb_astra_c5_final_v1_ctx_unk;
  ...
  task automatic wait_until_done;
    begin
      cap_v = 0; cap_st = 4'hF; cap_ans = 20'd0; cap_p0 = 20'd0; cap_p1 = 20'd0;
      cap_pok = 0; cap_allow = 0;
      guard = 0;
      while (!done && guard < 2000000) begin
        @(posedge clk);
        guard = guard + 1;
        if (c3res && !cap_v) begin
          cap_st = c3st; cap_ans = c3ans; cap_p0 = c3p0; cap_p1 = c3p1;
          cap_pok = pok; cap_allow = allow; cap_v = 1'b1;
        end
      end
  ...
  task automatic one_query(input string q);
    begin
      ...
      join
      $display("MEAS_Q q=%s npin=%0d st=%0d ans=%0d p0=%0d p1=%0d pok=%0d allow=%0d",
               q, npin, cap_st, cap_ans, cap_p0, cap_p1, cap_pok, cap_allow);
  ...
  endtask
  task automatic plant_wrong_ctx;
    begin
      wr_f(E0, C3_W_SUBJ[0], MID, C3_W_REL[0], C3_TR_CONF_W[0], 1, 1, 8'd9);
      wr_f(E1, MID, DST0, C3_W_REL[0], C3_TR_CONF_W[0], 1, 1, 8'd9);
    end
  endtask
  ...
  initial begin
    ...
    $display("C5_FINAL_V1_CTX_UNK PROGRAM=NO C5_MASTER=OPEN BOARD_PASS=OPEN");
    guard = 0;
    ...
    if ((cap_st == A7NG_C3_ST_ANSWER) && (cap_ans == DST0))
      $display("CLASS_base_two_hop HIT ans=%0d", cap_ans);
    else $display("CLASS_base_two_hop MISS st=%0d ans=%0d", cap_st, cap_ans);

    plant_wrong_ctx();
    one_query("pump requires indirect");
    $display("MEAS_CTX st=%0d ans=%0d p0=%0d pok=%0d allow=%0d fctx=9",
             cap_st, cap_ans, cap_p0, cap_pok, cap_allow);
    chk_safe("CTX_SAFE");
    if ((cap_st != A7NG_C3_ST_ANSWER) && (cap_ans != DST0) && !cap_pok && !cap_allow)
      $display("CLASS_wrong_context HIT st=%0d ans=%0d", cap_st, cap_ans);
    else $display("CLASS_wrong_context MISS st=%0d ans=%0d pok=%0d allow=%0d",
                  cap_st, cap_ans, cap_pok, cap_allow);

    one_query("chiller requires valve");
    $display("MEAS_UNK st=%0d ans=%0d pok=%0d allow=%0d",
             cap_st, cap_ans, cap_pok, cap_allow);
    chk("UNK_ST", cap_st == A7NG_C3_ST_UNKNOWN);
    chk("UNK_NO_PROOF", (cap_ans == 20'd0) && (cap_p0 == 20'd0) && !cap_pok);
    chk("UNK_NO_ALLOW", !cap_allow);
    chk_safe("UNK_SAFE");
    if ((cap_st == A7NG_C3_ST_UNKNOWN) && !cap_pok && !cap_allow)
      $display("CLASS_unrelated HIT st=%0d", cap_st);
    else $display("CLASS_unrelated MISS st=%0d ans=%0d", cap_st, cap_ans);
    ...
    if (fail == 0) $display("ASTRA_C5_FINAL_V1_CTX_UNK_XSIM_PASS");
    else $display("ASTRA_C5_FINAL_V1_CTX_UNK_XSIM_FAIL n=%0d first=%s", fail, first_div);
