`timescale 1ns / 1ps
// ASTRA-C5-FINAL-V1 WO §15-11: scalar reward changes later ranking/decision, once after reload.
...
module tb_astra_c5_final_v1_rew_rank;
  ...
  task automatic plant_two_rank;
    ...
    begin
      ...
      b0 = (b0 << 32) | E0;
      mem_wr(POST_HEAP, b0);
      mem_wr(dir_addr(0, C3_W_K0[0]), dir_pack(4));
      mem_wr(dir_addr(1, C3_W_K1[0]), dir_pack(4));
      mem_wr(dir_addr(2, C3_W_K2[0]), dir_pack(4));
      wr_f(E0, C3_W_SUBJ[0], MID, C3_W_REL[0], C3_TR_CONF_W[0], 1, 1, C3_TR_CX1);
      wr_f(E1, MID, DST0, C3_W_REL[0], C3_TR_CONF_W[0], 1, 1, C3_TR_CX2);
      wr_f(E2, C3_W_SUBJ[0], MIDN, C3_W_REL[0], C3_TR_CONF_W[4], 1, 1, A7NG_C3_CTX_INDIRECT);
      wr_f(E3, MIDN, DSTB, C3_W_REL[0], C3_TR_CONF_W[4], 1, 1, A7NG_C3_CTX_INDIRECT);
    end
  endtask
  ...
  initial begin
    ...
    $display("C5_FINAL_V1_REW_RANK PROGRAM=NO C5_MASTER=OPEN BOARD_PASS=OPEN");
    guard = 0;
    ...
    if ((cap_st == A7NG_C3_ST_ANSWER) && (cap_p0 == E0) && (cap_ans == DST0))
      $display("CLASS_base_two_hop HIT ans=%0d p0=%0d p1=%0d", cap_ans, cap_p0, cap_p1);
    else $display("CLASS_base_two_hop MISS st=%0d ans=%0d p0=%0d", cap_st, cap_ans, cap_p0);

    uart_rew(txn, gen, -8'sd3, 1'b0, 16'd7);
    wait_next_done();
    $display("MEAS_REW nupd=%0d pcmt=%0d pvalid=%0d pph=%0d w0=%0d w1=%0d w12=%0d w13=%0d",
             nupd, pcmt, pvalid, pph, cw[0], cw[1], cw[12], cw[13]);
    chk("MINUS3_NUPD", nupd == 16'd1);
    chk("MINUS3_CMT", pcmt == 1'b1);
    chk("MINUS3_PERS", pvalid === 1'b1);
    chk("MINUS3_W", w_nz());
    if (nupd == 16'd1) $display("CLASS_rew_minus3 HIT");
    else $display("CLASS_rew_minus3 MISS nupd=%0d", nupd);
    if (w_nz()) $display("CLASS_rew_w_changed HIT w0=%0d w12=%0d w13=%0d", cw[0], cw[12], cw[13]);
    else $display("CLASS_rew_w_changed MISS");
    snap_w();

    one_query("pump requires indirect");
    $display("MEAS_Q2 st=%0d ans=%0d p0=%0d p1=%0d pok=%0d nupd=%0d",
             cap_st, cap_ans, cap_p0, cap_p1, cap_pok, nupd);
    chk("Q2_C3", (cap_st == A7NG_C3_ST_ANSWER) && cap_pok);
    chk("Q2_FLIP", (cap_ans == DSTB) && (cap_p0 == E2) && (cap_p1 == E3));
    chk("Q2_NOT_Q1", (cap_ans != DST0) && (cap_p0 != E0));
    chk_safe("Q2_SAFE_NO_DICT");
    chk("Q2_NUPD", nupd == 16'd1);
    if ((cap_st == A7NG_C3_ST_ANSWER) && (cap_ans == DSTB) && (cap_p0 == E2) && (cap_p1 == E3))
      $display("CLASS_rew_rank_flip HIT ans=%0d p0=%0d p1=%0d", cap_ans, cap_p0, cap_p1);
    else $display("CLASS_rew_rank_flip MISS st=%0d ans=%0d p0=%0d p1=%0d",
                  cap_st, cap_ans, cap_p0, cap_p1);

    uart_byte(A7NG_C5P_CMD_FLUSH); uart_byte(A7NG_C5P_EOL);
    wait_next_done();
    $display("MEAS_FLUSH w_zero=%0d w0=%0d crest=%0d", w_zero(), cw[0], crest);
    chk("FLUSH_ZERO", w_zero());

    uart_byte(A7NG_C5P_CMD_RELOAD); uart_byte(A7NG_C5P_EOL);
    wait_next_done();
    $display("MEAS_RLD crest=%0d weq=%0d w0=%0d fail=%0d", crest, w_eq(), cw[0], cfail);
    chk("RLD_RESTORE", crest === 1'b1);
    chk("RLD_EXACT32", w_eq());

    one_query("pump requires indirect");
    $display("MEAS_Q3 st=%0d ans=%0d p0=%0d p1=%0d pok=%0d nupd=%0d",
             cap_st, cap_ans, cap_p0, cap_p1, cap_pok, nupd);
    chk("Q3_C3", (cap_st == A7NG_C3_ST_ANSWER) && cap_pok);
    chk("Q3_SAME_AS_Q2", (cap_ans == DSTB) && (cap_p0 == E2) && (cap_p1 == E3));
    chk_safe("Q3_SAFE_NO_DICT");
    if ((cap_st == A7NG_C3_ST_ANSWER) && (cap_ans == DSTB) && (cap_p0 == E2) && (cap_p1 == E3))
      $display("CLASS_rew_after_reload HIT ans=%0d p0=%0d", cap_ans, cap_p0);
    else $display("CLASS_rew_after_reload MISS st=%0d ans=%0d p0=%0d", cap_st, cap_ans, cap_p0);
    ...
    if (fail == 0) $display("ASTRA_C5_FINAL_V1_REW_RANK_XSIM_PASS");
    else $display("ASTRA_C5_FINAL_V1_REW_RANK_XSIM_FAIL n=%0d first=%s", fail, first_div);
