...
$files = @(
  ...
  (Join-Path $bag "tb_astra_c5_final_v1_rew_rank.sv")
)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5REW_LIVE_PROD_TOP" }
  if ($leaf -eq "a7ng_astra_c4_lm06_grounded_gen.sv") { throw "C5REW_GROUNDED_GEN" }
}
...
& "$bin\xelab.bat" tb_astra_c5_final_v1_rew_rank -s c5rewv1 -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) { throw "C5REW_XELAB_FAIL" }
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c5rewv1 -R -log $xsimLog
if ($LASTEXITCODE -ne 0) { throw "C5REW_XSIM_FAIL" }
$ok = (Select-String -Path $xsimLog -Pattern "ASTRA_C5_FINAL_V1_REW_RANK_XSIM_PASS" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_rew_rank_flip HIT" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_rew_after_reload HIT" -Quiet)
if (-not $ok) { throw "C5REW_MARKER_MISSING" }
Write-Host "ASTRA_C5_FINAL_V1_REW_RANK_RUN_OK"
