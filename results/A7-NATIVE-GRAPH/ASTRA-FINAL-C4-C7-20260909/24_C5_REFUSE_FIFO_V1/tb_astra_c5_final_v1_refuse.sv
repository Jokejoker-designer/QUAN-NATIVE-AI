`timescale 1ns / 1ps
// ASTRA-C5-FINAL-V1 WO §15 refuse + FIFO stream. PROGRAM=NO. Not C5_MASTER.
// Modeled AXI. TB alias is synthetic Confirm-V3 on C3 endpoints. PROD_DICT=OPEN.
...
module tb_astra_c5_final_v1_refuse;
  ...
  localparam int E0 = 16, E1 = 17, E2 = 20, MID = 32'h30, DST2 = 64, VALVE = 11;
  ...
  initial begin
    ...
    $display("C5_FINAL_V1_REFUSE PROGRAM=NO C5_MASTER=OPEN BOARD_PASS=OPEN");
    $display("MEAS_HIER CLK_HZ_DEFAULT=%0d BAUD_DEFAULT=%0d SIM_CLK=%0d SIM_BAUD=%0d",
             A7NG_C5P_CLK_HZ, A7NG_C5P_BAUD, SIM_CLK_HZ, SIM_BAUD);
    chk("HIER_CLK_83333333", A7NG_C5P_CLK_HZ == 83333333);
    chk("HIER_BAUD_115200", A7NG_C5P_BAUD == 115200);
    chk("SIM_OVERRIDE_NOT_IN_SVH", (SIM_CLK_HZ != A7NG_C5P_CLK_HZ) && (SIM_BAUD != A7NG_C5P_BAUD));
    guard = 0;
    ...
    one_query("pump valve requires");
    chk("AMB_C3", cap_st == A7NG_C3_ST_AMB);
    chk("AMB_NO_PROOF", !cap_pok && (cap_ans == 20'd0));
    chk_safe("AMB_SAFE");
    if (cap_st == A7NG_C3_ST_AMB) $display("CLASS_parser_amb HIT");
    else $display("CLASS_parser_amb MISS st=%0d", cap_st);

    one_query("pump not requires valve");
    chk("NEG_C3", cap_st == A7NG_C3_ST_NEG);
    chk("NEG_NO_PROOF", !cap_pok && (cap_ans == 20'd0));
    chk_safe("NEG_SAFE");
    if (cap_st == A7NG_C3_ST_NEG) $display("CLASS_parser_neg HIT");
    else $display("CLASS_parser_neg MISS st=%0d", cap_st);

    plant_index_ovf();
    one_query("pump requires indirect");
    chk("INCOMP_C3", cap_st == A7NG_C3_ST_INCOMP);
    chk("INCOMP_NO_PROOF", !cap_pok && (cap_ans == 20'd0));
    chk_safe("INCOMP_SAFE");
    if (cap_st == A7NG_C3_ST_INCOMP) $display("CLASS_search_incomp HIT");
    else $display("CLASS_search_incomp MISS st=%0d", cap_st);

    load_alias();
    one_query("pump requires valve");
    chk("F_ANSWER", (cap_st == A7NG_C3_ST_ANSWER) && (cap_ans == VALVE) && cap_pok);
    chk("FIFO_N", (npin == 5) && (nstr == 16'd5) && (novf == 16'd0));
    chk("FIFO_NOT_LASTGEN_ONLY", (npin > 1) && (glast == pinb[npin-1]));
    chk("FIFO_TIFH", (pinb[0]==8'd116) && (pinb[1]==8'd105) && (pinb[2]==8'd102)
        && (pinb[3]==8'd104) && (pinb[4]==8'd0));
    if ((npin == 5) && (nstr == 16'd5) && (novf == 16'd0))
      $display("CLASS_fifo_fullseq HIT nstr=%0d npin=%0d", nstr, npin);
    else $display("CLASS_fifo_fullseq MISS nstr=%0d npin=%0d novf=%0d", nstr, npin, novf);
    ...
    if (fail == 0) $display("ASTRA_C5_FINAL_V1_REFUSE_XSIM_PASS");
    else $display("ASTRA_C5_FINAL_V1_REFUSE_XSIM_FAIL n=%0d first=%s", fail, first_div);
