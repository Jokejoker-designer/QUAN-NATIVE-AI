`timescale 1ns / 1ps
// ASTRA-C5-FINAL-V1 UART 4-case smoke. PROGRAM=NO. Not C5_MASTER. Not BOARD.
// Modeled AXI (RRESP/RLAST/RID), pin UART. TB alias is synthetic Confirm-V3
// 4-char on real C3 endpoint IDs (zero-ext QSE subj, 20-bit ans). PROD_DICT=OPEN.
...
module tb_astra_c5_final_v1_uart_smoke;
  ...
  initial begin
    ...
    $display("C5_FINAL_V1_UART_SMOKE PROGRAM=NO C5_MASTER=OPEN BOARD_PASS=OPEN");
    ...
    chk("MISS_C3_ANSWER", (cap_st == A7NG_C3_ST_ANSWER) && (cap_ans == VALVE) && (cap_p0 == E2) && (cap_p1 == 20'd0) && cap_pok);
    chk("MISS_NOT_ALLOWED", !cap_allow && !cap_aok);
    chk_pin("ALIAS_MISS_SAFE", 3, 8'h6E, 8'h6F, 8'd0, 8'd0, 8'd0);

    load_alias();
    bank_r = 1'b0;
    one_query("pump requires valve");
    chk("F_C3_ANSWER", (cap_st == A7NG_C3_ST_ANSWER) && (cap_ans == VALVE) && cap_pok);
    chk("F_ALLOWED", cap_allow && cap_aok);
    chk_pin("F_LEARNED", 5, 8'd116, 8'd105, 8'd102, 8'd104, 8'd0);

    bank_r = 1'b1;
    one_query("pump requires valve");
    chk("R_C3_ANSWER", (cap_st == A7NG_C3_ST_ANSWER) && (cap_ans == VALVE) && cap_pok);
    chk("R_ALLOWED", cap_allow && cap_aok);
    chk("R_EXPLICIT_PIN", bank_r === 1'b1);
    ...
    if (fail == 0) begin
      $display("CLASS_host_tok0 HIT");
      $display("ASTRA_C5_FINAL_V1_UART_SMOKE_XSIM_PASS");
    end else begin
      $display("CLASS_host_tok0 MISS first=%s", first_div);
      $display("ASTRA_C5_FINAL_V1_UART_SMOKE_XSIM_FAIL n=%0d first=%s", fail, first_div);
    end
