...
$files = @(
  ...
  (Join-Path $bag "tb_astra_c5_final_v1_axi_err.sv")
)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5AXI_LIVE_PROD_TOP" }
  if ($leaf -eq "a7ng_astra_c4_lm06_grounded_gen.sv") { throw "C5AXI_GROUNDED_GEN" }
}
...
if ($LASTEXITCODE -ne 0) { throw "C5AXI_XVLOG_FAIL" }
& "$bin\xelab.bat" tb_astra_c5_final_v1_axi_err -s c5axiv1 -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) { throw "C5AXI_XELAB_FAIL" }
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c5axiv1 -R -log $xsimLog
if ($LASTEXITCODE -ne 0) { throw "C5AXI_XSIM_FAIL" }
$ok = (Select-String -Path $xsimLog -Pattern "ASTRA_C5_FINAL_V1_AXI_ERR_XSIM_PASS" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_adp_not_okay HIT" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_bresp_slverr HIT" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_rresp_slverr HIT" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_okay_persist_reload HIT" -Quiet)
if (-not $ok) { throw "C5AXI_MARKER_MISSING" }
Write-Host "ASTRA_C5_FINAL_V1_AXI_ERR_RUN_OK"
