`timescale 1ns / 1ps
// ASTRA-C5-FINAL-V1 AXI RRESP/BRESP/RID. PROGRAM=NO. Not C5_MASTER.
// Same class sequence as 04B, DUT is prod_top_final_v1 (not sibling resp).
...
module tb_astra_c5_final_v1_axi_err;
  ...
  int n_ar, n_r, n_aw, n_w, n_b, aw_cnt, w_cnt, b_wait;
  int inj_aw_stall, inj_w_stall, inj_b_stall;
  ...
  function automatic logic [1:0] rresp_of(input logic [27:0] a);
    if (a >= A7NG_C5P_BASE) rresp_of = inj_rresp_ckpt;
  ...
  initial begin
    ...
    $display("C5_FINAL_V1_AXI_ERR PROGRAM=NO C5_MASTER=OPEN BOARD_PASS=OPEN DUT=final_v1");
    guard = 0;
    ...
    inj_rresp_fact = SLVERR;
    uart_str("pump requires indirect");
    wait_until_done();
    $display("MEAS_ADP last_c3rr=%0d st=%0d n_ar=%0d n_r=%0d", last_c3rr, c3st, n_ar, n_r);
    chk("ADP_SLVERR", last_c3rr === SLVERR);
    if (last_c3rr === SLVERR) $display("CLASS_adp_not_okay HIT last_c3rr=2");
    ...
    chk("BERR_FAIL", cfail == A7NG_C5P_F_BRESP);
    chk("BERR_NOPERS", pvalid === 1'b0);
    chk("BERR_LAT", last_br === SLVERR);
    if ((cfail == A7NG_C5P_F_BRESP) && !pvalid && (last_br === SLVERR))
    ...
    chk("RERR_FAIL", cfail == A7NG_C5P_F_RRESP);
    chk("RERR_NOREST", crest === 1'b0);
    chk("RERR_NOINST", w_zero());
    chk("RERR_LAT", last_ckrr === SLVERR);
    if ((cfail == A7NG_C5P_F_RRESP) && !crest && w_zero() && (last_ckrr === SLVERR))
    ...
    $display("MEAS_COUNTS n_ar=%0d n_r=%0d n_aw=%0d n_w=%0d n_b=%0d nht=%0d nrok=%0d",
             n_ar, n_r, n_aw, n_w, n_b, nht, nrok);
    ...
    if (fail == 0) $display("ASTRA_C5_FINAL_V1_AXI_ERR_XSIM_PASS");
    else $display("ASTRA_C5_FINAL_V1_AXI_ERR_XSIM_FAIL n=%0d first=%s", fail, first_div);
