...
$files = @(
  ...
  (Join-Path $bag "tb_astra_c5_final_v1_e1a_dir.sv")
)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5E1A_LIVE_PROD_TOP" }
  if ($leaf -eq "a7ng_astra_c4_lm06_grounded_gen.sv") { throw "C5E1A_GROUNDED_GEN" }
}
...
if ($LASTEXITCODE -ne 0) { throw "C5E1A_XVLOG_FAIL" }
& "$bin\xelab.bat" tb_astra_c5_final_v1_e1a_dir -s c5e1adir -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) { throw "C5E1A_XELAB_FAIL" }
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c5e1adir -R -log $xsimLog
if ($LASTEXITCODE -ne 0) { throw "C5E1A_XSIM_FAIL" }
$ok = (Select-String -Path $xsimLog -Pattern "ASTRA_C5_FINAL_V1_E1A_DIR_XSIM_PASS" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "PASS F_LEARNED" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "PASS R_LEARNED" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "PASS F_AGAIN" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "PASS UNK_SAFE" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "PASS R_BANK_PIN0" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_e1a_opcode HIT" -Quiet)
if (-not $ok) { throw "C5E1A_MARKER_MISSING" }
Write-Host "ASTRA_C5_FINAL_V1_E1A_DIR_RUN_OK"
