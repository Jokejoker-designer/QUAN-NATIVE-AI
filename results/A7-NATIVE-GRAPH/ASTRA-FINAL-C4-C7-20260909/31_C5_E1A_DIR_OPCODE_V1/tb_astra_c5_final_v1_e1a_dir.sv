`timescale 1ns / 1ps
// E1a NEW GATE: F KEEP SVO + inverse REL-first. bank_r_i=0 both queries.
...
module tb_astra_c5_final_v1_e1a_dir;
  ...
  initial begin
    ...
    $display("C5_FINAL_V1_E1A_DIR PROGRAM=NO C5_MASTER=OPEN BOARD_PASS=OPEN");
    ...
    $display("E1A_LAW REL_FIRST OP_R=2d1 bank_r_i=0 both queries");
    guard = 0;
    ...
    chk("R_BANK_PIN0", bank_r === 1'b0);
    chk_pin("R_LEARNED", 5, 8'd118, 8'd109, 8'd103, 8'd104, 8'd0);

    bank_r = 1'b0;
    one_query("pump requires valve");
    chk("F2_C3_ANSWER", (cap_st == A7NG_C3_ST_ANSWER) && (cap_ans == VALVE) && cap_pok);
    chk("F2_ALLOWED", cap_allow && cap_aok);
    chk_pin("F_AGAIN", 5, 8'd116, 8'd105, 8'd102, 8'd104, 8'd0);
    ...
    if (fail == 0) begin
      $display("CLASS_e1a_opcode HIT");
      $display("ASTRA_C5_FINAL_V1_E1A_DIR_XSIM_PASS");
    end else begin
      $display("CLASS_e1a_opcode MISS first=%s", first_div);
      $display("ASTRA_C5_FINAL_V1_E1A_DIR_XSIM_FAIL n=%0d first=%s", fail, first_div);
    end
