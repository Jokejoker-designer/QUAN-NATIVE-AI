...
$files = @(
  ...
  (Join-Path $bag "tb_astra_c5_final_v1_elab.sv")
)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5FIN_LIVE_PROD_TOP" }
  if ($leaf -eq "a7ng_astra_c4_lm06_grounded_gen.sv") { throw "C5FIN_GROUNDED_GEN" }
}
...
if ($LASTEXITCODE -ne 0) { throw "C5FIN_XVLOG_FAIL" }
& "$bin\xelab.bat" tb_astra_c5_final_v1_elab -s c5finv1 -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) { throw "C5FIN_XELAB_FAIL" }
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c5finv1 -R -log $xsimLog
if ($LASTEXITCODE -ne 0) { throw "C5FIN_XSIM_FAIL" }
$ok = (Select-String -Path $xsimLog -Pattern "ASTRA_C5_FINAL_V1_ELAB_OK" -Quiet)
if (-not $ok) { throw "C5FIN_MARKER_MISSING" }
Write-Host "ASTRA_C5_FINAL_V1_ELAB_RUN_OK"
Write-Host "C5_MASTER=OPEN C6_SWITCH=NO PROGRAM=NO"
