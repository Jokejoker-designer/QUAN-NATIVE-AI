...
$files = @(
  ...
  (Join-Path $bag "tb_astra_c5_final_v1_regress.sv")
)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5REG_LIVE_PROD_TOP" }
  if ($leaf -eq "a7ng_astra_c4_lm06_grounded_gen.sv") { throw "C5REG_GROUNDED_GEN" }
}
...
if ($LASTEXITCODE -ne 0) { throw "C5REG_XVLOG_FAIL" }
& "$bin\xelab.bat" tb_astra_c5_final_v1_regress -s c5regv1 -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) { throw "C5REG_XELAB_FAIL" }
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c5regv1 -R -log $xsimLog
if ($LASTEXITCODE -ne 0) { throw "C5REG_XSIM_FAIL" }
$ok = (Select-String -Path $xsimLog -Pattern "ASTRA_C5_FINAL_V1_REGRESS_XSIM_PASS" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_base_two_hop HIT" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_rew_dup HIT" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_rew_stale HIT" -Quiet) -and
      (Select-String -Path $xsimLog -Pattern "CLASS_conflict HIT" -Quiet)
if (-not $ok) { throw "C5REG_MARKER_MISSING" }
Write-Host "ASTRA_C5_FINAL_V1_REGRESS_RUN_OK"
