`timescale 1ns / 1ps
// ASTRA-C5-FINAL-V1 WO §15 subset: 2-hop proof, CONFLICT refusal, dup/stale reward.
...
module tb_astra_c5_final_v1_regress;
  ...
  initial begin
    ...
    $display("C5_FINAL_V1_REGRESS PROGRAM=NO C5_MASTER=OPEN BOARD_PASS=OPEN");
    guard = 0;
    ...
    chk("TWOHOP_C3", (cap_st == A7NG_C3_ST_ANSWER) && (cap_ans == DST0) && (cap_p0 == E0) && (cap_p1 == E1) && cap_pok);
    chk_safe("TWOHOP_SAFE_NO_DICT");
    if ((cap_st == A7NG_C3_ST_ANSWER) && (cap_p0 == E0) && (cap_p1 == E1))
    ...
    if (nupd == 16'd1) $display("CLASS_rew_plus3 HIT");
    else $display("CLASS_rew_plus3 MISS nupd=%0d", nupd);
    snap_w();

    uart_rew(txn, gen, 8'sd3, 1'b0, 16'd7);
    guard = 0;
    while ((ndup == ndup_a[15:0]) && guard < 40000) begin @(posedge clk); guard = guard + 1; end
    ...
    guard = 0;
    while ((ncrc == ncrc_a[15:0]) && guard < 40000) begin @(posedge clk); guard = guard + 1; end
    ...
    guard = 0;
    while ((nbad == nbad_a[15:0]) && guard < 40000) begin @(posedge clk); guard = guard + 1; end
    ...
    plant_neg_same_dest();
    one_query("pump requires indirect");
    chk("CONFLICT_C3", cap_st == A7NG_C3_ST_CONFLICT);
    chk("CONFLICT_NO_PROOF", (cap_ans == 20'd0) && (cap_p0 == 20'd0) && !cap_pok);
    chk_safe("CONFLICT_SAFE");
    chk("CONFLICT_NUPD", nupd == 16'd1);
    if (cap_st == A7NG_C3_ST_CONFLICT) $display("CLASS_conflict HIT");
    else $display("CLASS_conflict MISS st=%0d", cap_st);
    ...
    if (fail == 0) $display("ASTRA_C5_FINAL_V1_REGRESS_XSIM_PASS");
    else $display("ASTRA_C5_FINAL_V1_REGRESS_XSIM_FAIL n=%0d first=%s", fail, first_div);
