...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C4G20_FILE_MISSING $p" }
...
if ($hashFail) { throw "C4G20_HASH_GATE_FAIL do_not_edit_keep_rtl" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c4_lm06_grounded_gen_heldout20.sv"
...
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C4G20_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C4G20_GOLD_HASH_DRIFT GOLDEN.json" }

$files = @($b256, $dut, $tb)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C4G20_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C4G20_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C4G20_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C4G20_LIVE_PROD_TOP_AS_DUT" }
}
...
Write-Host "C4G20_SHA_FROZEN"

if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null
Set-Location $work
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C4G20_XVLOG_NOT_FOUND" }
& "$bin\xvlog.bat" --sv -i $inci -i $bag $files
if ($LASTEXITCODE -ne 0) { throw "C4G20_XVLOG_FAIL" }
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
& "$bin\xelab.bat" tb_astra_c4_lm06_grounded_gen_heldout20 -s c4g20 -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) {
  if (Test-Path (Join-Path $work "xelab.log")) {
    Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
  }
  throw "C4G20_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c4g20 -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C4_LM06_GROUNDED_GEN_HELDOUT20_XSIM_PASS" -Quiet)
  $hasAcc = [bool](Select-String -Path $xsimLog -Pattern "CLASS_grounded_acc_ge90 HIT acc=20/20" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass) -or (-not $hasAcc)) {
  ...
  throw "C4G20_XSIM_FAIL_OR_MARKER_MISSING pass=$hasPass acc20=$hasAcc"
}
Write-Host "ASTRA_C4_LM06_GROUNDED_GEN_HELDOUT20_RUN_OK"
