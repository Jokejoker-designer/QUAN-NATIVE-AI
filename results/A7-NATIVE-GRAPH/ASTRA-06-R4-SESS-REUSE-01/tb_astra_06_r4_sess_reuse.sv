`timescale 1ns / 1ps
// ASTRA-06-R4-SESS-REUSE-01. PROGRAM=NO. Bag-local TB.
// Host sess_id reuse after rst without restore is n_stale/refuse;
// matching-sess restore still commits the snapshot.
module tb_astra_06_r4_sess_reuse;
  ...
  localparam logic [15:0] SESS_A  = 16'd7;
  localparam logic [15:0] SESS_B  = 16'd9;
  ...
  logic p_valid, p_acc, p_cmt, rl_busy, vic_v, sess_bound;
  ...
  logic [15:0] live_ep, sess_id, rew_epoch, epoch, nexh, p_epoch, p_sess;
  logic [15:0] s0_sess, s1_sess;
  logic [15:0] nref, nevict, nsch, nrlauto, nrlcmd, nreuse, nrej;
  ...
  logic [15:0] nupd0, nstale0, nreuse0, nrej0, nrlcmd0;
  ...
  a7ng_astra_06_r4_sess_reuse u_dut (
    .clk(clk), .rst_n(rst_n),
    ...
    .persist_sess_o(p_sess), .s0_sess_o(s0_sess), .s1_sess_o(s1_sess),
    .sess_bound_o(sess_bound), .n_sess_reuse_o(nreuse), .n_sess_rej_o(nrej)
  );
  ...
  task automatic dump(input string tag);
    $display("%s st=%0d npath=%0d ans=0x%05h p0=0x%05h acc=%0d cmt=%0d txn=%0d gen=%0d ep=%0d nupd=%0d nstale=%0d nbad=%0d w0=%0d phi0=%0d ost=%0d tbl=%0d bound=%0d psess=%0d s0sess=%0d s1sess=%0d nreuse=%0d nrej=%0d nrlcmd=%0d nrlauto=%0d s0v=%0d s0c=%0d s0txn=%0d s0phi=%0d s0w0=%0d s0ans=0x%05h s0p0=0x%05h s1v=%0d s1c=%0d s1txn=%0d s1phi=%0d s1w0=%0d s1ans=0x%05h s1p0=0x%05h",
      tag, st, npath, ans, p0, pend_acc, pend_cmt, txn, gen, epoch, nupd, nstale, nbad, wdut[0], phi0, axi_ost, tbl,
      sess_bound, p_sess, s0_sess, s1_sess, nreuse, nrej, nrlcmd, nrlauto,
  ...
  endtask
  task automatic pulse_reload_try;
    begin
      @(posedge clk); reload_req<=1;
      @(posedge clk); reload_req<=0;
  ...
  initial begin
    ...
    rew_v=0; rew=0; rew_txn=0; rew_gen=0; rew_epoch=0; live_ep=16'd7; sess_id=SESS_A;
    ...
    persist_en=1; restore_en=0; schema=SCH_OK; live_ep=16'd7; sess_id=SESS_A; slot_sel=2'd0;
    wipe_rst();
    plant_a(); q_text();
    dump("PICK_A");
    chk("PICK_A", (st===ST_ANSWER) && !tbl && (npath>=5'd2) && pend_acc && !pend_cmt
        && (txn===8'd1) && (gen===8'd1) && (epoch===SESS_A) && (phi0===8'sd50)
        && highid_a() && (s0_p1===HID_P1A) && (s0_w0===16'sd0) && live_a()
        && (s0_sess===SESS_A) && (p_sess===SESS_A) && sess_bound && s0v && !s0c);
    sav_ep=epoch; sav_txn=txn; sav_gen=gen;

    power_loss();
    dump("RST_LIVE");
    chk("RST_LIVE_CLEAR", (wdut[0]===16'sd0) && !axi_ost && !pend_acc && !sess_bound
        && s0v && !s0c && (s0_txn===sav_txn) && (s0_sess===SESS_A) && highid_a()
        && (nrlcmd===16'd0) && (nrlauto===16'd0) && (nreuse===16'd0));

    nstale0=nstale; nupd0=nupd;
    pulse_rew_id(-3, sav_ep, sav_txn, sav_gen);
    repeat(20) @(posedge clk);
    dump("REUSE_REW_STALE");
    chk("REUSE_REW_STALE", (nstale===nstale0+16'd1) && (nupd===nupd0) && !pend_acc
        && !pend_cmt && (wdut[0]===16'sd0) && (s0_w0===16'sd0) && !s0c
        && (s0_txn===sav_txn) && (s0_sess===SESS_A) && highid_a());

    nreuse0=nreuse;
    q_text();
    dump("REUSE_PICK_REFUSE");
    chk("REUSE_PICK_REFUSE", (nreuse===nreuse0+16'd1) && !pend_acc && !pend_cmt
        && (s0_txn===sav_txn) && (s0_sess===SESS_A) && (s0_w0===16'sd0) && !s0c
        && (txn!==sav_txn || !pend_acc) && (wdut[0]===16'sd0) && !sess_bound);
    retire_q();

    nrej0=nrej; nrlcmd0=nrlcmd;
    sess_id=SESS_B;
    pulse_reload_try();
    dump("RELOAD_MISMATCH");
    chk("RELOAD_MISMATCH", (nrej===nrej0+16'd1) && (nrlcmd===nrlcmd0) && !pend_acc
        && (wdut[0]===16'sd0) && (s0_sess===SESS_A) && (s0_txn===sav_txn)
        && (s0_w0===16'sd0) && !sess_bound && highid_a());

    sess_id=SESS_A; slot_sel=2'd0;
    nrlcmd0=nrlcmd;
    pulse_reload();
    dump("MATCH_RELOAD");
    chk("MATCH_RELOAD", (nrlcmd===nrlcmd0+16'd1) && (nrlauto===16'd0)
        && pend_acc && !pend_cmt && (txn===sav_txn) && (gen===sav_gen)
        && (epoch===sav_ep) && (phi0===8'sd50) && live_a() && sess_bound
        && (s0_sess===SESS_A) && (wdut[0]===16'sd0));

    nupd0=nupd;
    pulse_rew_id(-3, sav_ep, sav_txn, sav_gen);
    repeat(250) @(posedge clk);
    dump("MATCH_REW");
    chk("MATCH_REW", (nupd===nupd0+16'd1) && pend_cmt && (wdut[0]===-16'sd5)
        && s0c && (s0_w0===-16'sd5) && (s0_phi0===8'sd50) && highid_a()
        && (s0_sess===SESS_A) && sess_bound);
    retire_q();

    restore_en=0; persist_en=1;
    power_loss();
    sess_id=SESS_B; slot_sel=2'd0;
    plant_b(); q_text();
    dump("NEW_SESS_NO_PULL");
    chk("NEW_SESS_NO_PULL", pend_acc && !pend_cmt && (epoch===SESS_B) && (txn===8'd1)
        && (wdut[0]===16'sd0) && s0c && (s0_w0===-16'sd5) && (s0_sess===SESS_A)
        && highid_a() && (s1_sess===SESS_B) && (s1_w0===16'sd0) && s1v && !s1c
        && (s0_w0 !== wdut[0] || s0_w0===-16'sd5));

    nstale0=nstale; nupd0=nupd;
    pulse_rew_id(-3, sav_ep, sav_txn, sav_gen);
    repeat(20) @(posedge clk);
    dump("NEW_SESS_OLD_REW");
    chk("NEW_SESS_OLD_REW", (nstale===nstale0+16'd1) && (nupd===nupd0) && !pend_cmt
        && (s0_w0===-16'sd5) && (s0_sess===SESS_A) && (wdut[0]===16'sd0)
        && (epoch===SESS_B));
    retire_q();

    persist_en=1; restore_en=1; slot_sel=2'd0; schema=SCH_OK; sess_id=SESS_A;
    wipe_rst();
    plant_a(); q_text();
    power_loss();
    dump("UNREL_PRE");
    ...
    if (fail==0) $display("ASTRA_06_R4_SESS_REUSE_XSIM_PASS");
    else $display("ASTRA_06_R4_SESS_REUSE_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $finish;
  end
endmodule
