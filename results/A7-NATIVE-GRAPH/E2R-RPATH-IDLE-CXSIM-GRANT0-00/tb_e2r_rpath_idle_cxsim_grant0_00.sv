// E2R-RPATH-IDLE-CXSIM-GRANT0-00 — mux vehicle with TB B1 grant held 0
// Vehicle: copy of mux bag + a7ng_axi_read_cdc + B1 grant register STAYS 0
// + SoC mux (ungated cdc_rvalid=rvalid) + ONE shared stub. WDMA on same R bus.
// UNKNOWN: with grant=0 from reset, does DUT-driven dbg_tile_dst reach 4?
// Do not force dst / TILE_DST. Do not assign r_path_idle. Do not apply C-FIX.
// Do not instantiate soc_top or mig_native_wrap. XSim ≠ board.
`timescale 1ns/1ps

module tb_e2r_rpath_idle_cxsim_grant0_00;
  import a7ng_pkg::*;
  ...
  task automatic dump_row(input string phase_s);
    $display("PROBE t=%0t phase=%s dst=%0d drain=%0b fifo=%0d c_rvalid=%0b tr=%0d idle=%0b own=%0b grant=%0b own_ui=%0b cdc_rv=%0b stub_rv=%0b go=%0b busy=%0b rready=%0b rleft=%0d wst=%0d miss=%0b reqs1=%0b bst=%0d stall=%0b soa_done=%0b run=%0b fwd=%0b core_busy=%0b phase=%0d gv=%0d",
      ...
      phase, gv_count);
    $fdisplay(pf, "%0t,%s,%0d,%0b,%0d,%0b,%0d,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0d,%0b,%0b,%0d",
      $time, phase_s, tile_dst, p_drain, p_fifo, p_rvalid, p_tr, r_path_idle,
      wdma_owner, wdma_owner_grant, wdma_owner_ui, cdc_rvalid, rvalid,
  ...
  initial begin
    ...
    if (pf == 0) begin
      $display("E2R_RPATH_IDLE_CXSIM_GRANT0_00_XSIM_FAIL reason=probe_fopen");
      $fatal(1);
    end
    $fdisplay(pf, "t_ns,phase,tile_dst,r_drain_hold,fifo_cnt,m_axi_rvalid,tr_cnt,r_path_idle,wdma_owner,wdma_owner_grant,wdma_owner_ui,cdc_rvalid,stub_rvalid,wdma_go,wdma_busy,r_left,soa_done,core_busy,lm_phase");
    ...
    $display("E2R-RPATH-IDLE-CXSIM-GRANT0-00 START");
    $display("VEHICLE=a7ng_native_v1_ab_core SIM_FULL=0 do_lm=1 CDC+B1_GRANT0+MUX ungated_cdc_rvalid SHARED_STUB_WDMA");
    $display("PREREG_UNKNOWN: on mux vehicle with TB holding grant=0, does DUT-driven dbg_tile_dst reach 4?");
    $display("PREREG_H_CANDIDATE: dest=4 with grant=0; then classify AND terms (ONE/SET/NONE)");
    $display("PREREG_H_RIVAL: dest never 4 (FAIL_NO_DESTWAIT_GRANT0) — silicon TILE_DST=4 AND GRANT=0 not reproduced");
    $display("PREREG_FALSIFIER: force dst; product RTL; C-FIX; soc_top+MIG only");
    $display("PREREG_UNIT: one query, grant=0 from reset. First dst=4 occupancy, or timeout without dst=4");
    $display("FORBIDDEN: force TILE_DST/dst; assign r_path_idle=1; C-FIX; soc_top; MIG; board");
    ...
    if (!destwait_seen || !grant_stayed_0) begin
      $display("XSIM=FAIL_NO_DESTWAIT_GRANT0");
      $display("E2R_RPATH_IDLE_CXSIM_GRANT0_00_FAIL_NO_DESTWAIT_GRANT0 dst=%0d max_dst=%0d bst=%0d miss=%0b reqs1=%0b go=%0b busy=%0b soa=%0b fwd=%0b grant=%0b grant_stayed_0=%0b own_ui=%0b",
        tile_dst, max_dst, tile_bst, tile_miss, tile_req_s1, wdma_go, wdma_busy, soa_done_seen, start_fwd_seen,
        wdma_owner_grant, grant_stayed_0, wdma_owner_ui);
      $finish;
    end

    $display("XSIM=PASS");
    $display("E2R_RPATH_IDLE_CXSIM_GRANT0_00_XSIM_PASS verdict=%s wire=%s c_fix=NONE idle=%0b n_hot=%0d dst=%0d grant_stayed_0=%0b",
      verdict_class, wire_named, snap_idle, n_hot, snap_dst, grant_stayed_0);
    $finish;
  end
  ...
  initial begin
    ...
    $display("WALL_LIMIT");
    $display("XSIM=LIMIT");
    $display("E2R_RPATH_IDLE_CXSIM_GRANT0_00_FAIL_NO_DESTWAIT_GRANT0 reason=wall dst=%0d grant=%0b",
