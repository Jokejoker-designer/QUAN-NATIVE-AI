`timescale 1ns / 1ps
// ASTRA-07-SCALE-MID-01. PROGRAM=NO. Bag-local TB.
// Instantiates frozen a7ng_astra_09_r2_cand_ovf (not leftover A09) + frozen SGD ISO.
// Posting N=64 > CAND_CAP=16; DUT itself INCOMP ans=0. Smoke after reset ans=4.
// Not 65536. Not 800k. Not BOARD.
module tb_astra_07_scale_mid;
  localparam logic [27:0] INDEX_BASE = 28'h0500_0000, POST_HEAP = 28'h0504_0000, FACT_BASE = 28'h0580_0000;
  localparam int SLOTS=160;
  localparam int CAND_CAP=16, MAX_PATH=4, OVF_PLANT_N=64;
  ...
  logic pend_acc, pend_cmt, load_v, axi_ost, axi_abort, txn_exh, r_ovf, w_ovf;
  logic [1:0] ctrl, sel;
  logic [7:0] tok, txn, gen, rew_txn, rew_gen, phi0, qobj, qctx;
  logic [15:0] live_ep, sess_id, rew_epoch, epoch, nexh, ntrunc;
  ...
  a7ng_astra_09_r2_cand_ovf u_dut (
  ...
  task automatic plant_ovf_cand;
    integer id, beat_i, base;
    begin
      reset_mem();
      mem_wr(POST_HEAP+28'd0, post_beat(17,34,18,35));
      for (beat_i=1; beat_i<16; beat_i=beat_i+1) begin
        base = 100 + (beat_i-1)*4;
        mem_wr(POST_HEAP+(beat_i*28'd16), post_beat(base, base+1, base+2, base+3));
      end
      ...
      mem_wr(FACT_BASE+(18<<4), fact_pack(10,8,2,18,8,1,1,0));
      mem_wr(FACT_BASE+(35<<4), fact_pack(8,4,2,35,8,1,1,0));
      for (id=100; id<=159; id=id+1)
        mem_wr(FACT_BASE+(id<<4), fact_pack(99,98,2,id,8,1,1,0));
    end
  endtask
  task automatic dump(input string tag);
    $display("%s st=%0d npath=%0d ans=%0d p0=%0d acc=%0d ntrunc=%0d rov=%0d wov=%0d hier_st=%0d hier_ans=%0d hier_best=%0d tbl=%0d abort=%0d ost=%0d w0=%0d",
      tag, st, npath, ans, p0, pend_acc, ntrunc, r_ovf, w_ovf,
      u_dut.r_st, u_dut.ans_o, u_dut.best_a, tbl, axi_abort, axi_ost, wdut[0]);
  endtask
  ...
  initial begin
    ...
    chk("OVF_CAND_CAP", (st===ST_INCOMP) && (ans===20'd0) && (p0===20'd0)
        && !pend_acc && !tbl && (ntrunc!=16'd0) && r_ovf && !w_ovf
        && (OVF_PLANT_N > CAND_CAP));
    chk("OVF_CAND_NO_STALE_ANS4", (ans!==20'd4) && (st!==ST_ANSWER) && (p0!==20'd17));
    chk("OVF_HIER_NO_ANS4", (u_dut.ans_o===20'd0) && (u_dut.best_a===20'd0)
        && (u_dut.r_st===ST_INCOMP) && (u_dut.ans_o!==20'd4) && (u_dut.best_a!==20'd4));
    chk("OVF_DUT_NTRUNC", (ntrunc!=16'd0) && r_ovf && !w_ovf && (st===ST_INCOMP));

    hard_rst(); plant2();
    send_text("pump requires indirect"); wait_done();
    dump("SMOKE_AFTER_RST");
    chk("SMOKE_AFTER_RST", (st===ST_ANSWER) && !tbl && (npath>=5'd2)
        && (p0===20'd17) && (ans===20'd4) && !r_ovf && (ntrunc===16'd0));
    retire_q();
    ...
    if (fail==0) $display("ASTRA_07_SCALE_MID_XSIM_PASS");
    else $display("ASTRA_07_SCALE_MID_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $finish;
  end
endmodule
