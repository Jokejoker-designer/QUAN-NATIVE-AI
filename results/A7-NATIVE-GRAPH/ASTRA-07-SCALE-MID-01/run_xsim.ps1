...
$frozenA09r2 = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_09_r2_cand_ovf.sv"
$expectA09 = "9fdbe0d642dd5f36d1c43626de71a125cfbf7725ffa9dd81e920ecfebb5c776c"
$expectA09r2 = "15a919f19226bad2c8dc87f7862246a3869643db022303338cca5f8d8b70ee23"
$liveA09 = (Get-FileHash -Algorithm SHA256 -LiteralPath $frozenA09).Hash.ToLowerInvariant()
$liveA09r2 = (Get-FileHash -Algorithm SHA256 -LiteralPath $frozenA09r2).Hash.ToLowerInvariant()
if ($liveA09 -ne $expectA09) { throw "A07MID_FROZEN_A09_HASH_CHANGED $liveA09" }
if ($liveA09r2 -ne $expectA09r2) { throw "A07MID_FROZEN_A09R2_HASH_CHANGED $liveA09r2" }
...
$files = @(
  ...
  $frozenA09r2,
  (Join-Path $bag "tb_astra_07_scale_mid.sv")
)
$includes = @(
...
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "A07MID_FILE_MISSING $p" }
...
$pre += "# PROVENANCE leftover A09 / NARROW wrap / SGD (not compiled as DUT; not edited)"
...
Write-Host "A07MID_SHA_FROZEN $shaPre"
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "A07MID_XVLOG_NOT_FOUND" }
...
$post += (ShaLine $frozenA09r2)
[System.IO.File]::WriteAllLines((Join-Path $bag "SHA256_POST.txt"), $post)
$hasPass = $false
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_07_SCALE_MID_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "A07MID_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_07_SCALE_MID_RUN_OK"
