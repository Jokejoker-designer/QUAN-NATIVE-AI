...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5E_FILE_MISSING $p" }
...
if ($hashFail) { throw "C5E_HASH_GATE_FAIL do_not_edit_keep_rtl" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_edge_mut.sv"
$dut = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_prod_top.sv"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5E_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5E_GOLD_HASH_DRIFT GOLDEN.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5E_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C5E_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5E_SYNTH_MIG_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C5E_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C5E_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") { throw "C5E_CTX_8255a798_AS_DUT" }
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C5E_LEFTOVER_A09" }
}
...
$pre = @(
  ...
  "# LAW=causal edge delete/replace/reverse on one production hierarchy; modeled AXI not MIG"
) + $hashLines
$pre += "# COMPILED"
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C5_EDGE_MUTATION_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C5E_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C5_EDGE_MUTATION_RUN_OK"
