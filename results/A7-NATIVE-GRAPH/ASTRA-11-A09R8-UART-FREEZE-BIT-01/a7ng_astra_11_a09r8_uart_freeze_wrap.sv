// a7ng_astra_11_a09r8_uart_freeze_wrap.sv — ASTRA-11-A09R8-UART-FREEZE-BIT-01.
// Successor of B (a7ng_astra_11_a09r7_uart_impl_wrap). B KEEP, not this top.
// RCA fix: NO UART IOB pad FFs. Pad → frozen uart_rx 2-FF / uart_tx.tx.
// Instantiates frozen a7ng_astra_09_r2_cand_ovf (no copy-paste graph).
// R7 wrap KEEP, not this top. Frozen a7ng_astra_09_integ_path not instantiated.
// MMCM 100→50 + BUFG. UART pins uart_rxd_out=D10 uart_txd_in=A9.
// PROGRAM=NO (bit may be written; never program COM12/JTAG).
`timescale 1ns / 1ps
`include "a7ng_astra_11_a09r8_uart_freeze.svh"

module a7ng_astra_11_a09r8_uart_freeze_wrap (
  ...
  localparam int unsigned TX_N   = A7NG_A09R8_TX_N;
  localparam logic [7:0]  MAGIC  = A7NG_A09R8_MAGIC;
  localparam logic [7:0]  EOL    = A7NG_A09R8_EOL;
  localparam logic [7:0]  CMD_REW = A7NG_A09R8_CMD_REW;
  localparam logic [7:0]  CMD_RET = A7NG_A09R8_CMD_RET;
  localparam logic [7:0]  OBS_TAG = A7NG_A09R8_OBS_TAG;
  ...
  // RCA: do not insert IOB pad FFs. Frozen uart_rx has SR on the 2-FF
  // synchronizer so Vivado will not pack ILOGIC IFF (that pack is what
  // produced WHS=-4.915 vs uart_io_vclk). Do not patch uart_rx / uart_tx.
  (* keep_hierarchy = "yes" *)
  ...
  (* keep_hierarchy = "yes" *)
  a7ng_astra_11_a09r8_uart_freeze_plant u_plant (
  ...
  assign led[1] = busy;
  assign led[2] = result_v | r_ovf | pend_cmt | (|ntrunc);
