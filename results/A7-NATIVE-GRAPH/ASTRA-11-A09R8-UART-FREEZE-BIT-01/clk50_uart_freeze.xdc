# ASTRA-11-A09R8-UART-FREEZE-BIT-01. PROGRAM=NO until owner says nap.
...
#
# RCA of B (ASTRA-11-A09R7-UART-IMPL-ROUTE-01):
#   IOB FF on uart_txd_in vs virtual uart_io_vclk (SCD=0) vs clk50u MMCM insertion
#   produced WHS=-4.915 (ASTRA-11-A09R3-UART-IOBFF-01). FALSE_PATH_HOLD then
#   excepted that check so B DTS WHS=+0.104 is intra-clk50u, not pad hold MET.
#   IDELAYE2 TAP31 max ~2.5 ns cannot cover -4.915.
# Fix frozen here BEFORE impl (copy of proven ASTRA-11-A09R3-UART-IODELAY-01 envelope):
#   NO UART IOB FF. Pad feeds frozen uart_rx 2-FF (has SR, will not pack IOB).
#   delays IN/OUT max 2.000 min 0.500 vs uart_io_vclk.
#   NO set_false_path -hold on UART. Hold must be physically checked.
#   IOB FALSE on UART ports so Vivado does not re-pack the defect.
...
# HOLD_POLICY=RELATED_CHECK_NO_FALSE_PATH_HOLD (UART hold is timed, not excepted).
# IOB_FF_POLICY=FALSE — do not pack pad FFs; frozen uart_rx 2-FF is the CDC.
# ---------------------------------------------------------------------------
