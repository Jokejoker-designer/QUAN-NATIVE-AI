...
if ($liveA09 -ne $expectA09) { throw "A11A09R8_FROZEN_A09_HASH_CHANGED $liveA09" }
if ($liveR2  -ne $expectR2)  { throw "A11A09R8_DUT_HASH_CHANGED $liveR2" }
if ($liveSgd -ne $expectSgd) { throw "A11A09R8_SGD_HASH_CHANGED $liveSgd" }
if ($liveRx  -ne $expectRx)  { throw "A11A09R8_UART_RX_HASH_CHANGED $liveRx" }
if ($liveTx  -ne $expectTx)  { throw "A11A09R8_UART_TX_HASH_CHANGED $liveTx" }
$compiled = @(
  ...
  $uartRx,
  $uartTx,
  (Join-Path $bag "a7ng_astra_11_a09r8_uart_freeze_plant.sv"),
  (Join-Path $bag "a7ng_astra_11_a09r8_uart_freeze_wrap.sv"),
  (Join-Path $bag "clk50_uart_freeze.xdc"),
...
$includes = @(
  ...
  (Join-Path $bag "a7ng_astra_11_a09r8_uart_freeze.svh")
)
function Rel([string]$p) {
  return $p.Substring($root.Length).TrimStart('\', '/').Replace('\', '/')
}
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "A11A09R8_FILE_MISSING $p" }
  $h = (Get-FileHash -Algorithm SHA256 -LiteralPath $p).Hash.ToLowerInvariant()
  return "$h  $(Rel $p)"
}
foreach ($f in $compiled) {
  $bn = [IO.Path]::GetFileName($f)
  if ($bn -eq "a7ng_astra09_pipe.sv" -or $bn -eq "a7ng_astra_09_integ_path.sv" -or $bn -eq "arty_a7_astra09_soc_top.sv" -or $bn -eq "arty_a7_astra_rtp_soc_top.sv" -or $bn -eq "a7ng_astra_09_r7_uart_query_rew_wrap.sv" -or $bn -eq "a7ng_astra_11_a09r7_uart_impl_wrap.sv" -or $bn -match "axi_bram128") {
...
$lines += (ShaLine (Join-Path $bag "RCA.md"))
$lines += (ShaLine (Join-Path $bag "run_impl.ps1"))
$lines += "# KEEP_NOT_THIS_WRAPPER (hashed; not compiled)"
...
Write-Host "A11A09R8_UART_FREEZE_SHA_FROZEN $shaPath"
if (-not (Test-Path -LiteralPath $vivado)) { throw "A11A09R8_VIVADO_NOT_FOUND $vivado" }
...
if (Test-Path -LiteralPath $log) {
  $done = [bool](Select-String -Path $log -Pattern "ASTRA_11_A09R8_UART_FREEZE_DONE" -Quiet)
}
if (($exit -ne 0) -or (-not $done)) {
  $r0 = Join-Path $bag "vivado_fail_r0.log"
  if (Test-Path -LiteralPath $log) { Copy-Item $log $r0 -Force }
  throw "A11A09R8_UART_FREEZE_IMPL_FAIL exit=$exit done=$done"
}
...
$bit = Join-Path $bag "a7ng_astra_11_a09r8_uart_freeze_wrap.bit"
if (Test-Path -LiteralPath $bit) {
  $bh = (Get-FileHash -Algorithm SHA256 -LiteralPath $bit).Hash.ToLowerInvariant()
  $post += "$bh  $(Rel $bit)"
  [System.IO.File]::WriteAllLines((Join-Path $bag "SHA256_BIT.txt"), @("$bh  a7ng_astra_11_a09r8_uart_freeze_wrap.bit", "STATUS=UNPROGRAMMED", "PROGRAM=NO", "COM12=UNTOUCHED", "JTAG=210319BE776EA UNTOUCHED"))
  Write-Host "A11A09R8_BIT_SHA $bh"
} else {
  Write-Host "A11A09R8_BIT_ABSENT"
}
[System.IO.File]::WriteAllLines((Join-Path $bag "SHA256_POST.txt"), $post)
Write-Host "ASTRA_11_A09R8_UART_FREEZE_RUN_OK"
