#!/usr/bin/env python3
"""Capture COM12 for E2R-LM-PHASE-PROBE-00 — F1n W_STALL/PHASE probe."""
...
def main():
    ...
    while time.time() - t0 < args.seconds:
        ...
        if "CORE_BUSY" in lines and (time.time() - t0) > 90.0:
            if any(x.startswith("PHASE=") for x in lines) or "W_STALL" in lines:
    ...
    for k in ("BIND", "BIND_BUSY", "WDMA_BUSY", "WDMA_DONE", "CORE_BUSY",
              "W_STALL", "PRED_NZ", "CORE_DONE", "FWD", "LM", "PACK"):
    ...
    if not m:
        # also accept alternate exist row spellings from prior probes
        m = re.search(r"NATIVE_V1_EXIST_ROW,pred=(\d+)", text)
    if not m:
