# PREREG — ASTRA-C1-KEY-RELBIND-01
...
`a7ng_query_role_extract` + named `a7ng_query_role_keys_relbind` +
frozen `a7ng_sparse_dir_axi` + frozen `a7ng_route_valid_gate` inside
**new** `a7ng_query_axi_sparse_relbind`. Does **not** edit or compile as DUT
`a7ng_query_axi_sparse.sv` (hash-gated MATCH C0). Does **not** compile leftover
...
closed. Does not edit KEEP bags R1 / R2 / R3.

FAIL routing (auditor 20260907T0940Z): no threshold drop; no
`relevant=router_union`; no nid-derived keys; no silent C0 patch.
New registered law id only.

## Primary unknown (Master V1.1 §7, N=256 named key law)

At N=256, new law `qse-v2-relbind-01` with
`k2={rel_id,subj_cue[7:0]}` `k3={rel_id,obj_cue[7:0]}` (`k0,k1` unchanged
from frozen extract), does CLASS_distractor `leak_n=0` on R3 excluded gold
while CLASS_direct still retrieves its gold ids?

## Claim this bag may close

N=256 relbind key-law gate only.
...
k2                = {rel_id, subj_cue[7:0]}
k3                = {rel_id, obj_cue[7:0]}
```
...
Index rebuilt with the same relbind function as RTL (host twin).
