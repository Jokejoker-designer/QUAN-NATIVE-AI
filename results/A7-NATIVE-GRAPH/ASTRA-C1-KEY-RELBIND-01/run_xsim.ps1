...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C1RELBIND_FILE_MISSING $p" }
...
if ($hashFail) {
  ...
  throw "C1RELBIND_HASH_GATE_FAIL do_not_invent"
}
...
$tb = Join-Path $bag "tb_astra_c1_key_relbind.sv"
$hostPy = Join-Path $bag "host_astra_c1_key_relbind.py"
$rbKeys = Join-Path $root "rtl\native_graph\query\a7ng_query_role_keys_relbind.sv"
$rbSparse = Join-Path $root "rtl\native_graph\integrate\a7ng_query_axi_sparse_relbind.sv"
if (-not (Test-Path -LiteralPath $goldJson)) { throw "C1RELBIND_GOLD_MISSING GOLDEN.json" }
if (-not (Test-Path -LiteralPath $goldSvh)) { throw "C1RELBIND_GOLD_MISSING query_gold.svh" }
if (-not (Test-Path -LiteralPath $corpus)) { throw "C1RELBIND_GOLD_MISSING corpus.json" }
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C1RELBIND_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$liveSvh = Sha256 $goldSvh
$liveCorpus = Sha256 $corpus
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C1RELBIND_GOLD_HASH_DRIFT GOLDEN.json" }
if ($preTxt -notmatch $liveSvh) { throw "C1RELBIND_GOLD_HASH_DRIFT query_gold.svh" }
if ($preTxt -notmatch $liveCorpus) { throw "C1RELBIND_GOLD_HASH_DRIFT corpus.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") {
    throw "C1RELBIND_LEFTOVER_A09_ON_XVLOG_LIST"
  }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") {
    throw "C1RELBIND_FROZEN_AXI_SPARSE_MUST_NOT_BE_DUT"
  }
}
...
$pre = @(
  ...
  "# PROGRAM=NO C1_800K=OPEN CAND_CAP=16 N=256 LAW=qse-v2-relbind-01 POKE_V=0",
...
$pre += "$liveGold  results/A7-NATIVE-GRAPH/ASTRA-C1-KEY-RELBIND-01/GOLDEN.json"
$pre += "$liveSvh  results/A7-NATIVE-GRAPH/ASTRA-C1-KEY-RELBIND-01/query_gold.svh"
$pre += "$liveCorpus  results/A7-NATIVE-GRAPH/ASTRA-C1-KEY-RELBIND-01/corpus.json"
$pre += "# BAG"
...
Write-Host "C1RELBIND_SHA_FROZEN_BEFORE_XVLOG"

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C1RELBIND_XVLOG_NOT_FOUND" }
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C1RELBIND_XVLOG_FAIL"
}
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
$xvlogTxt = [System.IO.File]::ReadAllText((Join-Path $bag "xvlog.log"))
if ($xvlogTxt -match "a7ng_astra_09_integ_path") {
  throw "C1RELBIND_LEFTOVER_A09_COMPILED"
}
if ($xvlogTxt -match "Analyzing Verilog file .+a7ng_query_axi_sparse\.sv") {
  if ($xvlogTxt -notmatch "a7ng_query_axi_sparse_relbind") {
    throw "C1RELBIND_FROZEN_AXI_SPARSE_COMPILED"
  }
}
& "$bin\xelab.bat" tb_astra_c1_key_relbind -s astra_c1_key_relbind -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) {
  if (Test-Path (Join-Path $work "xelab.log")) {
    Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
  }
  throw "C1RELBIND_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" astra_c1_key_relbind -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C1_KEY_RELBIND_XSIM_PASS" -Quiet)
...
if ($hasRedCap) {
  ...
  throw "C1RELBIND_REDUCTION_X1000_EMITTED_AS_CAP_N"
}
if (($xsimExit -ne 0) -or (-not $hasPass) -or $hasDiv -or $hasLeak) {
  ...
  throw "C1RELBIND_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C1_KEY_RELBIND_RUN_OK"
