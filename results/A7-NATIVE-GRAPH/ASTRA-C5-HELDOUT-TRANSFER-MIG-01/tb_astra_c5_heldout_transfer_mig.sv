`timescale 1ps / 100fs
// ASTRA-C5-HELDOUT-TRANSFER-MIG-01. PROGRAM=NO. Bag-local TB (test-only plant).
...
module tb_astra_c5_heldout_transfer_mig;
  ...
  initial begin
    ...
    $display("C5_HELDOUT_TRANSFER_MIG PROGRAM=NO BOARD_PASS=REJECT train_subj=10 hold_subj=6 MIG=YES TO_CYC_TB_DEFPARAM=65535");
    ...
    if (fail == 0) $display("ASTRA_C5_HELDOUT_TRANSFER_MIG_XSIM_PASS");
    else $display("ASTRA_C5_HELDOUT_TRANSFER_MIG_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO quality=MIG_PHY_HELDOUT_PRE_POST UART_SIM_BAUD");
    $finish;
  end

  initial begin
    #20s;
    $display("FIRST_DIVERGENCE MIG_TIMEOUT calib_or_heldout hung");
    $display("RESULT=FAIL");
    $display("ASTRA_C5_HELDOUT_TRANSFER_MIG_XSIM_PASS ABSENT");
    $finish;
  end
endmodule
