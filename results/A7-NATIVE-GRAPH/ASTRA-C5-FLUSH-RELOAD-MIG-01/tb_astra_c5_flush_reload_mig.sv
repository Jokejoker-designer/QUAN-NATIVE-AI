...
module tb_astra_c5_flush_reload_mig;
  ...
  initial begin
    fail = 0; first_div = ""; saved_w0 = 16'sd0;
    ...
    $display("C5_FLUSH_RELOAD_MIG PROGRAM=NO BOARD_PASS=REJECT MIG=YES TO_CYC_TB_DEFPARAM=65535");
    ...
    wait_done("Q1_DONE");
    saved_w0 = pw0;
    $display("MEAS Q1 st=%0d ans=%0d p0=%0d w0=%0d pvalid=%0d", cap_st, cap_ans, cap_p0, pw0, pvalid);
    ...
    if (fail == 0) $display("ASTRA_C5_FLUSH_RELOAD_MIG_XSIM_PASS");
    else $display("ASTRA_C5_FLUSH_RELOAD_MIG_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO quality=MIG_PHY_FLUSH_RELOAD UART_SIM_BAUD");
    $finish;
  end

  initial begin
    #20s;
    $display("FIRST_DIVERGENCE MIG_TIMEOUT calib_or_flush_reload hung");
    $display("RESULT=FAIL");
    $display("ASTRA_C5_FLUSH_RELOAD_MIG_XSIM_PASS ABSENT");
    $finish;
  end
endmodule
