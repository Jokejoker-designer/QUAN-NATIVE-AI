// E2R-ACK-WHILE-R-CXSIM-00 — dest vs in-R from first dest=4 until dest=5
// Vehicle: copy of E2R-SDONE-STILLR-CXSIM-00 (SIM_FULL=0 + CDC + B1 + mux
// + shared stub + s_dma_idle=1'b0). STILLR CONTROL SHA 4F71A710… kept
// in-R through dest=4 then complete (SNAP_DONE0).
// SAME VEHICLE: do NOT pulse s_done / clear busy until AFTER first dest=4
// is latched (keep W_R + busy through dest=4), THEN complete.
// Do NOT complete before dest=4 (ROSE / FALSIFIER).
// Do NOT hold busy forever after dest=4 (MUX / FALSIFIER).
// Do NOT force dest=5. D_ACK is dest=5. dbg_tile_dst is dst_s1 (2-FF).
// UNKNOWN: can dest become 5 while responder still in-R/busy?
// H_CANDIDATE ACK_WHILE_R / H_RIVAL ACK_ONLY_AFTER_DONE
...
module tb_e2r_ack_while_r_cxsim_00;
  import a7ng_pkg::*;
  ...
  localparam int unsigned DEST5_TO = 8192;
  ...
  // Hierarchical READ of raw tile dest. Not a force. dbg_tile_dst = dst_s1.
  wire [2:0] raw_dst = dut.u_core.u_w.TILE.dst;
  ...
  bit destwait_seen;
  bit dest5_seen;
  bit done_seen_before_dest5;
  bit dest5_while_in_r;
  bit dest5_while_busy_nodone;
  always_ff @(posedge ui_clk or negedge ui_rst_n) begin
    if (!ui_rst_n) begin
      dest4_ui_m    <= 1'b0;
      dest4_seen_ui <= 1'b0;
  ...
  logic [2:0]  snap_wst;
  logic [2:0]  snap5_dst, snap5_raw;
  logic        snap5_in_r, snap5_sbusy, snap5_mbusy, snap5_sdone, snap5_mdone;
  logic        snap5_sdone_st, snap5_sdone_ever, snap5_done_before;
  logic [2:0]  snap5_wst;
  logic [31:0] snap5_cyc, first_done_cyc;
  logic        first_done_latched;
  ...
  always_ff @(posedge core_clk or negedge core_rst_n) begin
    if (!core_rst_n) begin
      destwait_seen      <= 1'b0;
      dest5_seen         <= 1'b0;
      done_seen_before_dest5 <= 1'b0;
      dest5_while_in_r   <= 1'b0;
      dest5_while_busy_nodone <= 1'b0;
      first_done_latched <= 1'b0;
      first_done_cyc     <= 32'd0;
      soa_done_seen      <= 1'b0;
      start_fwd_seen     <= 1'b0;
      wdma_go_seen       <= 1'b0;
      ...
      snap5_mdone    <= 1'b0;
      snap5_sdone_st <= 1'b0;
      snap5_sdone_ever <= 1'b0;
      snap5_done_before <= 1'b0;
      snap5_wst      <= 3'd0;
      snap5_cyc      <= 32'd0;
    end else begin
      if (wdma_owner_grant)
      ...
      if (destwait_seen && !dest5_seen)
        snap5_cyc <= snap5_cyc + 32'd1;
      if (!first_done_latched && (s_dma_done || wdma_done || s_done_ever_s
          || dbg_s_done_sticky || dbg_m_done_sticky)) begin
        first_done_latched <= 1'b1;
        first_done_cyc <= destwait_seen ? snap5_cyc : 32'd0;
      end
      if (!dest5_seen && destwait_seen && (tile_dst == 3'd5)) begin
        dest5_seen        <= 1'b1;
        snap5_dst         <= tile_dst;
        snap5_raw         <= raw_dst;
        snap5_in_r        <= (w_st == W_R) && (sbusy_dma_m2 | s_dma_busy);
        snap5_sbusy       <= sbusy_dma_m2 | s_dma_busy;
        snap5_mbusy       <= wdma_busy;
        snap5_sdone       <= s_dma_done;
        snap5_mdone       <= wdma_done;
        snap5_sdone_st    <= sdone_st_m2 | dbg_s_done_sticky;
        snap5_sdone_ever  <= sdone_ever_m2 | s_done_ever_s;
        snap5_done_before <= first_done_latched;
        snap5_wst         <= w_st;
        done_seen_before_dest5 <= first_done_latched;
        dest5_while_in_r  <= (w_st == W_R) && (sbusy_dma_m2 | s_dma_busy)
            && !(first_done_latched || s_dma_done || wdma_done
                 || s_done_ever_s || dbg_s_done_sticky || dbg_m_done_sticky);
        dest5_while_busy_nodone <= (wdma_busy || (sbusy_dma_m2 | s_dma_busy))
            && !(first_done_latched || s_dma_done || wdma_done
                 || s_done_ever_s || dbg_s_done_sticky || dbg_m_done_sticky);
      end
    end
  ...
  task automatic dump_row(input string phase_s);
    $display("PROBE t=%0t phase=%s dst=%0d raw=%0d drain=%0b fifo=%0d c_rvalid=%0b tr=%0d idle=%0b own=%0b grant=%0b own_ui=%0b cdc_rv=%0b stub_rv=%0b m_go=%0b s_go=%0b sgo_st=%0b mgo_st=%0b sgo_ever=%0b s_done=%0b sdone_st=%0b mdone_st=%0b sdone_ever=%0b cmd_st=%0d sbusy_pend=%0b s_busy=%0b m_busy=%0b m_done=%0b cmd_rd=%0b cmd_empty_mgo=%0b rleft=%0d wst=%0d miss=%0b reqs1=%0b bst=%0d soa_done=%0b fwd=%0b core_busy=%0b phase=%0d gv=%0d",
      $time, phase_s, tile_dst, raw_dst, p_drain, p_fifo, p_rvalid, p_tr, r_path_idle,
      ...
      dbg_cmd_st, dbg_sbusy_pend, s_dma_busy, wdma_busy, wdma_done, dbg_cmd_rd_sticky, dbg_cmd_empty_mgo,
      wdma_r_left, w_st,
      tile_miss, tile_req_s1, tile_bst, done, start_fwd, core_busy,
      phase, gv_count);
    $fdisplay(pf, "%0t,%s,%0d,%0d,%0b,%0d,%0b,%0d,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0d,%0b,%0b,%0b,%0b,%0b,%0d,%0d",
      $time, phase_s, tile_dst, raw_dst, p_drain, p_fifo, p_rvalid, p_tr, r_path_idle,
      wdma_owner, wdma_owner_grant, wdma_owner_ui,
      wdma_go, dma_go, dbg_s_go_sticky, dbg_m_go_sticky, s_go_ever_s,
      s_dma_done, dbg_s_done_sticky, dbg_m_done_sticky, s_done_ever_s,
      dbg_cmd_st, dbg_sbusy_pend, s_dma_busy, wdma_busy, wdma_done,
      done, phase, w_st);
  endtask

  task automatic dump_dest_busy(input string phase_s);
    $display("DEST_BUSY t=%0t phase=%s dest=%0d raw=%0d in_r=%0b s_busy=%0b m_busy=%0b s_done=%0b m_done=%0b sdone_ever=%0b sdone_st=%0b wst=%0d",
      $time, phase_s, tile_dst, raw_dst,
      (w_st == W_R) && s_dma_busy, s_dma_busy, wdma_busy,
      s_dma_done, wdma_done, s_done_ever_s, dbg_s_done_sticky, w_st);
    $fdisplay(pf, "%0t,%s,%0d,%0d,%0b,%0d,%0b,%0d,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0d,%0b,%0b,%0b,%0b,%0b,%0d,%0d",
      $time, phase_s, tile_dst, raw_dst, p_drain, p_fifo, p_rvalid, p_tr, r_path_idle,
      wdma_owner, wdma_owner_grant, wdma_owner_ui,
      wdma_go, dma_go, dbg_s_go_sticky, dbg_m_go_sticky, s_go_ever_s,
  ...
  initial begin
    ...
    if (pf == 0) begin
      $display("E2R_ACK_WHILE_R_CXSIM_00_XSIM_FAIL reason=probe_fopen");
      $fatal(1);
    end
    $fdisplay(pf, "t_ns,phase,tile_dst,raw_dst,r_drain_hold,fifo_cnt,m_axi_rvalid,tr_cnt,r_path_idle,wdma_owner,wdma_owner_grant,wdma_owner_ui,m_go,s_go,sgo_sticky,mgo_sticky,sgo_ever,s_done,sdone_sticky,mdone_sticky,sdone_ever,cmd_st,sbusy_pend,s_busy,wdma_busy,wdma_done,soa_done,lm_phase,w_st");
    ...
    $display("E2R-ACK-WHILE-R-CXSIM-00 START");
    ...
    $display("PREREG_UNKNOWN: can dest become 5 while responder still in-R/busy");
    $display("PREREG_H_CANDIDATE: ACK_WHILE_R dest=5 and in-R/busy same cycle");
    $display("PREREG_H_RIVAL: ACK_ONLY_AFTER_DONE dest=5 only after s_done or !busy");
    $display("PREREG_FALSIFIER: force dest; C-FIX; complete before dest=4; hold forever; soc_top+MIG; A2; LiteScope");
    $display("PREREG_UNIT: one query; dest vs in-R each core cycle from first dest=4 until dest=5");
    $display("FORBIDDEN: force TILE_DST/dst; complete before dest=4; hold busy forever; assign r_path_idle=1; C-FIX; soc_top; MIG; board; retie s_dma_idle");
    ...
    if (destwait_seen) begin
      ...
      dump_dest_busy("DEST4");
      begin
        int unsigned dw;
        logic [2:0] prev_dst, prev_raw, prev_wst;
        logic prev_in_r, prev_sbusy, prev_mbusy, prev_sdone, prev_mdone;
        dw = 0;
        prev_dst = tile_dst;
        prev_raw = raw_dst;
        prev_wst = w_st;
        prev_in_r = (w_st == W_R) && s_dma_busy;
        prev_sbusy = s_dma_busy;
        prev_mbusy = wdma_busy;
        prev_sdone = s_dma_done;
        prev_mdone = wdma_done;
        while (!dest5_seen && (dw < DEST5_TO)) begin
          @(posedge core_clk);
          dw = dw + 1;
          if ((tile_dst != prev_dst) || (raw_dst != prev_raw) || (w_st != prev_wst)
              || (((w_st == W_R) && s_dma_busy) != prev_in_r)
              || (s_dma_busy != prev_sbusy) || (wdma_busy != prev_mbusy)
              || (s_dma_done != prev_sdone) || (wdma_done != prev_mdone)
              || (dw <= 64) || dest5_seen || ((dw % HB_EVERY) == 0))
            dump_dest_busy("ACK_WATCH");
          prev_dst = tile_dst;
          prev_raw = raw_dst;
          prev_wst = w_st;
          prev_in_r = (w_st == W_R) && s_dma_busy;
        ...
        if (dest5_seen)
          dump_dest_busy("FIRST_DEST5");
        else
          dump_dest_busy("DEST5_WATCH_MISS");
      end
      end_dst        = tile_dst;
      end_fifo       = p_fifo;
    ...
    if (!destwait_seen)
      ...
      verdict_class = "FAIL_NO_ACK";
    else if (dest5_while_in_r || dest5_while_busy_nodone)
      verdict_class = "ACK_WHILE_R";
    else
      verdict_class = "ACK_ONLY_AFTER_DONE";
    ...
    $display("DEST5=%0b", dest5_seen && (snap5_dst == 3'd5));
    $display("DEST5_RAW=%0d", snap5_raw);
    $display("IN_R_AT_DEST5=%0b", snap5_in_r);
    $display("BUSY_AT_DEST5 s_busy=%0b m_busy=%0b wst=%0d", snap5_sbusy, snap5_mbusy, snap5_wst);
    $display("S_DONE_AT_DEST5=%0b", snap5_sdone | snap5_sdone_st | snap5_sdone_ever);
    $display("S_DONE_BEFORE_DEST5=%0b", done_seen_before_dest5 | snap5_done_before);
    $display("M_DONE_AT_DEST5=%0b", snap5_mdone);
    $display("FIRST_DEST5_CYC=%0d FIRST_DONE_CYC=%0d", snap5_cyc, first_done_cyc);
    $display("DEST5_WHILE_IN_R_NODONE=%0b DEST5_WHILE_BUSY_NODONE=%0b",
      dest5_while_in_r, dest5_while_busy_nodone);
    ...
    if (!destwait_seen || !((snap_wst == W_R) && snap_sbusy_dma)) begin
      $display("XSIM=FAIL_NO_DESTWAIT");
      $display("E2R_ACK_WHILE_R_CXSIM_00_FAIL_NO_DESTWAIT dst=%0d in_r=%0b wst=%0d s_busy=%0b",
        snap_dst, (snap_wst == W_R) && snap_sbusy_dma, snap_wst, snap_sbusy_dma);
      $finish;
    end

    if (verdict_class == "FAIL_NO_ACK") begin
      $display("XSIM=FAIL_NO_ACK");
      $display("E2R_ACK_WHILE_R_CXSIM_00_FAIL_NO_ACK dest4=1 dest5=0 in_r_dest4=%0b s_busy=%0b m_busy=%0b",
        (snap_wst == W_R) && snap_sbusy_dma, end_sbusy_dma, end_mbusy);
      $finish;
    end

    if ((verdict_class == "ACK_WHILE_R") || (verdict_class == "ACK_ONLY_AFTER_DONE")) begin
      $display("XSIM=PASS");
      $display("E2R_ACK_WHILE_R_CXSIM_00_XSIM_PASS class=%s c_fix=NONE dest5=%0d raw=%0d in_r_at_dest5=%0b s_done_before_dest5=%0b s_busy=%0b m_busy=%0b wst=%0d dest5_cyc=%0d done_cyc=%0d",
        verdict_class, snap5_dst, snap5_raw, snap5_in_r,
        done_seen_before_dest5 | snap5_done_before, snap5_sbusy, snap5_mbusy,
        snap5_wst, snap5_cyc, first_done_cyc);
    end else begin
      $display("XSIM=NO_PASS");
      $display("E2R_ACK_WHILE_R_CXSIM_00_NO_PASS class=%s dest5=%0b in_r=%0b",
        verdict_class, dest5_seen, snap5_in_r);
    end
    $finish;
  end
  ...
  initial begin
    ...
    $display("WALL_LIMIT");
    $display("XSIM=LIMIT");
    $display("E2R_ACK_WHILE_R_CXSIM_00_FAIL_NO_DESTWAIT reason=wall dst=%0d dest5=%0b",
      tile_dst, dest5_seen);
    $finish;
  end
endmodule
