...
$compiled = @(
  ...
  (Join-Path $bag "a7ng_astra_10_resource_wrap.sv"),
  (Join-Path $bag "clk50_ooc.xdc"),
...
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "A10_FILE_MISSING $p" }
  $h = (Get-FileHash -Algorithm SHA256 -LiteralPath $p).Hash.ToLowerInvariant()
  return "$h  $(Rel $p)"
}
foreach ($f in $compiled) {
  $bn = [IO.Path]::GetFileName($f)
  if ($bn -eq "a7ng_astra09_pipe.sv" -or $bn -match "pipe_r1|plant128|axi_bram128") {
...
$lines += (ShaLine (Join-Path $bag "run_synth.ps1"))
$lines += "# KEEP_NOT_THIS_WRAPPER (hashed; not copy-paste; astra09_pipe / axi_bram128 not compiled)"
...
Write-Host "A10_SHA_FROZEN $shaPath"
if (-not (Test-Path -LiteralPath $vivado)) { throw "A10_VIVADO_NOT_FOUND $vivado" }

function Invoke-A10Vivado {
  param([string]$LogName)
  $log = Join-Path $bag $LogName
  $jou = Join-Path $bag "vivado.jou"
  $tcl = Join-Path $bag "run_synth.tcl"
  Set-Location $bag
  cmd /c "`"$vivado`" -mode batch -notrace -source `"$tcl`" -log `"$log`" -journal `"$jou`""
  return [int]$LASTEXITCODE
}

$exit = Invoke-A10Vivado "vivado.log"
$done = $false
if (Test-Path -LiteralPath (Join-Path $bag "vivado.log")) {
  $done = [bool](Select-String -Path (Join-Path $bag "vivado.log") -Pattern "ASTRA_10_RESOURCE_BOUND_DONE" -Quiet)
}
if (($exit -ne 0) -or (-not $done)) {
  ...
  Write-Host "A10_SYNTH_FAIL_R0 exit=$exit done=$done one bounded re-run"
  $exit2 = Invoke-A10Vivado "vivado_r1.log"
  $done2 = $false
  if (Test-Path -LiteralPath (Join-Path $bag "vivado_r1.log")) {
    $done2 = [bool](Select-String -Path (Join-Path $bag "vivado_r1.log") -Pattern "ASTRA_10_RESOURCE_BOUND_DONE" -Quiet)
    Copy-Item (Join-Path $bag "vivado_r1.log") (Join-Path $bag "vivado.log") -Force
  }
  if (($exit2 -ne 0) -or (-not $done2)) {
    throw "A10_SYNTH_FAIL_AFTER_R1 exit=$exit2 done=$done2"
  }
}
$postStamp = (Get-Date).ToString("yyyy-MM-ddTHH:mm:ss.fffffffK")
$post = @("# SHA verify AFTER synth $postStamp")
...
Write-Host "ASTRA_10_RESOURCE_BOUND_RUN_OK"
