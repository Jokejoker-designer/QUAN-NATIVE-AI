// tb_astra01_u4_real_axi_sparse.sv
// ASTRA-01: raw QSE packet drives 4x4096 AXI directory. PROGRAM=NO.
`timescale 1ns / 1ps

module tb_astra01_u4_real_axi_sparse;
  import a7ng_pkg::*;
  ...
  logic [7:0] tok, eid, iid, relid, xid;
  ...
  // Live QSE keys/valid drive the walker. Poke path is protocol-only.
  assign wk0 = poke_en ? pk0 : k0;
  assign wk1 = poke_en ? pk1 : k1;
  assign wk2 = poke_en ? pk2 : k2;
  assign wk3 = poke_en ? pk3 : k3;
  assign wv0 = poke_en ? pv0 : v0;
  assign wv1 = poke_en ? pv1 : v1;
  assign wv2 = poke_en ? pv2 : v2;
  assign wv3 = poke_en ? pv3 : v3;
  ...
  a7ng_query_struct_extract u_qse (
    ...
    .entity_id_o(eid), .intent_id_o(iid), .relation_id_o(relid), .context_id_o(xid),
  ...
  task automatic check_cuts(input int q);
    ...
    begin
      ...
      if (G_MODE[q] == 0) begin
        if (eid !== G_EID[q] || iid !== G_IID[q] || relid !== G_RID[q] || xid !== G_XID[q])
          diverge("PACKET_MISMATCH", $sformatf("q=%0d eid=%h iid=%h rid=%h xid=%h", q, eid, iid, relid, xid));
        if (k0 !== G_K0[q] || k1 !== G_K1[q] || k2 !== G_K2[q] || k3 !== G_K3[q])
          diverge("KEY_MISMATCH", $sformatf("q=%0d k %h %h %h %h", q, k0, k1, k2, k3));
        if (v0 !== G_V0[q] || v1 !== G_V1[q] || v2 !== G_V2[q] || v3 !== G_V3[q])
          diverge("VALIDITY_MISMATCH", $sformatf("q=%0d v %0d%0d%0d%0d", q, v0, v1, v2, v3));
        if (poke_en)
          diverge("HOST_SEMANTIC_LEAK", "QSE query used poke mux");
      end
      ...
      if (n_pred != G_NPRED[q])
        diverge("PREDUP_MISMATCH", $sformatf("predup act=%0d exp=%0d", n_pred, G_NPRED[q]));
      for (s = 0; s < n_pred; s = s + 1)
        if (pred[s] !== G_PRED[q*G_MAX_PRED + s])
          diverge("PREDUP_MISMATCH", $sformatf("s=%0d act=%h exp=%h", s, pred[s], G_PRED[q*G_MAX_PRED + s]));
      ...
      for (s = 0; s < n_got; s = s + 1) begin
        if (got[s] !== G_EMIT[q*G_MAX_EMIT + s])
          diverge("CANDIDATE_ID_MISMATCH", $sformatf("s=%0d act=%h exp=%h", s, got[s], G_EMIT[q*G_MAX_EMIT + s]));
        for (j = 0; j < s; j = j + 1)
          if (got[j] === got[s])
      ...
      $fdisplay(fd_cut, "Q%0d\t%s\t%0d\t%0h\t%0h\t%0h\t%0h\t%0d%0d%0d%0d\t%0d\t%0d\t%0d\t%0d\t%0d\t%0d\t%0d",
        q, (G_MODE[q] ? "POKE" : "QSE"), G_MODE[q],
        (G_MODE[q] ? pk0 : k0), (G_MODE[q] ? pk1 : k1),
        (G_MODE[q] ? pk2 : k2), (G_MODE[q] ? pk3 : k3),
        (G_MODE[q] ? pv0 : v0), (G_MODE[q] ? pv1 : v1),
        (G_MODE[q] ? pv2 : v2), (G_MODE[q] ? pv3 : v3),
        n_dar, n_par, n_pred, n_got, n_dup, n_trunc, n_rbeat * 16);
  ...
  initial begin
    ...
    if (NG_DDR_INDEX_BASE !== 28'h05000000) diverge("DIR_ADDR_MISMATCH", "INDEX_BASE");

    fd_cut = $fopen("../CUT_TABLE.tsv", "w");
    $fdisplay(fd_cut, "q\tsrc\tmode\tk0\tk1\tk2\tk3\tv\tn_dir\tn_post\tn_pred\tn_emit\tn_dup\tn_trunc\tbytes");
    ...
    for (qi = 0; qi < G_N_Q; qi = qi + 1) begin
      if (G_MODE[qi] == 0) begin
        ...
        @(posedge clk);
        if (!valid)
          diverge("PACKET_MISMATCH", $sformatf("q=%0d NO_VALID", qi));
        walk(qi);
        check_cuts(qi);
        retire_qse();
      end else begin
        poke_en = 1'b1;
        pk0 = G_K0[qi]; pk1 = G_K1[qi]; pk2 = G_K2[qi]; pk3 = G_K3[qi];
        pv0 = G_V0[qi]; pv1 = G_V1[qi]; pv2 = G_V2[qi]; pv3 = G_V3[qi];
        walk(qi);
        check_cuts(qi);
        poke_en = 1'b0;
      end
    ...
    $fclose(fd_cut);
    $display("ASTRA_01_U4_REAL_AXI_SPARSE_PASS");
    $display("CLAIM=raw FPGA QSE packet/valid/keys drive exact 4x4096 AXI dir/posting and match independent host-golden IDs");
    $display("NOT_CLAIMED=800k_quality,P4_freeze,role_parse,board,BIT,PROGRAM,GATE14");
    #20 $finish;
  end
endmodule
