...
if ($liveA09 -ne $expectA09) { throw "A11A09R7_UART_IMPL_FROZEN_A09_HASH_CHANGED $liveA09" }
if ($liveR2  -ne $expectR2)  { throw "A11A09R7_UART_IMPL_DUT_HASH_CHANGED $liveR2" }
if ($liveSgd -ne $expectSgd) { throw "A11A09R7_UART_IMPL_SGD_HASH_CHANGED $liveSgd" }
if ($liveRx  -ne $expectRx)  { throw "A11A09R7_UART_IMPL_UART_RX_HASH_CHANGED $liveRx" }
if ($liveTx  -ne $expectTx)  { throw "A11A09R7_UART_IMPL_UART_TX_HASH_CHANGED $liveTx" }
$compiled = @(
  ...
  $uartRx,
  $uartTx,
  (Join-Path $bag "a7ng_astra_11_a09r7_uart_impl_plant.sv"),
  (Join-Path $bag "a7ng_astra_11_a09r7_uart_impl_wrap.sv"),
  (Join-Path $bag "clk50_uart_impl.xdc"),
...
$includes = @(
  ...
  (Join-Path $bag "a7ng_astra_11_a09r7_uart_impl.svh")
)
...
Write-Host "A11A09R7_UART_IMPL_SHA_FROZEN $shaPath"
if (-not (Test-Path -LiteralPath $vivado)) { throw "A11A09R7_UART_IMPL_VIVADO_NOT_FOUND $vivado" }
...
if (Test-Path -LiteralPath $log) {
  $done = [bool](Select-String -Path $log -Pattern "ASTRA_11_A09R7_UART_IMPL_ROUTE_DONE" -Quiet)
}
if (($exit -ne 0) -or (-not $done)) {
  $r0 = Join-Path $bag "vivado_fail_r0.log"
  if (Test-Path -LiteralPath $log) { Copy-Item $log $r0 -Force }
  throw "A11A09R7_UART_IMPL_IMPL_FAIL exit=$exit done=$done"
}
...
Write-Host "ASTRA_11_A09R7_UART_IMPL_ROUTE_RUN_OK"
