...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C1CCS_FILE_MISSING $p" }
...
if ($hashFail) {
  ...
  throw "C1CCS_HASH_GATE_FAIL do_not_invent do_not_edit_keep_rtl"
}
...
$tb = Join-Path $bag "tb_astra_c1_cand_cap_sweep.sv"
$tbTops = Join-Path $bag "tb_astra_c1_cand_cap_sweep_tops.sv"
$hostPy = Join-Path $bag "host_astra_c1_cand_cap_sweep.py"
...
if (-not (Test-Path -LiteralPath $goldJson)) { throw "C1CCS_GOLD_MISSING GOLDEN.json" }
if (-not (Test-Path -LiteralPath $goldSvh)) { throw "C1CCS_GOLD_MISSING query_gold.svh" }
if (-not (Test-Path -LiteralPath $corpus)) { throw "C1CCS_GOLD_MISSING corpus.json" }
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C1CCS_GOLD_NOT_HASHED_BEFORE_XVLOG" }
if (-not (Test-Path -LiteralPath $lexNamed)) { throw "C1CCS_LEX_MISSING qse_role_lexicon_semantic_800k.svh" }
if (-not (Test-Path -LiteralPath $lexShim)) { throw "C1CCS_LEX_SHIM_MISSING qse_role_lexicon.svh" }
if (-not (Test-Path -LiteralPath $synSvh)) { throw "C1CCS_SYN_SVH_MISSING bag qse_relctx_synonym_01.svh" }
if (-not (Test-Path -LiteralPath $procMem)) { throw "C1CCS_PROC_MEM_MISSING" }
if (-not (Test-Path -LiteralPath $genSvh)) { throw "C1CCS_GEN_SVH_MISSING" }
...
if ($preTxt -notmatch $liveGold) { throw "C1CCS_GOLD_HASH_DRIFT GOLDEN.json" }
if ($preTxt -notmatch $liveSvh) { throw "C1CCS_GOLD_HASH_DRIFT query_gold.svh" }
if ($preTxt -notmatch $liveCorpus) { throw "C1CCS_GOLD_HASH_DRIFT corpus.json" }
if ($preTxt -notmatch $liveLex) { throw "C1CCS_GOLD_HASH_DRIFT qse_role_lexicon_semantic_800k.svh" }
if ($preTxt -notmatch $liveSyn) { throw "C1CCS_GOLD_HASH_DRIFT qse_relctx_synonym_01.svh" }
if ($liveSyn -ne $liveSynRtl) {
  throw "C1CCS_SYN_SVH_BAG_RTL_MISMATCH bag=$liveSyn rtl=$liveSynRtl"
}
$goldTxt = [System.IO.File]::ReadAllText($goldJson)
if ($goldTxt -notmatch '"n": 800000') { throw "C1CCS_GOLD_N_NOT_800000" }
$svhTxt = [System.IO.File]::ReadAllText($goldSvh)
if ($svhTxt -notmatch "localparam int unsigned G_N = 800000;") { throw "C1CCS_SVH_N_NOT_800000" }
if ($svhTxt -match "localparam int unsigned G_N = 16384;") { throw "C1CCS_16K_CLONE_LABELED_800K" }
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") {
    throw "C1CCS_LEFTOVER_A09_ON_XVLOG_LIST"
  }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") {
    throw "C1CCS_FROZEN_AXI_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") {
    throw "C1CCS_STREAM02_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_page_skip.sv") {
    throw "C1CCS_PAGESKIP_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") {
    throw "C1CCS_CTX_DUT_8255a798_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_axi_mem_model.sv") {
    throw "C1CCS_C0_DENSE_MEM_MUST_NOT_BE_DUT"
  }
}
...
$pre += "$liveGold  results/A7-NATIVE-GRAPH/ASTRA-C1-CAND-CAP-SWEEP-01/GOLDEN.json"
$pre += "$liveSvh  results/A7-NATIVE-GRAPH/ASTRA-C1-CAND-CAP-SWEEP-01/query_gold.svh"
$pre += "$liveCorpus  results/A7-NATIVE-GRAPH/ASTRA-C1-CAND-CAP-SWEEP-01/corpus.json"
$pre += "$liveLex  results/A7-NATIVE-GRAPH/ASTRA-C1-CAND-CAP-SWEEP-01/qse_role_lexicon_semantic_800k.svh"
$pre += "$liveSyn  results/A7-NATIVE-GRAPH/ASTRA-C1-CAND-CAP-SWEEP-01/qse_relctx_synonym_01.svh"
$pre += "# BAG"
...
$pre += "# REDUCTION_VS_N_X1000 required; never reduction_x1000=cap/N tautology"
...
Write-Host "C1CCS_SHA_FROZEN_BEFORE_XVLOG"
...
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C1CCS_XVLOG_NOT_FOUND" }
...
foreach ($cap in $caps) {
  ...
  & "$bin\xelab.bat" "tb_sweep_$cap" -s $snap -timescale 1ns/1ps
  if ($LASTEXITCODE -ne 0) {
