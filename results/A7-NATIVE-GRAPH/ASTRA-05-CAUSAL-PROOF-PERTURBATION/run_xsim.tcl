# ASTRA-05 causal perturbation XSim. PROGRAM=NO.
...
set tb  [file join $bag tb_astra05_perturb.sv]
...
if {[catch {exec $xvlog_bin --sv -i $bag $rtl $tb > $xvlog_log 2>@1}]} {
  ...
  puts ASTRA05_XVLOG_FAIL
  exit 2
}
set xelab_log [file join $bag xelab.log]
if {[catch {exec $xelab_bin tb_astra05_perturb -s astra05 -timescale 1ns/1ps > $xelab_log 2>@1}]} {
  puts [read [open $xelab_log r]]
  puts FIRST_DIVERGENCE
  puts ASTRA05_XELAB_FAIL
  exit 3
}
set xsim_log [file join $bag xsim.log]
catch {exec $xsim_bin astra05 -R -log $xsim_log}
set body [read [open $xsim_log r]]
puts $body
if {[string match *FIRST_DIVERGENCE* $body]} { puts ASTRA05_FIRST_DIVERGENCE; exit 6 }
if {![string match *ASTRA05_CAUSAL_XSIM_PASS* $body]} { puts ASTRA05_NOT_PASS; exit 5 }
puts ASTRA05_XSIM_OK
exit 0
