`timescale 1ns / 1ps
// ASTRA-C3-HELD-OUT-800K-2HOP-01. PROGRAM=NO. Bag-local TB.
// 5-seed 4-arm on 800k cartesian facts + ctx-folded k0 overlay. Not MIG.
`include "a7ng_astra_c3_held_out.svh"
module tb_astra_c3_held_out_800k_2hop;
  ...
  initial begin
    ...
    load_v=0; load_idx=0; load_w=0; twohop_ok=0;
    ...
    $display("C3_800K_2HOP N=800000 FACT_BASE=0E000000 CTX_K0_OVERLAY=1 TO_CYC=65535 TR={13,15,16} HO={19,20} PROGRAM=NO BOARD_PASS=REJECT");
    ...
    $display("C3_SUM A=%0d/%0d B=%0d C=%0d D=%0d gain_pp=%0d pair_pos=%0d/%0d host_bad=%0d twohop_ctx=%0d",
      sum_a, 8*C3_N_SEED, sum_b, sum_c, sum_d, gain_pp, pair_pos, C3_N_SEED, host_bad, twohop_ok);
    ...
    if (fail==0) $display("ASTRA_C3_HELD_OUT_800K_2HOP_XSIM_PASS");
    else $display("ASTRA_C3_HELD_OUT_800K_2HOP_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C3_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO quality=800K_CARTESIAN_FACTS_CTX_K0_OVERLAY_2HOP_NOT_MIG");
    $finish;
  end
endmodule
