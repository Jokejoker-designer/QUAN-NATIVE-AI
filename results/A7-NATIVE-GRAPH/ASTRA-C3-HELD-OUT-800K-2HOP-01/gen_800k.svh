...
// Include-guard removed in this bag copy: mem and TB both in-module include.
...
function automatic int g_cidx(input int s, input int r, input int o);
  g_cidx = ((s - G_ENT0) * G_N_REL + (r - 1)) * (G_ENT_HI - G_ENT0) + g_oi(s, o);
endfunction
...
function automatic int g_occ_of(input int tbl, input int key);
  ...
  int ids [0:255];
  int n;
  if ((tbl == 0 && key == 3380) || (tbl == 1 && key == 3636))
    g_occ_of = 1;
  else begin
    s = (key >> 8) & 8'hFF;
    r = key & 8'hFF;
...
function automatic logic [127:0] g_pack_post(input int tbl, input int key, input int beat);
  ...
  int base;
  logic [127:0] v;
  if ((tbl == 0 && key == 3380) || (tbl == 1 && key == 3636)) begin
    n = 0;
    if (beat == 0) begin
      v = 128'd0;
      v[31:0] = 32'd121;
