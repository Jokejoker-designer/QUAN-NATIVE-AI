// a7ng_axi_mem_c3_800k_2hop.sv — bag-local. PROGRAM=NO.
// Directory/postings = gen_800k cartesian plus ctx-folded k0 overlay.
// Facts = C3 fact_pack at 0x0E000000. Not MIG. Not KEEP.
`timescale 1ns / 1ps

module a7ng_axi_mem_c3_800k_2hop (
  input  logic         clk,
  input  logic         rst_n,
  ...
  function automatic logic [127:0] rd_of(input logic [27:0] addr);
    ...
    begin
      if ((addr >= C3K_FACT_BASE) && (addr < (C3K_FACT_BASE + (C3K_NID_MAX << 4)))) begin
        ...
        rd_of = c3k2_pack_dir(s, live_epoch_i);
      end else if (c3k2_post_hit(addr, s)) begin
        rd_of = c3k2_pack_post(s);
      end else begin
        d = g_rdata_of(addr);
