// E2R-RPATH-IDLE-CXSIM-MUX-00 — mux half of C-XSIM leftover unknown
// Vehicle: a7ng_native_v1_ab_core SIM_FULL=0 + a7ng_axi_read_cdc + B1 grant
// + SoC mux (ungated cdc_rvalid=rvalid) + ONE shared stub. WDMA on same R bus.
// Probe four r_path_idle AND terms at first DUT-driven TILE_DST==4.
...
module tb_e2r_rpath_idle_cxsim_mux_00;
  import a7ng_pkg::*;
  ...
  initial begin
    ...
    if (pf == 0) begin
      $display("E2R_RPATH_IDLE_CXSIM_MUX_00_XSIM_FAIL reason=probe_fopen");
      $fatal(1);
    end
    ...
    $display("E2R-RPATH-IDLE-CXSIM-MUX-00 START");
    $display("VEHICLE=a7ng_native_v1_ab_core SIM_FULL=0 do_lm=1 CDC+B1+MUX ungated_cdc_rvalid SHARED_STUB_WDMA");
    $display("PREREG_UNKNOWN: which AND term is 1 at first legal TILE_DST==4 through shared stub+CDC+B1+ungated cdc_rvalid");
    $display("PREREG_H_CANDIDATE: shared-bus leftover or WDMA R via ungated cdc_rvalid holds c_rvalid → idle=0");
    $display("PREREG_H_RIVAL: dest-wait still idle=1 (NONE) — leftover is true MIG-only");
    $display("PREREG_FALSIFIER: FAIL_NO_DESTWAIT or force dst or more than one term (SET) or C-FIX");
    ...
    if (!destwait_seen) begin
      ...
      $display("E2R_RPATH_IDLE_CXSIM_MUX_00_FAIL_NO_DESTWAIT dst=%0d bst=%0d miss=%0b reqs1=%0b go=%0b busy=%0b soa=%0b fwd=%0b grant=%0b own_ui=%0b",
        tile_dst, tile_bst, tile_miss, tile_req_s1, wdma_go, wdma_busy, soa_done_seen, start_fwd_seen,
        wdma_owner_grant, wdma_owner_ui);
      $finish;
    end

    $display("XSIM=PASS");
    $display("E2R_RPATH_IDLE_CXSIM_MUX_00_XSIM_PASS verdict=%s wire=%s c_fix=NONE idle=%0b n_hot=%0d dst=%0d grant_rose=%0b",
      verdict_class, wire_named, snap_idle, n_hot, snap_dst, snap_grant_rose);
    $finish;
  end
  ...
  initial begin
    ...
    $display("WALL_LIMIT");
    $display("XSIM=LIMIT");
    $display("E2R_RPATH_IDLE_CXSIM_MUX_00_FAIL_NO_DESTWAIT reason=wall dst=%0d", tile_dst);
    $finish;
  end
endmodule
