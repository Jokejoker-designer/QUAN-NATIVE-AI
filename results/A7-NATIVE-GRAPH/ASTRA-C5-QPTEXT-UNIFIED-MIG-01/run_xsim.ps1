...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5QTUMIG_FILE_MISSING $p" }
...
if ($hashFail) { throw "C5QTUMIG_HASH_GATE_FAIL do_not_edit_keep_rtl_or_c4q_or_mig_prj" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_qptext_unified_mig.sv"
...
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5QTUMIG_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5QTUMIG_GOLD_HASH_DRIFT GOLDEN.json" }

$dutTxt = [System.IO.File]::ReadAllText($dut)
if ($dutTxt -match "evid_obj") { throw "C5QTUMIG_DUT_HAS_EVID_OBJ" }
if ($dutTxt -match "a7ng_astra_c4_lm06_grounded_gen") { throw "C5QTUMIG_DUT_HAS_GROUNDED_GEN" }
if ($dutTxt -match "a7ng_c4p_ch") { throw "C5QTUMIG_DUT_HAS_C3PROOF_DICT" }
if ($dutTxt -notmatch "a7ng_c4q_ch") { throw "C5QTUMIG_DUT_MISSING_FPGA_DICT" }
if ($dutTxt -notmatch "a7ng_astra_c3_held_out") { throw "C5QTUMIG_DUT_MISSING_C3" }
if ($dutTxt -notmatch "a7ng_astra_c4_lm06_byte256_qptext_gen") { throw "C5QTUMIG_DUT_MISSING_QPTEXT_GEN" }
...
foreach ($f in $svFiles) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C5QTUMIG_LEFTOVER_A09" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C5QTUMIG_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C5QTUMIG_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") { throw "C5QTUMIG_CTX_8255a798_AS_DUT" }
  if ($leaf -eq "a7ng_learned_prior_store.sv") { throw "C5QTUMIG_PRIOR_STORE" }
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5QTUMIG_TINYGPT_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C5QTUMIG_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5QTUMIG_SYNTH_MIG_AS_DUT" }
  if ($leaf -eq "a7ng_lm_graph_arb.sv") { throw "C5QTUMIG_LM_GRAPH_ARB_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5QTUMIG_LIVE_PROD_TOP_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_grounded_gen.sv") { throw "C5QTUMIG_GROUNDED_GEN_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_byte256_ctxcopy.sv") { throw "C5QTUMIG_CTXCOPY_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_byte256_dict.sv") { throw "C5QTUMIG_DICT_LUT_DUT" }
}
...
$pre = @(
  ...
  "# LAW=astra-c5-qptext-unified-mig-01 MIG_XSIM Digilent AXI mig_sim + ddr3_model c4q DUT unified UART + CONFLICT",
...
Write-Host "C5QTUMIG_SHA_FROZEN"
...
function Write-DivLog([string]$code, [string]$detail) {
  [System.IO.File]::WriteAllLines((Join-Path $bag "xsim.log"), @(
    ...
    "RESULT=FAIL",
    "ASTRA_C5_QPTEXT_UNIFIED_MIG_XSIM_PASS ABSENT",
    "PROGRAM=NO BOARD_PASS=NOT_CLAIMED C5_MASTER=OPEN"
  ))
}

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C5QTUMIG_XVLOG_NOT_FOUND" }
if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null

$prj = Join-Path $work "c5_qptext_unified_mig.prj"
$prjLines = @()
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C5QTUMIG_XVLOG_FAIL"
}
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
$xvlogTxt = [System.IO.File]::ReadAllText((Join-Path $bag "xvlog.log"))
if ($xvlogTxt -match "a7ng_astra_09_integ_path") { throw "C5QTUMIG_LEFTOVER_A09_COMPILED" }
if ($xvlogTxt -match "a7ng_query_axi_sparse_stream_intersect") { throw "C5QTUMIG_STREAM02_COMPILED" }
if ($xvlogTxt -match "a7ng_query_axi_sparse_intersect_context") { throw "C5QTUMIG_CTX_COMPILED" }
if ($xvlogTxt -match "tiny_gpt803k_core") { throw "C5QTUMIG_TINYGPT_COMPILED" }
if ($xvlogTxt -match "a7ng_evidence_compose") { throw "C5QTUMIG_COMPOSE_COMPILED" }
if ($xvlogTxt -match "a7ng_astra_c4_lm06_grounded_gen") { throw "C5QTUMIG_GROUNDED_GEN_COMPILED" }
if ($xvlogTxt -match "a7ng_astra_c4_lm06_byte256_ctxcopy") { throw "C5QTUMIG_CTXCOPY_COMPILED" }
if ($xvlogTxt -match "mig_7series_0_mig\.v" -and $xvlogTxt -notmatch "mig_7series_0_mig_sim") { throw "C5QTUMIG_SYNTH_MIG_COMPILED" }
if ($xvlogTxt -notmatch "a7ng_astra_c5_prod_top_c4q") { throw "C5QTUMIG_DUT_NOT_ANALYZED" }
if ($xvlogTxt -match "a7ng_astra_c5_prod_top\.sv") { throw "C5QTUMIG_LIVE_PROD_TOP_COMPILED" }
if ($xvlogTxt -notmatch "mig_7series_0_mig_sim") { throw "C5QTUMIG_MIG_SIM_NOT_ANALYZED" }
if ($xvlogTxt -notmatch "tb_astra_c5_qptext_unified_mig") { throw "C5QTUMIG_TB_NOT_ANALYZED" }
if ($xvlogTxt -notmatch "a7ng_astra_c4_lm06_byte256_qptext_gen") { throw "C5QTUMIG_QPTEXT_GEN_NOT_ANALYZED" }

& "$bin\xelab.bat" -mt off -O0 tb_astra_c5_qptext_unified_mig glbl -s c5qtumig -L unisims_ver -L unimacro_ver -L secureip -timescale 1ps/1ps
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C5QTUMIG_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c5qtumig -R -log $xsimLog
...
if (-not (Test-Path -LiteralPath $xsimLog)) { throw "C5QTUMIG_NO_XSIM_LOG" }
$hasPass = [bool](Select-String -Path $xsimLog -Pattern "^ASTRA_C5_QPTEXT_UNIFIED_MIG_XSIM_PASS$" -Quiet)
...
$need = @(
  ...
  "CLASS_role_reversal HIT",
  "CLASS_parser_amb HIT",
  "CLASS_teacher_off HIT",
...
if ($hasDiv -or (-not $hasPass) -or ($xsimExit -ne 0) -or ($missing.Count -gt 0)) {
  ...
  throw "C5QTUMIG_XSIM_CONTROL_FAIL pass=$hasPass div=$hasDiv exit=$xsimExit missing=$($missing -join ',')"
}
Write-Host "ASTRA_C5_QPTEXT_UNIFIED_MIG_RUN_OK PASS_THIS_GATE_ONLY PROGRAM=NO MIG_XSIM=1 C5_MASTER=OPEN"
