`timescale 1ps / 100fs
// ASTRA-C5-QPTEXT-UNIFIED-MIG-01. PROGRAM=NO. Bag-local TB (test-only plant).
// Existing a7ng_astra_c5_prod_top_c4q through official Digilent AXI MIG + ddr3_model.
// Unified UART classes plus polarity CONFLICT on dest 64, then restore plant.
// Not BOARD. Not Master. Does not edit c4q or live prod_top.
...
module tb_astra_c5_qptext_unified_mig;
  ...
  localparam int NEG0 = 18;
  localparam int NEG1 = 19;
  ...
  task automatic wait_done(input string tag);
    begin
      guard = 0; cap_v = 0; cap_cmt = 0; cap_st = 4'hF; cap_obj = 8'd0; cap_ans = 20'd0; cap_p0 = 20'd0; cap_p1 = 20'd0;
      while (!done && guard < Q_GUARD) begin
        @(posedge ui_clk);
        guard = guard + 1;
        if (c3res && !cap_v) begin
          cap_st = c3st; cap_obj = c3obj; cap_ans = c3ans; cap_p0 = c3p0; cap_p1 = c3p1; cap_cmt = pcmt; cap_v = 1'b1;
        end
      end
  ...
  endtask
  task automatic plant_neg_same_dest;
    begin
      wait_dut_idle();
      wr_f(NEG0, C3_W_SUBJ[0], MIDN, C3_W_REL[0], C3_TR_CONF_W[0], 1, 0, C3_TR_CX1);
      wr_f(NEG1, MIDN, C3_SHARED_DST, C3_W_REL[0], C3_TR_CONF_W[0], 1, 0, C3_TR_CX2);
      plant_release();
    end
  endtask
  task automatic plant_restore_distractors;
    int subj, rel, d0, d1;
    begin
      subj = C3_W_SUBJ[0]; rel = C3_W_REL[0];
      d0 = C3_TR_G0[0] + 2; d1 = C3_TR_G0[0] + 3;
      wait_dut_idle();
  ...
  initial begin
    ...
    $display("C5_QPTEXT_UNIFIED_MIG PROGRAM=NO BOARD_PASS=REJECT MIG=YES TO_CYC_TB_DEFPARAM=65535 CONFLICT=YES DUT=c4q");
    ...
    first_obj = cap_obj;
    saved_w0 = pw0;
    $display("MEAS Q1 st=%0d ans=%0d obj=%0d p0=%0d w0=%0d pvalid=%0d tok0=%0d tok1=%0d gdone=%0d",
      cap_st, cap_ans, cap_obj, cap_p0, pw0, pvalid, gtok0, gtok1, gdone);
    ...
    begin
      ...
      if ((cap_st == A7NG_C5P_ST_ANSWER) && (cap_ans == C3_SHARED_DST[19:0]))
        $display("CLASS_ans_matches_planted_dest HIT ans=%0d", cap_ans);
      else $display("CLASS_ans_matches_planted_dest MISS ans=%0d", cap_ans);
      ...
      if (gdone && (gtok1 === gold0) && (cap_ans == C3_SHARED_DST[19:0]))
        $display("CLASS_gen_tok1_hop2_obj_name HIT tok1=%0d dest=%0d", gtok1, cap_ans);
      else $display("CLASS_gen_tok1_hop2_obj_name MISS tok1=%0d gold0=%0d ans=%0d", gtok1, gold0, cap_ans);
      if (gtok0 !== gold0)
        $display("CLASS_tok0_not_ans_dict HIT tok0=%0d gold0=%0d", gtok0, gold0);
      else $display("CLASS_tok0_not_ans_dict MISS tok0=%0d", gtok0);
      chk("Q1_GLUE", gdone && (gtok0 === glue) && (gtok1 === gold0));
    end

    plant_neg_same_dest();
    uart_str("pump requires indirect");
    wait_done("QC_DONE");
    $display("MEAS QC conflict st=%0d ans=%0d p0=%0d p1=%0d cmt=%0d", cap_st, cap_ans, cap_p0, cap_p1, cap_cmt);
    if ((cap_st == A7NG_C5P_ST_CONFLICT) && (cap_ans == 20'd0))
      $display("CLASS_conflict HIT st=%0d", cap_st);
    else $display("CLASS_conflict MISS st=%0d ans=%0d", cap_st, cap_ans);
    chk("QC_CONFLICT", (cap_st == A7NG_C5P_ST_CONFLICT) && (cap_ans == 20'd0));
    if (!cap_cmt && !pcmt) $display("CLASS_conflict_no_upd HIT");
    else $display("CLASS_conflict_no_upd MISS cap_cmt=%0d pcmt=%0d", cap_cmt, pcmt);
    chk("QC_NO_UPD", !cap_cmt && !pcmt);

    wait_dut_idle();
    plant_restore_distractors();
    ...
    if (fail == 0) $display("ASTRA_C5_QPTEXT_UNIFIED_MIG_XSIM_PASS");
    else $display("ASTRA_C5_QPTEXT_UNIFIED_MIG_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO quality=MIG_PHY_C4Q_UNIFIED_UART_PLUS_POLARITY_CONFLICT UART_SIM_BAUD GLUE_PLUS_OBJECT");
    $finish;
  end

  initial begin
    #25s;
    $display("FIRST_DIVERGENCE MIG_TIMEOUT calib_or_qptext_unified hung");
    $display("RESULT=FAIL");
    $display("ASTRA_C5_QPTEXT_UNIFIED_MIG_XSIM_PASS ABSENT");
    $finish;
  end
endmodule
