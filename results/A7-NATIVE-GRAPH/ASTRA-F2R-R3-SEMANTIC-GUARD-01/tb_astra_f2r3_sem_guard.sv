...
module tb_astra_f2r3_sem_guard;
  ...
  a7ng_astra_f2r3_sem_guard u_dut (
  ...
  task automatic plant4(input int hi_slot);
    ...
    begin
      ...
      for (s=0;s<4;s=s+1) begin
        ...
        mem_wr(FACT_BASE+(e0<<4), fact_pack(10,mid,2,e0,c,1,1,0));
        mem_wr(FACT_BASE+(e1<<4), fact_pack(mid,4,2,e1,c,1,1,0));
      end
    end
  endtask
  task automatic plant5;
    logic [127:0] b0,b1,b2; int s, mid, e0, e1; int mids [0:4];
    begin
      mids[0]=1; mids[1]=2; mids[2]=3; mids[3]=8; mids[4]=9;
      reset_mem();
      b0 = 32'd23; b0=(b0<<32)|32'd22; b0=(b0<<32)|32'd21; b0=(b0<<32)|32'd20;
      b1 = 32'd27; b1=(b1<<32)|32'd26; b1=(b1<<32)|32'd25; b1=(b1<<32)|32'd24;
      b2 = 32'd0;  b2=(b2<<32)|32'd0;  b2=(b2<<32)|32'd29; b2=(b2<<32)|32'd28;
      mem_wr(POST_HEAP, b0);
      mem_wr(POST_HEAP+28'd16, b1);
      mem_wr(POST_HEAP+28'd32, b2);
      plant_keys(10);
      for (s=0;s<5;s=s+1) begin
        mid=mids[s]; e0=20+2*s; e1=21+2*s;
        mem_wr(FACT_BASE+(e0<<4), fact_pack(10,mid,2,e0,200,1,1,0));
        mem_wr(FACT_BASE+(e1<<4), fact_pack(mid,4,2,e1,200,1,1,0));
      end
    end
  endtask
  task automatic plant_polarity;
    logic [127:0] beat;
    begin
      reset_mem();
      beat = 32'd35; beat=(beat<<32)|32'd18; beat=(beat<<32)|32'd34; beat=(beat<<32)|32'd17;
      mem_wr(POST_HEAP, beat);
      plant_keys(4);
      mem_wr(FACT_BASE+(17<<4), fact_pack(10,1,2,17,8,1,1,0));
      mem_wr(FACT_BASE+(34<<4), fact_pack(1,4,2,34,8,1,1,0));
      mem_wr(FACT_BASE+(18<<4), fact_pack(10,8,2,18,200,1,1,0));
      mem_wr(FACT_BASE+(35<<4), fact_pack(8,4,2,35,200,1,0,0));
    end
  endtask
  task automatic plant_1hop;
    logic [127:0] beat;
    begin
      reset_mem();
      beat = 32'd0; beat=(beat<<32)|32'd0; beat=(beat<<32)|32'd0; beat=(beat<<32)|32'd40;
      mem_wr(POST_HEAP, beat);
      plant_keys(1);
      mem_wr(FACT_BASE+(40<<4), fact_pack(10,1,2,40,200,0,1,0));
    end
  endtask
  task automatic plant_ctx;
    logic [127:0] beat;
    begin
      reset_mem();
      beat = 32'd0; beat=(beat<<32)|32'd0; beat=(beat<<32)|32'd51; beat=(beat<<32)|32'd50;
      mem_wr(POST_HEAP, beat);
      plant_keys(2);
      mem_wr(FACT_BASE+(50<<4), fact_pack(10,11,2,50,200,0,1,2));
      mem_wr(FACT_BASE+(51<<4), fact_pack(10,1,2,51,8,0,1,1));
    end
  endtask
  ...
  initial begin
    ...
    $display("SMOKE npath=%0d p0=%0d p1=%0d ans=%0d sel=%0d phi0=%0d txn=%0d gen=%0d tbl=%0d obj=%0d ctx=%0d st=%0d",
      npath,p0,p1,ans,sel,phi0,txn,gen,tbl,qobj,qctx,st);
    chk("SMOKE_TWO_PROOFS", !tbl && npath>=2 && p0===20'd17 && ans===20'd4 && sel===2'd0
        && phi0===8'sd50 && st===ST_ANSWER && qobj===8'd0 && qctx===8'd2);
    snap(); oracle_upd(-3);
    pulse_rew(-3);
    ...
    send_text("pump requires indirect"); wait_done();
    $display("CONFLICT_DEST npath=%0d ans=%0d p0=%0d st=%0d acc=%0d", npath,ans,p0,st,pend_acc);
    chk("CONFLICT_DEST", st===ST_CONFLICT && ans===20'd0 && p0===20'd0 && !pend_acc && npath>=2);
    pulse_rew(-3);
    ...
    for (hi=0; hi<4; hi=hi+1) begin
      ...
      send_text("pump requires indirect"); wait_done();
      $display("SLOT%0d npath=%0d sel=%0d p0=%0d phi0=%0d ans=%0d st=%0d", hi, npath, sel, p0, phi0, ans, st);
      chk($sformatf("SLOT%0d_WIN", hi), npath===5'd4 && sel===hi[1:0] && phi0===8'sd50 && ans===20'd4 && st===ST_ANSWER);
      snap(); oracle_upd(-3);
      pulse_rew(-3);
      repeat(250) @(posedge clk);
    ...
    $display("WRONG_OBJ st=%0d npath=%0d ans=%0d p0=%0d obj=%0d ctx=%0d acc=%0d",
      st,npath,ans,p0,qobj,qctx,pend_acc);
    chk("WRONG_OBJ", st===ST_UNKNOWN && ans===20'd0 && p0===20'd0 && npath===5'd0
        && qobj===8'd11 && qctx===8'd2 && !pend_acc);
    retire_q();

    hard_rst(); plant2();
    send_text("pump requires compressor"); wait_done();
    $display("DIR_NO_STEAL st=%0d npath=%0d ans=%0d obj=%0d ctx=%0d", st,npath,ans,qobj,qctx);
    chk("DIR_NO_STEAL", st===ST_UNKNOWN && ans===20'd0 && npath===5'd0 && qobj===8'd4 && qctx===8'd0);
    retire_q();

    hard_rst(); plant2();
    send_text("pump requires indirect compressor"); wait_done();
    $display("IND_USE_2HOP st=%0d npath=%0d ans=%0d p0=%0d obj=%0d ctx=%0d", st,npath,ans,p0,qobj,qctx);
    chk("IND_USE_2HOP", st===ST_ANSWER && ans===20'd4 && p0===20'd17 && npath>=2 && qobj===8'd4 && qctx===8'd2);
    retire_q();

    hard_rst(); plant_1hop();
    send_text("pump requires chiller"); wait_done();
    $display("DIR_1HOP st=%0d npath=%0d ans=%0d p0=%0d p1=%0d obj=%0d", st,npath,ans,p0,p1,qobj);
    chk("DIR_1HOP", st===ST_ANSWER && ans===20'd1 && p0===20'd40 && p1===20'd0 && npath===5'd1 && qobj===8'd1);
    retire_q();

    hard_rst(); plant_1hop();
    send_text("pump requires indirect chiller"); wait_done();
    $display("IND_NO_1HOP st=%0d npath=%0d ans=%0d obj=%0d ctx=%0d", st,npath,ans,qobj,qctx);
    chk("IND_NO_1HOP", st===ST_UNKNOWN && ans===20'd0 && npath===5'd0 && qctx===8'd2);
    retire_q();

    hard_rst(); plant_polarity();
    send_text("pump requires indirect"); wait_done();
    $display("POLARITY st=%0d npath=%0d ans=%0d acc=%0d", st,npath,ans,pend_acc);
    chk("POLARITY", st===ST_CONFLICT && ans===20'd0 && p0===20'd0 && !pend_acc);
    retire_q();

    hard_rst(); plant_ctx(); load_w0(16'sd16);
    send_text("pump requires water"); wait_done();
    $display("CONTEXT st=%0d npath=%0d ans=%0d p0=%0d obj=%0d ctx=%0d", st,npath,ans,p0,qobj,qctx);
    chk("CONTEXT", st===ST_ANSWER && ans===20'd1 && p0===20'd51 && npath===5'd1 && qctx===8'd1 && qobj===8'd0);
    retire_q();

    hard_rst(); plant5();
    send_text("pump requires indirect"); wait_done();
    $display("FIFTH st=%0d npath=%0d ans=%0d p0=%0d acc=%0d", st,npath,ans,p0,pend_acc);
    chk("FIFTH", st===ST_INCOMP && npath>=5'd5 && ans===20'd0 && p0===20'd0 && !pend_acc);
    retire_q();

    hard_rst(); plant2();
    send_text("pump requires indirect"); wait_done();
    chk("PRE_UNREL", st===ST_ANSWER && npath>=2);
    retire_q();
    send_text("payroll tax form"); wait_done();
    $display("UNREL st=%0d npath=%0d ans=%0d p0=%0d", st,npath,ans,p0);
    chk("UNREL_NO_STALE", st===ST_UNKNOWN && npath===5'd0 && ans===20'd0 && p0===20'd0);

    if (fail==0) $display("ASTRA_F2R3_SEM_GUARD_XSIM_PASS");
    else $display("ASTRA_F2R3_SEM_GUARD_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $finish;
  end
endmodule
