// E2R-TILE-TOK-NLINE-CXSIM-00 — TOK nline after POS region
// Copy of tb_e2r_tile_thrash_next_cxsim_00.sv. ONE change: do not stop
// at first dest=4 after TOK switch. Completable stub for EVERY dma_go
// (8 R + done + busy clear). Watch until stall=0 again OR timeout
// >=2_000_000 dest-clk after switch. Classify TOK_DONE / TOK_HOLD /
// EARLY_IDLE / NO_THRASH only.
...
// After first stall=0, addr_a=OFF_TOK. Do not force dest/bst. Do not C-FIX.
// UNKNOWN: after TOK switch, dest4 increment == 1024 and stall=0?
// XSim ≠ board.
`timescale 1ns/1ps

module tb_e2r_tile_tok_nline_cxsim_00;
  import a7lm06_pkg::*;

  localparam realtime CLK_HALF = 5.0; // 10 ns → 100 MHz
  localparam int unsigned TO_FIRST = 1000000;
  localparam int unsigned TO_AFTER = 2000000;
  localparam int unsigned TOK_NLINE_LAW = 1024;
  ...
  int unsigned nline_tok;
  int unsigned stall0_cyc;
  int unsigned stall0_tok_cyc;
  ...
  task automatic emit_class(input string cls);
    begin
      ...
      $display("PROGRAM=NO");
      $display("XSIM=PASS");
      $display("TIMEOUT_FIRST=%0d TIMEOUT_AFTER=%0d TOK_NLINE_LAW=%0d",
               TO_FIRST, TO_AFTER, TOK_NLINE_LAW);
      $display("NLINE_MEAS=%0d NLINE_TOK=%0d NLINE_H=%0d NLINE_LAW_HOLD=%0d HOLD_RG=%0d CUR_RG=%0d DBG_CUR_RG=%0d CH=%0d",
               nline_meas, nline_tok, nline_live, nline_law(hold_rg_live),
      ...
      $display("STALL0_CYC=%0d STALL0_TOK_CYC=%0d SWITCH_CYC=%0d DEST4_POST_CYC=%0d STALL_END=%0d BST_END=%0d",
               stall0_cyc, stall0_tok_cyc, switch_cyc, dest4_post_cyc, stall_end, bst_end);
      ...
      $display("E2R_TILE_TOK_NLINE_CXSIM_00_XSIM_PASS class=%s c_fix=NONE dest4_cnt=%0d dest4_after_switch=%0d dma_go_burst=%0d nline=%0d nline_tok=%0d stall_end=%0d stall0_cyc=%0d stall0_tok_cyc=%0d switch_cyc=%0d dest4_post_cyc=%0d cur_rg_before=%0d cur_rg_after=%0d miss_before=%0d miss_after=%0d dest5=%0d dest0=%0d dest4_2=%0d dest4_after=%0d",
               cls, dest4_cnt, dest4_after_switch, dma_go_burst_cnt, nline_meas,
               nline_tok, stall_end, stall0_cyc, stall0_tok_cyc, switch_cyc, dest4_post_cyc,
               cur_rg_before, cur_rg_after, miss_before, miss_after,
               saw_d5_1, saw_idle_after_ack, saw_d4_2, saw_d4_after);
      $finish;
    end
  endtask

  task automatic classify_second_stall0;
    begin
      saw_stall0_tok = 1'b1;
      stall0_tok_cyc = cyc;
      dest4_after_switch = dest4_cnt - dest4_at_switch;
      if (nline_tok == 0)
        nline_tok = int'(nline_live);
      $display("SNAP_STALL0_AFTER_TOK cyc=%0d dest=%0d bst=%0d dest4=%0d dest4_after=%0d burst=%0d nline_h=%0d nline_tok=%0d hold_rg=%0d cur_rg=%0d miss=%0d stall=%0d ch=%0d",
               cyc, dest_live, bst_live, dest4_cnt, dest4_after_switch,
               dma_go_burst_cnt, nline_live, nline_tok, hold_rg_live,
               cached_rg, dbg_miss, stall, ch_live);
      if (dest4_after_switch == TOK_NLINE_LAW)
        emit_class("TOK_DONE");
      else
  ...
  initial begin
    ...
    $display("MISS_FORCE addr_a=OFF_POS(%0d) then after stall=0 addr_a=OFF_TOK(%0d) we_a=0",
             OFF_POS, OFF_TOK);
    $display("RG_NLINE_LAW POS_rg1=128 TOK_rg0=%0d rg_of(<OFF_POS)=0", TOK_NLINE_LAW);
    ...
    $display("WATCH first stall==0 then TOK switch; do NOT stop at first dest=4; stall=0 again OR timeout TO_AFTER=%0d; TO_FIRST=%0d",
             TO_AFTER, TO_FIRST);
    ...
    forever begin
      ...
      if (dma_go && !dma_go_d) begin
        ...
        if (dma_go_burst_cnt <= 2 || (dma_go_burst_cnt % 32) == 0 || switched)
          $display("SNAP_GO_BURST cyc=%0d burst=%0d dest=%0d dbg_dst=%0d bst=%0d stall=%0d nline_h=%0d hold_rg=%0d cur_rg=%0d miss=%0d ch=%0d dest4=%0d dest4_after=%0d switched=%0d",
                   cyc, dma_go_burst_cnt, dest_live, dbg_dst, bst_live, stall,
                   nline_live, hold_rg_live, cached_rg, dbg_miss, ch_live,
                   dest4_cnt, (dest4_cnt - dest4_at_switch), switched);
      end
      if (dma_go)
      ...
      if (dest_live == D_WAITDONE && !dest4_d) begin
        ...
        if (dest4_cnt == 1) begin
          ...
          dest4_after_switch = dest4_cnt - dest4_at_switch;
          nline_tok = int'(nline_live);
          $display("SNAP_D4_AFTER_SWITCH cyc=%0d dest=%0d bst=%0d stall=%0d dest4=%0d dest4_after=%0d nline_h=%0d hold_rg=%0d cur_rg=%0d miss=%0d ch=%0d burst=%0d CONTINUE_WATCH",
                   cyc, dest_live, bst_live, stall, dest4_cnt, dest4_after_switch,
                   nline_live, hold_rg_live, cached_rg, dbg_miss, ch_live,
                   dma_go_burst_cnt);
        end else if ((dest4_cnt % 32) == 0 || dest4_cnt == nline_meas ||
                     (switched && ((dest4_cnt - dest4_at_switch) == TOK_NLINE_LAW))) begin
          $display("SNAP_D4 cyc=%0d dest4=%0d dest4_after=%0d dest=%0d bst=%0d stall=%0d nline_h=%0d hold_rg=%0d cur_rg=%0d miss=%0d ch=%0d burst=%0d switched=%0d",
                   cyc, dest4_cnt, (dest4_cnt - dest4_at_switch), dest_live, bst_live, stall,
                   nline_live, hold_rg_live, cached_rg, dbg_miss, ch_live,
      ...
      // Same as THRASH-NEXT: after first stall=0, drive TOK address.
      ...
      // ONE change vs THRASH-NEXT: do not stop at first dest=4 after switch.
      // Require stall rose after the switch, then wait for stall=0 again.
      if (switched && stall == 1'b1)
        saw_stall1_after = 1'b1;

      if (!classified && switched && saw_stall1_after && stall == 1'b0) begin
        classify_second_stall0();
      end

      if (!classified && switched && (cyc - switch_cyc) >= TO_AFTER) begin
        dest4_after_switch = dest4_cnt - dest4_at_switch;
        $display("TIMEOUT_AFTER_SWITCH cyc=%0d delta=%0d dest=%0d bst=%0d stall=%0d dest4=%0d dest4_at_switch=%0d dest4_after=%0d burst=%0d cur_rg=%0d miss=%0d saw_d4_after=%0d",
                 cyc, (cyc - switch_cyc), dest_live, bst_live, stall, dest4_cnt,
                 dest4_at_switch, dest4_after_switch, dma_go_burst_cnt,
                 cached_rg, dbg_miss, saw_d4_after);
        if (!saw_d4_after)
          emit_class("NO_THRASH");
        else if (stall == 1'b0)
          classify_second_stall0();
        else
          emit_class("TOK_HOLD");
