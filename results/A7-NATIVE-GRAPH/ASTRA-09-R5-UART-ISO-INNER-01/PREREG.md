# PREREG — ASTRA-09-R5-UART-ISO-INNER-01

Frozen before xvlog. PROGRAM=NO. No board. No JTAG/xsdb/COM12/bitstream.
Does not edit ASTRA-09-R4-UART-ISO-01 / ASTRA-09-R3-UART-XSIM-01 /
...
Does **not** claim ASTRA-09-R4 sibling `u_iso` numbers as this bag.

## Claim this bag may close

One named UART-facing INNER ISO XSim (this bag only): a new wrap instantiates
frozen `a7ng_astra_09_r2_cand_ovf` (`15a919f1…`) **only**. The one SGD is
A09-R2 inner `u_sgd` (`a7ng_shared_rank_sgd_q8_sym_f2r2`, `b66ef328…`). Wrap/TB
must not instantiate a parallel ISO SGD for the pass numbers. Host UART 115200
8N1 delivers an isolated SGD command (`load_from_tb=0`) into inner
`go_upd`/`x`/`rew`. Byte stream / hierarchical `u_a09r2.u_sgd.w_o[0]` shows:
...
Can UART 8N1 drive `go_upd`/`x`/`rew` into **A09-R2 inner `u_sgd`**
(hierarchical `u_a09r2.u_sgd.w_o[0]`) so ISO_P3 w0=+5 and ISO_M3 w0=-6, with
`load_from_tb=0` — without a sibling SGD?

## Registered caps

```text
DUT_MODULE      = a7ng_astra_09_r2_cand_ovf (instantiated, not patched)
INNER_SGD       = u_a09r2.u_sgd (a7ng_shared_rank_sgd_q8_sym_f2r2 child; not a wrap sibling)
...
| Tag | Setup | Expect |
|-----|-------|--------|
| ISO_P3_X50_DW5 | UART ISO cmd x0=50 rew=+3 from w=0 | UART frame w0=+5 v=0 w1=0 tbl=0; hier `u_a09r2.u_sgd.w_o[0]`=+5 |
| ISO_M3_X64_DW6 | wrap reset (w=0); UART ISO cmd x0=64 rew=-3 | UART frame w0=-6 v=0 w1=0 tbl=0; hier `u_a09r2.u_sgd.w_o[0]`=-6 |
| HIER_TBL0 | hierarchical `load_from_tb_o` | 0 on both ISO frames |
| NO_SIBLING | wrap instantiates A09-R2 only | pass numbers from inner `u_sgd`, not a second SGD instance |
...
ASTRA-09-R4 sibling-ISO promotion. Silicon UART. Patching frozen A09 leftover
`ans=4`. Patching frozen A09-R2 DUT. Patching frozen SGD.
