...
$files = @(
  ...
  (Join-Path $bag "tb_astra_rtp_r1.sv")
)
& "$bin\xvlog.bat" --sv -i $incq -i $incc $files
if ($LASTEXITCODE -ne 0) { throw "R1_XVLOG_FAIL" }
& "$bin\xelab.bat" tb_astra_rtp_r1 -s r1 -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) { throw "R1_XELAB_FAIL" }
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" r1 -R -log $xsimLog
if ($LASTEXITCODE -ne 0) { throw "R1_XSIM_FAIL" }
if (-not (Select-String -Path $xsimLog -Pattern "ASTRA_RTP_R1_XSIM_PASS" -Quiet)) { throw "R1_PASS_MARKER_MISSING" }
Write-Host "ASTRA_RTP_R1_RUN_OK"
