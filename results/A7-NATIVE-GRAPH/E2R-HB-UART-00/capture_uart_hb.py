#!/usr/bin/env python3
"""Capture COM12 for E2R-HB-UART-00 — heartbeats + optional pred=664."""
...
STAGES = ["BOOT", "MIG_OK", "WMEM_OK", "SOA_OK", "CORE_START", "BIND_DONE", "LM_ACTIVE"]
...
def main():
    ...
    while time.time() - t0 < args.seconds:
        ...
        if b"pred=" in buf and b"\n" in buf[buf.find(b"pred="):]:
            # keep listening briefly for newline completeness already handled
            break
    ser.close()
    text = buf.decode("ascii", errors="replace")
    open(args.out, "w", encoding="utf-8").write(text)
    seen = [s for s in STAGES if s in text]
