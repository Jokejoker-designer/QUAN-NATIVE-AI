# ASTRA-11-A09R7-LED-IO-01. PROGRAM=NO. BIT=NO. write_bitstream forbidden.
# Instantiates a7ng_astra_09_r2_cand_ovf as u_a09r2. UART D10/A9 kept. LED LD4-LD7
# H5/J5/T9/T10 (cited constraints/arty_a7_100.xdc). LED delays 2.000/0.500 vs clk50u
# frozen in PREREG; applied post-synth via led_iodelay.xdc. UART envelope kept.
# Frozen a7ng_astra_09_integ_path.sv is not compiled. synth + opt + place + route. No bitstream.
set bag  [file normalize [file dirname [info script]]]
set root [file normalize [file join $bag ../../..]]
set env(XILINXD_LICENSE_FILE) {D:\Xilinx\licenses\vivado_basic.lic}
set part xc7a100tcsg324-1
set top  a7ng_astra_11_a09r7_led_io_wrap
set first_div ""
...
set files [list \
  ...
  [file join $bag a7ng_astra_11_a09r7_led_io_plant.sv] \
  [file join $bag a7ng_astra_11_a09r7_led_io_wrap.sv] \
]
set xdc [file join $bag clk50_led_io.xdc]
...
proc astra_forbid_bit {args} {
  ...
  puts ASTRA_11_A09R7_LED_IO_ABORTED
  exit 1
}
...
proc astra_copy_fail_r0 {} {
  ...
  puts "ASTRA_11_A09R7_LED_IO_FAIL_R0_PRESERVED $dst"
}
...
proc astra_force_iob_true {} {
  set n 0
  set cells [get_cells -quiet -hierarchical -filter {NAME =~ *uart_rx_iob_reg* || NAME =~ *uart_tx_iob_reg* || NAME =~ *led_q_reg*}]
  foreach c $cells {
...
if {[catch {synth_design -top $top -part $part} err]} {
  ...
  puts ASTRA_11_A09R7_LED_IO_SYNTH_FAIL
  exit 1
}
...
astra_util_extract [file join $bag util_synth.rpt] [file join $bag UTIL_EXTRACT_SYNTH.txt] Synthesized
astra_timing_extract [file join $bag timing_synth.rpt] [file join $bag TIMING_EXTRACT_SYNTH.txt] Synthesized
puts ASTRA_11_A09R7_LED_IO_SYNTH_DONE
...
if {[catch {opt_design} err]} {
  astra_fail OPT $err
  puts ASTRA_11_A09R7_LED_IO_OPT_FAIL
  exit 1
}
astra_force_iob_true
astra_apply_hold_policy
puts ASTRA_11_A09R7_LED_IO_OPT_DONE
...
if {[catch {place_design} err]} {
  ...
  puts ASTRA_11_A09R7_LED_IO_PLACE_FAIL
  exit 1
}
...
astra_apply_hold_policy
puts ASTRA_11_A09R7_LED_IO_PLACE_DONE
...
if {[catch {route_design} err]} {
  ...
  puts ASTRA_11_A09R7_LED_IO_ROUTE_FAIL
  exit 1
}
...
puts "ASTRA_11_A09R7_LED_IO_ROUTE_DONE WNS=$wns TNS=$tns WHS=$whs THS=$ths LED_IOB=YES UART_IOB=YES UART_IOBFF=$packed LED_IOBFF=$led_packed LED_OUT_MAX=2.000 LED_OUT_MIN=0.500 PROGRAM=NO BIT=NOT_BUILT"
...
if {$fail_whs || $fail_wns} {
  ...
  # Do not invent delay numbers. Do not patch uart_rx.
  astra_apply_hold_policy
  if {[catch {phys_opt_design} e2]} {
    puts "ASTRA_11_A09R7_LED_IO_EXP_PHYS_OPT_WARN $e2"
  } else {
    puts ASTRA_11_A09R7_LED_IO_EXP_PHYS_OPT_DONE
  }
  ...
  puts ASTRA_11_A09R7_LED_IO_EXP_DONE
  ...
  puts "ASTRA_11_A09R7_LED_IO_DONE WNS=$wns WHS=$whs LED_IOB=$led_iob UART_IOB=YES UART_IOBFF=$packed LED_IOBFF=$led_packed BIT=NOT_BUILT PROGRAM=NO RESULT=$result_tag PRODUCTION_TOP=UNKNOWN"
  exit 0
}

puts "ASTRA_11_A09R7_LED_IO_DONE WNS=$wns TNS=$tns WHS=$whs LED_IOB=YES UART_IOB=YES UART_IOBFF=$packed LED_IOBFF=$led_packed BIT=NOT_BUILT PROGRAM=NO PRODUCTION_TOP=UNKNOWN"
exit 0
