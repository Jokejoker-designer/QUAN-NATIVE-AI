`timescale 1ns / 1ps
// ASTRA-C5-CONFLICT-01. PROGRAM=NO. Bag-local TB (test-only plant).
// Same C5 production hierarchy. Modeled AXI, not MIG. A09 is not DUT.
`include "a7ng_astra_c5_prod_top.svh"
`include "a7ng_astra_c5_ddr_arb.svh"
`include "tb_oracles.svh"
module tb_astra_c5_conflict;
  ...
  task automatic wait_done(input string tag);
    begin
      guard = 0; cap_v = 0; cap_st = 4'hF; cap_ans = 20'd0; cap_p0 = 20'd0; cap_p1 = 20'd0; cap_cmt = 0;
      while (!done && guard < 400000) begin
        @(posedge clk);
        guard = guard + 1;
        if (c3res && !cap_v) begin
          cap_st = c3st; cap_ans = c3ans; cap_p0 = c3p0; cap_p1 = c3p1; cap_cmt = pcmt; cap_v = 1'b1;
        end
      end
  ...
  task automatic plant_neg_same_dest;
    logic [127:0] b0;
    begin
      // One AXI beat holds four 32-bit IDs. count=4 uses arlen=0.
      b0 = 32'd0;
      b0 = (b0 << 32) | E3;
      b0 = (b0 << 32) | E2;
      b0 = (b0 << 32) | E1;
  ...
  initial begin
    ...
    $display("C5_CONFLICT PROGRAM=NO BOARD_PASS=REJECT DDR_QUERY_BOUND_FINAL=NOT_FROZEN MIG=NO A09=NO");
    guard = 0;
    ...
    if (fail == 0) $display("ASTRA_C5_CONFLICT_XSIM_PASS");
    else $display("ASTRA_C5_CONFLICT_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO quality=MODELED_AXI_POLARITY_CONFLICT_NOT_A09");
    $finish;
  end
endmodule
