...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5A_FILE_MISSING $p" }
...
if ($hashFail) { throw "C5A_HASH_GATE_FAIL do_not_edit_keep_rtl" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_ddr_arb.sv"
...
$oldarb = Join-Path $root "rtl\native_graph\integrate\a7ng_lm_graph_arb.sv"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5A_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5A_GOLD_HASH_DRIFT GOLDEN.json" }

$files = @($dut, $tb)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5A_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5A_SYNTH_MIG_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C5A_C0_SPARSE_AS_DUT" }
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C5A_LEFTOVER_A09" }
}
...
$pre = @(
  ...
  "# LAW=six-client exclusive DDR owner; a7ng_lm_graph_arb not edited, not DUT"
) + $hashLines
$pre += "# COMPILED"
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C5_DDR_ARB_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C5A_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C5_DDR_ARB_RUN_OK"
