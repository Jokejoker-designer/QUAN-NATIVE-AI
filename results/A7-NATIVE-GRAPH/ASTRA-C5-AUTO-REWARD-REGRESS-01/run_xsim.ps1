...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5AR_FILE_MISSING $p" }
...
if ($hashFail) { throw "C5AR_HASH_GATE_FAIL do_not_edit_keep_rtl" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_auto_reward_regress.sv"
$dut = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_prod_top.sv"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5AR_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5AR_GOLD_HASH_DRIFT GOLDEN.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5AR_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C5AR_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5AR_SYNTH_MIG_AS_DUT" }
  if ($leaf -match "a7ng_astra_c5_prod_top_rew") { throw "C5AR_REW_TOP_NOT_THIS_FALSIFIER" }
}

$stamp = (Get-Date).ToString("yyyy-MM-ddTHH:mm:ss.fffffffK")
$pre = @(
  "# SHA freeze BEFORE xvlog $stamp",
  "# PROGRAM=NO BOARD_PASS=NOT_CLAIMED C5_MASTER=OPEN FALSIFIER=AUTO_REWARD MIG=NO"
) + $hashLines
$pre += "# COMPILED"
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C5_AUTO_REWARD_REGRESS_XSIM_PASS" -Quiet)
  $hasPcmt = [bool](Select-String -Path $xsimLog -Pattern "CLASS_auto_rew_pcmt HIT" -Quiet)
  $hasW0 = [bool](Select-String -Path $xsimLog -Pattern "CLASS_auto_rew_w0 HIT" -Quiet)
  $hasNoR = [bool](Select-String -Path $xsimLog -Pattern "CLASS_no_uart_reward_frame HIT" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass) -or (-not $hasPcmt) -or (-not $hasW0) -or (-not $hasNoR)) {
  if (Test-Path -LiteralPath $xsimLog) {
  ...
  throw "C5AR_XSIM_FAIL_OR_MARKER_MISSING pass=$hasPass pcmt=$hasPcmt w0=$hasW0 nor=$hasNoR"
}
Write-Host "ASTRA_C5_AUTO_REWARD_REGRESS_RUN_OK"
