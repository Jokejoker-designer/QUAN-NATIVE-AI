...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C1PAGESKIP_FILE_MISSING $p" }
...
if ($hashFail) {
  ...
  throw "C1PAGESKIP_HASH_GATE_FAIL do_not_invent"
}
...
$tb = Join-Path $bag "tb_astra_c1_page_skip.sv"
$hostPy = Join-Path $bag "host_astra_c1_page_skip.py"
...
if (Test-Path -LiteralPath $lexBag) { throw "C1PAGESKIP_BAG_LEXICON_SHADOW_FORBIDDEN" }
if (-not (Test-Path -LiteralPath $goldJson)) { throw "C1PAGESKIP_GOLD_MISSING GOLDEN.json" }
if (-not (Test-Path -LiteralPath $goldSvh)) { throw "C1PAGESKIP_GOLD_MISSING query_gold.svh" }
if (-not (Test-Path -LiteralPath $corpus)) { throw "C1PAGESKIP_GOLD_MISSING corpus.json" }
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C1PAGESKIP_GOLD_NOT_HASHED_BEFORE_XVLOG" }
if (-not (Test-Path -LiteralPath $pageDut)) { throw "C1PAGESKIP_DUT_MISSING a7ng_query_axi_sparse_page_skip.sv" }
$liveGold = Sha256 $goldJson
$liveSvh = Sha256 $goldSvh
$liveCorpus = Sha256 $corpus
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C1PAGESKIP_GOLD_HASH_DRIFT GOLDEN.json" }
if ($preTxt -notmatch $liveSvh) { throw "C1PAGESKIP_GOLD_HASH_DRIFT query_gold.svh" }
if ($preTxt -notmatch $liveCorpus) { throw "C1PAGESKIP_GOLD_HASH_DRIFT corpus.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") {
    throw "C1PAGESKIP_LEFTOVER_A09_ON_XVLOG_LIST"
  }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") {
    throw "C1PAGESKIP_FROZEN_AXI_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_relbind.sv") {
    throw "C1PAGESKIP_RELBIND_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect.sv") {
    throw "C1PAGESKIP_KEY_INTERSECT_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") {
    throw "C1PAGESKIP_STREAM02_MUST_NOT_BE_DUT"
  }
}
...
$pre = @(
  ...
  "# PROGRAM=NO C1_800K=OPEN CAND_CAP=16 N=256 N_BUCKETS=4096 LAW=qse-v2-page-skip-01 POKE_V=0",
...
$pre += "$liveGold  results/A7-NATIVE-GRAPH/ASTRA-C1-PAGE-SKIP-01/GOLDEN.json"
$pre += "$liveSvh  results/A7-NATIVE-GRAPH/ASTRA-C1-PAGE-SKIP-01/query_gold.svh"
$pre += "$liveCorpus  results/A7-NATIVE-GRAPH/ASTRA-C1-PAGE-SKIP-01/corpus.json"
$pre += "# BAG"
...
Write-Host "C1PAGESKIP_SHA_FROZEN_BEFORE_XVLOG"

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C1PAGESKIP_XVLOG_NOT_FOUND" }
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C1_PAGE_SKIP_XSIM_PASS" -Quiet)
  ...
  $hasHocFail = [bool](Select-String -Path $xsimLog -Pattern "FAIL HOC_" -Quiet)
  $hasLateMiss = [bool](Select-String -Path $xsimLog -Pattern "FAIL LATE_GOLD" -Quiet)
...
if ($hasRedCap) {
  ...
  throw "C1PAGESKIP_REDUCTION_X1000_EMITTED_AS_CAP_N"
}
if (($xsimExit -ne 0) -or (-not $hasPass) -or $hasDiv -or $hasLeak -or $hasHocFail -or $hasLateMiss -or $hasDirectFail) {
  $r0 = Join-Path $bag "xsim_fail_r0.log"
  if ((Test-Path -LiteralPath $xsimLog) -and (-not (Test-Path -LiteralPath $r0))) {
    Copy-Item $xsimLog $r0 -Force
  }
  throw "C1PAGESKIP_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C1_PAGE_SKIP_01_RUN_OK"
