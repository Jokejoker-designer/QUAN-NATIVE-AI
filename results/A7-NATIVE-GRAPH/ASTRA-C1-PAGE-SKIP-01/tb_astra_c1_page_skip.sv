// tb_astra_c1_page_skip.sv — C1 N=256 qse-v2-page-skip-01. PROGRAM=NO.
// Frozen QSE + relbind keys + NEW page-skip walker. poke_v=0. leftover A09 off.
// C0 59-word lexicon (no bag shadow). N_BUCKETS=4096. CAND_CAP=16 after emit.
// CLASS_direct must emit {110,144,145}. hoc POST_R_BEATS < AXI-BEAT 14 AND skip>=1.
// STREAM-02 late-gold nid 254 in corpus must emit; SEARCH_INCOMPLETE does not hide a miss.
// G_BYTES LSB-first: char0 at [7:0]; extract bytes[8*bi +: 8] only.
`timescale 1ns / 1ps

module tb_astra_c1_page_skip;
  import a7ng_pkg::*;
  ...
  logic [15:0] n_emit, n_dup, n_trunc, n_dir, n_post, n_skip, n_hdr;
  logic [3:0] pmask;
  ...
  integer arlen_bad, hoc_post_ok, hoc_skip_ok, direct_ids_ok;
  integer r_bytes;
  ...
  a7ng_query_axi_sparse_page_skip #(
    ...
    .n_page_skip_o(n_skip), .n_hdr_ar_o(n_hdr),
  ...
  a7ng_query_axi_rbeat_probe #(
    .INDEX_BASE(NG_DDR_INDEX_BASE), .N_TABLES(4), .N_BUCKETS(G_N_BUCKETS), .ENTRY_BYTES(16)
  ) u_rbeat (
  ...
  initial begin
    ...
    if (G_N_BUCKETS != 4096)
      diverge("DIR_WIDTH", "N_BUCKETS!=4096");
    if (G_PAGE_N != 16)
      diverge("PAGE_CONTRACT", "PAGE_N!=16");
    $display("C1_PAGE_SKIP_N=%0d N_BUCKETS=%0d CAND_CAP=%0d INDEX_HEAD=%0d PAGE_N=%0d LAW=qse-v2-page-skip-01 POKE_V=0 CAND_CAP_AFTER_EMIT=1 AXI_BEAT_HOC_POST_R=%0d REDUCTION_X1000=NOT_EMITTED CAND_CAP_FINAL=NOT_FROZEN DDR_QUERY_BOUND_FINAL=NOT_FROZEN",
      G_N, G_N_BUCKETS, CAND_CAP, G_INDEX_HEAD, G_PAGE_N, G_AXI_BEAT_POST_R);
    ...
    for (qi = 0; qi < G_NQ; qi = qi + 1) begin
      ...
      if (qi == 0) begin
        ...
        if ((n_got == 3) && (got[0] === 20'd110) && (got[1] === 20'd144) && (got[2] === 20'd145))
          direct_ids_ok = 1;
        else begin
          $display("FAIL DIRECT_EMIT_NOT_BIT_IDENTICAL n=%0d", n_got);
          fail = fail + 1;
        end
        found = 1'b1;
        for (i = 0; i < nrel; i = i + 1) begin
      ...
      if (qi == 4) begin
        ...
        $display("NOT_SELECTIVE class=wrong_context k0=%0d k1=%0d k2=%0d k3=%0d vs_direct k0=%0d k1=%0d k2=%0d k3=%0d keys_match=%0d emit_match=%0d law=qse-v2-page-skip-01 xid_not_directory_key rec_is_not_context_selectivity=1",
      ...
      r_bytes = total_axi_bytes;
      $display("AXI_BEAT class=%s DIR_R=%0d POST_R=%0d R_BYTES=%0d AR_DIR=%0d AR_POST=%0d skip=%0d hdr_ar=%0d vs_AXI_BEAT_hoc_POST_R=%0d",
        cname(qi), dir_r_beats, post_r_beats, r_bytes, dir_ar_beats, post_ar_beats,
        n_skip, n_hdr, G_AXI_BEAT_POST_R);
      ...
      if (qi == 7) begin
        $display("HOC_POST_R_BEATS=%0d AXI_BEAT_BASELINE=%0d LT14=%0d PAGE_SKIP=%0d",
          post_r_beats, G_AXI_BEAT_POST_R, (post_r_beats < G_AXI_BEAT_POST_R), n_skip);
        if (post_r_beats >= G_AXI_BEAT_POST_R) begin
          $display("FAIL HOC_POST_R_NOT_BELOW_AXI_BEAT POST_R=%0d baseline=%0d",
            post_r_beats, G_AXI_BEAT_POST_R);
          fail = fail + 1;
        end else
          hoc_post_ok = 1;
        if (n_skip < 16'd1) begin
          $display("FAIL HOC_NO_PAGE_SKIP skip=%0d (headers without skip is FAIL)", n_skip);
          fail = fail + 1;
        end else
          hoc_skip_ok = 1;
      end
      ...
      if ((qi == G_LATE_Q) && (G_LATE_GOLD_ID == 20'd254) && (late_hit == 0)) begin
        $display("FAIL LATE_GOLD_254_MUST_EMIT incomp=%0d (SEARCH_INCOMPLETE does not hide)", incomp);
        fail = fail + 1;
      end
    ...
    if (fail == 0 && dist_gold_ok && dist_excl_ok && dist_no_leak && unrelated_empty && wc_not_sel
        && (direct_tp > 0) && direct_ids_ok && late_hit && late_ok && hoc_post_ok && hoc_skip_ok) begin
      $display("ASTRA_C1_PAGE_SKIP_XSIM_PASS");
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,N_16384,CONTEXT_KEYS");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end else begin
      $display("ASTRA_C1_PAGE_SKIP_XSIM_NO_MARKER fail=%0d dist_gold_ok=%0d dist_excl_ok=%0d dist_no_leak=%0d unrelated_empty=%0d wc_not_sel=%0d direct_tp=%0d direct_ids_ok=%0d late_hit=%0d late_ok=%0d hoc_post_ok=%0d hoc_skip_ok=%0d",
        fail, dist_gold_ok, dist_excl_ok, dist_no_leak, unrelated_empty, wc_not_sel, direct_tp, direct_ids_ok, late_hit, late_ok, hoc_post_ok, hoc_skip_ok);
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,N_16384,CONTEXT_KEYS");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end
