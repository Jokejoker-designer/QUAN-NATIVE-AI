#!/usr/bin/env python3
"""ASTRA-C1-PAGE-SKIP-01 independent host gold for qse-v2-page-skip-01.

Law: STREAM-02 two-pointer AND plus posting PAGE min/max headers. Walker
fetches a header beat {min_id,max_id,nrec} then may omit remaining data
beats when page.max < other stream cursor (intersection skip, not a
range-query). C0 59-word lexicon (no bag shadow). N=256. N_BUCKETS=4096.
CAND_CAP=16 after emit. Direct {110,144,145}. Late-gold nid 254 must emit.
high_occupancy POST_R must be strictly < AXI-BEAT baseline 14 AND at least
one page skipped (headers-only is FAIL).

Does NOT patch a7ng_sparse_dir_axi.sv. Does NOT edit STREAM-02 walker.
Does NOT union k0 with k1. Does NOT probe k2/k3. Does NOT load full page BRAM.
Gold is label match, computed BEFORE xvlog.
...
bag lexicon shadow. PROGRAM=NO. BIT=NO.
...
HOC_SYNTH_N = 15
PAGE_N = 16
PAGE_DATA_BEATS = (PAGE_N + 3) // 4
PAGE_BEATS = PAGE_DATA_BEATS + 1
PAGE_BYTES = PAGE_BEATS * ENTRY
HDR_MAGIC = 0xA7A1
AXI_BEAT_HOC_POST_R = 14
...
class _Cur:
    """Host twin of page-skip cursor (header then optional data; skip on max<other)."""

    def __init__(self, ids: list[int]):
        self.head = list(ids[:INDEX_HEAD])
        self.ovf = list(ids[INDEX_HEAD:])
        self.tot = len(ids)
        self.pos = 0
        self.have = False
        self.hhdr = False
        self.dfetched = False
        self.done = self.tot == 0
        self.id = 0
        self.hmin = 0
        self.hmax = 0
        self.hnrec = 0
    ...
    def _region(self) -> tuple[list[int], int]:
        if self.pos < len(self.head):
            return self.head, self.pos
        return self.ovf, self.pos - len(self.head)

    def fetch_hdr(self) -> None:
        ids, pin = self._region()
        start = (pin // PAGE_N) * PAGE_N
        chunk = ids[start : start + PAGE_N]
        self.pstart = self.pos
        self.chunk = chunk
        self.hnrec = len(chunk)
        self.hmin = chunk[0] if chunk else 0
        self.hmax = chunk[-1] if chunk else 0
        self.hhdr = bool(chunk)
        self.dfetched = False
        self.have = False
        self.hdr_ar += 1
        if not chunk:
    ...
    def skip_page(self) -> None:
        ...
        self.have = False
        self.dfetched = False
        if self.pos >= self.tot:
            self.done = True

    def fetch_data(self) -> None:
        off = self.pos - self.pstart
        piece = self.chunk[off : off + 4]
        self.buf = piece
        self.lane = 0
        self.data_ar += 1
        self.dfetched = True
    ...
    def adv(self) -> None:
        ...
        if self.pos >= self.tot:
            ...
            self.dfetched = False
            return
        if self.lane < len(self.buf):
            self.id = self.buf[self.lane]
            self.have = True
            return
        self.have = False
        if (self.pos - self.pstart) >= self.hnrec:
            self.hhdr = False
            self.dfetched = False


def skip_walk(a: list[int], b: list[int], cap: int) -> dict:
    s0, s1 = _Cur(a), _Cur(b)
    emit: list[int] = []
    guard = 0
    while (not s0.done) and (not s1.done) and len(emit) < cap:
        guard += 1
        if guard > 100000:
            raise SystemExit("skip_walk diverged")
        if (not s0.have) and (not s0.hhdr) and (not s0.done):
            s0.fetch_hdr()
            continue
        if (not s1.have) and (not s1.hhdr) and (not s1.done):
            s1.fetch_hdr()
            continue
        if s0.have and s1.have:
            if s0.id == s1.id:
                emit.append(s0.id)
                s0.adv()
                s1.adv()
                ...
                s1.adv()
            continue
        other_lo1 = s1.id if s1.have else s1.hmin
        other_lo0 = s0.id if s0.have else s0.hmin
        if s0.hhdr and (not s0.have) and (not s0.dfetched) and (s1.have or s1.hhdr) and s0.hmax < other_lo1:
            s0.skip_page()
            continue
        if s1.hhdr and (not s1.have) and (not s1.dfetched) and (s0.have or s0.hhdr) and s1.hmax < other_lo0:
            s1.skip_page()
            continue
        if s0.hhdr and (not s0.have) and (not s0.dfetched) and s1.have and s0.hmin > s1.id:
            s1.adv()
            continue
        if s1.hhdr and (not s1.have) and (not s1.dfetched) and s0.have and s1.hmin > s0.id:
            s0.adv()
            continue
        if (not s0.have) and s0.hhdr:
            s0.fetch_data()
            continue
        if (not s1.have) and s1.hhdr:
            s1.fetch_data()
            continue
        break
    n_post = s0.hdr_ar + s0.data_ar + s1.hdr_ar + s1.data_ar
    return {
        "emit": emit,
        "n_post": n_post,
        "n_skip": s0.skip + s1.skip,
        "n_hdr": s0.hdr_ar + s1.hdr_ar,
        "n_data": s0.data_ar + s1.data_ar,
    }


def skip_one(ids: list[int], cap: int) -> dict:
    s = _Cur(ids)
    emit: list[int] = []
    guard = 0
    while (not s.done) and len(emit) < cap:
        guard += 1
        if guard > 100000:
            raise SystemExit("skip_one diverged")
        if (not s.have) and (not s.hhdr):
            s.fetch_hdr()
            continue
        if (not s.have) and s.hhdr:
            s.fetch_data()
            continue
        emit.append(s.id)
        s.adv()
...
def route(q: dict, heads, overflow, ovf_flag, post_len) -> dict:
    """qse-v2-page-skip-01: two-pointer AND + page min/max skip."""
    v0 = bool(q["k0_valid"])
    v1 = bool(q["k1_valid"])
    if v0 and v1:
        ...
        tr = skip_walk(a, b, CAND_CAP)
        if tr["emit"] != emit:
            raise SystemExit(f"skip twin emit mismatch {tr['emit']} vs {emit} q={q.get('name')}")
        b0 = q["k0"] & 0xFFF
        b1 = q["k1"] & 0xFFF
        n_dir = 2
        n_post = tr["n_post"]
        ...
        return {
            ...
            "n_hdr": tr["n_hdr"],
            "n_data": tr["n_data"],
            "noskip_post": nbeats(len(a)) + nbeats(len(b)),
        }
    if v0:
        a = posting_of(q, 0, heads, overflow)
        emit = a[:CAND_CAP]
        ...
        return {
            ...
            "n_hdr": tr["n_hdr"],
            "n_data": tr["n_data"],
            "noskip_post": nbeats(min(len(a), CAND_CAP + (1 if ntrunc else 0))),
        }
    if v1:
        b = posting_of(q, 1, heads, overflow)
        emit = b[:CAND_CAP]
        ...
        return {
            ...
            "n_hdr": tr["n_hdr"],
            "n_data": tr["n_data"],
            "noskip_post": nbeats(min(len(b), CAND_CAP + (1 if ntrunc else 0))),
        }
    return {
        "emit": [],
        "n_emit": 0,
...
def pack_page_hdr(ids: list[int]) -> int:
    if not ids:
        return 0
    nrec = len(ids) & 0xFFFF
    mn = ids[0] & 0xFFFFFFFF
...
def pack_region(ids: list[int], post_ptr: int, writes: dict[int, int]) -> tuple[int, int, int]:
    """Write paged postings. Returns (base, next_ptr, max_word)."""
    if not ids:
        return 0, post_ptr, 0
    base = post_ptr
    npages = (len(ids) + PAGE_N - 1) // PAGE_N
    max_word = 0
    for p in range(npages):
        chunk = ids[p * PAGE_N : (p + 1) * PAGE_N]
        addr = base + p * PAGE_BYTES
        w = word_of(addr)
        writes[w] = pack_page_hdr(chunk)
        max_word = max(max_word, w)
        n_db = (len(chunk) + 3) // 4
        for i in range(n_db):
            waddr = addr + ENTRY * (1 + i)
            ww = word_of(waddr)
            writes[ww] = pack_post_beat(chunk[i * 4 : i * 4 + 4])
            max_word = max(max_word, ww)
    return base, post_ptr + npages * PAGE_BYTES, max_word
...
def write_index(heads, overflow, ovf_flag) -> dict[int, int]:
    ...
    for t in range(N_TABLES):
        for b in range(N_BUCKETS):
            ...
            if not h and not o:
                continue
            base, post_ptr, mw = pack_region(h, post_ptr, writes)
            max_word = max(max_word, mw)
            ovf_base = 0
            if o:
                ovf_base, post_ptr, mw = pack_region(o, post_ptr, writes)
                max_word = max(max_word, mw)
...
def write_svh(path: Path, queries: list[dict], writes: dict[int, int], docs: list[dict]) -> None:
    ...
    lines = [
        "// generated by host_astra_c1_page_skip.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
        "// Law qse-v2-page-skip-01: stream AND + posting page min/max skip; CAND_CAP after emit",
        ...
        f"localparam int unsigned G_PAGE_N = {PAGE_N};",
        f"localparam int unsigned G_AXI_BEAT_POST_R = {AXI_BEAT_HOC_POST_R};",
        ...
        + "};",
        "localparam int G_NSKIP [0:G_NQ-1] = '{"
        + csv_int([int(q.get("n_skip") or 0) for q in queries], str)
        + "};",
        "localparam int G_NHDR [0:G_NQ-1] = '{"
        + csv_int([int(q.get("n_hdr") or 0) for q in queries], str)
        + "};",
...
def main() -> int:
    ...
    chk(int(qd["high_occupancy"].get("n_skip") or 0) >= 1, "hoc page skip==0 (headers without skip)")
    chk(int(qd["high_occupancy"]["n_post"]) < AXI_BEAT_HOC_POST_R,
        f"hoc n_post {qd['high_occupancy']['n_post']} not < AXI-BEAT {AXI_BEAT_HOC_POST_R}")
    ...
    if fails:
        print("ASTRA_C1_PAGE_SKIP_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        return 2
    ...
    def qpub(q: dict) -> dict:
        row = {
            ...
            "n_skip": q.get("n_skip", 0),
            "n_hdr": q.get("n_hdr", 0),
            "n_data": q.get("n_data", 0),
            "noskip_post": q.get("noskip_post", 0),
        }
    ...
    golden = {
        ...
        "key_rebind": "relbind reused; walker=page_skip_k0_intersect_k1; k2k3_not_probed",
        "intersect_law": "sorted-nid two-pointer AND; posting page min/max skip; CAND_CAP after emit",
        "page_n": PAGE_N,
        "page_bytes": PAGE_BYTES,
        "axi_beat_hoc_post_r_baseline": AXI_BEAT_HOC_POST_R,
        "n_buckets": N_BUCKETS,
        "n": N,
        "cand_cap": CAND_CAP,
        "index_head": INDEX_HEAD,
    ...
    print("ASTRA_C1_PAGE_SKIP_HOST_PASS")
    print("N", N, "CAND_CAP", CAND_CAP, "INDEX_HEAD", INDEX_HEAD, "PAGE_N", PAGE_N, "WRITES", len(writes))
    ...
    for q in queries:
        ...
        extra = f" n_post={q['n_post']} n_skip={q.get('n_skip', 0)}"
        if q["name"] == "wrong_context":
            extra += " NOT_SELECTIVE"
        if q["name"] == "unrelated":
        ...
        if q["name"] == "high_occupancy":
            extra = (
                f" PAGE_SKIP n_skip={q.get('n_skip', 0)} n_post={q['n_post']} "
                f"vs_AXI_BEAT={AXI_BEAT_HOC_POST_R} n_hdr={q.get('n_hdr', 0)}"
            )
        print(
