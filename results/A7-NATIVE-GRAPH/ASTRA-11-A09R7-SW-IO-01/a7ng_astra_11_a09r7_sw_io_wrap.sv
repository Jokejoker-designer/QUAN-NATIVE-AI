// a7ng_astra_11_a09r7_sw_io_wrap.sv — ASTRA-11-A09R7-SW-IO-01.
// PROGRAM=NO. BIT=NO. Named SW-IO wrap: R7 query-rew FSM + UART IOB FFs + LED IOB FFs + SW IOB FFs.
// Instantiates frozen a7ng_astra_09_r2_cand_ovf (no copy-paste graph).
// Prior a7ng_astra_11_a09r7_btn_io_wrap / btn_idelay_wrap / led_io_wrap KEEP, not this top.
// Not PRODUCTION_TOP. Frozen a7ng_astra_09_integ_path is not instantiated. MMCM 100→50 + BUFG.
// UART pins uart_rxd_out=D10 uart_txd_in=A9. LEDs LD4-LD7 H5/J5/T9/T10.
// Switches SW0-SW3 A8/C11/C10/A10 (cited constraints/arty_a7_100.xdc).
// Buttons kept for async reset only; false-pathed (BTN unknown already closed).
`timescale 1ns / 1ps
`include "a7ng_astra_11_a09r7_sw_io.svh"

module a7ng_astra_11_a09r7_sw_io_wrap (
  ...
  // SW IOB pad FFs (PREREG). INIT 0. No SR so Vivado can pack into IOB.
  // Pins SW0-SW3 = A8/C11/C10/A10 (cited constraints/arty_a7_100.xdc).
  (* IOB = "TRUE" *) logic [3:0] sw_q = 4'b0000;
  always_ff @(posedge clk) begin
    sw_q <= sw;
  end
  // btn[0] async reset (LED-wrap class 2-FF). False-pathed; not this bag's unknown.
  ...
  (* keep_hierarchy = "yes" *)
  a7ng_astra_11_a09r7_sw_io_plant u_plant (
    .clk(clk), .rst_n(rst_n), .plant_sel(sw_q[1:0]),
  ...
  // Use sw_q (IOB-sampled), never raw sw, so SW input delay is pad→IOB FF.
  (* IOB = "TRUE" *) logic [3:0] led_q = 4'b0000;
  always_ff @(posedge clk) begin
    led_q[0] <= rst_n & hb[24];
    led_q[1] <= busy;
    led_q[2] <= result_v | r_ovf | pend_cmt | (|ntrunc);
    led_q[3] <= (|sw_q) | tbl | w_ovf;
  end
  assign led = led_q;
endmodule
