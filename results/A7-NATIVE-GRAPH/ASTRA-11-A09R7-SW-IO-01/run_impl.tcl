# ASTRA-11-A09R7-SW-IO-01. PROGRAM=NO. BIT=NO. write_bitstream forbidden.
# Instantiates a7ng_astra_09_r2_cand_ovf as u_a09r2. UART D10/A9 kept. LED LD4-LD7
# H5/J5/T9/T10 kept. SW0-SW3 A8/C11/C10/A10 (cited constraints/arty_a7_100.xdc).
# SW input delays 2.000/0.500 vs clk50u frozen in PREREG; applied post-synth via
# sw_iodelay.xdc. LED delays kept. UART envelope kept. Do not false-path sw.
# Frozen a7ng_astra_09_integ_path.sv is not compiled. synth + opt + place + route. No bitstream.
set bag  [file normalize [file dirname [info script]]]
set root [file normalize [file join $bag ../../..]]
set env(XILINXD_LICENSE_FILE) {D:\Xilinx\licenses\vivado_basic.lic}
set part xc7a100tcsg324-1
set top  a7ng_astra_11_a09r7_sw_io_wrap
set first_div ""
...
set files [list \
  ...
  [file join $bag a7ng_astra_11_a09r7_sw_io_plant.sv] \
  [file join $bag a7ng_astra_11_a09r7_sw_io_wrap.sv] \
]
set xdc [file join $bag clk50_sw_io.xdc]
set ledxdc [file join $bag led_iodelay.xdc]
set swxdc [file join $bag sw_iodelay.xdc]
...
proc astra_forbid_bit {args} {
  ...
  puts ASTRA_11_A09R7_SW_IO_ABORTED
  exit 1
}
...
proc astra_copy_fail_r0 {} {
  ...
  foreach f {timing_route.rpt util_route.rpt route_status.rpt drc.rpt clocks_route.rpt \
             ...
             SW_IOB.txt SW_IODELAY.txt SW_IOBFF.txt timing_sw_in.rpt} {
    set src [file join $bag $f]
    if {[file exists $src]} {
      file copy -force $src [file join $dst $f]
    }
  }
  puts "ASTRA_11_A09R7_SW_IO_FAIL_R0_PRESERVED $dst"
}
...
proc astra_force_iob_true {} {
  set n 0
  set cells [get_cells -quiet -hierarchical -filter {NAME =~ *uart_rx_iob_reg* || NAME =~ *uart_tx_iob_reg* || NAME =~ *led_q_reg* || NAME =~ *sw_q_reg*}]
  foreach c $cells {
  ...
  if {[llength [info commands all_fanout]]} {
    set sw_end [all_fanout -quiet -flat -endpoints_only -only_cells [get_ports -quiet {sw[*]}]]
    foreach c $sw_end {
      if {[catch {set_property IOB TRUE $c} err]} {
        puts "IOB_TRUE_SW_FANOUT_WARN $c $err"
      } else {
        incr n
        puts "IOB_TRUE_SW_FANOUT=[get_property NAME $c]"
      }
    }
  }
...
proc astra_apply_sw_iodelay {} {
  global swxdc bag
  if {![file exists $swxdc]} {
    astra_fail FILE_MISSING $swxdc
    puts ASTRA_11_A09R7_SW_IO_ABORTED
    exit 1
  }
  if {![llength [get_clocks -quiet clk50u]]} {
    astra_fail SW_IODELAY_CLK "clk50u missing; SW delays cannot bind"
    puts ASTRA_11_A09R7_SW_IO_ABORTED
    exit 1
  }
  if {[catch {read_xdc $swxdc} err]} {
    astra_fail SW_IODELAY_XDC $err
    puts ASTRA_11_A09R7_SW_IO_ABORTED
    exit 1
  }
  puts "SW_IODELAY_APPLIED clk50u PERIOD=[get_property PERIOD [get_clocks clk50u]] MAX=2.000 MIN=0.500"
  return 1
}

proc astra_sw_iob_extract {rpt out} {
  set a8 "NO"
  set c11 "NO"
  set c10 "NO"
  set a10 "NO"
  if {[file exists $rpt]} {
    ...
    set txt [read $fh]
    close $fh
    if {[regexp {\|\s*A8\s+\|\s+sw\[0\]\s+\|} $txt]} { set a8 "YES" }
    if {[regexp {\|\s*C11\s+\|\s+sw\[1\]\s+\|} $txt]} { set c11 "YES" }
    if {[regexp {\|\s*C10\s+\|\s+sw\[2\]\s+\|} $txt]} { set c10 "YES" }
    if {[regexp {\|\s*A10\s+\|\s+sw\[3\]\s+\|} $txt]} { set a10 "YES" }
  }
  set of [open $out w]
  puts $of "SW0_A8=$a8"
  puts $of "SW1_C11=$c11"
  puts $of "SW2_C10=$c10"
  puts $of "SW3_A10=$a10"
  if {$a8 eq "YES" && $c11 eq "YES" && $c10 eq "YES" && $a10 eq "YES"} {
    puts $of "SW_IOB=YES"
  } else {
    puts $of "SW_IOB=NO"
  }
  ...
  puts $of "PROGRAM=NO"
  close $of
  return [list $a8 $c11 $c10 $a10]
}

proc astra_sw_iodelay_extract {out} {
  global bag
  set in_su [astra_path_slack -from [get_ports -quiet {sw[*]}] -setup]
  set in_ho [astra_path_slack -from [get_ports -quiet {sw[*]}] -hold]
  set clk50 "NO"
  set per "NA"
  if {[llength [get_clocks -quiet clk50u]]} {
    set clk50 "YES"
  ...
  set of [open $out w]
  puts $of "SW_IN_MAX_NS=2.000"
  puts $of "SW_IN_MIN_NS=0.500"
  puts $of "SW_CLK_REF=clk50u"
  puts $of "SW_HOLD_POLICY=RELATED_CLK50U_NO_FALSE_PATH_HOLD"
  puts $of "PIPE_CLK50U=$clk50"
  puts $of "PIPE_PERIOD_NS=$per"
  puts $of "SW_IN_SETUP_SLACK=$in_su"
  puts $of "SW_IN_HOLD_SLACK=$in_ho"
  puts $of "PROGRAM=NO"
  close $of
  return [list $in_su $in_ho $clk50]
}

proc astra_sw_iobff_extract {out} {
  set n 0
  set packed_n 0
  set lines {}
  set cells [get_cells -quiet -hierarchical -filter {NAME =~ *sw_q_reg*}]
  foreach c $cells {
    incr n
    set nm  [get_property NAME $c]
    set bel [get_property BEL $c]
    ...
    lappend lines "SW_CELL=$nm BEL=$bel LOC=$loc IOB=$iobp PACKED=$pack"
  }
  set packed "NO"
  if {$n >= 4 && $packed_n >= 4} { set packed "YES" }
  set of [open $out w]
  puts $of "SW_IOBFF_CELLS=$n"
  puts $of "SW_IOBFF_PACKED_CELLS=$packed_n"
  puts $of "SW_IOBFF=$packed"
  foreach ln $lines { puts $of $ln }
  puts $of "PROGRAM=NO"
  close $of
  return [list $packed $n $packed_n]
...
foreach f $files {
  ...
  if {$bn eq "a7ng_astra09_pipe.sv" || $bn eq "arty_a7_astra09_soc_top.sv" ||
      ...
      $bn eq "a7ng_astra_11_a09r7_btn_idelay_wrap.sv" ||
      $bn eq "a7ng_astra_11_a09r7_btn_idelay_plant.sv" ||
...
puts "SW2=C10 SW2"
puts "SW3=A10 SW3"
puts "SW_CITE=constraints/arty_a7_100.xdc"
...
if {[catch {synth_design -top $top -part $part} err]} {
  ...
  puts ASTRA_11_A09R7_SW_IO_SYNTH_FAIL
  exit 1
}
...
astra_util_extract [file join $bag util_synth.rpt] [file join $bag UTIL_EXTRACT_SYNTH.txt] Synthesized
astra_timing_extract [file join $bag timing_synth.rpt] [file join $bag TIMING_EXTRACT_SYNTH.txt] Synthesized
puts ASTRA_11_A09R7_SW_IO_SYNTH_DONE
...
set swff_cells [get_cells -quiet -hierarchical -filter {NAME =~ *sw_q_reg*}]
puts "SW_IOBFF_CELLS=[llength $swff_cells]"
if {[llength $swff_cells] < 4} {
  puts "SW_IOBFF_CELLS_WARN count=[llength $swff_cells] (SW IOB FF if it still packs; not a hard abort)"
}
...
if {[catch {opt_design} err]} {
  astra_fail OPT $err
  puts ASTRA_11_A09R7_SW_IO_OPT_FAIL
  exit 1
}
astra_force_iob_true
astra_apply_hold_policy
puts ASTRA_11_A09R7_SW_IO_OPT_DONE
...
if {[catch {place_design} err]} {
  ...
  puts ASTRA_11_A09R7_SW_IO_PLACE_FAIL
  exit 1
}
...
astra_apply_hold_policy
puts ASTRA_11_A09R7_SW_IO_PLACE_DONE
...
if {[catch {route_design} err]} {
  ...
  astra_sw_iob_extract [file join $bag io.rpt] [file join $bag SW_IOB.txt]
  astra_sw_iodelay_extract [file join $bag SW_IODELAY.txt]
  astra_sw_iobff_extract [file join $bag SW_IOBFF.txt]
  puts ASTRA_11_A09R7_SW_IO_ROUTE_FAIL
  exit 1
}
...
if {[catch {report_timing -from [get_ports {sw[*]}] -delay_type min_max -max_paths 8 -file [file join $bag timing_sw_in.rpt]}]} {}
...
set biob [astra_sw_iob_extract [file join $bag io.rpt] [file join $bag SW_IOB.txt]]
set biod [astra_sw_iodelay_extract [file join $bag SW_IODELAY.txt]]
set biff [astra_sw_iobff_extract [file join $bag SW_IOBFF.txt]]
set a9  [lindex $iob 0]
...
puts "SW_IOB A8=[lindex $biob 0] C11=[lindex $biob 1] C10=[lindex $biob 2] A10=[lindex $biob 3] SW_IOB=$sw_iob"
puts "SW_IODELAY IN_SU=[lindex $biod 0] IN_HO=[lindex $biod 1] CLK50=[lindex $biod 2]"
puts "SW_IOBFF=$sw_packed CELLS=[lindex $biff 1] PACKED_CELLS=[lindex $biff 2]"
...
if {$sw_iob ne "YES"} {
  astra_fail SW_IOB "io.rpt missing SW IOB A8/C11/C10/A10 A8=[lindex $biob 0] C11=[lindex $biob 1] C10=[lindex $biob 2] A10=[lindex $biob 3]"
  puts ASTRA_11_A09R7_SW_IO_ABORTED
  exit 1
}
...
if {[lindex $biod 0] eq "NA"} {
  astra_fail SW_IODELAY_PATH "SW I/O timing paths missing IN_SU=[lindex $biod 0]"
  puts ASTRA_11_A09R7_SW_IO_ABORTED
  exit 1
}
if {$packed ne "YES"} {
...
if {$sw_packed ne "YES"} {
  puts "SW_IOBFF_UNPACKED CELLS=[lindex $biff 1] PACKED=[lindex $biff 2] (SW IOB FF if it still packs; not a hard abort)"
}
set wns [lindex $tex 0]
set tns [lindex $tex 1]
set whs [lindex $tex 2]
set ths [lindex $tex 3]
puts "ASTRA_11_A09R7_SW_IO_ROUTE_DONE WNS=$wns TNS=$tns WHS=$whs THS=$ths SW_IOB=YES LED_IOB=YES UART_IOB=YES UART_IOBFF=$packed LED_IOBFF=$led_packed SW_IOBFF=$sw_packed SW_IN_MAX=2.000 SW_IN_MIN=0.500 PROGRAM=NO BIT=NOT_BUILT"
...
if {$fail_whs || $fail_wns} {
  ...
  astra_copy_fail_r0
  # phys_opt is exhausted on the BTN FAIL bag (pad hold). Do not silently retune
  # delays here. The bounded IDELAY experiment is a NEW PREREG tap after this run.
  # Still 50 MHz. Still no bitstream. Do not invent delay numbers. Do not patch uart_rx.
  # Do not set_false_path on sw[*].
  puts ASTRA_11_A09R7_SW_IO_FAIL_NEEDS_IDELAY_PREREG_EXP
  ...
  puts "ASTRA_11_A09R7_SW_IO_DONE WNS=$wns WHS=$whs SW_IOB=$sw_iob LED_IOB=$led_iob UART_IOB=YES UART_IOBFF=$packed LED_IOBFF=$led_packed SW_IOBFF=$sw_packed BIT=NOT_BUILT PROGRAM=NO RESULT=$result_tag PRODUCTION_TOP=UNKNOWN"
  exit 0
}

puts "ASTRA_11_A09R7_SW_IO_DONE WNS=$wns TNS=$tns WHS=$whs SW_IOB=YES LED_IOB=YES UART_IOB=YES UART_IOBFF=$packed LED_IOBFF=$led_packed SW_IOBFF=$sw_packed BIT=NOT_BUILT PROGRAM=NO PRODUCTION_TOP=UNKNOWN"
exit 0
