// tb_astra_c1_n4096_stream.sv — C1 N=4096 qse-v2-stream-intersect-02. PROGRAM=NO.
...
// SEARCH_INCOMPLETE on gold_n>=1 retrieve class FAILS the bag (no PASS marker).
// Distractor: G_GOLD_EXCLUDED=1; leak=FAIL not tp. Headline precision_all=TP/emit_n.
// MEM_DEPTH from gold (TB-only; must fit 4096-record index).
`timescale 1ns / 1ps

module tb_astra_c1_n4096_stream;
  import a7ng_pkg::*;
  ...
  initial begin
    ...
    if (G_SENTINEL_ID != 4095)
      diverge("SENTINEL_ID", "G_SENTINEL_ID!=4095");
    dist_gold_ok = 1;
    dist_excl_ok = 1;
    ...
    $display("C1_N4096_STREAM_02_N=%0d CAND_CAP=%0d INDEX_HEAD=%0d LAW=qse-v2-stream-intersect-02 POKE_V=0 PAGE_BUFFER_BEATS=1 RARE_LIST_FIRST=1 CAND_CAP_AFTER_EMIT=1 REDUCTION_X1000=NOT_EMITTED CAND_CAP_FINAL=NOT_FROZEN DDR_QUERY_BOUND_FINAL=NOT_FROZEN MEM_DEPTH=%0d",
      G_N, CAND_CAP, G_INDEX_HEAD, MEM_DEPTH);
    ...
    for (qi = 0; qi < G_NQ; qi = qi + 1) begin
      ...
      if (qi == G_LATE_Q) begin
        ...
        if (!found) begin
          $display("FAIL LATE_GOLD_MISS id=%0d emit_n=%0d incomp=%0d k0_idx=%0d k1_idx=%0d",
            G_LATE_GOLD_ID, n_got, incomp, G_LATE_K0_IDX, G_LATE_K1_IDX);
          fail = fail + 1;
        end else begin
          late_hit = 1;
          $display("LATE_GOLD_HIT id=%0d k0_idx=%0d k1_idx=%0d",
            G_LATE_GOLD_ID, G_LATE_K0_IDX, G_LATE_K1_IDX);
          $display("STREAM_HIT id=%0d k0_idx=%0d k1_idx=%0d",
            G_LATE_GOLD_ID, G_LATE_K0_IDX, G_LATE_K1_IDX);
        end
        if (G_CAP_THEN_AND_MISS[qi])
          $display("CAP_THEN_AND_WOULD_MISS id=%0d k0_idx=%0d k1_idx=%0d stream_hit=%0d",
            G_LATE_GOLD_ID, G_LATE_K0_IDX, G_LATE_K1_IDX, found);
        late_ok = 1;
      end
    ...
    if (fail == 0 && dist_gold_ok && dist_excl_ok && dist_no_leak && unrelated_empty && wc_not_sel && (direct_tp > 0) && late_hit && late_ok && (incomp_retrieve == 0)) begin
      $display("ASTRA_C1_N4096_STREAM_02_XSIM_PASS");
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,N_16384");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end else begin
      $display("ASTRA_C1_N4096_STREAM_02_XSIM_NO_MARKER fail=%0d dist_gold_ok=%0d dist_excl_ok=%0d dist_no_leak=%0d unrelated_empty=%0d wc_not_sel=%0d direct_tp=%0d late_hit=%0d late_ok=%0d incomp_retrieve=%0d",
        fail, dist_gold_ok, dist_excl_ok, dist_no_leak, unrelated_empty, wc_not_sel, direct_tp, late_hit, late_ok, incomp_retrieve);
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,N_16384");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end
