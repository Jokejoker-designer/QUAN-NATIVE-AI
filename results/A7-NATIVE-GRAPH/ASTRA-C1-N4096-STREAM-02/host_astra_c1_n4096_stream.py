#!/usr/bin/env python3
"""ASTRA-C1-N4096-STREAM-02 independent host gold for qse-v2-stream-intersect-02.

Same law as STREAM-INTERSECT-02 (N=256). Scale unknown only. N=4096.
...
Does NOT edit a7ng_query_axi_sparse_stream_intersect.sv.

LATE_GOLD nid at posting index ≥16 on both k0 and k1 (cap-then-AND misses;
stream AND-then-cap hits) and near 4095. Direct {110,144,145} remain if
those records stay. Retrieve class with gold_n>=1 that is SEARCH_INCOMPLETE
or miss FAILS the bag.
...
edit KEEP STREAM-02 / N4096-INTERSECT / KEY-INTERSECT bags.
PROGRAM=NO. BIT=NO.
    ...
    for s in range(1, 13):
        for r in range(1, 4):
            for o in range(1, 13):
                if s == o:
                    continue
                if (s, r, o) in used or (s, r, o) in PAD_BLOCK:
                    continue
...
def write_svh(
    ...
    lines = [
        "// generated by host_astra_c1_n4096_stream.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
        "// Law qse-v2-stream-intersect-02 N=4096: sorted-nid two-pointer; 1-beat page; rare-first; CAND_CAP after emit",
...
def main() -> int:
    ...
    chk(late_gold[0]["nid"] >= 4090, f"late gold nid {late_gold[0]['nid']} not near 4095")
    ...
    stream_hit, _, stream_incomp = two_pointer(k0_list, k1_list, CAND_CAP)
    chk(lg["nid"] in stream_hit, f"late gold not in stream AND {stream_hit}")
    chk(stream_incomp == 0, "late gold stream SEARCH_INCOMPLETE")
    print(
        f"CAP_THEN_AND_WOULD_MISS id={lg['nid']} k0_idx={idx0} k1_idx={idx1} "
        f"first16_and={cap_miss}"
    )
    print(
        f"STREAM_HIT id={lg['nid']} k0_idx={idx0} k1_idx={idx1} "
        f"stream_and={stream_hit}"
    )
    ...
    for name, text, pred in queries_spec:
        ...
        if name == "distractor":
            ...
            if incomp and name in must_hit and len(q["relevant"]) >= 1:
                fails.append(f"SEARCH_INCOMPLETE {name} gold_n={len(q['relevant'])} incomp does not PASS")
            q["leak_n"] = 0
            q["leak_ids"] = []
            q["tp"] = met["tp"]
            q["fp_ev1"] = met["fp_ev1"]
    ...
    chk(qd["high_id_sentinel"]["tp"] == 1, "sentinel tp")
    chk(qd["high_id_sentinel"]["search_incomplete"] == 0, "sentinel SEARCH_INCOMPLETE")
    chk((N - 1) in qd["high_id_sentinel"]["emit"], "sentinel not in emit")
    ...
    if fails:
        print("ASTRA_C1_N4096_STREAM_02_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        return 2
    ...
    def qpub(q: dict) -> dict:
        ...
        if q["name"] == "late_gold":
            ...
            row["k0_index"] = idx0
            row["k1_index"] = idx1
            row["CAP_THEN_AND_WOULD_MISS"] = True
            row["STREAM_HIT"] = True
    ...
    write_svh(BAG / "query_gold.svh", queries, writes, docs, idx0, idx1)
    ...
    print("ASTRA_C1_N4096_STREAM_02_HOST_PASS")
    ...
    for q in queries:
        ...
        if q["name"] == "late_gold":
            extra = (
                f" LATE_GOLD_HIT STREAM_HIT cap_then_and_miss={int(q['cap_then_and_miss'])} "
                f"k0_idx={idx0} k1_idx={idx1}"
            )
        print(
