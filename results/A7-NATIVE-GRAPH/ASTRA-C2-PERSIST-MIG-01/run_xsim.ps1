...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C2MIG_FILE_MISSING $p" }
...
$expect = @{
  ...
  "rtl/native_graph/integrate/a7ng_astra_c2_persist_multi_slot.sv" = "bc8b8993edbba8bc048bc03ebc6398744cf80da01d346801c3683a15881682fd"
  "rtl/native_graph/integrate/a7ng_astra_c2_persist_ddr_stall.sv" = "55f5ac0d4e9a5280d83951fb147fc85734417f7fff0ae22ed5d4422b65710862"
...
if ($hashFail) { throw "C2MIG_HASH_GATE_FAIL do_not_edit_keep_rtl_or_mig_prj" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c2_persist_mig.sv"
$dut = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c2_persist_mig.sv"
$svh = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c2_persist_mig.svh"
$keep = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c2_persist_commit.sv"
$wrap = Join-Path $root "rtl\ddr\mig_native_wrap.sv"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C2MIG_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C2MIG_GOLD_HASH_DRIFT GOLDEN.json" }

foreach ($f in @($dut, $tb)) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C2MIG_LEFTOVER_A09" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C2MIG_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_learned_prior_store.sv") { throw "C2MIG_PRIOR_STORE_AS_DUT" }
}

$stamp = (Get-Date).ToString("yyyy-MM-ddTHH:mm:ss.fffffffK")
$pre = @(
  "# SHA freeze BEFORE xvlog $stamp",
  "# PROGRAM=NO BOARD_PASS=NOT_CLAIMED DDR_QUERY_BOUND_FINAL=NOT_FROZEN",
  "# LAW=astra-c2-persist-mig-01 MIG_XSIM Digilent AXI mig_sim + ddr3_model",
  "# HASH_GATE MATCH vs C0 + persist KEEP + official mig.prj (not edited)"
) + $hashLines
$pre += "# COMPILED"
foreach ($f in @($dut, $keep, $wrap, $tb, $svh, $goldJson)) { $pre += (ShaLine $f) }
$pre += "# FORBIDDEN leftover A09 / STREAM-02 / prior_store as DUT; synth mig_7series_0_mig.v not compiled"
...
function Write-DivLog([string]$code, [string]$detail) {
  [System.IO.File]::WriteAllLines((Join-Path $bag "xsim.log"), @(
    ...
    "RESULT=FAIL",
    "ASTRA_C2_PERSIST_MIG_XSIM_PASS ABSENT",
    "PROGRAM=NO BOARD_PASS=NOT_CLAIMED"
  ))
}

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C2MIG_XVLOG_NOT_FOUND" }
if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null

$prj = Join-Path $work "c2_persist_mig.prj"
$prjLines = @()
$prjLines += "sv work `"$dut`""
$prjLines += "sv work `"$keep`""
$prjLines += "sv work `"$wrap`""
$prjLines += "sv work `"$tb`""
...
Set-Location $work
& "$bin\xvlog.bat" -prj $prj -i $incI -i $bag -i $migSimInc
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C2MIG_XVLOG_FAIL"
}
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
$xvlogTxt = [System.IO.File]::ReadAllText((Join-Path $bag "xvlog.log"))
if ($xvlogTxt -match "a7ng_astra_09_integ_path") { throw "C2MIG_LEFTOVER_A09_COMPILED" }
if ($xvlogTxt -match "a7ng_query_axi_sparse_stream_intersect") { throw "C2MIG_STREAM02_COMPILED" }
if ($xvlogTxt -match "a7ng_learned_prior_store") { throw "C2MIG_PRIOR_STORE_COMPILED" }
if ($xvlogTxt -match "mig_7series_0_mig\.v" -and $xvlogTxt -notmatch "mig_7series_0_mig_sim") { throw "C2MIG_SYNTH_MIG_COMPILED" }
if ($xvlogTxt -notmatch "a7ng_astra_c2_persist_mig") { throw "C2MIG_DUT_NOT_ANALYZED" }
if ($xvlogTxt -notmatch "mig_7series_0_mig_sim") { throw "C2MIG_MIG_SIM_NOT_ANALYZED" }

& "$bin\xelab.bat" -mt off -O0 tb_astra_c2_persist_mig glbl -s astra_c2_persist_mig -L unisims_ver -L unimacro_ver -L secureip -timescale 1ps/1ps
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C2MIG_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" astra_c2_persist_mig -R -log $xsimLog
if (-not (Test-Path -LiteralPath $xsimLog)) { throw "C2MIG_NO_XSIM_LOG" }
$hasPass = [bool](Select-String -Path $xsimLog -Pattern "^ASTRA_C2_PERSIST_MIG_XSIM_PASS$" -Quiet)
$hasDiv = [bool](Select-String -Path $xsimLog -Pattern "^FIRST_DIVERGENCE" -Quiet)
$need = @(
  "CLASS_mig_calib_complete HIT",
  "CLASS_persist_through_mig HIT",
  "CLASS_journal_addr_not_low16 HIT",
  "CLASS_alias_attempt HIT",
  "CLASS_bram_clear HIT",
  "CLASS_reload_from_mig HIT",
...
if ($hasDiv -or (-not $hasPass) -or ($missing.Count -gt 0)) {
  ...
  throw "C2MIG_XSIM_CONTROL_FAIL pass=$hasPass div=$hasDiv missing=$($missing -join ',')"
}
Write-Host "ASTRA_C2_PERSIST_MIG_RUN_OK PASS_THIS_GATE_ONLY PROGRAM=NO MIG_XSIM=1"
