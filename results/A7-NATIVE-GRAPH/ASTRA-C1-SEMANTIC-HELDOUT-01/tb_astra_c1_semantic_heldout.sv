// tb_astra_c1_semantic_heldout.sv — C1 N=16384 held-out queries. PROGRAM=NO.
...
// PASS marker only if FAIL=0 AND paraphrase gold hits AND fill-template control hits
// AND SEARCH_INCOMPLETE absent on gold_n>=1 retrieve.
// Gold miss on retrieve class = FAIL (incomp does not hide). Do not drop N from 16384.
...
module tb_astra_c1_semantic_heldout;
  import a7ng_pkg::*;
  ...
  integer unrelated_empty, wc_sel, dist_gold_ok, dist_excl_ok, dist_no_leak;
  integer keys_match, emit_match, fill_n, fill_tp, para_tp, late_hit, late_ok;
  integer incomp_retrieve, fill_ids_ok;
  integer arlen_bad;
  logic [ID_W-1:0] got [0:31];
  logic [ID_W-1:0] fill_got [0:31];
  logic found;
  ...
  initial begin
    ...
    if (G_N != 16384)
      diverge("N_DROP", "G_N!=16384 do_not_silently_drop_to_clone");
    if (CAND_CAP >= G_N)
    ...
    $display("C1_SEMANTIC_HELDOUT_N=%0d N_BUCKETS=%0d CAND_CAP=%0d INDEX_HEAD=%0d LAW=qse-v2-intersect-context-02 LEX=qse-v2-lex-semantic-16k-01 POKE_V=0 PAGE_BUFFER_BEATS=1 RARE_LIST_FIRST=1 CAND_CAP_AFTER_EMIT=1 REDUCTION_X1000=NOT_EMITTED CAND_CAP_FINAL=NOT_FROZEN DDR_QUERY_BOUND_FINAL=NOT_FROZEN MEM_DEPTH=%0d N_SUBJECTS=%0d N_RELS=%0d HELDOUT=1 FILL_TEMPLATE_CONTROL=1",
    ...
    for (qi = 0; qi < G_NQ; qi = qi + 1) begin
      ...
      if (qi == 4 && k0_cx === G_K0_PLAIN[qi])
        diverge("KEY_MISMATCH", "wrong_context k0 not folded");
      n_host = h_ent|h_int|h_hash|h_sh|h_bkt|h_cand|h_win|h_addr|h_rel|h_nxt|h_ans;
      ...
      if (qi == 0) begin
        ...
        fill_ids_ok = (n_got == G_NEMIT[0]);
        for (i = 0; i < n_got; i = i + 1)
          if (got[i] !== G_EMIT[0][i])
            fill_ids_ok = 0;
        if (!fill_ids_ok) begin
          $display("FAIL FILL_TEMPLATE_EMIT_NOT_BIT_IDENTICAL n=%0d", n_got);
          fail = fail + 1;
        end
      end
      if (qi == 1) begin
        para_tp = tp;
        if (para_tp < 1) begin
          $display("FAIL PARAPHRASE_GOLD_MISS tp=0");
          fail = fail + 1;
        end else begin
          $display("PARAPHRASE_HIT tp=%0d emit_n=%0d", para_tp, n_got);
        end
      end

      if (qi == 4) begin
        keys_match = (k0_cx === G_K0[0] && k1_cx === G_K1[0] && k2_cx === G_K2[0] && k3_cx === G_K3[0]);
        emit_match = (n_got == fill_n);
        if (emit_match)
          for (i = 0; i < n_got; i = i + 1)
            if (got[i] !== fill_got[i]) emit_match = 0;
        if (emit_match || keys_match) begin
          $display("NOT_SELECTIVE class=wrong_context k0=%0d k1=%0d k2=%0d k3=%0d vs_fill_template k0=%0d k1=%0d k2=%0d k3=%0d keys_match=%0d emit_match=%0d law=qse-v2-intersect-context-02",
            k0_cx, k1_cx, k2_cx, k3_cx, G_K0[0], G_K1[0], G_K2[0], G_K3[0], keys_match, emit_match);
          $display("FAIL WRONG_CONTEXT_NOT_SELECTIVE emit_match=%0d keys_match=%0d", emit_match, keys_match);
          fail = fail + 1;
          wc_sel = 0;
        end else begin
          $display("CONTEXT_SELECTIVE class=wrong_context k0=%0d k1=%0d k2=%0d k3=%0d vs_fill_template k0=%0d k1=%0d k2=%0d k3=%0d keys_match=0 emit_match=0 law=qse-v2-intersect-context-02",
            k0_cx, k1_cx, k2_cx, k3_cx, G_K0[0], G_K1[0], G_K2[0], G_K3[0]);
          wc_sel = 1;
        end
      end
    ...
    if (fail == 0 && dist_gold_ok && dist_excl_ok && dist_no_leak && unrelated_empty && wc_sel && (fill_tp > 0) && fill_ids_ok && (para_tp > 0) && late_hit && late_ok && (incomp_retrieve == 0)) begin
      $display("ASTRA_C1_SEMANTIC_HELDOUT_XSIM_PASS");
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,PAGE_SKIP_MIX");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end else begin
      $display("ASTRA_C1_SEMANTIC_HELDOUT_XSIM_NO_MARKER fail=%0d dist_gold_ok=%0d dist_excl_ok=%0d dist_no_leak=%0d unrelated_empty=%0d wc_sel=%0d fill_tp=%0d fill_ids_ok=%0d para_tp=%0d late_hit=%0d late_ok=%0d incomp_retrieve=%0d",
        fail, dist_gold_ok, dist_excl_ok, dist_no_leak, unrelated_empty, wc_sel, fill_tp, fill_ids_ok, para_tp, late_hit, late_ok, incomp_retrieve);
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,PAGE_SKIP_MIX");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end
