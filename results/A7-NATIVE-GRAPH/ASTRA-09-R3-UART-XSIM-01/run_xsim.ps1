...
if ($liveA09 -ne $expectA09) { throw "A09R3_FROZEN_A09_HASH_CHANGED $liveA09" }
if ($liveR2  -ne $expectR2)  { throw "A09R3_FROZEN_R2_HASH_CHANGED $liveR2" }
...
$files = @(
  ...
  (Join-Path $bag "a7ng_astra_09_r3_axi_plant.sv"),
  (Join-Path $bag "a7ng_astra_09_r3_uart_wrap.sv"),
  (Join-Path $bag "tb_astra_09_r3_uart_xsim.sv")
)
$includes = @(
...
foreach ($f in $files) {
  ...
  if ($bn -match "astra_09_integ_path|astra_rtp_soc_top|astra09_pipe") {
...
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "A09R3_FILE_MISSING $p" }
...
$pre += (ShaLine $stub)
$pre += "# KEEP_NOT_COMPILED (frozen leftover A09 + prior UART wrap top; not this DUT)"
...
Write-Host "A09R3_SHA_FROZEN $shaPre PROGRAM=NO PRODUCTION_TOP=UNKNOWN"
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "A09R3_XVLOG_NOT_FOUND" }
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "A09R3_XVLOG_FAIL"
}
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
if (-not (Test-Path -LiteralPath $glbl)) { throw "A09R3_GLBL_MISSING $glbl" }
& "$bin\xvlog.bat" $glbl
if ($LASTEXITCODE -ne 0) { throw "A09R3_GLBL_XVLOG_FAIL" }

$mmcmMode = "UNISIM_MMCME2_BASE"
& "$bin\xelab.bat" tb_astra_09_r3_uart_xsim glbl -s a09r3uart -timescale 1ns/1ps -L unisims_ver --debug typical
if ($LASTEXITCODE -ne 0) {
  Write-Host "UNISIM_XELAB_FAIL trying MMCM_STUB"
  $mmcmMode = "MMCM_STUB"
  & "$bin\xvlog.bat" --sv $stub
  if ($LASTEXITCODE -ne 0) { throw "A09R3_STUB_XVLOG_FAIL" }
  & "$bin\xelab.bat" tb_astra_09_r3_uart_xsim glbl -s a09r3uart -timescale 1ns/1ps --debug typical
  if ($LASTEXITCODE -ne 0) {
    if (Test-Path (Join-Path $work "xelab.log")) {
      Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
    }
    throw "A09R3_XELAB_FAIL"
  }
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
...
& "$bin\xsim.bat" a09r3uart -R --onerror quit -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_09_R3_UART_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "A09R3_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_09_R3_UART_XSIM_RUN_OK PROGRAM=NO PRODUCTION_TOP=UNKNOWN"
