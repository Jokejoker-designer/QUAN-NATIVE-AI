# ASTRA-11-A09R3-UART-IMPL-ROUTE-01. PROGRAM=NO. BIT=NO. write_bitstream forbidden.
# Instantiates a7ng_astra_09_r2_cand_ovf as u_a09r2. UART D10/A9. No copy-paste graph.
# Frozen a7ng_astra_09_integ_path.sv is not compiled. synth + opt + place + route. No bitstream.
set bag  [file normalize [file dirname [info script]]]
set root [file normalize [file join $bag ../../..]]
set env(XILINXD_LICENSE_FILE) {D:\Xilinx\licenses\vivado_basic.lic}
set part xc7a100tcsg324-1
set top  a7ng_astra_11_a09r3_uart_impl_wrap
set first_div ""
...
set files [list \
  ...
  [file join $bag a7ng_astra_11_a09r3_axi_plant.sv] \
  [file join $bag a7ng_astra_11_a09r3_uart_impl_wrap.sv] \
]
...
proc astra_forbid_bit {args} {
  ...
  puts ASTRA_11_A09R3_UART_IMPL_ROUTE_ABORTED
  exit 1
}
...
proc astra_copy_fail_r0 {} {
  ...
  foreach f {timing_route.rpt util_route.rpt route_status.rpt drc.rpt clocks_route.rpt \
             ...
             clock_util_route.rpt io.rpt UART_IOB.txt} {
    set src [file join $bag $f]
    if {[file exists $src]} {
      file copy -force $src [file join $dst $f]
    }
  }
  puts "ASTRA_11_A09R3_UART_FAIL_R0_PRESERVED $dst"
}
...
if {[catch {synth_design -top $top -part $part} err]} {
  ...
  puts ASTRA_11_A09R3_UART_IMPL_ROUTE_SYNTH_FAIL
  exit 1
}
...
astra_util_extract [file join $bag util_synth.rpt] [file join $bag UTIL_EXTRACT_SYNTH.txt] Synthesized
astra_timing_extract [file join $bag timing_synth.rpt] [file join $bag TIMING_EXTRACT_SYNTH.txt] Synthesized
puts ASTRA_11_A09R3_UART_IMPL_ROUTE_SYNTH_DONE
...
if {[catch {opt_design} err]} {
  astra_fail OPT $err
  puts ASTRA_11_A09R3_UART_IMPL_ROUTE_OPT_FAIL
  exit 1
}
puts ASTRA_11_A09R3_UART_IMPL_ROUTE_OPT_DONE
...
if {[catch {place_design} err]} {
  ...
  puts ASTRA_11_A09R3_UART_IMPL_ROUTE_PLACE_FAIL
  exit 1
}
if {[catch {phys_opt_design} err]} {
  puts "PHYS_OPT_WARN $err"
}
puts ASTRA_11_A09R3_UART_IMPL_ROUTE_PLACE_DONE
...
if {[catch {route_design} err]} {
  ...
  puts ASTRA_11_A09R3_UART_IMPL_ROUTE_ROUTE_FAIL
  exit 1
}
...
puts "ASTRA_11_A09R3_UART_IMPL_ROUTE_ROUTE_DONE WNS=$wns TNS=$tns WHS=$whs THS=$ths UART_IOB=YES PROGRAM=NO BIT=NOT_BUILT"
...
if {$wns_num ne "" && $wns_num < 0.0} {
  ...
  if {[catch {phys_opt_design} e2]} {
    puts "ASTRA_11_A09R3_UART_EXP_PHYS_OPT_WARN $e2"
  } else {
    puts ASTRA_11_A09R3_UART_EXP_PHYS_OPT_DONE
  }
  ...
  puts ASTRA_11_A09R3_UART_EXP_DONE
  puts "ASTRA_11_A09R3_UART_IMPL_ROUTE_DONE WNS=$wns WHS=$whs UART_IOB=YES BIT=NOT_BUILT PROGRAM=NO RESULT=FAIL_WNS PRODUCTION_TOP=UNKNOWN"
  exit 0
}

puts "ASTRA_11_A09R3_UART_IMPL_ROUTE_DONE WNS=$wns TNS=$tns WHS=$whs UART_IOB=YES BIT=NOT_BUILT PROGRAM=NO PRODUCTION_TOP=UNKNOWN"
exit 0
