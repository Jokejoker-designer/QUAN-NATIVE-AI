...
if ($liveA09 -ne $expectA09) { throw "A11A09R3_FROZEN_A09_HASH_CHANGED $liveA09" }
if ($liveR2  -ne $expectR2)  { throw "A11A09R3_DUT_HASH_CHANGED $liveR2" }
if ($liveSgd -ne $expectSgd) { throw "A11A09R3_SGD_HASH_CHANGED $liveSgd" }
if ($liveRx  -ne $expectRx)  { throw "A11A09R3_UART_RX_HASH_CHANGED $liveRx" }
if ($liveTx  -ne $expectTx)  { throw "A11A09R3_UART_TX_HASH_CHANGED $liveTx" }
$compiled = @(
  ...
  $uartRx,
  $uartTx,
  (Join-Path $bag "a7ng_astra_11_a09r3_axi_plant.sv"),
  (Join-Path $bag "a7ng_astra_11_a09r3_uart_impl_wrap.sv"),
...
foreach ($f in $compiled) {
  ...
  if ($bn -eq "a7ng_astra09_pipe.sv" -or $bn -eq "a7ng_astra_09_integ_path.sv" -or $bn -eq "arty_a7_astra09_soc_top.sv" -or $bn -eq "arty_a7_astra_rtp_soc_top.sv" -or $bn -eq "a7ng_astra_09_r3_uart_wrap.sv" -or $bn -match "pipe_r1|plant128|axi_bram128") {
...
Write-Host "A11A09R3_SHA_FROZEN $shaPath"
if (-not (Test-Path -LiteralPath $vivado)) { throw "A11A09R3_VIVADO_NOT_FOUND $vivado" }
...
if (Test-Path -LiteralPath $log) {
  $done = [bool](Select-String -Path $log -Pattern "ASTRA_11_A09R3_UART_IMPL_ROUTE_DONE" -Quiet)
}
if (($exit -ne 0) -or (-not $done)) {
  $r0 = Join-Path $bag "vivado_fail_r0.log"
  if (Test-Path -LiteralPath $log) { Copy-Item $log $r0 -Force }
  throw "A11A09R3_IMPL_FAIL exit=$exit done=$done"
}
...
Write-Host "ASTRA_11_A09R3_UART_IMPL_ROUTE_RUN_OK"
