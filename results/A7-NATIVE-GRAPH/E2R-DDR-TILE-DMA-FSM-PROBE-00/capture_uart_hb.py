#!/usr/bin/env python3
"""Capture COM12 for E2R-DDR-TILE-DMA-FSM-PROBE-00 — F1u DMA_ST + SGO probes."""
import argparse
import re
import sys
import time
...
def main():
    ...
    while time.time() - t0 < args.seconds:
        ...
        if "CORE_BUSY" in lines and (time.time() - t0) > 90.0:
            if any(x.startswith("SGO=") for x in lines) and any(x.startswith("DMA_ST=") for x in lines):
    ...
    dma_st = parse_val(dma_st_line)
    sgo = parse_val(sgo_line)
    ...
    probe_pass = dma_st_line is not None and sgo_line is not None
