...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C4B_FILE_MISSING $p" }
...
if ($hashFail) { throw "C4B_HASH_GATE_FAIL do_not_edit_keep_rtl" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c4_lm06_byte256.sv"
...
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C4B_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C4B_GOLD_HASH_DRIFT GOLDEN.json" }

$files = @($dut, $tb)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C4B_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C4B_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C4B_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") { throw "C4B_CTX_8255a798_AS_DUT" }
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C4B_LEFTOVER_A09" }
}
...
$pre = @(
  ...
  "# LAW=lm06-byte256-contract-01 4-cand physical head10 + mask; frozen LM-06 not DUT"
) + $hashLines
$pre += "# COMPILED"
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C4_LM06_BYTE256_CONTRACT_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C4B_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C4_LM06_BYTE256_CONTRACT_RUN_OK"
