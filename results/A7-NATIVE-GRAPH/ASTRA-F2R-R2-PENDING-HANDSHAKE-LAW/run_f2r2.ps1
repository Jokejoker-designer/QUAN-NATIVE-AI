...
$files = @(
  ...
  (Join-Path $bag "tb_astra_f2r2_hs_law.sv")
)
$includes = @(
...
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "F2R2_FILE_MISSING $p" }
...
$pre += (ShaLine (Join-Path $bag "run_f2r2.ps1"))
$pre += "# PROVENANCE floor-shift F2R (not compiled; do not edit)"
...
Write-Host "F2R2_SHA_FROZEN $shaPre"
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "F2R2_XVLOG_NOT_FOUND" }

Set-Location $work
& "$bin\xvlog.bat" --sv -i $incq -i $incc $files
if ($LASTEXITCODE -ne 0) { throw "F2R2_XVLOG_FAIL" }
...
& "$bin\xelab.bat" tb_astra_f2r2_hs_law -s f2r2 -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) {
  if (Test-Path (Join-Path $work "xelab.log")) {
    Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
  }
  throw "F2R2_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" f2r2 -R -log $xsimLog
if ($LASTEXITCODE -ne 0) { throw "F2R2_XSIM_FAIL" }
if (-not (Select-String -Path $xsimLog -Pattern "ASTRA_F2R2_HS_LAW_XSIM_PASS" -Quiet)) {
  throw "F2R2_PASS_MARKER_MISSING"
}
...
Write-Host "ASTRA_F2R2_HS_LAW_RUN_OK"
