...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5REW_FILE_MISSING $p" }
...
if ($hashFail) { throw "C5REW_HASH_GATE_FAIL do_not_edit_keep_or_live_prod_top" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_explicit_reward_01a.sv"
$dut = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_prod_top_rew.sv"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5REW_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5REW_GOLD_HASH_DRIFT GOLDEN.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5REW_LIVE_PROD_TOP_COMPILED" }
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5REW_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5REW_SYNTH_MIG_AS_DUT" }
}

$stamp = (Get-Date).ToString("yyyy-MM-ddTHH:mm:ss.fffffffK")
$pre = @(
  "# SHA freeze BEFORE xvlog $stamp",
  "# PROGRAM=NO BOARD_PASS=NOT_CLAIMED C5_MASTER=OPEN 01B=OPEN MIG=NO"
) + $hashLines
$pre += "# COMPILED"
foreach ($f in $files) { $pre += (ShaLine $f) }
$pre += (ShaLine (Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_prod_top_rew.svh"))
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C5_EXPLICIT_REWARD_01A_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C5REW_XSIM_FAIL_OR_MARKER_MISSING pass=$hasPass"
}
Write-Host "ASTRA_C5_EXPLICIT_REWARD_01A_RUN_OK"
