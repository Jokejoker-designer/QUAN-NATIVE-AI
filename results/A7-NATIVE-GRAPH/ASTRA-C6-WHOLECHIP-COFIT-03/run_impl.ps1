...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C6WC03_FILE_MISSING $p" }
...
if ($hashFail) { throw "C6WC03_HASH_GATE_FAIL do_not_edit_keep_rtl" }
...
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C6WC03_GOLD_NOT_HASHED_BEFORE_VIVADO" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C6WC03_GOLD_HASH_DRIFT GOLDEN.json" }
...
$pre = @(
  ...
  "# LAW=in-context route of C5 prod_top + official MIG after C3 polarity CONFLICT; A09 is not this top"
) + $hashLines
$pre += "# SCRIPTS"
$pre += (ShaLine $tcl)
...
Write-Host "C6WC03_SHA_FROZEN"

if (-not (Test-Path -LiteralPath "$bin\vivado.bat")) { throw "C6WC03_VIVADO_NOT_FOUND" }
...
if (Test-Path -LiteralPath $log) {
  $hasPass = [bool](Select-String -Path $log -Pattern '(?m)^ASTRA_C6_WHOLECHIP_COFIT_03_PASS$' -Quiet)
}
$need = @(
...
if ($hasPass -and $miss.Count -eq 0 -and $vivExit -eq 0) {
  Write-Host "ASTRA_C6_WHOLECHIP_COFIT_03_RUN_OK"
  exit 0
}
Write-Host "C6WC03_PS_MISS viv_exit=$vivExit pass_marker=$hasPass miss=$($miss -join ',')"
exit 1
