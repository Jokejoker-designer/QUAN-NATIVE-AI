#!/usr/bin/env python3
"""R-upsampled D=32 float copy. PROGRAM=NO. Keep best held.
"""
from __future__ import annotations

import json
from pathlib import Path

import numpy as np

from train_float import (
    EOS,
    HELD_ENT,
    Net,
    decode_f,
    decode_i,
    eval_rows,
    make_split,
    make_unrel,
    pack16,
    rname,
    train_seq,
    zeros_g,
)
from train_float_best import restore, snapshot


def fresh_row_weighted(rng):
    src, dst = rname(rng), rname(rng)
    while dst == src:
        dst = rname(rng)
    u = float(rng.random())
    if u < 0.50:
        return {"ctx": pack16(f"R {src} {src}>{dst}"), "ans": dst, "unrel": False}
    if u < 0.80:
        return {"ctx": pack16(f"F {dst} {src}>{dst}"), "ans": src, "unrel": False}
    other = rname(rng)
    while other in (src, dst):
        other = rname(rng)
    return {"ctx": pack16(f"F {dst} {src}>{other}"), "ans": "no", "unrel": True}


def main():
    bag = Path(__file__).resolve().parent
    rng = np.random.default_rng(7)
    n = Net(rng)
    adam = {"t": 0, "m": zeros_g(n), "v": zeros_g(n)}
    data_rng = np.random.default_rng(21)
    held_g = (make_split(HELD_ENT, "F") + make_split(HELD_ENT, "R"))[:20]
    held_u = make_unrel(HELD_ENT)[:12]
    hist = []
    best_acc = -1.0
    best_snap = snapshot(n)
    bs, epochs = 160, 50
    for ep in range(epochs):
        lr = 0.005 if ep < 20 else (0.002 if ep < 35 else 0.0007)
        tot = tf_ok = tf_n = 0
        for _ in range(bs):
            r = fresh_row_weighted(data_rng)
            tgt = [ord(c) for c in r["ans"]] + [EOS]
            loss, a, b = train_seq(n, r["ctx"].encode(), tgt, adam, lr)
            tot += loss
            tf_ok += a
            tf_n += b
        hg_f = eval_rows(n, held_g, decode_f)
        rec = {
            "ep": ep,
            "tf": tf_ok / tf_n if tf_n else 0.0,
            "held": hg_f["acc"],
            "held_ok": hg_f["ok"],
        }
        hist.append(rec)
        if hg_f["acc"] >= best_acc:
            best_acc = hg_f["acc"]
            best_snap = snapshot(n)
        if ep % 4 == 0 or hg_f["ok"] >= 10 or ep == epochs - 1:
            print(
                f"RUP EP {ep} tf={rec['tf']:.3f} held={hg_f['acc']:.3f} "
                f"({hg_f['ok']}/20) best={best_acc:.3f}",
                flush=True,
            )
        if hg_f["acc"] >= 0.90:
            best_snap = snapshot(n)
            break
    restore(n, best_snap)
    hg_f = eval_rows(n, held_g, decode_f)
    hg_i = eval_rows(n, held_g, decode_i)
    hu_f = eval_rows(n, held_u, decode_f)
    n_f = sum(1 for s in hg_f["samples"] if s[0].startswith("F") and s[2].startswith(s[1]))
    n_r = sum(1 for s in hg_f["samples"] if s[0].startswith("R") and s[2].startswith(s[1]))
    hall = hu_f["hall"] / hu_f["n"] if hu_f["n"] else 1.0
    out = {
        "bag": "ASTRA-C4-LM06-RED-FLOATTFM-01",
        "mode": "r_upsample_best",
        "program": "NO",
        "c4_master": False,
        "best_held_float": best_acc,
        "held_float": hg_f["acc"],
        "held_float_ok": hg_f["ok"],
        "held_f_ok": n_f,
        "held_r_ok": n_r,
        "held_int": hg_i["acc"],
        "hall_float": hall,
        "lang_90_float": hg_f["acc"] >= 0.90,
        "lang_90_int": hg_i["acc"] >= 0.90,
        "hist": hist,
        "held_samples": [{"ctx": a, "ans": b, "got": c} for a, b, c in hg_f["samples"]],
        "unrel_samples": [{"ctx": a, "ans": b, "got": c} for a, b, c in hu_f["samples"][:6]],
    }
    (bag / "TRAIN_METRICS_RUP.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
    print("SUMMARY", json.dumps({k: out[k] for k in ("best_held_float", "held_float", "held_f_ok", "held_r_ok", "held_int", "hall_float", "lang_90_float")}), flush=True)


if __name__ == "__main__":
    main()
