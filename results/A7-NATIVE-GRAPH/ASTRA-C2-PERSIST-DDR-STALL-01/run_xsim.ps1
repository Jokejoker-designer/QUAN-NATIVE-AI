...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C2ST_FILE_MISSING $p" }
...
if ($hashFail) { throw "C2ST_HASH_GATE_FAIL do_not_edit_keep_rtl" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c2_persist_ddr_stall.sv"
$dut = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c2_persist_ddr_stall.sv"
$svh = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c2_persist_ddr_stall.svh"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C2ST_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C2ST_GOLD_HASH_DRIFT GOLDEN.json" }

$files = @($dut, $tb)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C2ST_LEFTOVER_A09" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C2ST_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C2ST_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_learned_prior_store.sv") { throw "C2ST_PRIOR_STORE_AS_DUT" }
  if ($leaf -eq "a7ng_astra_06_warm_persist.sv") { throw "C2ST_ASTRA06_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c2_persist_commit.sv") { throw "C2ST_IDENTITY_KEEP_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c2_persist_multi_slot.sv") { throw "C2ST_MULTISLOT_KEEP_AS_DUT" }
}
...
$pre = @(
  ...
  "# LAW=astra-c2-persist-stall-01 STALL_AW=8 STALL_W=5 STALL_B=6 AXI_JOURNAL PERSIST_BASE=0x06000000",
  "# HASH_GATE MATCH vs C0 + persist-commit/multi-slot KEEP (not compiled as DUT)"
) + $hashLines
$pre += "# COMPILED"
foreach ($f in $files) { $pre += (ShaLine $f) }
$pre += (ShaLine $svh)
$pre += (ShaLine $goldJson)
$pre += "# FORBIDDEN leftover A09 / C0 sparse / STREAM-02 / prior_store / ASTRA-06 / persist KEEP not DUT"
...
function Write-DivLog([string]$code, [string]$detail) {
  [System.IO.File]::WriteAllLines((Join-Path $bag "xsim.log"), @(
    ...
    "RESULT=FAIL",
    "ASTRA_C2_PERSIST_DDR_STALL_XSIM_PASS ABSENT",
    "PROGRAM=NO BOARD_PASS=NOT_CLAIMED"
  ))
}

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C2ST_XVLOG_NOT_FOUND" }
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C2ST_XVLOG_FAIL"
}
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
$xvlogTxt = [System.IO.File]::ReadAllText((Join-Path $bag "xvlog.log"))
if ($xvlogTxt -match "a7ng_astra_09_integ_path") { throw "C2ST_LEFTOVER_A09_COMPILED" }
if ($xvlogTxt -match "a7ng_query_axi_sparse_stream_intersect") { throw "C2ST_STREAM02_COMPILED" }
if ($xvlogTxt -match "a7ng_learned_prior_store") { throw "C2ST_PRIOR_STORE_COMPILED" }
if ($xvlogTxt -match "a7ng_astra_06_warm_persist") { throw "C2ST_ASTRA06_COMPILED" }
if ($xvlogTxt -match "a7ng_astra_c2_persist_commit\.sv") { throw "C2ST_IDENTITY_KEEP_COMPILED" }
if ($xvlogTxt -match "a7ng_astra_c2_persist_multi_slot\.sv") { throw "C2ST_MULTISLOT_KEEP_COMPILED" }
if ($xvlogTxt -notmatch "a7ng_astra_c2_persist_ddr_stall") { throw "C2ST_DUT_NOT_ANALYZED" }

& "$bin\xelab.bat" tb_astra_c2_persist_ddr_stall -s astra_c2_persist_stall -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C2ST_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" astra_c2_persist_stall -R -log $xsimLog
if (-not (Test-Path -LiteralPath $xsimLog)) { throw "C2ST_NO_XSIM_LOG" }
$hasPass = [bool](Select-String -Path $xsimLog -Pattern "^ASTRA_C2_PERSIST_DDR_STALL_XSIM_PASS$" -Quiet)
$hasDiv = [bool](Select-String -Path $xsimLog -Pattern "^FIRST_DIVERGENCE" -Quiet)
$need = @(
  "CLASS_aw_stall HIT",
  "CLASS_w_stall HIT",
  "CLASS_b_stall HIT",
  "CLASS_wdata_stable HIT",
  "CLASS_not_persisted_before_b HIT",
  "CLASS_in_place_dirty HIT",
  "CLASS_writeback_after_stall HIT",
  "CLASS_high_id_identity HIT",
  "CLASS_alias_attempt HIT",
  "CLASS_journal_addr_not_low16 HIT",
  "CLASS_reset_during_stall HIT",
  "CLASS_reload_after_stall HIT",
...
if ($hasDiv -or (-not $hasPass) -or ($missing.Count -gt 0)) {
  ...
  throw "C2ST_XSIM_CONTROL_FAIL pass=$hasPass div=$hasDiv missing=$($missing -join ',')"
}
Write-Host "ASTRA_C2_PERSIST_DDR_STALL_RUN_OK PASS_THIS_GATE_ONLY PROGRAM=NO"
