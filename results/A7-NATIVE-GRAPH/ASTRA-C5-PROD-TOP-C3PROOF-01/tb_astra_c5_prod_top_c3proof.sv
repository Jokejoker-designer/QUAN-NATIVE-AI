`timescale 1ns / 1ps
// ASTRA-C5-PROD-TOP-C3PROOF-01. PROGRAM=NO. Bag-local TB (test-only plant).
// New named C5 hierarchy. Modeled AXI, not MIG. Not BOARD.
...
module tb_astra_c5_prod_top_c3proof;
  ...
  a7ng_astra_c5_prod_top_c4p u_dut (
    ...
    .gen_done_o(gdone), .gen_last_o(glast), .gen_tok0_o(gtok0),
  ...
  initial begin
    ...
    $display("C5_PROD_TOP_C3PROOF PROGRAM=NO BOARD_PASS=REJECT DDR_QUERY_BOUND_FINAL=NOT_FROZEN MIG=NO A09=NO");
    $display("CLASS_not_compose_renderer HIT dut=a7ng_astra_c5_prod_top_c4p");
    ...
    gold0 = a7ng_c4p_ch(C3_SHARED_DST[7:0], 0);
    $display("MEAS nurx=%0d nutx=%0d seen=%02h dual=%0d st=%0d obj=%0d ans=%0d tok0=%0d gold0=%0d gdone=%0d nht=%0d",
      nurx, nutx, seen, dual, c3st, c3obj, c3ans, gtok0, gold0, gdone, nht);
    ...
    if (gdone && (nht == 16'd0)) $display("CLASS_lm06_gen HIT glast=%0d tok0=%0d", glast, gtok0);
    ...
    if (gdone && (gtok0 === gold0) && (c3ans == C3_SHARED_DST[19:0]))
      $display("CLASS_gen_tok0_fpga_dict_of_c3_ans HIT tok0=%0d dest=%0d", gtok0, c3ans);
    else $display("CLASS_gen_tok0_fpga_dict_of_c3_ans MISS tok0=%0d gold0=%0d ans=%0d", gtok0, gold0, c3ans);
    ...
    chk("TOK0_DICT", gtok0 === gold0);

    if (fail == 0) $display("ASTRA_C5_PROD_TOP_C3PROOF_XSIM_PASS");
    else $display("ASTRA_C5_PROD_TOP_C3PROOF_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO DDR_QUERY_BOUND_FINAL=NOT_FROZEN quality=C5_HIER_C3_DEST_DICT_CTXCOPY_NOT_802K");
    $finish;
  end
endmodule
