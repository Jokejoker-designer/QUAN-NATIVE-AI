...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5C4P_FILE_MISSING $p" }
...
if ($hashFail) { throw "C5C4P_HASH_GATE_FAIL do_not_edit_keep_or_live_prod_top" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_prod_top_c3proof.sv"
$dut = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_prod_top_c4p.sv"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5C4P_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5C4P_GOLD_HASH_DRIFT GOLDEN.json" }

$dutTxt = [System.IO.File]::ReadAllText($dut)
if ($dutTxt -match "evid_obj") { throw "C5C4P_DUT_HAS_EVID_OBJ" }
if ($dutTxt -match "a7ng_astra_c4_lm06_grounded_gen") { throw "C5C4P_DUT_HAS_GROUNDED_GEN" }
if ($dutTxt -notmatch "a7ng_c4p_ch") { throw "C5C4P_DUT_MISSING_FPGA_DICT" }
if ($dutTxt -notmatch "a7ng_astra_c3_held_out") { throw "C5C4P_DUT_MISSING_C3" }
...
$files += (Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c4_lm06_byte256_ctxcopy.sv")
...
foreach ($f in $files) {
  ...
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5C4P_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C5C4P_COMPOSE_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_byte256_dict.sv") { throw "C5C4P_DICT_LUT_DUT" }
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5C4P_LIVE_PROD_TOP_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_grounded_gen.sv") { throw "C5C4P_GROUNDED_GEN_AS_DUT" }
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C5C4P_LEFTOVER_A09" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C5C4P_C0_SPARSE_AS_DUT" }
}
...
$pre = @(
  ...
  "# LAW=C5 hierarchy C3 dest FPGA dict then ctxcopy"
) + $hashLines
$pre += "# COMPILED"
...
Write-Host "C5C4P_SHA_FROZEN"

if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null
Set-Location $work
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C5C4P_XVLOG_NOT_FOUND" }
& "$bin\xvlog.bat" --sv -i $incq -i $incc -i $inci -i $incb -i $bag $files
if ($LASTEXITCODE -ne 0) { throw "C5C4P_XVLOG_FAIL" }
...
if ($xvlogTxt -match "tiny_gpt803k_core") { throw "C5C4P_TINYGPT_COMPILED" }
if ($xvlogTxt -match "a7ng_astra_c4_lm06_grounded_gen") { throw "C5C4P_GROUNDED_GEN_COMPILED" }
if ($xvlogTxt -notmatch "a7ng_astra_c5_prod_top_c4p") { throw "C5C4P_DUT_NOT_ANALYZED" }
& "$bin\xelab.bat" tb_astra_c5_prod_top_c3proof -s c5c4p -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) {
  if (Test-Path (Join-Path $work "xelab.log")) {
    Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
  }
  throw "C5C4P_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c5c4p -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "^ASTRA_C5_PROD_TOP_C3PROOF_XSIM_PASS$" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C5C4P_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C5_PROD_TOP_C3PROOF_RUN_OK PASS_THIS_GATE_ONLY PROGRAM=NO C5_MASTER=OPEN"
