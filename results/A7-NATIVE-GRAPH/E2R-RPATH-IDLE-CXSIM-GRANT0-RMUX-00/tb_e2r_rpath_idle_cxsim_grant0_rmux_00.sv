// E2R-RPATH-IDLE-CXSIM-GRANT0-RMUX-00 — GRANT0 vehicle + 8 leftover mux R after dest==3
// Vehicle: RINJ/GRANT0 copy. Grant register STAYS 0. After first dbg_tile_dst==3,
// inject 8 combinational-legal leftover beats on the shared stub R into the mux
// (ungated cdc_rvalid=rvalid). Do NOT drive CDC-slave dma_r_valid as the inject.
// UNKNOWN: do those 8 mux leftover R beats make DUT-driven dbg_tile_dst==4 while grant=0?
...
module tb_e2r_rpath_idle_cxsim_grant0_rmux_00;
  import a7ng_pkg::*;
  ...
  // Shared stub / mux AXI (ui). Stub R is split so leftover can occupy mux rvalid.
  logic [3:0]  arid, rid;
  logic [27:0] araddr;
  logic [7:0]  arlen;
  logic [2:0]  arsize;
  ...
  logic [3:0]  stub_rid;
  logic [1:0]  stub_rresp;
  logic [127:0] stub_rdata;
  logic        stub_rlast, stub_rvalid, stub_rready;
  logic [3:0]  awid, bid;
  logic [27:0] awaddr;
  logic [7:0]  awlen;
  logic [2:0]  awsize;
  ...
  assign cdc_rvalid  = rvalid; // UNGATED — leftover occupies this mux path
  ...
  a7ng_axi_soa_mem_stub u_mem (
    ...
    .s_axi_rid(stub_rid), .s_axi_rdata(stub_rdata), .s_axi_rresp(stub_rresp),
    .s_axi_rlast(stub_rlast), .s_axi_rvalid(stub_rvalid), .s_axi_rready(stub_rready)
  );
  ...
  // RMUX: after dest==3, leftover R occupies shared stub R into the mux.
  // Combinational-legal: registered valid; mux handshake = leftover && cdc_rready
  // (own_ui=0). Do NOT drive CDC-slave dma_r_valid as the inject (that is RINJ).
  logic        rmux_arm;
  logic        rmux_arm_u, rmux_arm_u2;
  logic        rmux_active;
  logic        rmux_done;
  logic        rmux_v;
  logic [3:0]  rmux_left;
  ...
  logic        dma_rv_during_rmux;

  assign rvalid = rmux_active ? rmux_v : stub_rvalid;
  assign rlast  = rmux_active ? (rmux_left == 4'd1) : stub_rlast;
  assign rdata  = rmux_active ? {124'd0, rmux_left} : stub_rdata;
  assign rid    = rmux_active ? 4'hE : stub_rid;
  assign rresp  = rmux_active ? 2'b00 : stub_rresp;
  assign stub_rready = rmux_active ? 1'b0 : rready;

  assign dma_r_valid = (w_st == W_R) && stub_rvalid && wdma_owner_ui;
  assign dma_r_data  = stub_rdata;
  ...
  always_ff @(posedge ui_clk or negedge ui_rst_n) begin
    if (!ui_rst_n) begin
      ...
      rmux_arm_u  <= rmux_arm;
      rmux_arm_u2 <= rmux_arm_u;
      // Start leftover only when stub R is idle (leftover, not steal-live-R).
      if (rmux_arm_u2 && !rmux_active && !rmux_done && !stub_rvalid) begin
        rmux_active <= 1'b1;
        rmux_v      <= 1'b1;
        rmux_left   <= 4'(RINJ_N);
      end else if (rmux_active && rmux_v && cdc_rready) begin
        rinj_accepted <= rinj_accepted + 1;
        if (rmux_left == 4'd1) begin
          rmux_active <= 1'b0;
          rmux_v      <= 1'b0;
          rmux_done   <= 1'b1;
          rmux_left   <= 4'd0;
        end else
          rmux_left <= rmux_left - 4'd1;
      end
    end
  end
  ...
  always_ff @(posedge core_clk or negedge core_rst_n) begin
    if (!core_rst_n) begin
      ...
      if (rmux_active && dma_r_valid)
        dma_rv_during_rmux <= 1'b1;
  ...
  initial begin
    ...
    if (pf == 0) begin
      $display("E2R_RPATH_IDLE_CXSIM_GRANT0_RMUX_00_XSIM_FAIL reason=probe_fopen");
      $fatal(1);
    end
    $fdisplay(pf, "t_ns,phase,tile_dst,r_drain_hold,fifo_cnt,m_axi_rvalid,tr_cnt,r_path_idle,wdma_owner,wdma_owner_grant,wdma_owner_ui,cdc_rvalid,stub_rvalid,wdma_go,wdma_busy,r_left,soa_done,core_busy,lm_phase,rinj_acc,rmux_left,rmux_done");
    ...
    $display("E2R-RPATH-IDLE-CXSIM-GRANT0-RMUX-00 START");
    $display("VEHICLE=a7ng_native_v1_ab_core SIM_FULL=0 do_lm=1 CDC+B1_GRANT0+MUX ungated_cdc_rvalid SHARED_STUB_WDMA RMUX8_AFTER_DST3");
    $display("PREREG_UNKNOWN: grant=0 own_ui=0; after first dbg_tile_dst==3, do 8 leftover beats on shared stub R into mux (cdc_rvalid=rvalid) make dest==4?");
    $display("PREREG_H_CANDIDATE: mux leftover R without grant reaches dest=4 and holds idle=0; then ONE/SET/NONE");
    $display("PREREG_H_RIVAL: dest stays 3 after 8 mux leftover R (FAIL_NO_DESTWAIT_GRANT0)");
    $display("PREREG_FALSIFIER: force dst; raise grant; CDC-slave dma_r_valid inject (RINJ); product RTL; C-FIX; board");
    $display("PREREG_UNIT: one query, grant=0 from reset, inject exactly 8 leftover beats on shared stub R after first dest==3");
    $display("FORBIDDEN: force TILE_DST/dst; assign r_path_idle=1; raise grant; C-FIX; dma_r_valid inject; soc_top; MIG; board");
    ...
    if (!drain3_seen) begin
      ...
      $display("E2R_RPATH_IDLE_CXSIM_GRANT0_RMUX_00_FAIL_NO_DESTWAIT_GRANT0 reason=no_dst3 dst=%0d grant=%0b",
    ...
    timeout = 0;
    while (!rmux_done && timeout < RINJ_TO) begin
      @(posedge ui_clk);
      timeout = timeout + 1;
    end
    dump_row("AFTER_RMUX");
    $display("R_INJECTED=%0d rmux_done=%0b dma_rv_during_rmux=%0b", rinj_accepted, rmux_done, dma_rv_during_rmux);
    ...
    $display("OWN_UI=%0b", wdma_owner_ui);
    $display("DMA_RV_DURING_RMUX=%0b", dma_rv_during_rmux);
    ...
    if (!destwait_seen || !grant_stayed_0 || (rinj_accepted != 8) || dma_rv_during_rmux) begin
      $display("XSIM=FAIL_NO_DESTWAIT_GRANT0");
      $display("E2R_RPATH_IDLE_CXSIM_GRANT0_RMUX_00_FAIL_NO_DESTWAIT_GRANT0 dst=%0d max_dst=%0d bst=%0d miss=%0b reqs1=%0b go=%0b busy=%0b soa=%0b fwd=%0b grant=%0b grant_stayed_0=%0b own_ui=%0b rinj=%0d dma_rv_during_rmux=%0b",
        tile_dst, max_dst, tile_bst, tile_miss, tile_req_s1, wdma_go, wdma_busy, soa_done_seen, start_fwd_seen,
        wdma_owner_grant, grant_stayed_0, wdma_owner_ui, rinj_accepted, dma_rv_during_rmux);
      $finish;
    end

    $display("XSIM=PASS");
    $display("E2R_RPATH_IDLE_CXSIM_GRANT0_RMUX_00_XSIM_PASS verdict=%s wire=%s c_fix=NONE idle=%0b n_hot=%0d dst=%0d grant_stayed_0=%0b rinj=%0d dma_rv_during_rmux=%0b",
      verdict_class, wire_named, snap_idle, n_hot, snap_dst, grant_stayed_0, rinj_accepted, dma_rv_during_rmux);
    $finish;
  end
  ...
  initial begin
    ...
    $display("WALL_LIMIT");
    $display("XSIM=LIMIT");
    $display("E2R_RPATH_IDLE_CXSIM_GRANT0_RMUX_00_FAIL_NO_DESTWAIT_GRANT0 reason=wall dst=%0d grant=%0b rinj=%0d",
