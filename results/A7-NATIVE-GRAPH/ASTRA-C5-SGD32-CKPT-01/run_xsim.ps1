...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5K_FILE_MISSING $p" }
...
if ($hashFail) { throw "C5K_HASH_GATE_FAIL do_not_edit_c2_or_live_prod_top" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_sgd32_ckpt.sv"
...
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5K_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5K_GOLD_HASH_DRIFT GOLDEN.json" }

$files = @($sgd, $dut, $tb)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "a7ng_astra_c2_persist_commit.sv") { throw "C5K_C2_KEEP_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5K_LIVE_PROD_TOP_AS_DUT" }
}
...
$hasPass = $false
$hasEx = $false; $hasCrc = $false; $hasB = $false
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C5_SGD32_CKPT_XSIM_PASS" -Quiet)
...
if (($xsimExit -ne 0) -or (-not $hasPass) -or (-not $hasEx) -or (-not $hasCrc) -or (-not $hasB)) {
  if (Test-Path -LiteralPath $xsimLog) {
    Copy-Item $xsimLog (Join-Path $bag "xsim_fail.log") -Force
  }
  throw "C5K_XSIM_FAIL_OR_MARKER_MISSING pass=$hasPass ex=$hasEx crc=$hasCrc b=$hasB"
}
Write-Host "ASTRA_C5_SGD32_CKPT_RUN_OK"
