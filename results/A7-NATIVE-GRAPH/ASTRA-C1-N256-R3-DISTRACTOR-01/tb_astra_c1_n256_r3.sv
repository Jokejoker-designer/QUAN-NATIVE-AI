// tb_astra_c1_n256_r3.sv — C1 N=256 R3 distractor polarity. PROGRAM=NO.
// Frozen QSE + sparse AXI. poke_v=0. leftover A09 not compiled.
...
module tb_astra_c1_n256_r3;
  ...
  integer keys_match, emit_match, direct_n;
  ...
  initial begin
    ...
    $display("C1_R3_N=%0d CAND_CAP=%0d INDEX_HEAD=%0d LAW_SEL=1 POKE_V=0 REDUCTION_X1000=NOT_EMITTED",
    ...
    for (qi = 0; qi < G_NQ; qi = qi + 1) begin
      ...
      if (qi == 4) begin
        keys_match = (k0 === G_K0[0] && k1 === G_K1[0] && k2 === G_K2[0] && k3 === G_K3[0]);
        emit_match = (n_got == direct_n);
        if (emit_match)
          for (i = 0; i < n_got; i = i + 1)
            if (got[i] !== direct_got[i]) emit_match = 0;
        $display("NOT_SELECTIVE class=wrong_context k0=%0d k1=%0d k2=%0d k3=%0d vs_direct k0=%0d k1=%0d k2=%0d k3=%0d keys_match=%0d emit_match=%0d frozen_law=xid_not_directory_key rec_is_not_context_selectivity=1",
          k0, k1, k2, k3, G_K0[0], G_K1[0], G_K2[0], G_K3[0], keys_match, emit_match);
        wc_not_sel = 1;
      end
    ...
    if (fail == 0 && dist_gold_ok && dist_excl_ok && dist_no_leak && unrelated_empty && wc_not_sel) begin
      $display("ASTRA_C1_N256_R3_XSIM_PASS");
      ...
      $display("ASTRA_C1_N256_R3_XSIM_NO_MARKER fail=%0d dist_gold_ok=%0d dist_excl_ok=%0d dist_no_leak=%0d unrelated_empty=%0d wc_not_sel=%0d",
        fail, dist_gold_ok, dist_excl_ok, dist_no_leak, unrelated_empty, wc_not_sel);
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,N_gt_256");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end
