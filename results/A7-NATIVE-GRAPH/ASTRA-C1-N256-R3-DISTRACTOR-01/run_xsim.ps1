...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C1R3_FILE_MISSING $p" }
...
if ($hashFail) {
  ...
  throw "C1R3_HASH_GATE_FAIL do_not_invent"
}
...
$tb = Join-Path $bag "tb_astra_c1_n256_r3.sv"
if (-not (Test-Path -LiteralPath $goldJson)) { throw "C1R3_GOLD_MISSING GOLDEN.json" }
if (-not (Test-Path -LiteralPath $goldSvh)) { throw "C1R3_GOLD_MISSING query_gold.svh" }
if (-not (Test-Path -LiteralPath $corpus)) { throw "C1R3_GOLD_MISSING corpus.json" }
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C1R3_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$liveSvh = Sha256 $goldSvh
$liveCorpus = Sha256 $corpus
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C1R3_GOLD_HASH_DRIFT GOLDEN.json" }
if ($preTxt -notmatch $liveSvh) { throw "C1R3_GOLD_HASH_DRIFT query_gold.svh" }
if ($preTxt -notmatch $liveCorpus) { throw "C1R3_GOLD_HASH_DRIFT corpus.json" }
...
foreach ($f in $files) {
  if ((Split-Path -Leaf $f) -match "a7ng_astra_09_integ_path") {
    throw "C1R3_LEFTOVER_A09_ON_XVLOG_LIST"
  }
}
...
$pre = @(
  ...
  "# PROGRAM=NO C1_800K=OPEN CAND_CAP=16 N=256 LAW_SEL=1 POKE_V=0",
...
$pre += "$liveGold  results/A7-NATIVE-GRAPH/ASTRA-C1-N256-R3-DISTRACTOR-01/GOLDEN.json"
$pre += "$liveSvh  results/A7-NATIVE-GRAPH/ASTRA-C1-N256-R3-DISTRACTOR-01/query_gold.svh"
$pre += "$liveCorpus  results/A7-NATIVE-GRAPH/ASTRA-C1-N256-R3-DISTRACTOR-01/corpus.json"
$pre += "# BAG"
$pre += (ShaLine (Join-Path $bag "PREREG.md"))
$pre += (ShaLine (Join-Path $bag "ACK.json"))
$pre += (ShaLine (Join-Path $bag "run_xsim.ps1"))
$pre += (ShaLine (Join-Path $bag "host_astra_c1_r3.py"))
$pre += (ShaLine $tb)
...
Write-Host "C1R3_SHA_FROZEN_BEFORE_XVLOG"

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C1R3_XVLOG_NOT_FOUND" }
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C1_N256_R3_XSIM_PASS" -Quiet)
...
if ($hasRedCap) {
  ...
  throw "C1R3_REDUCTION_X1000_EMITTED_AS_CAP_N"
}
if (($xsimExit -ne 0) -or (-not $hasPass) -or $hasDiv -or $hasLeak) {
  ...
  throw "C1R3_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C1_N256_R3_RUN_OK"
