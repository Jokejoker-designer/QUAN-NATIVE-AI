`timescale 1ns / 1ps
module tb_astra_f2r4_axi_drain;
  ...
  int fail, i, nslot, inj, late_cnt;
  string first_div;
  logic tok_v, tok_r, fire, retire, rew_v, busy, result_v, tbl;
  logic pend_acc, pend_cmt, load_v, axi_ost, axi_abort;
  logic [1:0] ctrl, sel;
  ...
  typedef enum logic [1:0] { M_IDLE, M_BEAT, M_LATE, M_NOLAST } mst_t;
  mst_t mst;
  logic [7:0] rem; logic [27:0] ra;
  logic cross_saved, cross_arm, cross_go;
  int cross_dly;
  logic [3:0] cross_rid;
  logic [127:0] cross_data;
  logic [15:0] d0;
  ...
  assign arready = (mst==M_IDLE) && !((inj==4) && arvalid && (araddr>=FACT_BASE));
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      mst<=M_IDLE; rvalid<=0; rlast<=0; rdata<=0; rid<=0; rresp<=0; rem<=0; ra<=0;
      late_cnt<=0; cross_saved<=0; cross_arm<=0; cross_dly<=0; cross_rid<=0; cross_data<=0;
    end else begin
      if (cross_go) begin cross_arm<=1'b1; cross_dly<=40; end
      else if (cross_arm && (cross_dly>0)) cross_dly <= cross_dly-1;
      unique case (mst)
        M_IDLE: begin
          if (arvalid && arready) begin
            ra<=araddr; rem<=arlen;
            if ((araddr>=FACT_BASE) && (inj==3)) begin
              cross_rid<=arid; cross_data<=mem_rd(araddr); cross_saved<=1'b1;
            end else if ((araddr>=FACT_BASE) && ((inj==5)||(inj==6))) begin
              late_cnt <= (inj==5) ? 20 : 80;
              rid<=arid; rresp<=2'b00; rlast<=1'b1; rdata<=mem_rd(araddr);
              mst<=M_LATE;
            end else if ((araddr>=FACT_BASE) && (inj==2)) begin
              rid<=arid; rresp<=2'b00; rlast<=1'b0; rvalid<=1'b1;
              rdata<=mem_rd(araddr); mst<=M_NOLAST;
            end else if ((araddr>=FACT_BASE) && (inj==1)) begin
              rid<=arid; rresp<=2'b10; rlast<=1'b1; rvalid<=1'b1;
              rdata<=mem_rd(araddr); mst<=M_BEAT;
            end else begin
              rid<=arid; rresp<=2'b00; rlast<=(arlen==8'd0); rvalid<=1'b1;
              rdata<=mem_rd(araddr); mst<=M_BEAT;
            end
          end else if (cross_saved && cross_arm && (cross_dly==0) && !rvalid) begin
            rid<=cross_rid; rdata<=cross_data; rresp<=2'b00; rlast<=1'b1;
            rvalid<=1'b1; cross_saved<=1'b0; cross_arm<=1'b0; mst<=M_BEAT;
          end
        end
        ...
        M_NOLAST: if (rvalid && rready) begin
          rdata<=128'd0; rlast<=1'b1; rvalid<=1'b1; rresp<=2'b00; mst<=M_BEAT;
        end
        M_LATE: begin
          if (late_cnt>1) late_cnt<=late_cnt-1;
          else if (late_cnt==1) begin rvalid<=1'b1; late_cnt<=0; end
          if (rvalid && rready) begin rvalid<=1'b0; mst<=M_IDLE; end
        end
      endcase
    end
  end

  a7ng_astra_f2r4_axi_drain u_dut (
  ...
  task automatic hard_rst;
    begin rst_n=0; load_v=0; rew_v=0; fire=0; retire=0; tok_v=0; ctrl=0; inj=0; cross_go=0;
  ...
  task automatic dump(input string tag);
    $display("%s st=%0d npath=%0d ans=%0d p0=%0d acc=%0d abort=%0d ost=%0d arvalid=%0d narto=%0d nrto=%0d nerr=%0d ndrain=%0d naban=%0d dead=%0h tbl=%0d",
      tag, st, npath, ans, p0, pend_acc, axi_abort, axi_ost, arvalid, narto, nrto, nerr, ndrain, naban, dead_mask, tbl);
  endtask
  task automatic chk_abort(input string tag);
    begin
      dump(tag);
      chk(tag, (st===ST_INCOMP) && (ans===20'd0) && (p0===20'd0) && !pend_acc
          && !arvalid && !axi_ost && !tbl && axi_abort);
    end
  endtask
  task automatic chk_smoke(input string tag);
    begin
      dump(tag);
      chk(tag, (st===ST_ANSWER) && !tbl && (npath>=5'd2) && (p0===20'd17)
          && (ans===20'd4) && !arvalid && !axi_ost);
    end
  endtask
  ...
  initial begin
    ...
    go_s=0; go_u=0; iso_load=0; iso_rew=0; rst_n=0; cross_go=0;
    ...
    chk_smoke("SMOKE_TWO_PROOFS");
    retire_q();

    hard_rst(); inj=5; plant2();
    send_text("pump requires indirect"); wait_done();
    chk_smoke("LATE_R_OK");
    retire_q(); inj=0;

    hard_rst(); inj=4; plant2();
    send_text("pump requires indirect"); wait_done();
    chk_abort("AR_STALL");
    chk("AR_STALL_NARTO", narto>=16'd1);
    retire_q(); inj=0;
    ...
    hard_rst(); inj=2; plant2();
    send_text("pump requires indirect"); wait_done();
    chk_abort("NOLAST");
    chk("NOLAST_NERR", nerr>=16'd1);
    chk("NOLAST_NDRAIN", ndrain>=16'd1);
    retire_q(); inj=0;

    hard_rst(); inj=3; plant2();
    send_text("pump requires indirect"); wait_done();
    chk_abort("NORESP");
    chk("NORESP_NRTO", nrto>=16'd1);
    chk("NORESP_ABANDON", (naban>=16'd1) || (ndrain>=16'd1));
    retire_q(); inj=0;

    hard_rst(); inj=6; plant2();
    send_text("pump requires indirect"); wait_done();
    chk_abort("R_AFTER_TO");
    chk("R_AFTER_TO_NOT_ANS4", ans!==20'd4);
    chk("R_AFTER_TO_DRAIN", (ndrain>=16'd1) || (naban>=16'd1) || (nrto>=16'd1));
    retire_q(); inj=0;

    hard_rst(); inj=3; plant2();
    send_text("pump requires indirect"); wait_done();
    chk_abort("CROSS_SETUP");
    d0 = ndrain;
    retire_q();
    inj=0; plant2();
    @(posedge clk); cross_go<=1; @(posedge clk); cross_go<=0;
    send_text("pump requires indirect"); wait_done();
    chk_smoke("CROSS_NEXT_SMOKE");
    chk("CROSS_DRAINED_OLD", ndrain>d0);
    retire_q();

    hard_rst(); inj=0; plant2();
    send_text("pump requires indirect"); wait_done();
    chk_smoke("SMOKE_AFTER");
    retire_q();
    send_text("payroll tax form"); wait_done();
    dump("UNREL");
    chk("UNREL_NO_STALE", st===ST_UNKNOWN && npath===5'd0 && ans===20'd0 && p0===20'd0);

    if (fail==0) $display("ASTRA_F2R4_AXI_DRAIN_XSIM_PASS");
    else $display("ASTRA_F2R4_AXI_DRAIN_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $finish;
  end
endmodule
