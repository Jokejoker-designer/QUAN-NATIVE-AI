...
$files = @(
  ...
  (Join-Path $bag "tb_astra_f2r4_axi_drain.sv")
)
$includes = @(
...
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "F2R4_FILE_MISSING $p" }
...
$pre += "# PROVENANCE frozen F2R3/F2R2 (not compiled; do not edit)"
...
Write-Host "F2R4_SHA_FROZEN $shaPre"
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "F2R4_XVLOG_NOT_FOUND" }

Set-Location $work
& "$bin\xvlog.bat" --sv -i $incq -i $incc -i $inci $files
if ($LASTEXITCODE -ne 0) { throw "F2R4_XVLOG_FAIL" }
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_F2R4_AXI_DRAIN_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  if (Test-Path -LiteralPath $xsimLog) {
    Copy-Item $xsimLog (Join-Path $bag "xsim_fail.log") -Force
  }
  throw "F2R4_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_F2R4_AXI_DRAIN_RUN_OK"
