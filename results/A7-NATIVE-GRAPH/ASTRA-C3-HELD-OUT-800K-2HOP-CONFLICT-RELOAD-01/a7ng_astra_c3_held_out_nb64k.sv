// a7ng_astra_c3_held_out_nb64k.sv — ASTRA-C3-HELD-OUT-800K-2HOP-CONFLICT-RTL-01. PROGRAM=NO.
// Bag-local copy of live CONFLICT C3 wrap. Live a7ng_astra_c3_held_out.sv is not edited.
// Deltas vs live wrap: module name, N_BUCKETS=65536, MAX_PATH=16, sel_idx_o[3:0] TB match.
// Instantiates C1 synonym KEEP + frozen a7ng_shared_rank_sgd_q8_sym_f2r2.
// Two positive dests still rank. Polarity CONFLICT only. Not BOARD.
`timescale 1ns / 1ps
...
module a7ng_astra_c3_held_out_nb64k #(
  ...
  assign sel_idx_o = {2'b00, sel_idx};
  assign ans_o = best_a;
    ...
    if (!rst_n) begin
      ...
      unique case (st)
        S_IDLE: begin
          ...
          r_conf <= 1'b0; have_pos <= 1'b0; have_neg <= 1'b0;
          concl0 <= '0; neg0 <= '0;
