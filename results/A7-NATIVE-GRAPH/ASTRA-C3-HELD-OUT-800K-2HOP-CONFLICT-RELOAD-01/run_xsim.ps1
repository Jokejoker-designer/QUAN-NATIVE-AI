...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C3K2HCR_FILE_MISSING $p" }
...
if ($hashFail) { throw "C3K2HCR_HASH_GATE_FAIL do_not_edit_keep_rtl" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c3_held_out_800k_2hop_conflict_reload.sv"
$mem = Join-Path $bag "a7ng_axi_mem_c3_800k_2hop.sv"
$dut = Join-Path $bag "a7ng_astra_c3_held_out_nb64k.sv"
$svh = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c3_held_out.svh"
$c2 = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c2_persist_commit.sv"
$c2h = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c2_persist_commit.svh"
$sgd = Join-Path $root "rtl\native_graph\learn\a7ng_shared_rank_sgd_q8_sym_f2r2.sv"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C3K2HCR_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C3K2HCR_GOLD_HASH_DRIFT GOLDEN.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C3K2HCR_LEFTOVER_A09" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C3K2HCR_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C3K2HCR_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") { throw "C3K2HCR_CTX_8255a798_AS_DUT" }
  if ($leaf -eq "a7ng_learned_prior_store.sv") { throw "C3K2HCR_PRIOR_STORE" }
  if ($leaf -eq "a7ng_astra_06_warm_persist.sv") { throw "C3K2HCR_ASTRA06" }
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C3K2HCR_TINYGPT_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c3_held_out.sv") { throw "C3K2HCR_KEEP_C3_COMPILED_USE_NB64K" }
}
...
$pre = @(
  ...
  '# LAW=800k cartesian 2-hop ctx overlay + C2 KEEP w0 reload on live CONFLICT C3 copy'
) + $hashLines
$pre += "# COMPILED"
...
$pre += (ShaLine (Join-Path $bag "c3_800k_facts.svh"))
$pre += (ShaLine (Join-Path $bag "c3_800k_2hop.svh"))
$pre += (ShaLine (Join-Path $bag "run_xsim.ps1"))
[System.IO.File]::WriteAllLines((Join-Path $bag "SHA256.txt"), $pre)
Write-Host "C3K2HCR_SHA_FROZEN"

if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null
Set-Location $work
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C3K2HCR_XVLOG_NOT_FOUND" }
& "$bin\xvlog.bat" --sv -i $bag -i $incq -i $incc -i $inci $files
if ($LASTEXITCODE -ne 0) { throw "C3K2HCR_XVLOG_FAIL" }
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
& "$bin\xelab.bat" tb_astra_c3_held_out_800k_2hop_conflict_reload -s c3k2hcr -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) {
  if (Test-Path (Join-Path $work "xelab.log")) {
    Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
  }
  throw "C3K2HCR_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c3k2hcr -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "^ASTRA_C3_HELD_OUT_800K_2HOP_CONFLICT_RELOAD_XSIM_PASS$" -Quiet)
  $need = @(
...
if (($xsimExit -ne 0) -or (-not $hasPass) -or ($missing.Count -gt 0)) {
  ...
  throw "C3K2HCR_XSIM_FAIL_OR_MARKER_MISSING pass=$hasPass missing=$($missing -join ',')"
}
Write-Host "ASTRA_C3_HELD_OUT_800K_2HOP_CONFLICT_RELOAD_RUN_OK PASS_THIS_GATE_ONLY PROGRAM=NO C3_MASTER=OPEN"
