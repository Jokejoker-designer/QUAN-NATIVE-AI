`timescale 1ns / 1ps
// ASTRA-C3-HELD-OUT-800K-2HOP-CONFLICT-RELOAD-01. PROGRAM=NO. Bag-local TB.
// One unknown: retention after C2 persist_clr+reload on live CONFLICT C3 copy.
...
module tb_astra_c3_held_out_800k_2hop_conflict_reload;
  ...
  mx_t mx;
  int c2g, step, drop_pp, max_drop, sum_pre, sum_post, acc_pre, acc_post, host_bad, dj_fail, n_conf;
  ...
  initial begin
    ...
    host_bad=0; dj_fail=0; sum_pre=0; sum_post=0; max_drop=0; n_conf=0;
    ...
    $display("C3_800K_2HOP_CONFLICT_RELOAD N=800000 FACT_BASE=0E000000 CTX_K0_OVERLAY=1 TO_CYC=65535 TR={13,15,16} HO={19,20} PROGRAM=NO BOARD_PASS=REJECT");
    ...
    $display("C3_RELOAD_SUM pre=%0d/%0d post=%0d drop_pp=%0d max_seed_drop=%0d host_bad=%0d saw_base=%0d n_false=%0d n_conf=%0d",
      sum_pre, 8*C3_N_SEED, sum_post, drop_pp, max_drop, host_bad, saw_base, c2_nfalse, n_conf);
    ...
    if (fail==0) $display("ASTRA_C3_HELD_OUT_800K_2HOP_CONFLICT_RELOAD_XSIM_PASS");
    else $display("ASTRA_C3_HELD_OUT_800K_2HOP_CONFLICT_RELOAD_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C3_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO quality=800K_CARTESIAN_2HOP_LIVE_CONFLICT_C2_W0_NOT_MIG");
    $finish;
  end
endmodule
