...
if ($liveA09 -ne $expectA09) { throw "A11A09R2_FROZEN_A09_HASH_CHANGED $liveA09" }
if ($liveR2  -ne $expectR2)  { throw "A11A09R2_DUT_HASH_CHANGED $liveR2" }
if ($liveSgd -ne $expectSgd) { throw "A11A09R2_SGD_HASH_CHANGED $liveSgd" }
$compiled = @(
  ...
  (Join-Path $root "rtl\native_graph\learn\a7ng_shared_rank_sgd_q8_sym_f2r2.sv"),
  (Join-Path $root "rtl\native_graph\integrate\a7ng_astra_09_r2_cand_ovf.sv"),
  (Join-Path $bag "a7ng_astra_11_a09r2_impl_wrap.sv"),
...
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "A11A09R2_FILE_MISSING $p" }
  $h = (Get-FileHash -Algorithm SHA256 -LiteralPath $p).Hash.ToLowerInvariant()
  return "$h  $(Rel $p)"
}
foreach ($f in $compiled) {
  $bn = [IO.Path]::GetFileName($f)
  if ($bn -eq "a7ng_astra09_pipe.sv" -or $bn -eq "uart_rx.sv" -or $bn -eq "uart_tx.sv" -or $bn -eq "a7ng_astra_09_integ_path.sv" -or $bn -match "pipe_r1|plant128|axi_bram128") {
...
$lines += "# KEEP_NOT_THIS_WRAPPER (hashed; not copy-paste; astra09_pipe / axi_bram128 / UART / frozen A09 not compiled)"
...
$lines += (ShaLine (Join-Path $root "results\A7-NATIVE-GRAPH\ASTRA-11-A09-IMPL-ROUTE-01\a7ng_astra_11_a09_impl_wrap.sv"))
...
Write-Host "A11A09R2_SHA_FROZEN $shaPath"
if (-not (Test-Path -LiteralPath $vivado)) { throw "A11A09R2_VIVADO_NOT_FOUND $vivado" }
...
if (Test-Path -LiteralPath $log) {
  $done = [bool](Select-String -Path $log -Pattern "ASTRA_11_A09R2_IMPL_ROUTE_DONE" -Quiet)
}
if (($exit -ne 0) -or (-not $done)) {
  $r0 = Join-Path $bag "vivado_fail_r0.log"
  if (Test-Path -LiteralPath $log) { Copy-Item $log $r0 -Force }
  throw "A11A09R2_IMPL_FAIL exit=$exit done=$done"
}
...
Write-Host "ASTRA_11_A09R2_IMPL_ROUTE_RUN_OK"
