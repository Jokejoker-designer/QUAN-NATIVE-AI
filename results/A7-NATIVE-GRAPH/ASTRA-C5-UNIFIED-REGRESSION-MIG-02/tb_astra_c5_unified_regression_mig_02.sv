`timescale 1ps / 100fs
// ASTRA-C5-UNIFIED-REGRESSION-MIG-02. PROGRAM=NO. Bag-local TB (test-only plant).
...
module tb_astra_c5_unified_regression_mig_02;
  ...
  initial begin
    ...
    $display("C5_UNIFIED_REGRESSION_MIG_02 PROGRAM=NO BOARD_PASS=REJECT MIG=YES TO_CYC_TB_DEFPARAM=65535 CONFLICT=YES");
    ...
    if (fail == 0) $display("ASTRA_C5_UNIFIED_REGRESSION_MIG_02_XSIM_PASS");
    else $display("ASTRA_C5_UNIFIED_REGRESSION_MIG_02_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO quality=MIG_PHY_UNIFIED_UART_PLUS_POLARITY_CONFLICT UART_SIM_BAUD");
    $finish;
  end

  initial begin
    #25s;
    $display("FIRST_DIVERGENCE MIG_TIMEOUT calib_or_unified_regression_02 hung");
    $display("RESULT=FAIL");
    $display("ASTRA_C5_UNIFIED_REGRESSION_MIG_02_XSIM_PASS ABSENT");
    $finish;
  end
endmodule
