...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5URM2_FILE_MISSING $p" }
...
if ($hashFail) { throw "C5URM2_HASH_GATE_FAIL do_not_edit_keep_rtl_or_mig_prj" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_unified_regression_mig_02.sv"
$dut = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_prod_top.sv"
$wrap = Join-Path $root "rtl\ddr\mig_native_wrap.sv"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5URM2_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5URM2_GOLD_HASH_DRIFT GOLDEN.json" }
...
foreach ($f in $svFiles) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C5URM2_LEFTOVER_A09" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C5URM2_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C5URM2_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") { throw "C5URM2_CTX_8255a798_AS_DUT" }
  if ($leaf -eq "a7ng_learned_prior_store.sv") { throw "C5URM2_PRIOR_STORE" }
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5URM2_TINYGPT_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C5URM2_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5URM2_SYNTH_MIG_AS_DUT" }
  if ($leaf -eq "a7ng_lm_graph_arb.sv") { throw "C5URM2_LM_GRAPH_ARB_AS_DUT" }
}
...
$pre = @(
  ...
  "# LAW=astra-c5-unified-regression-mig-02 MIG_XSIM Digilent AXI mig_sim + ddr3_model unified UART + polarity CONFLICT",
...
function Write-DivLog([string]$code, [string]$detail) {
  [System.IO.File]::WriteAllLines((Join-Path $bag "xsim.log"), @(
    ...
    "RESULT=FAIL",
    "ASTRA_C5_UNIFIED_REGRESSION_MIG_02_XSIM_PASS ABSENT",
    "PROGRAM=NO BOARD_PASS=NOT_CLAIMED C5_MASTER=OPEN"
  ))
}

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C5URM2_XVLOG_NOT_FOUND" }
if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null

$prj = Join-Path $work "c5_unified_regression_mig_02.prj"
$prjLines = @()
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C5URM2_XVLOG_FAIL"
}
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
$xvlogTxt = [System.IO.File]::ReadAllText((Join-Path $bag "xvlog.log"))
if ($xvlogTxt -match "a7ng_astra_09_integ_path") { throw "C5URM2_LEFTOVER_A09_COMPILED" }
if ($xvlogTxt -match "a7ng_query_axi_sparse_stream_intersect") { throw "C5URM2_STREAM02_COMPILED" }
if ($xvlogTxt -match "a7ng_query_axi_sparse_intersect_context") { throw "C5URM2_CTX_COMPILED" }
if ($xvlogTxt -match "tiny_gpt803k_core") { throw "C5URM2_TINYGPT_COMPILED" }
if ($xvlogTxt -match "a7ng_evidence_compose") { throw "C5URM2_COMPOSE_COMPILED" }
if ($xvlogTxt -match "mig_7series_0_mig\.v" -and $xvlogTxt -notmatch "mig_7series_0_mig_sim") { throw "C5URM2_SYNTH_MIG_COMPILED" }
if ($xvlogTxt -notmatch "a7ng_astra_c5_prod_top") { throw "C5URM2_DUT_NOT_ANALYZED" }
if ($xvlogTxt -notmatch "mig_7series_0_mig_sim") { throw "C5URM2_MIG_SIM_NOT_ANALYZED" }
if ($xvlogTxt -notmatch "tb_astra_c5_unified_regression_mig_02") { throw "C5URM2_TB_NOT_ANALYZED" }

& "$bin\xelab.bat" -mt off -O0 tb_astra_c5_unified_regression_mig_02 glbl -s c5urm2 -L unisims_ver -L unimacro_ver -L secureip -timescale 1ps/1ps
if ($LASTEXITCODE -ne 0) {
...
& "$bin\xsim.bat" c5urm2 -R -log $xsimLog 1> $xsimOut 2>&1
...
if (-not (Test-Path -LiteralPath $xsimLog)) { throw "C5URM2_NO_XSIM_LOG" }
$hasPass = [bool](Select-String -Path $xsimLog -Pattern "^ASTRA_C5_UNIFIED_REGRESSION_MIG_02_XSIM_PASS$" -Quiet)
...
if ($hasDiv -or (-not $hasPass) -or ($xsimExit -ne 0) -or ($missing.Count -gt 0)) {
  $rFail = Join-Path $bag "xsim_fail_r0.log"
  if (-not (Test-Path -LiteralPath $rFail)) { Copy-Item $xsimLog $rFail -Force }
  throw "C5URM2_XSIM_CONTROL_FAIL pass=$hasPass div=$hasDiv exit=$xsimExit missing=$($missing -join ',')"
}
Write-Host "ASTRA_C5_UNIFIED_REGRESSION_MIG_02_RUN_OK PASS_THIS_GATE_ONLY PROGRAM=NO MIG_XSIM=1 C5_MASTER=OPEN"
