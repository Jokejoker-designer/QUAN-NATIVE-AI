// a7ng_astra_11_a09r7_btn_idelay_wrap.sv — ASTRA-11-A09R7-BTN-IDELAY-01.
// PROGRAM=NO. BIT=NO. Named BTN-IDELAY wrap: R7 query-rew FSM + UART IOB FFs +
// LED IOB FFs + IDELAYE2/IDELAYCTRL on BTN + BTN IOB FFs.
// Instantiates frozen a7ng_astra_09_r2_cand_ovf (no copy-paste graph).
// Prior a7ng_astra_11_a09r7_btn_io_wrap KEEP, not this top. Not PRODUCTION_TOP.
// Frozen a7ng_astra_09_integ_path is not instantiated. MMCM 100→50 + 100→200 + BUFG.
// UART pins uart_rxd_out=D10 uart_txd_in=A9. LEDs LD4-LD7 H5/J5/T9/T10.
// Buttons BTN0-BTN3 D9/C9/B9/B8 (cited constraints/arty_a7_100.xdc).
// IDELAYE2 IDELAY_VALUE=31 FIXED REFCLK=200 MHz frozen in PREREG before impl.
`timescale 1ns / 1ps
`include "a7ng_astra_11_a09r7_btn_idelay.svh"

module a7ng_astra_11_a09r7_btn_idelay_wrap (
  ...
  // PREREG freeze: IDELAY_VALUE=31, REFCLK=200 MHz, IDELAYCTRL required.
  localparam int unsigned BTN_IDELAY_VALUE = 31;

  logic clk, clkfb, clk50u, clk200u, clk200, locked, idelay_rdy;
  MMCME2_BASE #(
    .CLKIN1_PERIOD(10.0),
  ...
  BUFG u_bufg200 (.I(clk200u), .O(clk200));

  (* IODELAY_GROUP = "ASTRA_BTN_IDELAY" *)
  IDELAYCTRL u_btn_idelayctrl (
    .RDY(idelay_rdy),
    .REFCLK(clk200),
    .RST(~locked)
  );
  ...
  genvar gi;
  generate
    for (gi = 0; gi < 4; gi++) begin : g_btn_idelay
      (* IODELAY_GROUP = "ASTRA_BTN_IDELAY" *)
      IDELAYE2 #(
        .CINVCTRL_SEL("FALSE"),
        .DELAY_SRC("IDATAIN"),
  ...
  // BTN IOB pad FFs (PREREG). Sample IDELAYE2 DATAOUT, never raw btn.
  // INIT 0. No SR so Vivado can pack into IOB.
  // Pins BTN0-BTN3 = D9/C9/B9/B8 (cited constraints/arty_a7_100.xdc).
  (* IOB = "TRUE" *) logic [3:0] btn_q = 4'b0000;
  always_ff @(posedge clk) begin
    btn_q <= btn_dly;
  end
  (* ASYNC_REG = "TRUE" *) logic btn0_sync;
  (* ASYNC_REG = "TRUE" *) logic [1:0] idelay_rdy_sync = 2'b00;
  logic [7:0] por_cnt;
  logic       rst_n;
  always_ff @(posedge clk) begin
    btn0_sync <= btn_q[0];
    idelay_rdy_sync <= {idelay_rdy_sync[0], idelay_rdy};
    if (!locked || !idelay_rdy_sync[1] || btn0_sync)
      por_cnt <= 8'd0;
    else if (por_cnt != 8'hFF)
      por_cnt <= por_cnt + 8'd1;
  end
  assign rst_n = locked && idelay_rdy_sync[1] && (por_cnt == 8'hFF);
  ...
  (* keep_hierarchy = "yes" *)
  a7ng_astra_11_a09r7_btn_idelay_plant u_plant (
  ...
  // Use btn_q (IDELAYE2 then IOB-sampled), never raw btn, so hold path is pad→IDELAY→IOB FF.
  (* IOB = "TRUE" *) logic [3:0] led_q = 4'b0000;
  always_ff @(posedge clk) begin
    led_q[0] <= rst_n & hb[24];
    led_q[1] <= busy;
    led_q[2] <= result_v | r_ovf | pend_cmt | (|ntrunc);
    led_q[3] <= sw[0] | (|sw[3:1]) | btn_q[1] | (|btn_q[3:2]) | tbl | w_ovf;
  end
  assign led = led_q;
endmodule
