#!/usr/bin/env python3
"""Capture COM12 for E2R-TILE-DMA-BUSY-SRC-PROBE-00 — F1r dma busy source probe."""
import argparse
import re
import sys
import time
...
def main():
    ...
    tile_dma_busy_line = find_line("TILE_DMA_BUSY=")
    tile_dma_own_line = find_line("TILE_DMA_OWN=")

    last = seen[-1] if seen else "NONE"
    for candidate in (
        wdma_own_ui_line, wdma_busy_line, sdma_busy_line,
        tile_dma_own_line, tile_dma_busy_line, tile_req_line,
        tile_bst_line, tile_dst_line, phase_line,
    ):
        if candidate:
            last = candidate
            break
    ...
    probe_pass = (
        ...
        and sdma_busy_line is not None
        and wdma_busy_line is not None
        and wdma_own_ui_line is not None
    )
