...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C1INTERSECT_FILE_MISSING $p" }
...
if ($hashFail) {
  ...
  throw "C1INTERSECT_HASH_GATE_FAIL do_not_invent"
}
...
$tb = Join-Path $bag "tb_astra_c1_key_intersect.sv"
$hostPy = Join-Path $bag "host_astra_c1_key_intersect.py"
...
if (-not (Test-Path -LiteralPath $goldJson)) { throw "C1INTERSECT_GOLD_MISSING GOLDEN.json" }
if (-not (Test-Path -LiteralPath $goldSvh)) { throw "C1INTERSECT_GOLD_MISSING query_gold.svh" }
if (-not (Test-Path -LiteralPath $corpus)) { throw "C1INTERSECT_GOLD_MISSING corpus.json" }
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C1INTERSECT_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$liveSvh = Sha256 $goldSvh
$liveCorpus = Sha256 $corpus
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C1INTERSECT_GOLD_HASH_DRIFT GOLDEN.json" }
if ($preTxt -notmatch $liveSvh) { throw "C1INTERSECT_GOLD_HASH_DRIFT query_gold.svh" }
if ($preTxt -notmatch $liveCorpus) { throw "C1INTERSECT_GOLD_HASH_DRIFT corpus.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") {
    throw "C1INTERSECT_LEFTOVER_A09_ON_XVLOG_LIST"
  }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") {
    throw "C1INTERSECT_FROZEN_AXI_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_relbind.sv") {
    throw "C1INTERSECT_RELBIND_SPARSE_MUST_NOT_BE_DUT"
  }
}
...
$pre += "$liveGold  results/A7-NATIVE-GRAPH/ASTRA-C1-KEY-INTERSECT-01/GOLDEN.json"
$pre += "$liveSvh  results/A7-NATIVE-GRAPH/ASTRA-C1-KEY-INTERSECT-01/query_gold.svh"
$pre += "$liveCorpus  results/A7-NATIVE-GRAPH/ASTRA-C1-KEY-INTERSECT-01/corpus.json"
$pre += "# BAG"
...
Write-Host "C1INTERSECT_SHA_FROZEN_BEFORE_XVLOG"

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C1INTERSECT_XVLOG_NOT_FOUND" }
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C1_KEY_INTERSECT_XSIM_PASS" -Quiet)
...
if ($hasRedCap) {
  ...
  throw "C1INTERSECT_REDUCTION_X1000_EMITTED_AS_CAP_N"
}
if (($xsimExit -ne 0) -or (-not $hasPass) -or $hasDiv -or $hasLeak) {
  ...
  throw "C1INTERSECT_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C1_KEY_INTERSECT_RUN_OK"
