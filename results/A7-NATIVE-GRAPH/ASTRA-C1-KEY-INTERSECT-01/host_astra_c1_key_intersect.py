#!/usr/bin/env python3
"""ASTRA-C1-KEY-INTERSECT-01 independent host gold for qse-v2-intersect-01.

Frozen extract (qse-v2-role-00) instantiated unchanged. Relbind keys reused
(k2={rel_id,subj_cue[7:0]} k3={rel_id,obj_cue[7:0]}; k0/k1 pass through).
...
fake context keys; k2/k3-only rebind as exclusion.
PROGRAM=NO. BIT=NO.
...
def write_svh(path: Path, queries: list[dict], writes: dict[int, int], docs: list[dict]) -> None:
    ...
    lines = [
        "// generated by host_astra_c1_key_intersect.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
        "// Law qse-v2-intersect-01: emit k0∩k1 when both valid; relbind keys reused; no k0∪k1 union",
...
def main() -> int:
    ...
    if 255 not in qd["high_id_sentinel"]["emit"]:
        chk(
    ...
    # relevant is the label predicate, not set(emit). Intersect may equal gold.
    ...
    if fails:
        print("ASTRA_C1_KEY_INTERSECT_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        return 2
    ...
    print("ASTRA_C1_KEY_INTERSECT_HOST_PASS")
