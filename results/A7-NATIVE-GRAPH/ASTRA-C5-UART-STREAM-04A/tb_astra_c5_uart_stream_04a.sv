`timescale 1ns / 1ps
// ASTRA-C5-UART-STREAM-04A. PROGRAM=NO.
// Named stream top. Final CLK/BAUD defaults. Pin-level UART decode.
// Not BOARD. Does not close C5_MASTER / 04B.
...
module tb_astra_c5_uart_stream_04a;
  localparam int unsigned CLK_HZ = A7NG_C5P_CLK_HZ;
  localparam int unsigned BAUD   = A7NG_C5P_BAUD;
  ...
  endtask
  function automatic logic [127:0] dir_pack(input int count, input logic [27:0] heap);
    dir_pack = {48'd0, 16'd7, 16'd0, count[15:0], 4'd0, heap};
  endfunction
  ...
  a7ng_astra_c5_prod_top_stream #(.CLK_HZ(CLK_HZ), .BAUD(BAUD)) u_dut (
  ...
  task automatic plant_index(input int nfact, input int k0, input int k1, input int k2);
    begin
      mem_wr(dir_addr(0, k0), dir_pack(nfact, POST_HEAP));
      mem_wr(dir_addr(1, k1), dir_pack(nfact, POST_HEAP));
      mem_wr(dir_addr(2, k2), dir_pack(nfact, POST_HEAP));
    end
  endtask
  ...
  task automatic plant_train0;
    ...
    begin
      ...
      mem_wr(POST_HEAP + 28'd32, 32'd20);
      mem_wr(dir_addr(0, 16'h0A02), dir_pack(1, POST_HEAP + 28'd32));
      mem_wr(dir_addr(1, 16'h0B02), dir_pack(1, POST_HEAP + 28'd32));
      wr_f(20, subj, 11, rel, C3_TR_CONF_W[0], 0, 1, 0);
    end
  endtask
  task automatic one_query_stream(input string q, output int ntok);
    integer t;
    begin
      fork
        pin_line();
      ...
      join
      ntok = npin;
      $display("MEAS_STR q=%s n_stream=%0d gnout=%0d npin=%0d last=%0d obj=%0d ovf=%0d nutx=%0d",
               q, nstr, gnout, npin, glast, c3obj, novf, nutx);
  ...
  initial begin
    ...
    $display("C5_UART_STREAM_04A PROGRAM=NO BOARD_PASS=REJECT DUT=prod_top_stream");
    $display("MEAS_BAUD CLK_HZ=%0d BAUD=%0d CPB=%0d SIM_OVERRIDE_8000_800=NO", CLK_HZ, BAUD, CPB);
    chk("DEFAULT_NOT_8000", CLK_HZ != 8000);
    chk("DEFAULT_115200", BAUD == 115200);
    guard = 0;
    ...
    begin : q1
      int ntok;
      one_query_stream("pump requires valve", ntok);
      chk("Q1_GE3", ntok >= 3);
      chk("Q1_NSTR", nstr == ntok[15:0]);
      chk("Q1_GNOUT", gnout == ntok[7:0]);
      chk("Q1_OVF", novf == 16'd0);
      if (ntok >= 3) $display("CLASS_stream_ge3 HIT n=%0d", ntok);
      else $display("CLASS_stream_ge3 MISS n=%0d", ntok);
      if (ntok > 1) $display("CLASS_not_last_only HIT n=%0d", ntok);
      else $display("CLASS_not_last_only MISS n=%0d", ntok);
      if ((nstr == ntok[15:0]) && (npin == ntok)) $display("CLASS_pin_decode HIT");
      else $display("CLASS_pin_decode MISS nstr=%0d npin=%0d", nstr, npin);
      if (gnout == ntok[7:0]) $display("CLASS_n_eq_gen HIT");
      else $display("CLASS_n_eq_gen MISS gnout=%0d ntok=%0d", gnout, ntok);
      if (novf == 16'd0) $display("CLASS_fifo_ovf0 HIT");
      else $display("CLASS_fifo_ovf0 MISS ovf=%0d", novf);
    end
    ...
    begin : q2
      ...
      one_query_stream("pump requires valve", ntok);
      chk("Q2_GE3", ntok >= 3);
      if (ntok >= 3) $display("CLASS_consec_stream HIT n=%0d", ntok);
      else $display("CLASS_consec_stream MISS n=%0d", ntok);
    end

    if (fail == 0) $display("ASTRA_C5_UART_STREAM_04A_XSIM_PASS");
    else $display("ASTRA_C5_UART_STREAM_04A_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO 04B=OPEN");
    $finish;
  end
endmodule
