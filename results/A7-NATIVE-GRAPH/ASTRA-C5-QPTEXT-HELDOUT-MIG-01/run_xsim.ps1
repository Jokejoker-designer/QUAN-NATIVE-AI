...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5QTHMIG_FILE_MISSING $p" }
...
if ($hashFail) { throw "C5QTHMIG_HASH_GATE_FAIL do_not_edit_keep_rtl_or_c4q_or_mig_prj" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_qptext_heldout_mig.sv"
...
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5QTHMIG_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5QTHMIG_GOLD_HASH_DRIFT GOLDEN.json" }

$dutTxt = [System.IO.File]::ReadAllText($dut)
if ($dutTxt -match "evid_obj") { throw "C5QTHMIG_DUT_HAS_EVID_OBJ" }
if ($dutTxt -match "a7ng_astra_c4_lm06_grounded_gen") { throw "C5QTHMIG_DUT_HAS_GROUNDED_GEN" }
if ($dutTxt -match "a7ng_c4p_ch") { throw "C5QTHMIG_DUT_HAS_C3PROOF_DICT" }
if ($dutTxt -notmatch "a7ng_c4q_ch") { throw "C5QTHMIG_DUT_MISSING_FPGA_DICT" }
if ($dutTxt -notmatch "a7ng_astra_c3_held_out") { throw "C5QTHMIG_DUT_MISSING_C3" }
if ($dutTxt -notmatch "a7ng_astra_c4_lm06_byte256_qptext_gen") { throw "C5QTHMIG_DUT_MISSING_QPTEXT_GEN" }
...
foreach ($f in $svFiles) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C5QTHMIG_LEFTOVER_A09" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C5QTHMIG_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C5QTHMIG_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") { throw "C5QTHMIG_CTX_8255a798_AS_DUT" }
  if ($leaf -eq "a7ng_learned_prior_store.sv") { throw "C5QTHMIG_PRIOR_STORE" }
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5QTHMIG_TINYGPT_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C5QTHMIG_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5QTHMIG_SYNTH_MIG_AS_DUT" }
  if ($leaf -eq "a7ng_lm_graph_arb.sv") { throw "C5QTHMIG_LM_GRAPH_ARB_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5QTHMIG_LIVE_PROD_TOP_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_grounded_gen.sv") { throw "C5QTHMIG_GROUNDED_GEN_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_byte256_ctxcopy.sv") { throw "C5QTHMIG_CTXCOPY_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_byte256_dict.sv") { throw "C5QTHMIG_DICT_LUT_DUT" }
}
...
$pre = @(
  ...
  "# LAW=astra-c5-qptext-heldout-mig-01 MIG_XSIM Digilent AXI mig_sim + ddr3_model c4q DUT held-out PRE/POST",
...
Write-Host "C5QTHMIG_SHA_FROZEN"
...
function Write-DivLog([string]$code, [string]$detail) {
  [System.IO.File]::WriteAllLines((Join-Path $bag "xsim.log"), @(
    ...
    "RESULT=FAIL",
    "ASTRA_C5_QPTEXT_HELDOUT_MIG_XSIM_PASS ABSENT",
    "PROGRAM=NO BOARD_PASS=NOT_CLAIMED C5_MASTER=OPEN"
  ))
}

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C5QTHMIG_XVLOG_NOT_FOUND" }
if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null

$prj = Join-Path $work "c5_qptext_heldout_mig.prj"
$prjLines = @()
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C5QTHMIG_XVLOG_FAIL"
}
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
$xvlogTxt = [System.IO.File]::ReadAllText((Join-Path $bag "xvlog.log"))
if ($xvlogTxt -match "a7ng_astra_09_integ_path") { throw "C5QTHMIG_LEFTOVER_A09_COMPILED" }
if ($xvlogTxt -match "a7ng_query_axi_sparse_stream_intersect") { throw "C5QTHMIG_STREAM02_COMPILED" }
if ($xvlogTxt -match "a7ng_query_axi_sparse_intersect_context") { throw "C5QTHMIG_CTX_COMPILED" }
if ($xvlogTxt -match "tiny_gpt803k_core") { throw "C5QTHMIG_TINYGPT_COMPILED" }
if ($xvlogTxt -match "a7ng_evidence_compose") { throw "C5QTHMIG_COMPOSE_COMPILED" }
if ($xvlogTxt -match "a7ng_astra_c4_lm06_grounded_gen") { throw "C5QTHMIG_GROUNDED_GEN_COMPILED" }
if ($xvlogTxt -match "a7ng_astra_c4_lm06_byte256_ctxcopy") { throw "C5QTHMIG_CTXCOPY_COMPILED" }
if ($xvlogTxt -match "mig_7series_0_mig\.v" -and $xvlogTxt -notmatch "mig_7series_0_mig_sim") { throw "C5QTHMIG_SYNTH_MIG_COMPILED" }
if ($xvlogTxt -notmatch "a7ng_astra_c5_prod_top_c4q") { throw "C5QTHMIG_DUT_NOT_ANALYZED" }
if ($xvlogTxt -match "a7ng_astra_c5_prod_top\.sv") { throw "C5QTHMIG_LIVE_PROD_TOP_COMPILED" }
if ($xvlogTxt -notmatch "mig_7series_0_mig_sim") { throw "C5QTHMIG_MIG_SIM_NOT_ANALYZED" }
if ($xvlogTxt -notmatch "tb_astra_c5_qptext_heldout_mig") { throw "C5QTHMIG_TB_NOT_ANALYZED" }
if ($xvlogTxt -notmatch "a7ng_astra_c4_lm06_byte256_qptext_gen") { throw "C5QTHMIG_QPTEXT_GEN_NOT_ANALYZED" }

& "$bin\xelab.bat" -mt off -O0 tb_astra_c5_qptext_heldout_mig glbl -s c5qthmig -L unisims_ver -L unimacro_ver -L secureip -timescale 1ps/1ps
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C5QTHMIG_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c5qthmig -R -log $xsimLog
...
if (-not (Test-Path -LiteralPath $xsimLog)) { throw "C5QTHMIG_NO_XSIM_LOG" }
$hasPass = [bool](Select-String -Path $xsimLog -Pattern "^ASTRA_C5_QPTEXT_HELDOUT_MIG_XSIM_PASS$" -Quiet)
...
$need = @(
  ...
  "CLASS_heldout_pre HIT",
  "CLASS_reward_update HIT",
  "CLASS_heldout_post HIT",
  "CLASS_heldout_changed HIT",
...
if ($hasDiv -or (-not $hasPass) -or ($xsimExit -ne 0) -or ($missing.Count -gt 0)) {
  ...
  throw "C5QTHMIG_XSIM_CONTROL_FAIL pass=$hasPass div=$hasDiv exit=$xsimExit missing=$($missing -join ',')"
}
Write-Host "ASTRA_C5_QPTEXT_HELDOUT_MIG_RUN_OK PASS_THIS_GATE_ONLY PROGRAM=NO MIG_XSIM=1 C5_MASTER=OPEN"
