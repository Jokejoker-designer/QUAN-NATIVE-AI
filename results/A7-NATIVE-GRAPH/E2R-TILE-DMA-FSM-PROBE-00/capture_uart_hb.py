#!/usr/bin/env python3
"""Capture COM12 for E2R-TILE-DMA-FSM-PROBE-00 — F1p TILE_BST/TILE_DST probe."""
import argparse
import re
import sys
import time
...
def main():
    ...
    while time.time() - t0 < args.seconds:
        ...
        if "CORE_BUSY" in lines and (time.time() - t0) > 90.0:
            if any(x.startswith("TILE_BST=") for x in lines) or any(
                x.startswith("TILE_DST=") for x in lines
            ):
