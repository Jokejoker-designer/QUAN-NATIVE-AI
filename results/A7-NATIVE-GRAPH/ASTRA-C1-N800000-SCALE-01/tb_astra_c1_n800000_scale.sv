// tb_astra_c1_n800000_scale.sv — C1 N800000 SCALE. PROGRAM=NO.
...
module tb_astra_c1_n800000_scale;
  import a7ng_pkg::*;
  ...
  function automatic string cname(input int q);
    if (q == G_FILL_Q)
      ...
      cname = "wrong_relation";
    else if (q == G_WCTX_Q)
      cname = "wrong_context";
    else if (q == G_DIST_Q)
      cname = "distractor";
    else if (q == G_HOCC_Q)
      cname = "high_occupancy";
    else if (q == G_LATE_Q)
      cname = "late_gold";
    else if (q == G_HIGH_Q)
  ...
  initial begin
    ...
    if (G_N != 800000)
      diverge("N_DROP", "G_N!=800000 do_not_silently_drop_N");
    if (G_PROC_MEM != 1)
    ...
    if (G_NQ != 11)
      diverge("NQ", "G_NQ!=11 Master class set");
    if (G_FILL_Q != 0)
    ...
    if (G_LATE_NID != 20'd799998)
      diverge("LATE_NID", "G_LATE_NID!=799998");
    ...
    for (qi = 0; qi < G_NQ; qi = qi + 1) begin
      ...
      if (qi == G_DIST_Q) begin
        for (i = 0; i < n_got; i = i + 1)
          if (got[i] === 20'd120 || got[i] === 20'd121 || got[i] === 20'd122) begin
            $display("FAIL DISTRACTOR_LEAK fill_nid=%0d", got[i]);
            fail = fail + 1;
          end
        $display("DISTRACTOR_LEAK_N=0 emit_n=%0d", n_got);
      end
      if (qi == G_WCTX_Q) begin
        if ((n_got != 1) || (got[0] !== 20'd121)) begin
          $display("FAIL WRONG_CONTEXT_NOT_SELECTIVE emit_n=%0d", n_got);
          fail = fail + 1;
        end else
          $display("WRONG_CONTEXT_SELECTIVE nid=121");
      end
      if (qi == G_REV_Q) begin
        if (tp < 1) begin
          $display("FAIL ROLE_REVERSAL_MISS");
          fail = fail + 1;
        end else
          $display("ROLE_REVERSAL_HIT tp=%0d emit_n=%0d", tp, n_got);
      end
      if (qi == G_LATE_Q) begin
        late_tp = tp;
        late_hit = 0;
        for (i = 0; i < n_got; i = i + 1)
          if (got[i] === G_LATE_NID) late_hit = 1;
    ...
    $display("N800000_SUMMARY fill_tp=%0d nl_hit=%0d late_hit=%0d high_hit=%0d emit_has_131=%0d incomp_retrieve=%0d fail=%0d G_N=%0d G_NQ=%0d",
      fill_tp, nl_hit, late_hit, high_hit, emit_has_131, incomp_retrieve, fail, G_N, G_NQ);
    if ((fail == 0) && unrelated_empty && (fill_tp > 0) && fill_ids_ok && (keys_match == 0) && keys_match_syn && nl_hit && (nl_tp >= 3) && (emit_has_131 == 0) && late_hit && high_hit && (incomp_retrieve == 0) && (G_N == 800000) && (G_NQ == 11)) begin
