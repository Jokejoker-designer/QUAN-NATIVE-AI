#!/usr/bin/env python3
"""ASTRA-C1-N800000-SCALE-01 independent host gold.
...
NEW_ENT = NEW_ENT_16K + NEW_ENT_EXTRA
REL_EXTRA = [
    ("recirculates", 2, 9),
    ("equalizes", 2, 10),
    ("throttles", 2, 11),
...
def write_svh(path: Path, queries: list[dict]) -> None:
    ...
    lines = [
        "// generated by host_astra_c1_n800000_scale.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
        ...
        f"localparam int unsigned G_PARA_Q = {next(i for i, q in enumerate(queries) if q['name'] == 'paraphrase')};",
        f"localparam int unsigned G_NL_Q = {nl_i};",
        f"localparam int unsigned G_REV_Q = {next(i for i, q in enumerate(queries) if q['name'] == 'role_reversal')};",
        f"localparam int unsigned G_WREL_Q = {next(i for i, q in enumerate(queries) if q['name'] == 'wrong_relation')};",
        f"localparam int unsigned G_WCTX_Q = {next(i for i, q in enumerate(queries) if q['name'] == 'wrong_context')};",
        f"localparam int unsigned G_DIST_Q = {next(i for i, q in enumerate(queries) if q['name'] == 'distractor')};",
        f"localparam int unsigned G_HOCC_Q = {next(i for i, q in enumerate(queries) if q['name'] == 'high_occupancy')};",
...
def main() -> int:
    ...
    chk(N == 800000, "N dropped from registered 800000")
    ...
    chk(nids_of_sro(SEN_S, SEN_R, SEN_O) == [799999], "sentinel nid")
    ...
    chk(sen_and == [799999], f"sent AND {sen_and}")
    ...
    chk(799998 in late_a, "late nid not on k0 list")
    chk(late_a.index(799998) >= 16, f"late k0_idx={late_a.index(799998)} < 16")
    rev_nids = nids_of_sro(14, 4, 13)
    wrel_nids = nids_of_sro(13, 5, 14)
    dist_nids = nids_of_sro(13, 4, 15)
    hocc_nids = nids_of_sro(17, 7, 18)
    chk(rev_nids and set(rev_nids).isdisjoint({120, 121, 122}), f"rev {rev_nids}")
    chk(wrel_nids and set(wrel_nids).isdisjoint({120, 121, 122}), f"wrel {wrel_nids}")
    chk(dist_nids and set(dist_nids).isdisjoint({120, 121, 122}), f"dist {dist_nids}")
    chk(hocc_nids, f"hocc {hocc_nids}")
    queries_spec = [
        ("fill_template", triple_text(DIR_S, DIR_R, DIR_O), [120, 121, 122]),
        ("paraphrase", "the boiler feeds the header", [120, 121, 122]),
        ("nl_synonym", NL_TEXT, [120, 121, 122]),
        ("role_reversal", "header feeds boiler", rev_nids),
        ("wrong_relation", "boiler isolates header", wrel_nids),
        ("wrong_context", "boiler feeds header glycol", [121]),
        ("distractor", "boiler feeds coil", dist_nids),
        ("high_occupancy", "damper modulates grille", hocc_nids),
        ("late_gold", late_text, [799998]),
    ...
    chk(qd["paraphrase"]["emit"] == [120, 121, 122], f"para emit {qd['paraphrase']['emit']}")
    chk(qd["paraphrase"]["rel_id"] == REL_FEEDS, "para rel")
    chk(120 not in qd["role_reversal"]["emit"], "rev leaked fill")
    chk(qd["role_reversal"]["tp"] == 1, "rev tp")
    chk(120 not in qd["wrong_relation"]["emit"], "wrel leaked fill")
    chk(qd["wrong_context"]["emit"] == [121], f"wctx emit {qd['wrong_context']['emit']}")
    chk(qd["wrong_context"]["k0"] == 3380 and qd["wrong_context"]["k1"] == 3636, "wctx keys")
    chk(set(qd["distractor"]["emit"]).isdisjoint({120, 121, 122}), "dist leak fill")
    chk(max(qd["high_occupancy"]["occupancies"] or [0]) >= 16, "hocc not high")
    chk(qd["high_occupancy"]["search_incomplete"] == 0, "hocc SEARCH_INCOMPLETE")
    chk(qd["high_id_sentinel"]["emit"] == [799999], f"high emit {qd['high_id_sentinel']['emit']}")
    ...
    if fails:
        print("ASTRA_C1_N800000_SCALE_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        return 2
    ...
    corpus = {
        "gate": GATE,
        "n": N,
        "generator": "cartesian_n800000_v1",
        "procedural": True,
        "not_16k_clone": True,
        "n_subjects": N_ENT,
        "n_rels": N_REL,
    ...
    print("ASTRA_C1_N800000_SCALE_HOST_OK")
