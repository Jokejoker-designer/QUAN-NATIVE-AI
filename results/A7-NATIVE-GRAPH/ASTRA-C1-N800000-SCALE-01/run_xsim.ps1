...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C1800KS_FILE_MISSING $p" }
...
if ($hashFail) {
  ...
  throw "C1800KS_HASH_GATE_FAIL do_not_invent do_not_edit_keep_rtl"
}
...
$tb = Join-Path $bag "tb_astra_c1_n800000_scale.sv"
$hostPy = Join-Path $bag "host_astra_c1_n800000_scale.py"
...
if (-not (Test-Path -LiteralPath $goldJson)) { throw "C1800KS_GOLD_MISSING GOLDEN.json" }
if (-not (Test-Path -LiteralPath $goldSvh)) { throw "C1800KS_GOLD_MISSING query_gold.svh" }
if (-not (Test-Path -LiteralPath $corpus)) { throw "C1800KS_GOLD_MISSING corpus.json" }
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C1800KS_GOLD_NOT_HASHED_BEFORE_XVLOG" }
if (-not (Test-Path -LiteralPath $lexNamed)) { throw "C1800KS_LEX_MISSING qse_role_lexicon_semantic_800k.svh" }
if (-not (Test-Path -LiteralPath $lexShim)) { throw "C1800KS_LEX_SHIM_MISSING qse_role_lexicon.svh" }
if (-not (Test-Path -LiteralPath $synSvh)) { throw "C1800KS_SYN_SVH_MISSING bag qse_relctx_synonym_01.svh" }
if (-not (Test-Path -LiteralPath $procMem)) { throw "C1800KS_PROC_MEM_MISSING" }
if (-not (Test-Path -LiteralPath $genSvh)) { throw "C1800KS_GEN_SVH_MISSING" }
...
if ($preTxt -notmatch $liveGold) { throw "C1800KS_GOLD_HASH_DRIFT GOLDEN.json" }
if ($preTxt -notmatch $liveSvh) { throw "C1800KS_GOLD_HASH_DRIFT query_gold.svh" }
if ($preTxt -notmatch $liveCorpus) { throw "C1800KS_GOLD_HASH_DRIFT corpus.json" }
if ($preTxt -notmatch $liveLex) { throw "C1800KS_GOLD_HASH_DRIFT qse_role_lexicon_semantic_800k.svh" }
if ($preTxt -notmatch $liveSyn) { throw "C1800KS_GOLD_HASH_DRIFT qse_relctx_synonym_01.svh" }
if ($liveSyn -ne $liveSynRtl) {
  throw "C1800KS_SYN_SVH_BAG_RTL_MISMATCH bag=$liveSyn rtl=$liveSynRtl"
}
$goldTxt = [System.IO.File]::ReadAllText($goldJson)
if ($goldTxt -notmatch '"n": 800000') { throw "C1800KS_GOLD_N_NOT_800000" }
$svhTxt = [System.IO.File]::ReadAllText($goldSvh)
if ($svhTxt -notmatch "localparam int unsigned G_N = 800000;") { throw "C1800KS_SVH_N_NOT_800000" }
if ($svhTxt -match "localparam int unsigned G_N = 16384;") { throw "C1800KS_16K_CLONE_LABELED_800K" }
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") {
    throw "C1800KS_LEFTOVER_A09_ON_XVLOG_LIST"
  }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") {
    throw "C1800KS_FROZEN_AXI_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") {
    throw "C1800KS_STREAM02_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_page_skip.sv") {
    throw "C1800KS_PAGESKIP_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") {
    throw "C1800KS_CTX_DUT_8255a798_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_axi_mem_model.sv") {
    throw "C1800KS_C0_DENSE_MEM_MUST_NOT_BE_DUT"
  }
}
...
$pre += "$liveGold  results/A7-NATIVE-GRAPH/ASTRA-C1-N800000-SCALE-01/GOLDEN.json"
$pre += "$liveSvh  results/A7-NATIVE-GRAPH/ASTRA-C1-N800000-SCALE-01/query_gold.svh"
$pre += "$liveCorpus  results/A7-NATIVE-GRAPH/ASTRA-C1-N800000-SCALE-01/corpus.json"
$pre += "$liveLex  results/A7-NATIVE-GRAPH/ASTRA-C1-N800000-SCALE-01/qse_role_lexicon_semantic_800k.svh"
$pre += "$liveSyn  results/A7-NATIVE-GRAPH/ASTRA-C1-N800000-SCALE-01/qse_relctx_synonym_01.svh"
$pre += "# BAG"
...
Write-Host "C1800KS_SHA_FROZEN_BEFORE_XVLOG"
...
function Write-DivLog([string]$code, [string]$detail) {
  ...
  $lines = @(
    ...
    "RESULT=FAIL",
    "ASTRA_C1_N800000_SCALE_XSIM_PASS ABSENT",
    "PROGRAM=NO BOARD_PASS=NOT_CLAIMED CAND_CAP_FINAL=NOT_FROZEN"
  )
  [System.IO.File]::WriteAllLines($xsimLog, $lines)
}

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C1800KS_XVLOG_NOT_FOUND" }
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C1800KS_XELAB_FAIL FIRST_DIVERGENCE XELAB"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" astra_c1_n800000_scale -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C1_N800000_SCALE_XSIM_PASS" -Quiet)
  ...
  $hasLate = [bool](Select-String -Path $xsimLog -Pattern "LATE_GOLD_HIT nid=799998" -Quiet)
  $hasWctx = [bool](Select-String -Path $xsimLog -Pattern "WRONG_CONTEXT_SELECTIVE nid=121" -Quiet)
  $hasRev = [bool](Select-String -Path $xsimLog -Pattern "ROLE_REVERSAL_HIT" -Quiet)
  $hasDist = [bool](Select-String -Path $xsimLog -Pattern "DISTRACTOR_LEAK_N=0" -Quiet)
  $hasFail = [bool](Select-String -Path $xsimLog -Pattern "^FAIL " -Quiet)
  $hasN800 = [bool](Select-String -Path $xsimLog -Pattern "C1_N800000_SCALE_N=800000" -Quiet)
}
if ($hasRedCap) {
  ...
  throw "C1800KS_REDUCTION_X1000_EMITTED_AS_CAP_N"
}
if ($hasNDrop) {
  $r0 = Join-Path $bag "xsim_fail_r0.log"
  if ((Test-Path -LiteralPath $xsimLog) -and (-not (Test-Path -LiteralPath $r0))) {
  ...
  throw "C1800KS_N_DROP_SILENT_16K_FORBIDDEN"
}
if ($hasDiv -or $hasFillFail -or (-not $hasFillHit) -or $hasIncomp -or (-not $hasHigh) -or (-not $hasLate) -or (-not $hasN800) -or (-not $hasWctx) -or (-not $hasRev) -or (-not $hasDist)) {
  $r0 = Join-Path $bag "xsim_fail_r0.log"
  if ((Test-Path -LiteralPath $xsimLog) -and (-not (Test-Path -LiteralPath $r0))) {
    Copy-Item $xsimLog $r0 -Force
  }
  throw "C1800KS_XSIM_CONTROL_FAIL fillHit=$hasFillHit high=$hasHigh late=$hasLate n800=$hasN800 wctx=$hasWctx rev=$hasRev dist=$hasDist div=$hasDiv incomp=$hasIncomp"
}
if ($hasPass) {
  if ($hasFail) {
    $r0 = Join-Path $bag "xsim_fail_r0.log"
    ...
    throw "C1800KS_PASS_MARKER_WITH_FAIL"
  }
  Write-Host "ASTRA_C1_N800000_SCALE_RUN_OK PASS N=800000 FILL_TEMPLATE_HIT LATE_GOLD_HIT HIGH_ID_HIT"
} else {
  $r0 = Join-Path $bag "xsim_fail_r0.log"
  if ((Test-Path -LiteralPath $xsimLog) -and (-not (Test-Path -LiteralPath $r0))) {
    Copy-Item $xsimLog $r0 -Force
  }
  throw "C1800KS_XSIM_FAIL pass=$hasPass fillHit=$hasFillHit high=$hasHigh exit=$xsimExit"
}
