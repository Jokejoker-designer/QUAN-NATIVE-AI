...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C1262K_FILE_MISSING $p" }
...
if ($hashFail) {
  ...
  throw "C1262K_HASH_GATE_FAIL do_not_invent do_not_edit_keep_rtl"
}
...
$tb = Join-Path $bag "tb_astra_c1_n262144_scale.sv"
$hostPy = Join-Path $bag "host_astra_c1_n262144_scale.py"
...
if (-not (Test-Path -LiteralPath $goldJson)) { throw "C1262K_GOLD_MISSING GOLDEN.json" }
if (-not (Test-Path -LiteralPath $goldSvh)) { throw "C1262K_GOLD_MISSING query_gold.svh" }
if (-not (Test-Path -LiteralPath $corpus)) { throw "C1262K_GOLD_MISSING corpus.json" }
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C1262K_GOLD_NOT_HASHED_BEFORE_XVLOG" }
if (-not (Test-Path -LiteralPath $lexNamed)) { throw "C1262K_LEX_MISSING qse_role_lexicon_semantic_800k.svh" }
if (-not (Test-Path -LiteralPath $lexShim)) { throw "C1262K_LEX_SHIM_MISSING qse_role_lexicon.svh" }
if (-not (Test-Path -LiteralPath $synSvh)) { throw "C1262K_SYN_SVH_MISSING bag qse_relctx_synonym_01.svh" }
if (-not (Test-Path -LiteralPath $procMem)) { throw "C1262K_PROC_MEM_MISSING" }
if (-not (Test-Path -LiteralPath $genSvh)) { throw "C1262K_GEN_SVH_MISSING" }
...
if ($preTxt -notmatch $liveGold) { throw "C1262K_GOLD_HASH_DRIFT GOLDEN.json" }
if ($preTxt -notmatch $liveSvh) { throw "C1262K_GOLD_HASH_DRIFT query_gold.svh" }
if ($preTxt -notmatch $liveCorpus) { throw "C1262K_GOLD_HASH_DRIFT corpus.json" }
if ($preTxt -notmatch $liveLex) { throw "C1262K_GOLD_HASH_DRIFT qse_role_lexicon_semantic_800k.svh" }
if ($preTxt -notmatch $liveSyn) { throw "C1262K_GOLD_HASH_DRIFT qse_relctx_synonym_01.svh" }
if ($liveSyn -ne $liveSynRtl) {
  throw "C1262K_SYN_SVH_BAG_RTL_MISMATCH bag=$liveSyn rtl=$liveSynRtl"
}
$goldTxt = [System.IO.File]::ReadAllText($goldJson)
if ($goldTxt -notmatch '"n": 262144') { throw "C1262K_GOLD_N_NOT_800000" }
$svhTxt = [System.IO.File]::ReadAllText($goldSvh)
if ($svhTxt -notmatch "localparam int unsigned G_N = 262144;") { throw "C1262K_SVH_N_NOT_800000" }
if ($svhTxt -match "localparam int unsigned G_N = 16384;") { throw "C1262K_16K_CLONE_LABELED_800K" }
Write-Host "GOLD_HASH_LOCKED_BEFORE_XVLOG GOLDEN=$liveGold SVH=$liveSvh CORPUS=$liveCorpus LEX=$liveLex SYN=$liveSyn"
Write-Host "N=262144 procedural mem; named lex 800k; C0 FILE unedited MATCH"
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") {
    throw "C1262K_LEFTOVER_A09_ON_XVLOG_LIST"
  }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") {
    throw "C1262K_FROZEN_AXI_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") {
    throw "C1262K_STREAM02_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_page_skip.sv") {
    throw "C1262K_PAGESKIP_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") {
    throw "C1262K_CTX_DUT_8255a798_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_axi_mem_model.sv") {
    throw "C1262K_C0_DENSE_MEM_MUST_NOT_BE_DUT"
  }
}
...
$pre = @(
  ...
  "# PROGRAM=NO CAND_CAP=16 N=262144 N_BUCKETS=65536 LAW=qse-v2-relctx-synonym-01 LEX=qse-v2-lex-semantic-800k-01 POKE_V=0 PROC_MEM=1",
...
$pre += "$liveGold  results/A7-NATIVE-GRAPH/ASTRA-C1-N262144-SCALE-01/GOLDEN.json"
$pre += "$liveSvh  results/A7-NATIVE-GRAPH/ASTRA-C1-N262144-SCALE-01/query_gold.svh"
$pre += "$liveCorpus  results/A7-NATIVE-GRAPH/ASTRA-C1-N262144-SCALE-01/corpus.json"
$pre += "$liveLex  results/A7-NATIVE-GRAPH/ASTRA-C1-N262144-SCALE-01/qse_role_lexicon_semantic_800k.svh"
$pre += "$liveSyn  results/A7-NATIVE-GRAPH/ASTRA-C1-N262144-SCALE-01/qse_relctx_synonym_01.svh"
$pre += "# BAG"
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C1262K_XELAB_FAIL FIRST_DIVERGENCE XELAB"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" astra_c1_n262144_scale -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C1_N262144_SCALE_XSIM_PASS" -Quiet)
  ...
  $hasHigh = [bool](Select-String -Path $xsimLog -Pattern "HIGH_ID_HIT nid=262143" -Quiet)
  $hasLate = [bool](Select-String -Path $xsimLog -Pattern "LATE_GOLD_HIT nid=262142" -Quiet)
  $hasFail = [bool](Select-String -Path $xsimLog -Pattern "^FAIL " -Quiet)
  $hasN800 = [bool](Select-String -Path $xsimLog -Pattern "C1_N262144_SCALE_N=262144" -Quiet)
}
if ($hasRedCap) {
  ...
  throw "C1262K_REDUCTION_X1000_EMITTED_AS_CAP_N"
}
if ($hasNDrop) {
  $r0 = Join-Path $bag "xsim_fail_r0.log"
  if ((Test-Path -LiteralPath $xsimLog) -and (-not (Test-Path -LiteralPath $r0))) {
  ...
  throw "C1262K_N_DROP_SILENT_16K_FORBIDDEN"
}
if ($hasDiv -or $hasFillFail -or (-not $hasFillHit) -or $hasIncomp -or (-not $hasHigh) -or (-not $hasLate) -or (-not $hasN800)) {
  ...
  throw "C1262K_XSIM_CONTROL_FAIL fillHit=$hasFillHit high=$hasHigh late=$hasLate n800=$hasN800 div=$hasDiv incomp=$hasIncomp"
}
if ($hasPass) {
  if ($hasFail) {
    $r0 = Join-Path $bag "xsim_fail_r0.log"
    ...
    throw "C1262K_PASS_MARKER_WITH_FAIL"
  }
  Write-Host "ASTRA_C1_N262144_SCALE_RUN_OK PASS N=262144 FILL_TEMPLATE_HIT LATE_GOLD_HIT HIGH_ID_HIT"
} else {
  $r0 = Join-Path $bag "xsim_fail_r0.log"
  if ((Test-Path -LiteralPath $xsimLog) -and (-not (Test-Path -LiteralPath $r0))) {
    Copy-Item $xsimLog $r0 -Force
  }
  throw "C1262K_XSIM_FAIL pass=$hasPass fillHit=$hasFillHit high=$hasHigh exit=$xsimExit"
}
