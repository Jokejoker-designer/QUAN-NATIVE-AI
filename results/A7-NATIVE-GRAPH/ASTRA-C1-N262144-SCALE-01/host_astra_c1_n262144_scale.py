#!/usr/bin/env python3
"""ASTRA-C1-N262144-SCALE-01 independent host gold.
...
NEW_ENT = NEW_ENT_16K + NEW_ENT_EXTRA[:70]
REL_EXTRA = [
    ("recirculates", 2, 9),
    ("equalizes", 2, 10),
    ("throttles", 2, 11),
...
def write_svh(path: Path, queries: list[dict]) -> None:
    ...
    lines = [
        "// generated by host_astra_c1_n262144_scale.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
...
def main() -> int:
    ...
    if N != 262144 or N_STREAM + 3 + 1 != N:
        raise SystemExit('N != 262144')
    if SEN_NID != 262143:
    ...
    chk(N == 262144, "N dropped from registered 262144")
    ...
    chk(stream_to_nid(N_STREAM - 1) == 262142, "last stream nid")
    chk(nids_of_sro(DIR_S, DIR_R, DIR_O) == [120, 121, 122], "fill-grid nids")
    chk(nids_of_sro(SEN_S, SEN_R, SEN_O) == [262143], "sentinel nid")
    ...
    chk(sen_and == [262143], f"sent AND {sen_and}")
    ...
    chk(262142 in late_a, "late nid not on k0 list")
    chk(late_a.index(262142) >= 16, f"late k0_idx={late_a.index(262142)} < 16")
    queries_spec = [
        ("fill_template", triple_text(DIR_S, DIR_R, DIR_O), [120, 121, 122]),
        ("nl_synonym", NL_TEXT, [120, 121, 122]),
        ("late_gold", late_text, [262142]),
        ("high_id_sentinel", sen_text, [262143]),
    ...
    chk(qd["high_id_sentinel"]["emit"] == [262143], f"high emit {qd['high_id_sentinel']['emit']}")
    ...
    if fails:
        print("ASTRA_C1_N262144_SCALE_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        return 2
    ...
    corpus = {
        "gate": GATE,
        "n": N,
        "generator": "cartesian_n262144_v1",
        "procedural": True,
        "not_16k_clone": True,
        "n_subjects": N_ENT,
        "n_rels": N_REL,
    ...
    print("ASTRA_C1_N262144_SCALE_HOST_OK")
