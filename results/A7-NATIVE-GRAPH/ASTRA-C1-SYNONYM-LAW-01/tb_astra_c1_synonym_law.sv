// tb_astra_c1_synonym_law.sv — C1 SYNONYM-LAW. PROGRAM=NO.
// Frozen QSE cd7baf49 + NEW named synonym overlay qse-v2-relctx-synonym-01
// + ctx keys 124be808 (not edited) + NEW DUT wrap (STREAM-02 walker copy).
// Context DUT 8255a798 KEEP not compiled as DUT. STREAM-02 14f75db7 not DUT.
// Named lexicon qse-v2-lex-semantic-16k-01 copied df0e8833; not silent C0 59-word.
// poke_v=0. leftover A09 off.
// G_BYTES LSB-first: char0 at [7:0]; extract bytes[8*bi +: 8] only.
// ONE UNKNOWN: named synonym overlay so "what does the boiler supply to the
// header" HIT fill-meaning gold {120,121,122}, NOT nid 131.
// Frozen extract keys vs fill MUST stay keys_match=0 (C0 unpatched).
// Synonym DUT keys MUST remap to fill 3332/3588. Do not relabel gold to {131}.
// Marker ASTRA_C1_SYNONYM_LAW_XSIM_PASS only if NL_GOLD_HIT tp=3 on
// {120,121,122} AND FAIL=0 AND fill-template control hits.
// Do not drop N from 16384. Headline precision_all=TP/emit_n.
`timescale 1ns / 1ps

module tb_astra_c1_synonym_law;
  import a7ng_pkg::*;
  ...
  initial begin
    ...
    for (qi = 0; qi < G_NQ; qi = qi + 1) begin
      ...
      if (qi == G_NL_Q) begin
        ...
        $display("KEYS_VS_FILL class=nl_synonym k0=%0d k1=%0d vs_fill_template k0=%0d k1=%0d keys_match=%0d law=qse-v2-relctx-synonym-01",
          k0_syn, k1_syn, G_FILL_K0, G_FILL_K1, keys_match_syn);
        if (keys_match) begin
    ...
    $display("NL_SUMMARY keys_match_frozen=%0d keys_match_syn=%0d nl_hit=%0d nl_tp=%0d fill_tp=%0d emit_has_131=%0d fail=%0d",
      keys_match, keys_match_syn, nl_hit, nl_tp, fill_tp, emit_has_131, fail);
    if ((fail == 0) && unrelated_empty && (fill_tp > 0) && fill_ids_ok && (keys_match == 0) && keys_match_syn && nl_hit && (nl_tp >= 3) && (emit_has_131 == 0) && (incomp_retrieve == 0) && (G_N == 16384)) begin
      $display("ASTRA_C1_SYNONYM_LAW_XSIM_PASS");
      ...
      $display("ASTRA_C1_SYNONYM_LAW_XSIM_NO_MARKER fail=%0d unrelated_empty=%0d fill_tp=%0d fill_ids_ok=%0d keys_match_frozen=%0d keys_match_syn=%0d nl_hit=%0d nl_tp=%0d emit_has_131=%0d incomp_retrieve=%0d G_N=%0d",
        fail, unrelated_empty, fill_tp, fill_ids_ok, keys_match, keys_match_syn, nl_hit, nl_tp, emit_has_131, incomp_retrieve, G_N);
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,PAGE_SKIP_MIX,N_65536,MASTER_95,ACCEPT_BOARD");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end
