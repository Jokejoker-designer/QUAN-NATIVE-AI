#!/usr/bin/env python3
"""ASTRA-C1-SYNONYM-LAW-01 independent host gold.

ONE UNKNOWN: NEW named synonym law qse-v2-relctx-synonym-01 so the NL probe
"what does the boiler supply to the header" retrieves fill-meaning gold
{120,121,122} (feeds), NOT cartesian nid 131 ("boiler supplies header").
Do not relabel gold as {131}. Do not silent-patch C0 extract.

Frozen extract still binds supply RELCTX id=1 (keys 3329/3585). Named overlay
remaps rel 1 → feeds id=4 so DUT keys become 3332/3588 and walker HITs
{120,121,122}. Frozen keys_match vs fill MUST stay 0 (C0 unpatched).
Synonym keys_match vs fill MUST be 1 (law applied). Gold HIT required.

Instantiate frozen extract cd7baf49 + NEW synonym overlay + ctx keys 124be808
(not edited) + NEW thin DUT wrap (STREAM-02 walker copy; context DUT 8255a798
KEEP not compiled as DUT). Named lexicon df0e8833 copied. Corpus KEEP copy
6991adc7. leftover A09 off poke_v=0 PROGRAM=NO. C1 800k OPEN.
...
def apply_synonym_rel(x: dict) -> dict:
    """Named overlay qse-v2-relctx-synonym-01. Does not patch extract."""
    y = dict(x)
...
def write_svh(
    ...
    lines = [
        "// generated by host_astra_c1_synonym_law.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
        f"// Law {LAW} + ctx {LAW_CTX} + named lexicon {LAW_LEX}; synonym DUT wrap; N=16384 SYNONYM-LAW",
...
def main() -> int:
    ...
    must_hit = {"fill_template", "nl_synonym"}
    queries = []
    for name, text, pred in queries_spec:
        qf = feat(text)
        qs = feat_syn(text)
    ...
    chk(qd["nl_synonym"]["k0"] == FILL_K0 and qd["nl_synonym"]["k1"] == FILL_K1, "syn NL keys not fill keys")
    chk(qd["nl_synonym"]["k0_plain"] == 3329 and qd["nl_synonym"]["k1_plain"] == 3585, "frozen NL plain keys drifted")
    chk(qd["nl_synonym"]["emit"] == [120, 121, 122], f"nl emit {qd['nl_synonym']['emit']} want fill-meaning not 131")
    ...
    predicted_not_selective = 0
    if not predicted_nl_hit:
        fails.append("NL_GOLD_MISS after synonym overlay")
    if 131 in qd["nl_synonym"]["emit"]:
        fails.append("emit contains nid 131 after synonym overlay")

    if fails:
        print("ASTRA_C1_SYNONYM_LAW_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        return 2
    ...
    def qpub(q: dict) -> dict:
        ...
        if q["name"] == "nl_synonym":
            ...
            row["keys_match_fill_frozen"] = int(
                q["k0_frozen"] == qd["fill_template"]["k0"] and q["k1_frozen"] == qd["fill_template"]["k1"]
            )
            ...
            row["not_nid_131"] = 131 not in q["emit"]
        if q["name"] == "unrelated":
    ...
    golden = {
        ...
        "synonym_bind": "qse-v2-relctx-synonym-01: REL id 1 supply → REL id 4 feeds; combinational overlay after frozen extract",
        "key_rebind": "instantiate a7ng_query_role_keys_ctx 124be808 not edited; NEW DUT wrap a7ng_query_axi_sparse_intersect_synonym; context DUT 8255a798 KEEP not compiled as DUT; STREAM-02 14f75db7 not compiled as DUT; k2k3_not_probed",
        "intersect_law": "sorted-nid two-pointer AND; 1-beat page; rare-first; CAND_CAP after emit; synonym rel then ctx folded into k0/k1 when ctx_valid",
        "n": N,
        "cand_cap": CAND_CAP,
        ...
        "nl_keys_match_fill_frozen": keys_match_frozen,
        "nl_keys_match_fill_syn": keys_match_syn,
        "nl_gold_hit_predicted": int(predicted_nl_hit),
        "predicted_not_selective_nl": int(predicted_not_selective),
        "gold_not_relabeled_131": True,
    ...
    ysha = sha256_file(BAG / "qse_relctx_synonym_01.svh")
    (BAG / "GOLD_HASH_PRE_XVLOG.txt").write_text(
        "\n".join(
            [
                ...
                f"{ysha}  qse_relctx_synonym_01.svh",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    print("ASTRA_C1_SYNONYM_LAW_HOST_GOLD_WRITTEN")
    ...
    for q in queries:
        ...
        if q["name"] == "nl_synonym":
            extra = (
                f" keys_match_frozen={keys_match_frozen} keys_match_syn={keys_match_syn}"
                f" syn_hit={q['syn_hit']} NL_GOLD_HIT={int(q['tp'] > 0)}"
            )
        if q["name"] == "unrelated":
    ...
    if (
        predicted_nl_hit
        and keys_match_frozen == 0
        and keys_match_syn == 1
        and qd["nl_synonym"]["emit"] == [120, 121, 122]
    ):
        print("PREDICTED_RESULT=PASS_THIS_GATE_ONLY synonym remap AND fill-meaning gold HIT not nid 131")
    else:
