// ASTRA-09-R4-UART-ISO-01. PROGRAM=NO. Bag-local TB.
// Instantiates a7ng_astra_09_r4_uart_iso_wrap (UART ISO around frozen SGD + A09-R2).
// Does not claim ASTRA-09-R3 query frames. Not BOARD_PASS.
`timescale 1ns / 1ps

module tb_astra_09_r4_uart_iso;
  ...
  a7ng_astra_09_r4_uart_iso_wrap dut (
    .CLK100MHZ(CLK100MHZ),
    .sw(sw),
    .btn(btn),
    .led(led),
  ...
  always @(posedge dut.clk) begin
    ...
    if (dut.dn)
      $display("ISO_DONE w0=%0d viso=%0d w1=%0d tbl=%0d a09_w0=%0d t=%0t",
        dut.wiso[0], dut.viso, dut.wiso[1], dut.tbl, dut.w_a09[0], $time);
  end
  ...
  task automatic decode_iso(
    ...
    logic signed [15:0] w0_u, viso_u, w1_u;
    ...
    begin
      ...
      viso_u = $signed({fr[5], fr[4]});
      w1_u   = $signed({fr[9], fr[8]});
      x0_u   = $signed(fr[6]);
      rew_u  = $signed(fr[7]);
      tbl_u  = fr[1][7];
      iso_u  = fr[1][6];
      $display("%s w0=%0d viso=%0d w1=%0d x0=%0d rew=%0d tbl=%0d iso=%0d hier_w0=%0d hier_tbl=%0d a09_w0=%0d",
        tag, w0_u, viso_u, w1_u, x0_u, rew_u, tbl_u, iso_u,
        dut.wiso[0], dut.tbl, dut.w_a09[0]);
      ...
      chk({tag, "_VISO0"}, viso_u === 16'sd0);
      chk({tag, "_W1_0"}, w1_u === 16'sd0);
      chk({tag, "_HIER_W0"}, dut.wiso[0] === exp_w0);
      chk({tag, "_HIER_W1"}, dut.wiso[1] === 16'sd0);
      chk({tag, "_A09_W0_UNTOUCHED"}, dut.w_a09[0] === 16'sd0);
    end
  endtask
  ...
  initial begin
    ...
    btn = 4'd0;
    uart_txd_in = 1'b1;
    $display("TB ASTRA-09-R4-UART-ISO DUT=a7ng_astra_09_r4_uart_iso_wrap INNER=a7ng_astra_09_r2_cand_ovf ISO=a7ng_shared_rank_sgd_q8_sym_f2r2 PROGRAM=NO");
    ...
    if (dut.rst_n !== 1'b1) begin
      $display("ASTRA_09_R4_UART_ISO_FAIL n=%0d first=%s", fail, first_div);
      $finish;
    end
    ...
    chk("ISO_P3_X50_DW5", (dut.wiso[0] === 16'sd5) && (dut.wiso[1] === 16'sd0)
        && (dut.viso === 16'sd0) && (dut.tbl === 1'b0));

    wrap_rst();
    chk("ISO_M3_PRE_W0", dut.wiso[0] === 16'sd0);

    $display("ISO_M3_CMD_SENT x0=64 rew=-3 t=%0t", $time);
    uart_send_iso(8'sd64, -8'sd3);
    decode_iso("ISO_M3", -16'sd6, 8'sd64, -8'sd3);
    chk("ISO_M3_X64_DW6", (dut.wiso[0] === -16'sd6) && (dut.wiso[1] === 16'sd0)
        && (dut.viso === 16'sd0) && (dut.tbl === 1'b0));

    if (fail == 0)
      $display("ASTRA_09_R4_UART_ISO_PASS");
    else
      $display("ASTRA_09_R4_UART_ISO_FAIL n=%0d first=%s", fail, first_div);
    $finish;
  end
