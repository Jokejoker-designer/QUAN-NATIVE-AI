// a7ng_astra_09_r4_uart_iso_wrap.sv — ASTRA-09-R4-UART-ISO-01. PROGRAM=NO.
// Bag-local UART ISO glue. Instantiates frozen a7ng_astra_09_r2_cand_ovf
// and frozen a7ng_shared_rank_sgd_q8_sym_f2r2. Not PRODUCTION_TOP. Not a bit.
`timescale 1ns / 1ps

module a7ng_astra_09_r4_uart_iso_wrap (
  ...
  typedef enum logic [2:0] {
    C_IDLE, C_X, C_REW, C_EOL, C_GO, C_WAIT, C_TX
  } cmd_t;
  cmd_t cst;
      ...
      unique case (cst)
        ...
        C_WAIT: begin
          if (dn) begin
            ...
            tx_bytes[2]  <= wiso[0][7:0];
            tx_bytes[3]  <= wiso[0][15:8];
            tx_bytes[4]  <= viso[7:0];
            tx_bytes[5]  <= viso[15:8];
            tx_bytes[6]  <= x0_lat;
            tx_bytes[7]  <= rew8_lat;
            tx_bytes[8]  <= wiso[1][7:0];
            tx_bytes[9]  <= wiso[1][15:8];
            tx_bytes[10] <= 8'd0;
            tx_bytes[11] <= 8'd0;
            tx_bytes[12] <= 8'd0;
            tx_bytes[13] <= 8'd0;
