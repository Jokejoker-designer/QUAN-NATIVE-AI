...
$files = @(
  ...
  (Join-Path $bag "tb_astra_09_integ_path.sv")
)
$includes = @(
...
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "A09_FILE_MISSING $p" }
...
$pre += "# PROVENANCE frozen persist/R2/R3/R4/F3R4/F3R3/F3R2/F3/F2R5/F2R4/F2R3/F2R2 (not compiled; do not edit)"
...
Write-Host "A09_SHA_FROZEN $shaPre"
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "A09_XVLOG_NOT_FOUND" }

Set-Location $work
& "$bin\xvlog.bat" --sv -i $incq -i $incc -i $inci $files
if ($LASTEXITCODE -ne 0) { throw "A09_XVLOG_FAIL" }
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_09_INTEGRATED_PATH_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "A09_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_09_INTEGRATED_PATH_RUN_OK"
