# ASTRA-11-A09R3-UART-IOBFF-HOLD-01. PROGRAM=NO. BIT=NO.
...
#
# HOLD POLICY FROZEN in PREREG before impl: FALSE_PATH_HOLD_ASYNC_UART.
# Setup max 2.000 kept. Do NOT apply -min 0.500. Do not retune the +0.115 bag.
# IOB FF policy FROZEN in PREREG: pack UART pad FFs (IOB=TRUE).
...
# ---------------------------------------------------------------------------
# Setup envelope (PREREG). uart_io_vclk is the async FTDI I/O reference
# at the same 20.000 ns period as pipe clk50u. Pipe clock remains MMCM+BUFG
# clk50u (real, not virtual). IN/OUT max 2.000 only. -min is NOT applied.
# ---------------------------------------------------------------------------
...
# Do not check hold vs 20 ns virtual uart_io_vclk. Do not apply -min 0.500.
