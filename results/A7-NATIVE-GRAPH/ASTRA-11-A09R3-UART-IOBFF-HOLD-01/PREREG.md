# PREREG — ASTRA-11-A09R3-UART-IOBFF-HOLD-01

Frozen before impl. PROGRAM=NO. No board program. No JTAG/xsdb/COM12/bitstream/write_bitstream/hw_server.
Does not edit ASTRA-11-A09R3-UART-IOBFF-01 / ASTRA-11-A09R3-UART-IODELAY-01 /
...
Does not overwrite IOB-FF wrap WNS=+0.115 WHS=−4.915, I/O-delay wrap WNS=+0.681,
...
One unknown, this bag only: can a **new STA envelope** (HOLD_POLICY frozen here before impl)
keep UART IOB FFs packed and make Design Timing Summary **WHS ≥ 0 as well as WNS ≥ 0**
after implement+route of a **new named UART wrap** that **instantiates** frozen
`a7ng_astra_09_r2_cand_ovf` on `xc7a100tcsg324-1` at a **declared 50 MHz** constraint
with **clock-network delay included** — **without a bitstream** and without claiming BOARD_PASS.

ASTRA-11-A09R3-UART-IOBFF-01 routed WNS=+0.115 WHS=−4.915 (IOB FF YES, delays 2.000/0.500) is **not** this result.
...
## Frozen HOLD policy (BEFORE impl) — pick one, write it

Handoff allowed: false-path-hold, async UART exception, extra IOB sample, or a different virtual clock.
...
- UART 115200 is asynchronous to pipe `clk50u` (bit period ~8.68 µs). There is no related
  FTDI launch clock on the FPGA.
- Auditor 20260906T1830Z: packing ILOGIC IFF puts **0.000 ns** IBUF→IFF route against
  virtual `uart_io_vclk` SCD=0 and real MMCM+BUFG destination delay. That collision is
  WHS=−4.915 in the prior bag. Intra-`clk50u` hold was MET +0.131.
- IOB FF + frozen `uart_rx` 2-FF synchronizer is the CDC. Hold vs a 20 ns virtual I/O
  clock is not a 115200 silicon hold requirement.
- Do **not** retune frozen min 0.500 from the +0.115 bag. Do **not** copy-paste
  2.000/0.500 min delay then hide WHS. This bag does **not** apply `-min`.
- Extra IOB sample is not picked: it would not remove IBUF→IFF as the hold endpoint.
- Different virtual clock (period change) is not picked: hold is same-edge; period
  does not fix SCD=0 vs DCD. Fitted source-latency numbers would chase the −4.915 residual.
...
Setup envelope **kept** (not a hold retune):

```text
UART_TXD_IN     = A9    FPGA RX   INPUT   set_input_delay  -max 2.000
UART_RXD_OUT    = D10   FPGA TX   OUTPUT  set_output_delay -max 2.000
UART_IN_MAX_NS  = 2.000
UART_OUT_MAX_NS = 2.000
UART_IN_MIN_NS  = NOT_APPLIED   (FALSE_PATH_HOLD; do not reuse 0.500)
UART_OUT_MIN_NS = NOT_APPLIED   (FALSE_PATH_HOLD; do not reuse 0.500)
...
`uart_io_vclk` remains the **I/O timing reference** for setup only. Pipe clock remains
MMCM 100→50 + BUFG `clk50u` (propagated, generated, auto-derived).
...
Can HOLD_POLICY=FALSE_PATH_HOLD_ASYNC_UART (frozen here before impl) keep IOB FFs packed
and make Design Timing Summary WHS ≥ 0 as well as WNS ≥ 0 — without a bitstream and
...
- Not `a7ng_astra_11_a09r3_uart_iobff_wrap` (prior bag).
...
`uart_rx` `8e802d0b…` / `uart_tx` `b4b7d097…`. Prior IOB-FF wrap KEEP
(not compiled as this top). Prior I/O-delay wrap KEEP. Prior UART impl wrap KEEP.

## If WHS < 0

FAIL this bag, keep the routed report, one bounded experiment in this bag
(post-route `phys_opt_design`; **same frozen HOLD_POLICY**; still 50 MHz;
still IOB=TRUE; still setup max 2.000; still no `-min` 0.500). Still no bitstream.
Do not program a failing bit. Do not invent min-delay numbers after seeing WHS.
...
Retune of frozen 2.000/0.500 inside ASTRA-11-A09R3-UART-IOBFF-01.
