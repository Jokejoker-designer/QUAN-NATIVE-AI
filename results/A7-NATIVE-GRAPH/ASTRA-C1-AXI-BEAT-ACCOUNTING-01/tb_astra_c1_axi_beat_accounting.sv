// tb_astra_c1_axi_beat_accounting.sv — C1 N=256 P0-2 R-beat accounting. PROGRAM=NO.
// Replay frozen KEY-INTERSECT queries through instantiated intersect DUT.
// Probe: DIR_R_BEATS/POST_R_BEATS on rvalid&&rready; class by AR address.
// Emit must be bit-identical to KEY-INTERSECT GOLDEN. poke_v=0. leftover A09 off.
// G_BYTES LSB-first: char0 at [7:0]; extract bytes[8*bi +: 8] only.
`timescale 1ns / 1ps

module tb_astra_c1_axi_beat_accounting;
  import a7ng_pkg::*;
  ...
  integer ar_bytes, r_bytes, ratio_x1000, emit_ident, rbeat_ok;
  integer all_emit_ident, all_rbeat_ok;
  logic [ID_W-1:0] got [0:31];
  logic [ID_W-1:0] direct_got [0:31];
  logic found;
  logic probe_clr;
  logic [31:0] dir_r_beats, post_r_beats, dir_ar_beats, post_ar_beats;
  logic [31:0] unk_ar, unk_r, total_axi_bytes;
  string emit_list;
  ...
  a7ng_query_axi_sparse_intersect #(
  ...
  a7ng_query_axi_rbeat_probe #(
    .INDEX_BASE(NG_DDR_INDEX_BASE), .N_TABLES(4), .N_BUCKETS(4096), .ENTRY_BYTES(16)
  ) u_rbeat (
    .clk(clk), .rst_n(rst_n), .clr_i(probe_clr),
    .araddr(araddr), .arvalid(arvalid), .arready(arready),
    .rvalid(rvalid), .rready(rready), .rlast(rlast),
    .dir_r_beats_o(dir_r_beats), .post_r_beats_o(post_r_beats),
    .dir_ar_beats_o(dir_ar_beats), .post_ar_beats_o(post_ar_beats),
    .unk_ar_o(unk_ar), .unk_r_o(unk_r),
    .total_axi_bytes_o(total_axi_bytes)
  );
  ...
  initial begin
    ...
    $display("C1_AXI_BEAT_N=%0d CAND_CAP=%0d INDEX_HEAD=%0d LAW=qse-v2-intersect-01 POKE_V=0 REDUCTION_X1000=NOT_EMITTED DDR_QUERY_BOUND_FINAL=NOT_FROZEN",
    ...
    for (qi = 0; qi < G_NQ; qi = qi + 1) begin
      ...
      if (v0_rb !== G_V0[qi] || v1_rb !== G_V1[qi] || v2_rb !== G_V2[qi] || v3_rb !== G_V3[qi])
        diverge("VALIDITY_MISMATCH", $sformatf("q=%0d %s relbind valid", qi, cname(qi)));
      if (qi == 0 && k2_rb === k2)
      ...
      if (qi == 0) begin
        if ((n_got != 3) || (got[0] !== 20'd110) || (got[1] !== 20'd144) || (got[2] !== 20'd145)) begin
          emit_ident = 0;
          $display("FAIL DIRECT_EMIT_NOT_GOLD n=%0d", n_got);
          fail = fail + 1;
        end
      end
      all_emit_ident = all_emit_ident && emit_ident;

      ar_bytes = (integer'(n_dir) + integer'(n_post)) * 16;
      r_bytes  = (integer'(dir_r_beats) + integer'(post_r_beats)) * 16;
      rbeat_ok = 1;
      if (unk_ar != 32'd0 || unk_r != 32'd0) begin
        $display("FAIL AXI_UNCLASSIFIED class=%s unk_ar=%0d unk_r=%0d", cname(qi), unk_ar, unk_r);
        rbeat_ok = 0;
        fail = fail + 1;
      end
      if ((integer'(dir_ar_beats) != integer'(n_dir)) || (integer'(post_ar_beats) != integer'(n_post))) begin
        $display("FAIL AXI_AR_CLASS_MISMATCH class=%s probe_dir_ar=%0d dut_n_dir=%0d probe_post_ar=%0d dut_n_post=%0d",
          cname(qi), dir_ar_beats, n_dir, post_ar_beats, n_post);
        rbeat_ok = 0;
        fail = fail + 1;
      end
      if (ar_bytes > 0) begin
        if ((dir_r_beats + post_r_beats) == 32'd0) begin
          $display("FAIL AXI_R_BEATS_ZERO class=%s AR_BYTES=%0d", cname(qi), ar_bytes);
          rbeat_ok = 0;
          fail = fail + 1;
        end
        if (r_bytes < ar_bytes) begin
          $display("FAIL AXI_R_LT_AR class=%s R_BYTES=%0d AR_BYTES=%0d", cname(qi), r_bytes, ar_bytes);
          rbeat_ok = 0;
          fail = fail + 1;
        end
      end
      all_rbeat_ok = all_rbeat_ok && rbeat_ok;
      if (ar_bytes == 0) ratio_x1000 = -1;
      else ratio_x1000 = (r_bytes * 1000) / ar_bytes;
      ...
      if (qi == 4) begin
        ...
        $display("NOT_SELECTIVE class=wrong_context k0=%0d k1=%0d k2=%0d k3=%0d vs_direct k0=%0d k1=%0d k2=%0d k3=%0d keys_match=%0d emit_match=%0d law=qse-v2-intersect-01 xid_not_directory_key rec_is_not_context_selectivity=1",
      ...
      for (i = 0; i < n_got; i = i + 1) begin
        ...
        if (i == 0) emit_list = $sformatf("%0d", got[i]);
        else emit_list = $sformatf("%s,%0d", emit_list, got[i]);
      end
      $display("EMIT_IDS class=%s n=%0d list=%s EMIT_IDENTICAL=%0d",
        cname(qi), n_got, emit_list, emit_ident);
      if (ar_bytes == 0)
        $display("AXI_BEAT class=%s DIR_R_BEATS=%0d POST_R_BEATS=%0d TOTAL_AXI_BYTES=%0d AR_BYTES=%0d R_BYTES=%0d RATIO_x1000=NA n_dir_ar=%0d n_post_ar=%0d rbeat_ok=%0d",
          cname(qi), dir_r_beats, post_r_beats, total_axi_bytes, ar_bytes, r_bytes,
          n_dir, n_post, rbeat_ok);
      else
        $display("AXI_BEAT class=%s DIR_R_BEATS=%0d POST_R_BEATS=%0d TOTAL_AXI_BYTES=%0d AR_BYTES=%0d R_BYTES=%0d RATIO_x1000=%0d n_dir_ar=%0d n_post_ar=%0d rbeat_ok=%0d",
          cname(qi), dir_r_beats, post_r_beats, total_axi_bytes, ar_bytes, r_bytes, ratio_x1000,
          n_dir, n_post, rbeat_ok);
    ...
    if (fail == 0 && dist_gold_ok && dist_excl_ok && dist_no_leak && unrelated_empty && wc_not_sel
        && (direct_tp > 0) && all_emit_ident && all_rbeat_ok) begin
      $display("ASTRA_C1_AXI_BEAT_XSIM_PASS");
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,N_gt_256,qse-v2-stream-intersect-02");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO DDR_QUERY_BOUND_FINAL=NOT_FROZEN CAND_CAP_FINAL=NOT_FROZEN");
      $display("REDUCTION_X1000=NOT_EMITTED");
      $display("STREAM_INTERSECT_02=NOT_THIS_BAG");
    end else begin
      $display("ASTRA_C1_AXI_BEAT_XSIM_NO_MARKER fail=%0d dist_gold_ok=%0d dist_excl_ok=%0d dist_no_leak=%0d unrelated_empty=%0d wc_not_sel=%0d direct_tp=%0d all_emit_ident=%0d all_rbeat_ok=%0d",
        fail, dist_gold_ok, dist_excl_ok, dist_no_leak, unrelated_empty, wc_not_sel, direct_tp,
        all_emit_ident, all_rbeat_ok);
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,N_gt_256,qse-v2-stream-intersect-02");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO DDR_QUERY_BOUND_FINAL=NOT_FROZEN CAND_CAP_FINAL=NOT_FROZEN");
      $display("REDUCTION_X1000=NOT_EMITTED");
      $display("STREAM_INTERSECT_02=NOT_THIS_BAG");
