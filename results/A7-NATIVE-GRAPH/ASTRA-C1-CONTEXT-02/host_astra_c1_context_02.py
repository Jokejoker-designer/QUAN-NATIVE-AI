#!/usr/bin/env python3
"""ASTRA-C1-CONTEXT-02 independent host gold for qse-v2-intersect-context-02.

Law: frozen extract (qse-v2-role-00) unchanged. NEW named key bind folds C0
ctx_id into k0/k1 when ctx_valid. Walker is STREAM-02 two-pointer (1-beat,
rare-first, CAND_CAP after emit) in a new thin wrapper — STREAM-02 file
14f75db7 is not edited and is not compiled as DUT.

ctx_valid is the bind flag, independent of a zero ctx_id inside the packer.
Frozen extract does not export xh. C0 CLS_CTX ids are {1,2} never 0, so
ctx_valid := (ctx_id != 0) ≡ xh on this freeze.

if ctx_valid:
  k0 = {subj[7:0], ctx[3:0], rel[3:0]}
  k1 = {obj[7:0],  ctx[3:0], rel[3:0]}
else:
  k0, k1 = frozen {subj,rel}/{obj,rel}

Host indexes every record under plain k0/k1 (so CLASS_direct still retrieves
{110,144,145}) AND under ctx-packed keys when the record bound a ctx token.

ONE UNKNOWN: CLASS_wrong_context emit ≠ CLASS_direct {110,144,145} AND
CLASS_direct still retrieves those gold ids.

FORBIDDEN: nid-derived keys; relevant=router_union; bag lexicon shadow;
patch C0 extract/lexicon/dir/sparse; edit STREAM-02 / PAGE-SKIP;
regenerate gold after FAIL; freeze CAND_CAP_FINAL / DDR_QUERY_BOUND_FINAL /
C1 800k; poke_v=1; leftover A09 as DUT; write independent audit tree.
PROGRAM=NO. BIT=NO.
...
def ctx_keys(x: dict) -> dict:
    """Same packing as a7ng_query_role_keys_ctx.sv. Valids unchanged."""
...
def write_svh(path: Path, queries: list[dict], writes: dict[int, int], docs: list[dict]) -> None:
    ...
    lines = [
        "// generated by host_astra_c1_context_02.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
        "// Law qse-v2-intersect-context-02: ctx fold into k0/k1 when ctx_valid; STREAM-02 two-pointer walker",
...
def main() -> int:
    ...
    chk(all(d["ctx_valid"] == int(bool(d["ctx_bind"])) for d in docs), "ctx_valid != extract xh")
    chk(all(d["k0_plain"] == pack_plain(d["subj_id"], d["rel_id"]) for d in docs), "k0_plain packing")
    chk(all(d["k0"] != d["nid"] and d["k1"] != d["nid"] for d in docs), "nid-derived keys")
    ...
    chk(gold_map["wrong_context"] == [144], f"wrong_context gold {gold_map['wrong_context']}")
    ...
    chk(qd["direct"]["emit"] == [110, 144, 145], f"direct emit {qd['direct']['emit']}")
    ...
    chk(qd["distractor"]["emit"] == qd["direct"]["emit"], "distractor emit != direct")

    wc = qd["wrong_context"]
    dd = qd["direct"]
    chk(wc["ctx_id"] == 1, "wrong_context ctx_id")
    chk(wc["ctx_valid"] == 1, "wrong_context ctx_valid")
    chk(wc["ctx_bind"] == 1, "wrong_context extract xh")
    chk(wc["k0"] != dd["k0"] or wc["k1"] != dd["k1"], "NOT_SELECTIVE keys still match; C0 cannot distinguish")
    chk(wc["k0"] != wc["k0_plain"], "wrong_context k0 not folded")
    chk(wc["emit"] != dd["emit"], "NOT_SELECTIVE emit still == direct")
    chk(set(wc["emit"]) != {110, 144, 145}, "wrong_context emit still CLASS_direct ids")
    chk(wc["emit"] == [144], f"wrong_context emit {wc['emit']}")
    chk(wc["tp"] == 1, "wrong_context tp")
    chk(110 not in wc["emit"] and 145 not in wc["emit"], "wrong_context leaked other PSC")

    not_selective = (wc["k0"] == dd["k0"] and wc["k1"] == dd["k1"] and wc["emit"] == dd["emit"])

    if fails:
        print("ASTRA_C1_CONTEXT_02_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        if not_selective:
            print("NOT_SELECTIVE C0 lexicon/extract cannot bind distinguishable ctx; do not fake nid keys")
        return 2
    ...
    golden = {
        ...
        "key_rebind": "NEW a7ng_query_role_keys_ctx; walker=stream_k0_intersect_k1; k2k3_not_probed",
    ...
    print("ASTRA_C1_CONTEXT_02_HOST_PASS")
    print("N", N, "CAND_CAP", CAND_CAP, "INDEX_HEAD", INDEX_HEAD, "N_BUCKETS", N_BUCKETS, "WRITES", len(writes))
