// tb_astra_c1_context_02.sv — C1 N=256 qse-v2-intersect-context-02. PROGRAM=NO.
// Frozen QSE + NEW ctx key bind + STREAM-02 two-pointer walker wrapper.
// poke_v=0. leftover A09 off. STREAM-02 14f75db7 not compiled as DUT.
// G_BYTES LSB-first: char0 at [7:0]; extract bytes[8*bi +: 8] only.
// PASS marker only if emit_wrong_context != emit_direct AND direct_tp>0.
...
module tb_astra_c1_context_02;
  ...
  integer incomp_retrieve, direct_ids_ok;
  integer arlen_bad;
  ...
  a7ng_query_axi_sparse_intersect_context #(
    .N_TABLES(4), .N_BUCKETS(G_N_BUCKETS_TB), .CAND_CAP(CAND_CAP),
  ...
  initial begin
    ...
    $display("C1_CONTEXT_02_N=%0d N_BUCKETS=%0d CAND_CAP=%0d INDEX_HEAD=%0d LAW=qse-v2-intersect-context-02 POKE_V=0 PAGE_BUFFER_BEATS=1 RARE_LIST_FIRST=1 CAND_CAP_AFTER_EMIT=1 REDUCTION_X1000=NOT_EMITTED CAND_CAP_FINAL=NOT_FROZEN DDR_QUERY_BOUND_FINAL=NOT_FROZEN",
      G_N, G_N_BUCKETS, CAND_CAP, G_INDEX_HEAD);
    ...
    for (qi = 0; qi < G_NQ; qi = qi + 1) begin
      ...
      if (qi == 0) begin
        ...
        direct_ids_ok = (n_got == 3 && got[0] == 20'd110 && got[1] == 20'd144 && got[2] == 20'd145);
        if (!direct_ids_ok) begin
      ...
      if (qi == 4) begin
        ...
        if (emit_match || keys_match) begin
          $display("NOT_SELECTIVE class=wrong_context k0=%0d k1=%0d k2=%0d k3=%0d vs_direct k0=%0d k1=%0d k2=%0d k3=%0d keys_match=%0d emit_match=%0d law=qse-v2-intersect-context-02",
          ...
          $display("CONTEXT_SELECTIVE class=wrong_context k0=%0d k1=%0d k2=%0d k3=%0d vs_direct k0=%0d k1=%0d k2=%0d k3=%0d keys_match=0 emit_match=0 law=qse-v2-intersect-context-02",
    ...
    if (fail == 0 && dist_gold_ok && dist_excl_ok && dist_no_leak && unrelated_empty && wc_sel && (direct_tp > 0) && direct_ids_ok && late_hit && late_ok && (incomp_retrieve == 0)) begin
      $display("ASTRA_C1_CONTEXT_02_XSIM_PASS");
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,N_16384,PAGE_SKIP_MIX");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end else begin
      $display("ASTRA_C1_CONTEXT_02_XSIM_NO_MARKER fail=%0d dist_gold_ok=%0d dist_excl_ok=%0d dist_no_leak=%0d unrelated_empty=%0d wc_sel=%0d direct_tp=%0d direct_ids_ok=%0d late_hit=%0d late_ok=%0d incomp_retrieve=%0d",
        fail, dist_gold_ok, dist_excl_ok, dist_no_leak, unrelated_empty, wc_sel, direct_tp, direct_ids_ok, late_hit, late_ok, incomp_retrieve);
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,N_16384,PAGE_SKIP_MIX");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end
