...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5QTMIG_FILE_MISSING $p" }
...
$expect = @{
  ...
  "rtl/native_graph/integrate/a7ng_astra_c5_prod_top_c4q.sv" = "e8b5d8e4020117c3909e733fbbfd8f990829b92b6c71301eff25b06e6585cc71"
  "rtl/native_graph/integrate/a7ng_astra_c4_lm06_byte256_qptext_gen.sv" = "ded1af7d75b8240c35ca69ec5385f55103f4869aeb8e6c767a9bcb1f9706f4a6"
}
...
if ($hashFail) { throw "C5QTMIG_HASH_GATE_FAIL do_not_edit_keep_rtl_or_c4q_or_mig_prj" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_prod_top_qptext_mig.sv"
$dut = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_prod_top_c4q.sv"
...
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5QTMIG_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5QTMIG_GOLD_HASH_DRIFT GOLDEN.json" }

$dutTxt = [System.IO.File]::ReadAllText($dut)
if ($dutTxt -match "evid_obj") { throw "C5QTMIG_DUT_HAS_EVID_OBJ" }
if ($dutTxt -match "a7ng_astra_c4_lm06_grounded_gen") { throw "C5QTMIG_DUT_HAS_GROUNDED_GEN" }
if ($dutTxt -match "a7ng_c4p_ch") { throw "C5QTMIG_DUT_HAS_C3PROOF_DICT" }
if ($dutTxt -notmatch "a7ng_c4q_ch") { throw "C5QTMIG_DUT_MISSING_FPGA_DICT" }
if ($dutTxt -notmatch "a7ng_astra_c3_held_out") { throw "C5QTMIG_DUT_MISSING_C3" }
if ($dutTxt -notmatch "a7ng_astra_c4_lm06_byte256_qptext_gen") { throw "C5QTMIG_DUT_MISSING_QPTEXT_GEN" }
...
foreach ($f in $svFiles) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C5QTMIG_LEFTOVER_A09" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C5QTMIG_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C5QTMIG_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") { throw "C5QTMIG_CTX_8255a798_AS_DUT" }
  if ($leaf -eq "a7ng_learned_prior_store.sv") { throw "C5QTMIG_PRIOR_STORE" }
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5QTMIG_TINYGPT_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C5QTMIG_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5QTMIG_SYNTH_MIG_AS_DUT" }
  if ($leaf -eq "a7ng_lm_graph_arb.sv") { throw "C5QTMIG_LM_GRAPH_ARB_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5QTMIG_LIVE_PROD_TOP_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_grounded_gen.sv") { throw "C5QTMIG_GROUNDED_GEN_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_byte256_ctxcopy.sv") { throw "C5QTMIG_CTXCOPY_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_byte256_dict.sv") { throw "C5QTMIG_DICT_LUT_DUT" }
}
...
$pre = @(
  ...
  "# LAW=astra-c5-prod-top-qptext-mig-01 MIG_XSIM Digilent AXI mig_sim + ddr3_model c4q DUT",
  "# HASH_GATE MATCH vs C0/C1/C2 KEEP + live C3 wrap + live prod_top unedited + c4q + official mig.prj"
) + $hashLines
$pre += "# COMPILED"
...
$pre += "# FORBIDDEN live prod_top / grounded_gen / ctxcopy / TinyGPT / synth mig / A09 as DUT"
[System.IO.File]::WriteAllLines((Join-Path $bag "SHA256.txt"), $pre)
Write-Host "C5QTMIG_SHA_FROZEN"
...
function Write-DivLog([string]$code, [string]$detail) {
  [System.IO.File]::WriteAllLines((Join-Path $bag "xsim.log"), @(
    ...
    "RESULT=FAIL",
    "ASTRA_C5_PROD_TOP_QPTEXT_MIG_XSIM_PASS ABSENT",
    "PROGRAM=NO BOARD_PASS=NOT_CLAIMED C5_MASTER=OPEN"
  ))
}

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C5QTMIG_XVLOG_NOT_FOUND" }
if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null

$prj = Join-Path $work "c5_prod_top_qptext_mig.prj"
$prjLines = @()
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C5QTMIG_XVLOG_FAIL"
}
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
$xvlogTxt = [System.IO.File]::ReadAllText((Join-Path $bag "xvlog.log"))
if ($xvlogTxt -match "a7ng_astra_09_integ_path") { throw "C5QTMIG_LEFTOVER_A09_COMPILED" }
if ($xvlogTxt -match "a7ng_query_axi_sparse_stream_intersect") { throw "C5QTMIG_STREAM02_COMPILED" }
if ($xvlogTxt -match "a7ng_query_axi_sparse_intersect_context") { throw "C5QTMIG_CTX_COMPILED" }
if ($xvlogTxt -match "tiny_gpt803k_core") { throw "C5QTMIG_TINYGPT_COMPILED" }
if ($xvlogTxt -match "a7ng_evidence_compose") { throw "C5QTMIG_COMPOSE_COMPILED" }
if ($xvlogTxt -match "a7ng_astra_c4_lm06_grounded_gen") { throw "C5QTMIG_GROUNDED_GEN_COMPILED" }
if ($xvlogTxt -match "a7ng_astra_c4_lm06_byte256_ctxcopy") { throw "C5QTMIG_CTXCOPY_COMPILED" }
if ($xvlogTxt -match "mig_7series_0_mig\.v" -and $xvlogTxt -notmatch "mig_7series_0_mig_sim") { throw "C5QTMIG_SYNTH_MIG_COMPILED" }
if ($xvlogTxt -notmatch "a7ng_astra_c5_prod_top_c4q") { throw "C5QTMIG_DUT_NOT_ANALYZED" }
if ($xvlogTxt -match "a7ng_astra_c5_prod_top\.sv") { throw "C5QTMIG_LIVE_PROD_TOP_COMPILED" }
if ($xvlogTxt -notmatch "mig_7series_0_mig_sim") { throw "C5QTMIG_MIG_SIM_NOT_ANALYZED" }
if ($xvlogTxt -notmatch "tb_astra_c5_prod_top_qptext_mig") { throw "C5QTMIG_TB_NOT_ANALYZED" }
if ($xvlogTxt -notmatch "a7ng_astra_c4_lm06_byte256_qptext_gen") { throw "C5QTMIG_QPTEXT_GEN_NOT_ANALYZED" }

& "$bin\xelab.bat" -mt off -O0 tb_astra_c5_prod_top_qptext_mig glbl -s astra_c5_prod_top_qptext_mig -L unisims_ver -L unimacro_ver -L secureip -timescale 1ps/1ps
if ($LASTEXITCODE -ne 0) {
  ...
  throw "C5QTMIG_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" astra_c5_prod_top_qptext_mig -R -log $xsimLog
...
if (-not (Test-Path -LiteralPath $xsimLog)) { throw "C5QTMIG_NO_XSIM_LOG" }
$hasPass = [bool](Select-String -Path $xsimLog -Pattern "^ASTRA_C5_PROD_TOP_QPTEXT_MIG_XSIM_PASS$" -Quiet)
$hasDiv = [bool](Select-String -Path $xsimLog -Pattern "^FIRST_DIVERGENCE" -Quiet)
$need = @(
  "CLASS_mig_calib_complete HIT",
  "CLASS_uart_ingress HIT",
  "CLASS_role_parser HIT",
  "CLASS_sparse_index HIT",
  "CLASS_desc_retrieval HIT",
  "CLASS_shared_scorer HIT",
  "CLASS_typed_proof HIT",
  "CLASS_pending_reward HIT",
  "CLASS_persist HIT",
  "CLASS_evid_materializer HIT",
  "CLASS_lm06_gen HIT",
  "CLASS_uart_egress HIT",
  "CLASS_one_ddr_owner HIT",
  "CLASS_six_clients HIT",
  "CLASS_no_qid_map HIT",
  "CLASS_no_host_winner HIT",
  "CLASS_no_a09_top HIT",
  "CLASS_no_plant_in_dut HIT",
  "CLASS_ckpt_addr_06000000 HIT",
  "CLASS_ans_matches_planted_dest HIT",
  "CLASS_gen_tok0_glue_from_weights HIT",
  "CLASS_gen_tok1_hop2_obj_name HIT",
  "CLASS_tok0_not_ans_dict HIT",
...
if ($hasDiv -or (-not $hasPass) -or ($xsimExit -ne 0) -or ($missing.Count -gt 0)) {
  ...
  throw "C5QTMIG_XSIM_CONTROL_FAIL pass=$hasPass div=$hasDiv exit=$xsimExit missing=$($missing -join ',')"
}
Write-Host "ASTRA_C5_PROD_TOP_QPTEXT_MIG_RUN_OK PASS_THIS_GATE_ONLY PROGRAM=NO MIG_XSIM=1 C5_MASTER=OPEN"
