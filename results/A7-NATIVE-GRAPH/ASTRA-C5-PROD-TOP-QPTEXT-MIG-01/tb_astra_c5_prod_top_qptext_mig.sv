`timescale 1ps / 100fs
// ASTRA-C5-PROD-TOP-QPTEXT-MIG-01. PROGRAM=NO. Bag-local TB.
// Official Digilent AXI MIG + ddr3_model. Plants compact 2-hop via MIG AW.
// Instantiates existing a7ng_astra_c5_prod_top_c4q (not edited). Not BOARD. Not Master.
...
module tb_astra_c5_prod_top_qptext_mig;
  ...
  logic [3:0] c3st, pph;
  logic [7:0] c3obj, glast, gtok0, gtok1, gold0, glue;
  logic [19:0] c3ans;
  logic [15:0] c5_wstrb;
  ...
  a7ng_astra_c5_prod_top_c4q u_dut (
    ...
    .c3_ans_o(c3ans), .c3_result_v_o(c3res), .c3_pend_cmt_o(pcmt),
  ...
  initial begin
    ...
    $display("C5_PROD_TOP_QPTEXT_MIG PROGRAM=NO BOARD_PASS=REJECT C5_MASTER=OPEN MIG_XSIM=1 TO_CYC_TB_DEFPARAM=65535");
    ...
    dut_rst_n = 1'b1;
    wait_ui(16);
    $display("C5_PROD_TOP_QPTEXT PROGRAM=NO BOARD_PASS=REJECT DDR_QUERY_BOUND_FINAL=NOT_FROZEN MIG=YES A09=NO");
    ...
    $display("MEAS nurx=%0d nutx=%0d seen=%02h dual=%0d st=%0d obj=%0d ans=%0d tok0=%0d tok1=%0d glue=%0d gold0=%0d gdone=%0d nht=%0d last_aw=%h nsw=%0d guard=%0d",
      nurx, nutx, seen, dual, c3st, c3obj, c3ans, gtok0, gtok1, glue, gold0, gdone, nht, last_aw, nsw, guard);
    ...
    chk("ANS64", c3ans == C3_SHARED_DST[19:0]);
    chk("GLUE61", gdone && (gtok0 === glue) && (gtok0 !== gold0));
    chk("TOK1", gdone && (gtok1 === gold0));
    chk("UART_TX", nutx != 0);
    chk("SIX", seen == 6'h3F);
    chk("DUAL0", !dual);
    chk("NOQID", !qid);
    ...
    if (fail == 0) $display("ASTRA_C5_PROD_TOP_QPTEXT_MIG_XSIM_PASS");
    else $display("ASTRA_C5_PROD_TOP_QPTEXT_MIG_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO DDR_QUERY_BOUND_FINAL=NOT_FROZEN quality=MIG_PHY_PLANTED_2HOP_GLUE_PLUS_OBJECT UART_SIM_BAUD COMPACT_LM_NOT_802K");
    $finish;
  end

  initial begin
    #10s;
    $display("FIRST_DIVERGENCE MIG_TIMEOUT calib_or_c5_qptext hung");
    $display("RESULT=FAIL");
    $display("ASTRA_C5_PROD_TOP_QPTEXT_MIG_XSIM_PASS ABSENT");
    $finish;
  end
endmodule
