#!/usr/bin/env python3
"""ASTRA-C1-N4096-INTERSECT-01 independent host gold for qse-v2-intersect-01.

Same law as KEY-INTERSECT (N=256). Scale unknown only. N=4096.
Frozen extract instantiated unchanged. Relbind keys instantiated, not edited.
Walker emits k0∩k1 when both valid. Does NOT union k0 with k1. Does NOT
probe k2/k3 as the exclusion mechanism. Index rebuilt with the SAME
relbind function.
...
inspect/generate N=16384; patch frozen C0 RTL; leftover A09 as DUT;
regenerate GOLDEN/corpus/query_gold after FAIL; emit reduction=1-CAND_CAP/N;
reuse query "supply duct" with gold {72,73,74,75}; threshold drop;
fake context keys; k2/k3-only rebind as exclusion; edit KEEP bags.
PROGRAM=NO. BIT=NO.
G_BYTES: LSB-first (char0 at bits[7:0]) matching TB bytes[8*bi +: 8].
Posting-selectivity (if reported) = emit vs posting occupancy, labeled.
"""
...
def is_two_role_or_psc(s: int, r: int, o: int) -> bool:
    psc = s == 10 and r == 1 and o == 1
    wo = s == 10 and r == 1 and o != 1
    wr = s == 10 and r != 1 and o == 1
    we = s != 10 and r == 1 and o == 1
    return psc or wo or wr or we


def build_corpus() -> list[dict]:
    docs: list[dict] = []
    used: set[tuple[int, int, int]] = set()
    reserved = {(12, 3, 8), (12, 3, 9), (6, 2, 1)}
    ...
    for s in range(1, 13):
        for r in range(1, 4):
            for o in range(1, 13):
                if s == o:
                    continue
                if (s, r, o) in reserved:
                    continue
                add(triple_text(s, r, o), 1, "unique", (s, r, o))
    ...
    for _ in range(OVF_TAIL):
        add(ovf_txt, 0, "ovf_tail")

    fillers: list[tuple[int, int, int]] = []
    for s in range(1, 13):
        for r in range(1, 4):
            for o in range(1, 13):
                if s == o:
                ...
                if (s, r, o) in reserved:
                    continue
                if is_two_role_or_psc(s, r, o):
                    continue
                fillers.append((s, r, o))
    if not fillers:
        raise SystemExit("no pad fillers")

    fi = 0
    while len(docs) < N - 1:
        s, r, o = fillers[fi % len(fillers)]
...
def walk_one_table(q: dict, t: int, heads, overflow, ovf_flag, post_len) -> dict:
    """Frozen dir twin for a single table. Does not union other tables."""
    seen: list[int] = []
...
def route(q: dict, heads, overflow, ovf_flag, post_len) -> dict:
    """qse-v2-intersect-01: emit k0∩k1 when both valid. Never union k2/k3."""
    v0 = bool(q["k0_valid"])
    v1 = bool(q["k1_valid"])
    if v0 and v1:
        a = walk_one_table(q, 0, heads, overflow, ovf_flag, post_len)
        b = walk_one_table(q, 1, heads, overflow, ovf_flag, post_len)
        bset = set(b["emit"])
        emit: list[int] = []
        ntrunc_x = 0
        for nid in a["emit"]:
            if nid in bset:
                if len(emit) >= CAND_CAP:
                    ntrunc_x += 1
                else:
                    emit.append(nid)
        n_dir = a["n_dir"] + b["n_dir"]
        n_post = a["n_post"] + b["n_post"]
        return {
            "emit": emit,
            "n_emit": len(emit),
            "n_dir": n_dir,
            "n_post": n_post,
            "n_dup": a["n_dup"] + b["n_dup"],
            "n_trunc": a["n_trunc"] + b["n_trunc"] + ntrunc_x,
            "overflow_flag": 1 if (a["overflow_flag"] or b["overflow_flag"]) else 0,
            "occupancies": a["occupancies"] + b["occupancies"],
            "posting_lens": a["posting_lens"] + b["posting_lens"],
            "probed": a["probed"] + b["probed"],
            "bytes": (n_dir + n_post) * ENTRY,
        }
    if v0:
        a = walk_one_table(q, 0, heads, overflow, ovf_flag, post_len)
        a["bytes"] = (a["n_dir"] + a["n_post"]) * ENTRY
        return a
    if v1:
        b = walk_one_table(q, 1, heads, overflow, ovf_flag, post_len)
        b["bytes"] = (b["n_dir"] + b["n_post"]) * ENTRY
        return b
    return {
        "emit": [],
        "n_emit": 0,
...
def table_posting(docs: list[dict], key_field: str, key: int, valid_field: str) -> list[int]:
    return [d["nid"] for d in docs if d[valid_field] and d[key_field] == key]
...
def write_svh(path: Path, queries: list[dict], writes: dict[int, int], docs: list[dict]) -> None:
    ...
    lines = [
        "// generated by host_astra_c1_n4096_intersect.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
        "// Law qse-v2-intersect-01 N=4096: emit k0∩k1 when both valid; relbind keys reused; no k0∪k1 union",
    ...
    lines.append(wrap_items(rel_rows, 1))
    lines.append("};")
    lines.append("")
...
def posting_sel_row(q: dict) -> dict:
    occ = max(q["occupancies"] or [0])
    return {
        "label": "emit_vs_posting_occupancy",
        "emit_n": q["n_emit"],
        "posting_occ": occ,
        "n": N,
        "cand_cap": CAND_CAP,
        "posting_bytes": q["bytes"],
        "not_one_minus_cap_over_n": True,
    }
...
def main() -> int:
    ...
    chk(docs[N - 1]["nid"] == 4095, "sentinel nid 4095")
    ...
    queries_spec = [
        ...
        ("high_id_sentinel", "sensor connects tower", lambda d: d["nid"] == N - 1),
    ]
    ...
    chk(set(gold_map["direct"]) != {72, 73, 74, 75}, "direct gold not supply-duct")
    ...
    for name, text, pred in queries_spec:
        ...
        if q["k0_valid"] and q["k1_valid"]:
            p0 = table_posting(docs, "k0", q["k0"], "k0_valid")[:CAND_CAP]
            p1 = table_posting(docs, "k1", q["k1"], "k1_valid")[:CAND_CAP]
            indep = [nid for nid in p0 if nid in set(p1)][:CAND_CAP]
            chk(q["emit"] == indep, f"{name} emit != independent k0∩k1 prefix {q['emit']} vs {indep}")
        if name == "distractor":
        ...
        q["posting_selectivity"] = posting_sel_row(q)
    ...
    chk(qd["paraphrase"]["k2"] == qd["direct"]["k2"] and qd["paraphrase"]["k3"] == qd["direct"]["k3"], "para relbind keys")
    ...
    chk(qd["role_reversal"]["k2"] == qd["direct"]["k3"], "reverse relbind k2k3")
    chk(qd["role_reversal"]["k3"] == qd["direct"]["k2"], "reverse relbind k3k2")
    chk(qd["direct"]["k2"] != qd["direct"]["k2_frozen"], "relbind k2 did not change vs frozen cue")
    chk(qd["direct"]["k3"] != qd["direct"]["k3_frozen"], "relbind k3 did not change vs frozen cue")
    chk(qd["direct"]["k2"] == (((qd["direct"]["rel_id"] & 0xFF) << 8) | (qd["direct"]["subj_cue"] & 0xFF)), "k2 packing")
    chk(qd["direct"]["k3"] == (((qd["direct"]["rel_id"] & 0xFF) << 8) | (qd["direct"]["obj_cue"] & 0xFF)), "k3 packing")
    chk(qd["direct"]["k0"] == (((qd["direct"]["subj_id"] & 0xFF) << 8) | (qd["direct"]["rel_id"] & 0xFF)), "k0 packing")
    chk(qd["direct"]["k1"] == (((qd["direct"]["obj_id"] & 0xFF) << 8) | (qd["direct"]["rel_id"] & 0xFF)), "k1 packing")
    chk(qd["wrong_relation"]["k0"] != qd["direct"]["k0"], "wrongrel key collapse")
    chk(qd["wrong_relation"]["k2"] != qd["direct"]["k2"], "wrongrel k2 still cue-only")
    ...
    if (N - 1) not in qd["high_id_sentinel"]["emit"]:
        chk(
            (qd["high_id_sentinel"]["n_trunc"] > 0) or qd["high_id_sentinel"]["overflow_flag"],
            "sentinel not emitted without budget",
        )
    if ovf_gold[0]["nid"] not in qd["overflow_page"]["emit"]:
        chk(
            (qd["overflow_page"]["n_trunc"] > 0) or qd["overflow_page"]["overflow_flag"],
            "ovf gold not emitted without budget",
        )
    ...
    chk(set(gold_map["direct"]).issubset(set(qd["direct"]["emit"])), "direct gold not subset of intersect emit")

    wc = qd["wrong_context"]
    dd = qd["direct"]
    chk(
        wc["k0"] == dd["k0"] and wc["k1"] == dd["k1"] and wc["k2"] == dd["k2"] and wc["k3"] == dd["k3"],
        "wrong_context keys != direct (relbind still has no ctx key)",
    )
    chk(wc["emit"] == dd["emit"], "wrong_context emit != direct (xid still not a directory key)")
    chk(qd["distractor"]["emit"] == dd["emit"], "distractor emit != direct (same query tokens)")

    if fails:
        print("ASTRA_C1_N4096_INTERSECT_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        return 2
    ...
    def qpub(q: dict) -> dict:
        row = {
            ...
            "posting_selectivity": q["posting_selectivity"],
        }
    ...
    golden = {
        ...
        "key_rebind": "relbind reused; walker=k0_intersect_k1; k2k3_not_probed",
        "intersect_law": "emit ids in k0 AND k1 when both valid; no k0 union k1",
        "n": N,
        "cand_cap": CAND_CAP,
        "index_head": INDEX_HEAD,
        "n_host": 0,
    ...
    print("ASTRA_C1_N4096_INTERSECT_HOST_PASS")
    print("N", N, "CAND_CAP", CAND_CAP, "INDEX_HEAD", INDEX_HEAD, "WRITES", len(writes), "MEM_DEPTH", MEM_DEPTH)
    ...
    for q in queries:
        ...
        print(
            f"POSTING_SELECTIVITY class={q['name']} emit_n={q['n_emit']} "
            f"posting_occ={occ} n={N} cand_cap={CAND_CAP} labeled=emit_vs_posting_occupancy"
        )
    return 0
