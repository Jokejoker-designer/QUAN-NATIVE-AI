...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C1N4096_FILE_MISSING $p" }
...
if ($hashFail) {
  ...
  throw "C1N4096_HASH_GATE_FAIL do_not_invent"
}
...
$tb = Join-Path $bag "tb_astra_c1_n4096_intersect.sv"
$hostPy = Join-Path $bag "host_astra_c1_n4096_intersect.py"
...
if (-not (Test-Path -LiteralPath $goldJson)) { throw "C1N4096_GOLD_MISSING GOLDEN.json" }
if (-not (Test-Path -LiteralPath $goldSvh)) { throw "C1N4096_GOLD_MISSING query_gold.svh" }
if (-not (Test-Path -LiteralPath $corpus)) { throw "C1N4096_GOLD_MISSING corpus.json" }
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C1N4096_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$liveSvh = Sha256 $goldSvh
$liveCorpus = Sha256 $corpus
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C1N4096_GOLD_HASH_DRIFT GOLDEN.json" }
if ($preTxt -notmatch $liveSvh) { throw "C1N4096_GOLD_HASH_DRIFT query_gold.svh" }
if ($preTxt -notmatch $liveCorpus) { throw "C1N4096_GOLD_HASH_DRIFT corpus.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") {
    throw "C1N4096_LEFTOVER_A09_ON_XVLOG_LIST"
  }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") {
    throw "C1N4096_FROZEN_AXI_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_relbind.sv") {
    throw "C1N4096_RELBIND_SPARSE_MUST_NOT_BE_DUT"
  }
}
...
$pre = @(
  ...
  "# PROGRAM=NO C1_800K=OPEN CAND_CAP=16 N=4096 LAW=qse-v2-intersect-01 POKE_V=0",
  "# HASH_GATE MATCH vs C0 FINAL_CONTRACT (frozen files not patched)",
  "# DUT a7ng_query_axi_sparse_intersect MATCH KEY-INTERSECT live SHA",
  "# FROZEN_RTL"
)
foreach ($rel in @(
...
$pre += "$liveGold  results/A7-NATIVE-GRAPH/ASTRA-C1-N4096-INTERSECT-01/GOLDEN.json"
$pre += "$liveSvh  results/A7-NATIVE-GRAPH/ASTRA-C1-N4096-INTERSECT-01/query_gold.svh"
$pre += "$liveCorpus  results/A7-NATIVE-GRAPH/ASTRA-C1-N4096-INTERSECT-01/corpus.json"
$pre += "# BAG"
...
$pre += "# POSTING_SELECTIVITY labeled emit_vs_posting_occupancy"
$pre += "# DISTRACTOR_GOLD_POLARITY excluded leak=FAIL"
[System.IO.File]::WriteAllLines((Join-Path $bag "SHA256.txt"), $pre)
Write-Host "C1N4096_SHA_FROZEN_BEFORE_XVLOG"

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C1N4096_XVLOG_NOT_FOUND" }
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C1_N4096_INTERSECT_XSIM_PASS" -Quiet)
...
if ($hasRedCap) {
  ...
  throw "C1N4096_REDUCTION_X1000_EMITTED_AS_CAP_N"
}
if (($xsimExit -ne 0) -or (-not $hasPass) -or $hasDiv -or $hasLeak) {
  $r0 = Join-Path $bag "xsim_fail_r0.log"
  if ((Test-Path -LiteralPath $xsimLog) -and (-not (Test-Path -LiteralPath $r0))) {
    Copy-Item $xsimLog $r0 -Force
  }
  throw "C1N4096_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C1_N4096_INTERSECT_RUN_OK"
