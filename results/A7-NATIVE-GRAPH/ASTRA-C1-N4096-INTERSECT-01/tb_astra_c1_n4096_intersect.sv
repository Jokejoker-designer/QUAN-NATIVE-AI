// tb_astra_c1_n4096_intersect.sv — C1 N=4096 qse-v2-intersect-01. PROGRAM=NO.
// Frozen QSE + relbind keys + k0∩k1 walker. poke_v=0. leftover A09 off.
// G_BYTES LSB-first: char0 at [7:0]; extract bytes[8*bi +: 8] only.
// Distractor: G_GOLD_EXCLUDED=1; G_RELEVANT is EXCLUDED set; leak=FAIL not tp.
// MEM_DEPTH from gold (TB-only; must fit 4096-record index).
`timescale 1ns / 1ps

module tb_astra_c1_n4096_intersect;
  import a7ng_pkg::*;
  ...
  initial begin
    ...
    if (G_N !== 4096)
      diverge("SELECTIVITY_TAUTOLOGY", "G_N!=4096");
    if (G_NREL[5] < 1)
    ...
    $display("C1_N4096_INTERSECT_N=%0d CAND_CAP=%0d INDEX_HEAD=%0d LAW=qse-v2-intersect-01 POKE_V=0 REDUCTION_X1000=NOT_EMITTED MEM_DEPTH=%0d",
    ...
    for (qi = 0; qi < G_NQ; qi = qi + 1) begin
      ...
      $display("POSTING_SELECTIVITY class=%s emit_n=%0d posting_occ=%0d n=%0d cand_cap=%0d labeled=emit_vs_posting_occupancy",
        cname(qi), n_got, G_OCC[qi], G_N, CAND_CAP);
      ...
      if (qi == 9) begin
        ...
        if (!found && !incomp)
          diverge("CANDIDATE_ID_MISMATCH", "high_id_sentinel 4095 missing");
      end
    ...
    if (fail == 0 && dist_gold_ok && dist_excl_ok && dist_no_leak && unrelated_empty && wc_not_sel && (direct_tp > 0)) begin
      $display("ASTRA_C1_N4096_INTERSECT_XSIM_PASS");
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,N_16384");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end else begin
      $display("ASTRA_C1_N4096_INTERSECT_XSIM_NO_MARKER fail=%0d dist_gold_ok=%0d dist_excl_ok=%0d dist_no_leak=%0d unrelated_empty=%0d wc_not_sel=%0d direct_tp=%0d",
