// ASTRA-09-R6-UART-ISO-PUBLIC-01. PROGRAM=NO. Bag-local TB.
// Instantiates a7ng_astra_09_r6_uart_iso_public_wrap. UART 8N1 into
// public iso ports. Inner u_sgd is the one SGD. Not BOARD_PASS.
`timescale 1ns / 1ps

module tb_astra_09_r6_uart_iso_public;
  ...
  a7ng_astra_09_r6_uart_iso_public_wrap dut (
    .CLK100MHZ(CLK100MHZ),
    .sw(sw),
    .btn(btn),
    .led(led),
  ...
  always @(posedge dut.clk) begin
    ...
    if (dut.inner_dn)
      $display("INNER_DONE w0=%0d v=%0d w1=%0d tbl=%0d pub_w0=%0d t=%0t",
        dut.inner_w0, dut.inner_v, dut.inner_w1, dut.tbl, dut.w_pub[0], $time);
  end
  ...
  task automatic decode_iso(
    ...
    begin
      ...
      tbl_u = fr[1][7];
      iso_u = fr[1][6];
      $display("%s w0=%0d v=%0d w1=%0d x0=%0d rew=%0d tbl=%0d iso=%0d inner_w0=%0d hier_sgd_w0=%0d pub_w0=%0d hier_tbl=%0d",
        tag, w0_u, v_u, w1_u, x0_u, rew_u, tbl_u, iso_u,
        dut.inner_w0, dut.u_r6.u_sgd.w_o[0], dut.w_pub[0], dut.tbl);
      ...
      chk({tag, "_HIER_SGD_W0"}, dut.u_r6.u_sgd.w_o[0] === exp_w0);
      chk({tag, "_PUB_W0_IS_INNER"}, dut.w_pub[0] === exp_w0);
  ...
  initial begin
    ...
    btn = 4'd0;
    uart_txd_in = 1'b1;
    $display("TB ASTRA-09-R6-UART-ISO-PUBLIC DUT=a7ng_astra_09_r6_uart_iso_public_wrap INNER=a7ng_astra_09_r6_iso_pub.u_sgd NO_SIBLING_SGD=1 PORT_DRIVEN=1 PROGRAM=NO");
    ...
    if (dut.rst_n !== 1'b1) begin
      $display("ASTRA_09_R6_UART_ISO_PUBLIC_FAIL n=%0d first=%s", fail, first_div);
      $finish;
    end
    ...
    chk("ISO_P3_X50_DW5", (dut.inner_w0 === 16'sd5) && (dut.inner_w1 === 16'sd0)
        ...
        && (dut.u_r6.u_sgd.w_o[0] === 16'sd5)
        && (dut.w_pub[0] === 16'sd5));

    wrap_rst();
    chk("ISO_M3_PRE_W0", (dut.inner_w0 === 16'sd0)
        && (dut.u_r6.u_sgd.w_o[0] === 16'sd0));
    ...
    chk("ISO_M3_X64_DW6", (dut.inner_w0 === -16'sd6) && (dut.inner_w1 === 16'sd0)
        && (dut.inner_v === 16'sd0) && (dut.tbl === 1'b0)
        && (dut.u_r6.u_sgd.w_o[0] === -16'sd6)
        && (dut.w_pub[0] === -16'sd6));

    if (fail == 0)
      $display("ASTRA_09_R6_UART_ISO_PUBLIC_PASS");
    else
      $display("ASTRA_09_R6_UART_ISO_PUBLIC_FAIL n=%0d first=%s", fail, first_div);
    $finish;
  end
