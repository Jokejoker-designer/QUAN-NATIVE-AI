// tb_astra01_query_axi_sparse.sv
// ASTRA-01: raw tokens → RTL QSE+valid-gate+4x4096 AXI dir. PROGRAM=NO.
`timescale 1ns / 1ps

module tb_astra01_query_axi_sparse;
  import a7ng_pkg::*;
  ...
  logic [15:0] k0, k1, k2, k3, n_host_any;
  logic v0, v1, v2, v3;
  ...
  task automatic observe_walk(input int q);
    integer lane;
    begin
  ...
  initial begin
    ...
    poke_v = 0; pk0 = 0; pk1 = 0; pk2 = 0; pk3 = 0;
    ...
    for (qi = 0; qi < G_N_Q; qi = qi + 1) begin
      if (G_MODE[qi] == 0) begin
        ...
        if (eid !== G_EID[qi] || iid !== G_IID[qi] || rid !== G_RID[qi] || xid !== G_XID[qi])
          diverge("PACKET_MISMATCH", $sformatf("q=%0d pkt %0d %0d %0d %0d", qi, eid, iid, rid, xid));
        observe_walk(qi);
        check_cuts(qi);
        retire_qse();
      end else begin
    ...
    $display("ASTRA01_AXI_SPARSE_PASS");
    $display("CLAIM=raw FPGA query packet drives 4x4096 AXI sparse dir/posting and matches host-golden IDs");
    $display("EXTRACTOR_WIRE=YES");
    $display("NOT_CLAIMED=800k,P4_quality,role_parse,board");
    #20 $finish;
  end
endmodule
