#!/usr/bin/env python3
"""ASTRA-01 host golden: frozen corpus index + cuts A-F including query packet.

Independent of RTL output. Law qse-v1-lexicon-hdc-00 / P4_4k_h64 / U4A-R6 validity.
Does not reopen U4A-R4/R5/R6 or schemaV2.
"""
...
def freeze_check(name, text, q):
    ...
    ids = g.get("ids", {})
    if ids:
        if q["eid"] != ids.get("entity_id", q["eid"]):
            raise SystemExit(f"PACKET_MISMATCH entity {name}")
        if q["iid"] != ids.get("intent_id", q["iid"]):
            raise SystemExit(f"PACKET_MISMATCH intent {name}")
        if q["rid"] != ids.get("relation_id", q["rid"]):
            raise SystemExit(f"PACKET_MISMATCH relation {name}")
        if q["xid"] != ids.get("context_id", q["xid"]):
            raise SystemExit(f"PACKET_MISMATCH context {name}")
...
def main() -> int:
    ...
    lines.append(
        "localparam logic [7:0] G_EID [0:G_N_Q-1] = '{"
        + ", ".join(f"8'd{r['eid']}" for r in queries)
        + "};"
    )
    lines.append(
        "localparam logic [7:0] G_IID [0:G_N_Q-1] = '{"
        + ", ".join(f"8'd{r['iid']}" for r in queries)
        + "};"
    )
    lines.append(
        "localparam logic [7:0] G_RID [0:G_N_Q-1] = '{"
        + ", ".join(f"8'd{r['rid']}" for r in queries)
        + "};"
    )
    lines.append(
        "localparam logic [7:0] G_XID [0:G_N_Q-1] = '{"
        + ", ".join(f"8'd{r['xid']}" for r in queries)
        + "};"
    )
    ...
    print("ASTRA01_HOST_GOLDEN_DONE")
