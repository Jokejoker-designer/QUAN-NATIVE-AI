...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C1SEMUNSEEN16KR2_FILE_MISSING $p" }
...
if ($hashFail) {
  ...
  throw "C1SEMUNSEEN16KR2_HASH_GATE_FAIL do_not_invent do_not_edit_keep_rtl"
}
...
$tb = Join-Path $bag "tb_astra_c1_semantic_unseen_sro_16k_r2.sv"
$hostPy = Join-Path $bag "host_astra_c1_semantic_unseen_sro_16k_r2.py"
...
if (-not (Test-Path -LiteralPath $goldJson)) { throw "C1SEMUNSEEN16KR2_GOLD_MISSING GOLDEN.json" }
if (-not (Test-Path -LiteralPath $goldSvh)) { throw "C1SEMUNSEEN16KR2_GOLD_MISSING query_gold.svh" }
if (-not (Test-Path -LiteralPath $corpus)) { throw "C1SEMUNSEEN16KR2_GOLD_MISSING corpus.json" }
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C1SEMUNSEEN16KR2_GOLD_NOT_HASHED_BEFORE_XVLOG" }
if (-not (Test-Path -LiteralPath $lexNamed)) { throw "C1SEMUNSEEN16KR2_LEX_MISSING qse_role_lexicon_semantic_16k.svh" }
if (-not (Test-Path -LiteralPath $lexShim)) { throw "C1SEMUNSEEN16KR2_LEX_SHIM_MISSING qse_role_lexicon.svh (NEW law include-name, not C0)" }
if (-not (Test-Path -LiteralPath $ctxDut)) { throw "C1SEMUNSEEN16KR2_DUT_MISSING a7ng_query_axi_sparse_intersect_context.sv" }
if (-not (Test-Path -LiteralPath $ctxKeys)) { throw "C1SEMUNSEEN16KR2_KEYS_MISSING a7ng_query_role_keys_ctx.sv" }
...
if ($preTxt -notmatch $liveGold) { throw "C1SEMUNSEEN16KR2_GOLD_HASH_DRIFT GOLDEN.json" }
if ($preTxt -notmatch $liveSvh) { throw "C1SEMUNSEEN16KR2_GOLD_HASH_DRIFT query_gold.svh" }
if ($preTxt -notmatch $liveCorpus) { throw "C1SEMUNSEEN16KR2_GOLD_HASH_DRIFT corpus.json" }
if ($preTxt -notmatch $liveLex) { throw "C1SEMUNSEEN16KR2_GOLD_HASH_DRIFT qse_role_lexicon_semantic_16k.svh" }
if ($liveCorpus -ne "6991adc75ffc4d0c50bdf780f455bac5a8eb97ecf4e575d49f42f1716e0aa597") {
  throw "C1SEMUNSEEN16KR2_CORPUS_NOT_KEEP_COPY live=$liveCorpus"
}
if ($liveLex -ne "df0e8833ff9664cd4e21a6aa6d3d223112c8d6733871d6398b133e37296e1aa4") {
  throw "C1SEMUNSEEN16KR2_NAMED_LEX_NOT_KEEP_COPY live=$liveLex"
}
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") {
    throw "C1SEMUNSEEN16KR2_LEFTOVER_A09_ON_XVLOG_LIST"
  }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") {
    throw "C1SEMUNSEEN16KR2_FROZEN_AXI_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_relbind.sv") {
    throw "C1SEMUNSEEN16KR2_RELBIND_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect.sv") {
    throw "C1SEMUNSEEN16KR2_KEY_INTERSECT_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") {
    throw "C1SEMUNSEEN16KR2_STREAM02_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_page_skip.sv") {
    throw "C1SEMUNSEEN16KR2_PAGESKIP_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_role_keys_relbind.sv") {
    throw "C1SEMUNSEEN16KR2_RELBIND_KEYS_MUST_NOT_BE_COMPILED"
  }
}
...
$pre = @(
  ...
  "# PROGRAM=NO C1_800K=OPEN CAND_CAP=16 N=16384 N_BUCKETS=65536 LAW=qse-v2-intersect-context-02 LEX=qse-v2-lex-semantic-16k-01 POKE_V=0 R2_8_OF_8",
...
$pre += "$liveGold  results/A7-NATIVE-GRAPH/ASTRA-C1-SEMANTIC-UNSEEN-SRO-16K-R2-01/GOLDEN.json"
$pre += "$liveSvh  results/A7-NATIVE-GRAPH/ASTRA-C1-SEMANTIC-UNSEEN-SRO-16K-R2-01/query_gold.svh"
$pre += "$liveCorpus  results/A7-NATIVE-GRAPH/ASTRA-C1-SEMANTIC-UNSEEN-SRO-16K-R2-01/corpus.json"
$pre += "$liveLex  results/A7-NATIVE-GRAPH/ASTRA-C1-SEMANTIC-UNSEEN-SRO-16K-R2-01/qse_role_lexicon_semantic_16k.svh"
$pre += "# BAG"
...
Write-Host "C1SEMUNSEEN16KR2_SHA_FROZEN_BEFORE_XVLOG"

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C1SEMUNSEEN16KR2_XVLOG_NOT_FOUND" }
...
$hasFillHit = $false
$hasUnseenHit = $false
$hasUnboundEmpty = $false
$hasIncomp = $false
$hasNDrop = $false
$hasEight = $false
$unseenHitN = 0
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C1_SEMANTIC_UNSEEN_SRO_16K_R2_XSIM_PASS" -Quiet)
  ...
  $hasUnseenFail = [bool](Select-String -Path $xsimLog -Pattern "FAIL UNSEEN_SRO_" -Quiet)
  $hasLeak = [bool](Select-String -Path $xsimLog -Pattern "FAIL UNBOUND_FILL_GRID_LEAK" -Quiet)
  $hasFillHit = [bool](Select-String -Path $xsimLog -Pattern "FILL_TEMPLATE_HIT" -Quiet)
  $hasUnseenHit = [bool](Select-String -Path $xsimLog -Pattern "UNSEEN_SRO_HIT tp=" -Quiet)
  $hasUnboundEmpty = [bool](Select-String -Path $xsimLog -Pattern "UNBOUND_EMPTY_WALK" -Quiet)
  ...
  $hasEight = [bool](Select-String -Path $xsimLog -Pattern "UNSEEN_SRO_8_OF_8_HIT" -Quiet)
  $unseenHitN = @(Select-String -Path $xsimLog -Pattern "UNSEEN_SRO_HIT tp=").Count
}
if ($hasRedCap) {
  ...
  throw "C1SEMUNSEEN16KR2_REDUCTION_X1000_EMITTED_AS_CAP_N"
}
if ($hasNDrop) {
  $r0 = Join-Path $bag "xsim_fail_r0.log"
  if ((Test-Path -LiteralPath $xsimLog) -and (-not (Test-Path -LiteralPath $r0))) {
  ...
  throw "C1SEMUNSEEN16KR2_N_DROP_SILENT_256_FORBIDDEN"
}
if (($xsimExit -ne 0) -or (-not $hasPass) -or $hasDiv -or $hasLeak -or $hasFillFail -or $hasUnseenFail -or (-not $hasFillHit) -or (-not $hasUnseenHit) -or ($unseenHitN -ne 8) -or (-not $hasEight) -or (-not $hasUnboundEmpty) -or $hasIncomp) {
  $r0 = Join-Path $bag "xsim_fail_r0.log"
  if ((Test-Path -LiteralPath $xsimLog) -and (-not (Test-Path -LiteralPath $r0))) {
    Copy-Item $xsimLog $r0 -Force
  }
  throw "C1SEMUNSEEN16KR2_XSIM_FAIL_OR_MARKER_MISSING unseenHitN=$unseenHitN eight=$hasEight pass=$hasPass"
}
Write-Host "ASTRA_C1_SEMANTIC_UNSEEN_SRO_16K_R2_RUN_OK"
