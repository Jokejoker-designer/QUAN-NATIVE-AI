#!/usr/bin/env python3
"""ASTRA-C1-SEMANTIC-UNSEEN-SRO-16K-R2-01 independent host gold.

ONE UNKNOWN: N=16384 same KEEP plants 123-130; query EACH of 8 off-grid
SROs and all 8 HIT their nid; unbound SRO still UNBOUND_EMPTY_WALK and
must NOT emit fill-grid {120,121,122}. SEARCH_INCOMPLETE on any of 8
retrieve = FAIL. Instantiate ctx keys 124be808 + DUT 8255a798 not
edited. Named lexicon qse-v2-lex-semantic-16k-01 copied SHA df0e8833.
Corpus copied hashed from KEEP UNSEEN-SRO-16K-01 (SHA 6991adc7); do
not edit KEEP bag. leftover A09 off poke_v=0 PROGRAM=NO.

If XSim MEM cannot host 16384, FAIL honestly FIRST_DIVERGENCE MEM —
do not silently drop to N=256. C1 800k stays OPEN. BOARD_PASS not
claimed. Do not write independent audit tree. G_BYTES: LSB-first
(char0 at bits[7:0]) matching TB bytes[8*bi +: 8].
"""
...
GATE = "ASTRA-C1-SEMANTIC-UNSEEN-SRO-16K-R2-01"
...
# R2 queries ALL 8 (KEEP 16K only retrieved nid 123).
PLANTED = [
    (1, 1, 2),    # chiller supplies condenser     nid 123  RETRIEVE
    (1, 2, 3),    # chiller requires evaporator    nid 124  RETRIEVE
    (4, 1, 2),    # compressor supplies condenser  nid 125  RETRIEVE
    (6, 3, 7),    # ahu connects duct              nid 126  RETRIEVE
    (10, 1, 11),  # pump supplies valve            nid 127  RETRIEVE (not psc clone)
    (9, 8, 2),    # tower discharges condenser     nid 128  RETRIEVE
    (12, 5, 6),   # sensor isolates ahu            nid 129  RETRIEVE
    (3, 6, 4),    # evaporator bypasses compressor nid 130  RETRIEVE
]
...
def write_svh(
    ...
    unseen_i = next(i for i, q in enumerate(queries) if q["name"] == "unseen_sro_123")
    ...
    lines = [
        "// generated by host_astra_c1_semantic_unseen_sro_16k_r2.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
        f"// Law {LAW} + named lexicon {LAW_LEX}; STREAM-02 walker in context wrapper; N=16384 unseen SRO R2 8/8",
        ...
        f"localparam int unsigned G_FILL_Q = 0;",
        f"localparam int unsigned G_PLANTED_Q0 = 1;",
...
def main() -> int:
    ...
    for i, sro in enumerate(PLANTED):
        nid = UNSEEN_NID + i
        queries_spec.append(
            (f"unseen_sro_{nid}", triple_text(*sro), pred_sro(*sro))
        )
    queries_spec.extend(
        [
    ...
    for i, sro in enumerate(PLANTED):
        nid = UNSEEN_NID + i
        name = f"unseen_sro_{nid}"
        chk(gold_map[name] == [nid], f"{name} gold {gold_map[name]} want [{nid}]")
        chk(set(gold_map[name]).isdisjoint(set(gold_map["fill_template"])), f"{name} overlaps fill-grid")
    ...
    for i in range(1, 1 + len(PLANTED)):
        chk(FILL_RE.match(queries_spec[i][1]) is not None, f"unseen query {i} not SRO surface")
    chk(FILL_RE.match(queries_spec[1 + len(PLANTED)][1]) is not None, "unbound query not SRO surface")

    heads, overflow, ovf_flag, post_len = index_docs(docs)

    must_hit = {"fill_template"} | {f"unseen_sro_{n}" for n in PLANTED_NIDS}
    queries = []
    for name, text, pred in queries_spec:
        q = feat(text)
        q["name"] = name
    ...
    for i, sro in enumerate(PLANTED):
        nid = UNSEEN_NID + i
        name = f"unseen_sro_{nid}"
        chk(qd[name]["subj_id"] == sro[0], f"{name} subj")
        chk(qd[name]["rel_id"] == sro[1], f"{name} rel")
        chk(qd[name]["obj_id"] == sro[2], f"{name} obj")
        chk(
            qd[name]["k0"] != qd["fill_template"]["k0"] or qd[name]["k1"] != qd["fill_template"]["k1"],
            f"{name} keys == fill-grid",
        )
        chk(qd[name]["emit"] == [nid], f"{name} emit {qd[name]['emit']} want [{nid}]")
        chk(nid in qd[name]["emit"], f"{name} HIT miss nid={nid}")
        chk(qd[name]["tp"] == 1, f"{name} tp")
        chk(qd[name]["search_incomplete"] == 0, f"{name} SEARCH_INCOMPLETE")
    ...
    if fails:
        print("ASTRA_C1_SEMANTIC_UNSEEN_SRO_16K_R2_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        return 2
    ...
    def qpub(q: dict) -> dict:
        ...
        if q["name"].startswith("unseen_sro_"):
            want_nid = int(q["name"].rsplit("_", 1)[1])
            row["UNSEEN_SRO_HIT"] = want_nid in q["emit"]
            row["sro_in_cartesian_fill"] = False
            row["planted_nid"] = want_nid
    ...
    print("ASTRA_C1_SEMANTIC_UNSEEN_SRO_16K_R2_HOST_PASS")
    ...
    print("FILL_SRO_N", len(fill_sros), "PLANTED_N", len(PLANTED), "PLANTED_NIDS", PLANTED_NIDS, "FILL_GRID", list(FILL_GRID_NIDS))
