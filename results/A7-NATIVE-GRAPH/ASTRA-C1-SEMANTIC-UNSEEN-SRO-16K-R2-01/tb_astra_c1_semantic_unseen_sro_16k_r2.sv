// tb_astra_c1_semantic_unseen_sro_16k_r2.sv — C1 N=16384 unseen SRO R2 8/8. PROGRAM=NO.
...
// PASS marker only if FAIL=0 AND all 8 planted SROs HIT their nids 123-130
// AND fill-template control hits AND unbound does not emit {120,121,122}
// AND SEARCH_INCOMPLETE absent on any of 8 retrieve. Gold miss = FAIL.
...
module tb_astra_c1_semantic_unseen_sro_16k_r2;
  import a7ng_pkg::*;
  ...
  integer unrelated_empty, fill_n, fill_tp, unseen_tp, unseen_hit_n, unbound_empty, unbound_no_leak;
  ...
  function automatic string cname(input int q);
    if (q == 0)
      cname = "fill_template";
    else if ((q >= 1) && (q <= G_PLANTED_N))
      cname = $sformatf("unseen_sro_%0d", G_PLANTED_NIDS[q-1]);
    else if (q == G_UNBOUND_Q)
      cname = "unbound_sro";
  ...
  initial begin
    ...
    if (G_PLANTED_N != 8)
      diverge("PLANTED_N", "G_PLANTED_N!=8");
    if (G_NQ != (1 + G_PLANTED_N + 2))
      diverge("NQ", "G_NQ!=11 (fill+8 plants+unbound+unrelated)");
    if (G_UNSEEN_Q != 1)
      diverge("UNSEEN_IDX", "G_UNSEEN_Q!=1");
    if (G_PLANTED_Q0 != 1)
      diverge("PLANTED_Q0", "G_PLANTED_Q0!=1");
    if (G_UNBOUND_Q != 9)
      diverge("UNBOUND_IDX", "G_UNBOUND_Q!=9");
    if (G_UNRELATED_Q != 10)
      diverge("UNRELATED_IDX", "G_UNRELATED_Q!=10");
    for (i = 0; i < G_PLANTED_N; i = i + 1)
      if (G_PLANTED_NIDS[i] != (20'd123 + i))
        diverge("PLANTED_NIDS", $sformatf("G_PLANTED_NIDS[%0d]!=%0d", i, 123 + i));
    ...
    $display("C1_SEMANTIC_UNSEEN_SRO_16K_R2_N=%0d N_BUCKETS=%0d CAND_CAP=%0d INDEX_HEAD=%0d LAW=qse-v2-intersect-context-02 LEX=qse-v2-lex-semantic-16k-01 POKE_V=0 PAGE_BUFFER_BEATS=1 RARE_LIST_FIRST=1 CAND_CAP_AFTER_EMIT=1 REDUCTION_X1000=NOT_EMITTED CAND_CAP_FINAL=NOT_FROZEN DDR_QUERY_BOUND_FINAL=NOT_FROZEN MEM_DEPTH=%0d N_SUBJECTS=%0d N_RELS=%0d PLANTED_N=%0d UNSEEN_SRO=8 FILL_GRID=120,121,122 PLANTED_NIDS=123,124,125,126,127,128,129,130",
      G_N, G_N_BUCKETS, CAND_CAP, G_INDEX_HEAD, MEM_DEPTH, G_N_SUBJECTS, G_N_RELS, G_PLANTED_N);
    ...
    for (qi = 0; qi < G_NQ; qi = qi + 1) begin
      ...
      if ((qi >= 1) && (qi <= G_PLANTED_N)) begin
        unseen_tp = tp;
        found = 1'b0;
        for (i = 0; i < n_got; i = i + 1)
          if (got[i] === G_PLANTED_NIDS[qi-1]) found = 1'b1;
        if (unseen_tp < 1 || !found) begin
          $display("FAIL UNSEEN_SRO_GOLD_MISS tp=%0d nid=%0d", unseen_tp, G_PLANTED_NIDS[qi-1]);
          fail = fail + 1;
        end else begin
          $display("UNSEEN_SRO_HIT tp=%0d emit_n=%0d nid=%0d", unseen_tp, n_got, G_PLANTED_NIDS[qi-1]);
          unseen_hit_n = unseen_hit_n + 1;
        end
        if (k0_cx === G_K0[0] && k1_cx === G_K1[0])
          diverge("KEY_MISMATCH", $sformatf("unseen keys identical to fill-grid q=%0d nid=%0d", qi, G_PLANTED_NIDS[qi-1]));
      end
      if (qi == G_UNBOUND_Q) begin
        if (n_got == 0) begin
    ...
    $display("UNSEEN_SRO_HIT_N=%0d OF %0d", unseen_hit_n, G_PLANTED_N);
    if (fail == 0 && unrelated_empty && (fill_tp > 0) && fill_ids_ok && (unseen_hit_n == G_PLANTED_N) && unbound_empty && unbound_no_leak && (incomp_retrieve == 0) && (G_N == 16384) && (G_PLANTED_N == 8)) begin
      $display("UNSEEN_SRO_8_OF_8_HIT");
      $display("ASTRA_C1_SEMANTIC_UNSEEN_SRO_16K_R2_XSIM_PASS");
      ...
      $display("ASTRA_C1_SEMANTIC_UNSEEN_SRO_16K_R2_XSIM_NO_MARKER fail=%0d unrelated_empty=%0d fill_tp=%0d fill_ids_ok=%0d unseen_hit_n=%0d unbound_empty=%0d unbound_no_leak=%0d incomp_retrieve=%0d G_N=%0d planted=%0d",
        fail, unrelated_empty, fill_tp, fill_ids_ok, unseen_hit_n, unbound_empty, unbound_no_leak, incomp_retrieve, G_N, G_PLANTED_N);
      $display("NOT_CLAIMED=C1_800k,BOARD_PASS,ASTRA-13,CAND_CAP_FINAL,DDR_QUERY_BOUND_FINAL,PAGE_SKIP_MIX,N_65536,MASTER_95,ACCEPT_BOARD");
      $display("C1_800K=OPEN BOARD_PASS=NOT_CLAIMED PROGRAM=NO");
      $display("REDUCTION_X1000=NOT_EMITTED");
    end
