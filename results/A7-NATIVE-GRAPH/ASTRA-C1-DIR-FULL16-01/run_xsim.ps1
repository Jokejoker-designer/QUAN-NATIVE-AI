...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C1DIRFULL16_FILE_MISSING $p" }
...
if ($hashFail) {
  ...
  throw "C1DIRFULL16_HASH_GATE_FAIL do_not_invent do_not_edit_stream_rtl do_not_patch_c0_dir"
}
...
$tb = Join-Path $bag "tb_astra_c1_dir_full16.sv"
$hostPy = Join-Path $bag "host_astra_c1_dir_full16.py"
$bagLex = Join-Path $bag "qse_role_lexicon.svh"
...
$c0Lex = Join-Path $root "rtl\native_graph\query\qse_role_lexicon.svh"
if (-not (Test-Path -LiteralPath $goldJson)) { throw "C1DIRFULL16_GOLD_MISSING GOLDEN.json" }
if (-not (Test-Path -LiteralPath $goldSvh)) { throw "C1DIRFULL16_GOLD_MISSING query_gold.svh" }
if (-not (Test-Path -LiteralPath $corpus)) { throw "C1DIRFULL16_GOLD_MISSING corpus.json" }
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C1DIRFULL16_GOLD_NOT_HASHED_BEFORE_XVLOG" }
if (-not (Test-Path -LiteralPath $streamDut)) { throw "C1DIRFULL16_DUT_MISSING a7ng_query_axi_sparse_stream_intersect.sv" }
if (-not (Test-Path -LiteralPath $bagLex)) { throw "C1DIRFULL16_BAG_LEXICON_MISSING" }
$liveGold = Sha256 $goldJson
$liveSvh = Sha256 $goldSvh
$liveCorpus = Sha256 $corpus
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C1DIRFULL16_GOLD_HASH_DRIFT GOLDEN.json" }
if ($preTxt -notmatch $liveSvh) { throw "C1DIRFULL16_GOLD_HASH_DRIFT query_gold.svh" }
if ($preTxt -notmatch $liveCorpus) { throw "C1DIRFULL16_GOLD_HASH_DRIFT corpus.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") {
    throw "C1DIRFULL16_LEFTOVER_A09_ON_XVLOG_LIST"
  }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") {
    throw "C1DIRFULL16_FROZEN_AXI_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_relbind.sv") {
    throw "C1DIRFULL16_RELBIND_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect.sv") {
    throw "C1DIRFULL16_KEY_INTERSECT_MUST_NOT_BE_DUT"
  }
}
...
$pre = @(
  ...
  "# PROGRAM=NO C1_800K=OPEN CAND_CAP=16 N=256 N_BUCKETS=65536 LAW=qse-v2-stream-intersect-02 POKE_V=0",
...
$pre += (ShaLine $rbKeys)
$pre += (ShaLine $keepIx)
$pre += "# STREAM_02_DUT_INSTANTIATE_NOT_EDIT N_BUCKETS=65536"
$pre += (ShaLine $streamDut)
$pre += "# COMPILED"
foreach ($f in $files) { $pre += (ShaLine $f) }
$pre += "# TRANSITIVE_INCLUDES"
foreach ($f in $includes) { $pre += (ShaLine $f) }
$pre += "# TB_INCLUDE_PATH_LEXICON_EXTENSION (C0 file unedited MATCH above; bag copy adds boiler=26 header=27)"
$pre += (ShaLine $bagLex)
$pre += "# INDEPENDENT_GOLD_BEFORE_XVLOG"
$pre += "$liveGold  results/A7-NATIVE-GRAPH/ASTRA-C1-DIR-FULL16-01/GOLDEN.json"
$pre += "$liveSvh  results/A7-NATIVE-GRAPH/ASTRA-C1-DIR-FULL16-01/query_gold.svh"
$pre += "$liveCorpus  results/A7-NATIVE-GRAPH/ASTRA-C1-DIR-FULL16-01/corpus.json"
$pre += "# BAG"
...
Write-Host "C1DIRFULL16_SHA_FROZEN_BEFORE_XVLOG"

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C1DIRFULL16_XVLOG_NOT_FOUND" }
if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null
$incq = Join-Path $root "rtl\native_graph\query"
$incc = Join-Path $root "rtl\native_graph\control"
Set-Location $work
# Bag include path FIRST so extract sees boiler/header extension; C0 lexicon FILE remains unedited.
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C1_DIR_FULL16_XSIM_PASS" -Quiet)
  ...
  $hasFull16 = [bool](Select-String -Path $xsimLog -Pattern "FULL16_HIT" -Quiet)
  $hasCollide = [bool](Select-String -Path $xsimLog -Pattern "FAIL ALIAS_12BIT_COLLISION_EMIT" -Quiet)
...
if ($hasRedCap) {
  ...
  throw "C1DIRFULL16_REDUCTION_X1000_EMITTED_AS_CAP_N"
}
if (($xsimExit -ne 0) -or (-not $hasPass) -or $hasDiv -or $hasLeak -or (-not $hasFull16) -or $hasCollide -or $hasIncomp) {
  $r0 = Join-Path $bag "xsim_fail_r0.log"
  if ((Test-Path -LiteralPath $xsimLog) -and (-not (Test-Path -LiteralPath $r0))) {
    Copy-Item $xsimLog $r0 -Force
  }
  throw "C1DIRFULL16_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C1_DIR_FULL16_RUN_OK"
