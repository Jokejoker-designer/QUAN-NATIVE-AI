...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5CKPT_FILE_MISSING $p" }
...
$c3p = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c3_held_out.sv"
$c3h = Sha256 $c3p
$hashLines += "$c3h  rtl/native_graph/integrate/a7ng_astra_c3_held_out.sv"
Write-Host "LIVE_C3_WRAP $c3h"
if ($hashFail) { throw "C5CKPT_HASH_GATE_FAIL do_not_edit_keep_or_live_prod_top" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_explicit_reward_01b.sv"
$dut = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_prod_top_ckpt.sv"
$ckpt = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_sgd32_ckpt.sv"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5CKPT_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5CKPT_GOLD_HASH_DRIFT GOLDEN.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C5CKPT_LIVE_PROD_TOP_COMPILED" }
  if ($leaf -eq "a7ng_astra_c2_persist_commit.sv") { throw "C5CKPT_C2_COMPILED_ON_SAME_MAP" }
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5CKPT_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5CKPT_SYNTH_MIG_AS_DUT" }
}

$stamp = (Get-Date).ToString("yyyy-MM-ddTHH:mm:ss.fffffffK")
$pre = @(
  "# SHA freeze BEFORE xvlog $stamp",
  "# PROGRAM=NO BOARD_PASS=NOT_CLAIMED C5_MASTER=OPEN 01B_TOP=XSIM MIG=NO"
) + $hashLines
$pre += "# COMPILED"
foreach ($f in $files) { $pre += (ShaLine $f) }
$pre += (ShaLine (Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_prod_top_ckpt.svh"))
...
Write-Host "C5CKPT_SHA_FROZEN"

if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null
Set-Location $work
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C5CKPT_XVLOG_NOT_FOUND" }
& "$bin\xvlog.bat" --sv -i $incq -i $incc -i $inci -i $bag $files
if ($LASTEXITCODE -ne 0) { throw "C5CKPT_XVLOG_FAIL" }
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
& "$bin\xelab.bat" tb_astra_c5_explicit_reward_01b -s c5ckpt -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) {
  if (Test-Path (Join-Path $work "xelab.log")) {
    Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
  }
  throw "C5CKPT_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c5ckpt -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C5_SGD32_CKPT_TOP_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C5CKPT_XSIM_FAIL_OR_MARKER_MISSING pass=$hasPass"
}
Write-Host "ASTRA_C5_SGD32_CKPT_TOP_RUN_OK"
