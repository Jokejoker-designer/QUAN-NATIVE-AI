`timescale 1ns / 1ps
// ASTRA-C5-PROD-TOP-01. PROGRAM=NO. Bag-local TB (test-only plant).
// One production hierarchy. Modeled AXI, not MIG. Not BOARD.
`include "a7ng_astra_c5_prod_top.svh"
`include "a7ng_astra_c5_ddr_arb.svh"
`include "tb_oracles.svh"
module tb_astra_c5_prod_top;
  ...
  initial begin
    ...
    $display("C5_PROD_TOP PROGRAM=NO BOARD_PASS=REJECT DDR_QUERY_BOUND_FINAL=NOT_FROZEN MIG=NO A09=NO");
    guard = 0;
    ...
    $display("MEAS nurx=%0d nutx=%0d seen=%02h dual=%0d st=%0d obj=%0d res=%0d pcmt=%0d pvalid=%0d pph=%0d gdone=%0d glast=%0d ehas=%0d nhw=%0d nht=%0d last_aw=%h imask=%04h nsw=%0d",
      nurx, nutx, seen, dual, c3st, c3obj, c3res, pcmt, pvalid, pph, gdone, glast, ehas, nhw, nht, last_aw, imask, nsw);
    ...
    if (fail == 0) $display("ASTRA_C5_PROD_TOP_XSIM_PASS");
    else $display("ASTRA_C5_PROD_TOP_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO DDR_QUERY_BOUND_FINAL=NOT_FROZEN quality=MODELED_AXI_NOT_MIG_COMPACT_LM_NOT_802K");
    $finish;
  end
endmodule
