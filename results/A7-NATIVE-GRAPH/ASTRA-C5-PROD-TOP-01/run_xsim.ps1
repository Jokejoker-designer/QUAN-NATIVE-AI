...
if ($hashFail) { throw "C5P_HASH_GATE_FAIL do_not_edit_keep_rtl" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_prod_top.sv"
...
foreach ($f in $files) {
  ...
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5P_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C5P_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5P_SYNTH_MIG_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C5P_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C5P_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") { throw "C5P_CTX_8255a798_AS_DUT" }
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C5P_LEFTOVER_A09" }
}
...
$pre = @(
  ...
  "# LAW=one production hierarchy; A09 not top; modeled AXI not MIG"
) + $hashLines
$pre += "# COMPILED"
...
Set-Location $work
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C5P_XVLOG_NOT_FOUND" }
...
& "$bin\xelab.bat" tb_astra_c5_prod_top -s c5pt -timescale 1ns/1ps
...
& "$bin\xsim.bat" c5pt -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C5_PROD_TOP_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C5P_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C5_PROD_TOP_RUN_OK"
