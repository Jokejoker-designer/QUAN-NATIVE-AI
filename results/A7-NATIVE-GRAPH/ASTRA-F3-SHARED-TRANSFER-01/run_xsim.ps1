...
$files = @(
  ...
  (Join-Path $bag "tb_astra_f3_shared_xfer.sv")
)
$includes = @(
...
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "F3_FILE_MISSING $p" }
...
$pre += "# PROVENANCE frozen F2R5/F2R4/F2R3/F2R2 (not compiled; do not edit)"
...
Write-Host "F3_SHA_FROZEN $shaPre"
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "F3_XVLOG_NOT_FOUND" }

Set-Location $work
& "$bin\xvlog.bat" --sv -i $incq -i $incc -i $inci -i $bag $files
if ($LASTEXITCODE -ne 0) { throw "F3_XVLOG_FAIL" }
...
& "$bin\xelab.bat" tb_astra_f3_shared_xfer -s f3x -timescale 1ns/1ps
...
& "$bin\xsim.bat" f3x -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_F3_SHARED_TRANSFER_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "F3_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_F3_SHARED_TRANSFER_RUN_OK"
