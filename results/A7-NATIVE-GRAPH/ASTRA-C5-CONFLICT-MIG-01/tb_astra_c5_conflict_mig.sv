`timescale 1ps / 100fs
// ASTRA-C5-CONFLICT-MIG-01. PROGRAM=NO. Bag-local TB (test-only plant).
// Same C5 production hierarchy through official Digilent AXI MIG + ddr3_model.
// Polarity CONFLICT (st=5) after opposing 2-hop. A09 is not DUT. Not BOARD.
`include "a7ng_astra_c5_prod_top.svh"
`include "a7ng_astra_c5_ddr_arb.svh"
`include "tb_oracles.svh"

module tb_astra_c5_conflict_mig;
  ...
  task automatic plant_pos;
    ...
    begin
      ...
      axi_wr(POST_HEAP, b0);
      plant_index(2, C3_W_K0[0], C3_W_K1[0], C3_W_K2[0]);
  ...
  task automatic plant_neg_same_dest;
    ...
    begin
      ...
      b0 = (b0 << 32) | E0;
      axi_wr(POST_HEAP, b0);
      plant_index(4, C3_W_K0[0], C3_W_K1[0], C3_W_K2[0]);
      wr_f(E2, C3_W_SUBJ[0], MIDN, C3_W_REL[0], C3_TR_CONF_W[0], 1, 0, C3_TR_CX1);
      wr_f(E3, MIDN, DST0, C3_W_REL[0], C3_TR_CONF_W[0], 1, 0, C3_TR_CX2);
      plant_release();
    end
  endtask
  ...
  initial begin
    ...
    $display("C5_CONFLICT_MIG PROGRAM=NO BOARD_PASS=REJECT MIG=YES TO_CYC_TB_DEFPARAM=65535 A09=NO");
    ...
    wait_done("Q1_DONE");
    $display("MEAS Q1 st=%0d ans=%0d p0=%0d p1=%0d cmt=%0d", cap_st, cap_ans, cap_p0, cap_p1, cap_cmt);
    ...
    uart_str("pump requires indirect");
    wait_done("Q2_DONE");
    $display("MEAS Q2 conflict st=%0d ans=%0d p0=%0d p1=%0d cmt=%0d", cap_st, cap_ans, cap_p0, cap_p1, cap_cmt);
    if ((cap_st == A7NG_C5P_ST_CONFLICT) && (cap_ans == 20'd0) && (cap_p0 == 20'd0) && (cap_p1 == 20'd0))
      $display("CLASS_conflict HIT st=%0d", cap_st);
    else $display("CLASS_conflict MISS st=%0d ans=%0d p0=%0d p1=%0d", cap_st, cap_ans, cap_p0, cap_p1);
    chk("Q2_CONFLICT", (cap_st == A7NG_C5P_ST_CONFLICT) && (cap_ans == 20'd0));
    if (!cap_cmt && !pcmt)
      $display("CLASS_conflict_no_upd HIT");
    else $display("CLASS_conflict_no_upd MISS cap_cmt=%0d pcmt=%0d", cap_cmt, pcmt);
    chk("Q2_NO_UPD", !cap_cmt && !pcmt);
    ...
    if (fail == 0) $display("ASTRA_C5_CONFLICT_MIG_XSIM_PASS");
    else $display("ASTRA_C5_CONFLICT_MIG_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO quality=MIG_PHY_POLARITY_CONFLICT_NOT_A09");
    $finish;
  end

  initial begin
    #20s;
    $display("FIRST_DIVERGENCE MIG_TIMEOUT calib_or_conflict hung");
    $display("RESULT=FAIL");
    $display("ASTRA_C5_CONFLICT_MIG_XSIM_PASS ABSENT");
    $finish;
  end
endmodule
