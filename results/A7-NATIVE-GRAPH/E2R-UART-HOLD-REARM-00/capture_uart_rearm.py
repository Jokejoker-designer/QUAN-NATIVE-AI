#!/usr/bin/env python3
"""Hold-past-ATOM COM12 capture for E2R-UART-HOLD-REARM-00.

Open style copied from E2R-UART-HOLD-PRED-00/listen_uart_hold.py
(DTR/RTS false). Never treat ATOM rows as a stop. Hold >=300 s after
ATOM1 or until pred=664. Absolute max >=600 s. Stop early only on
...
def capture_rearm(
    ...
    print(
        f"REARM_HOLD {port} @{baud} max_s={max_seconds} hold_after_atom_s={hold_after_atom} "
...
def main() -> int:
    ...
    ap.add_argument("--hold-after-atom", type=float, default=300.0)
    ap.add_argument("--out", default="uart_rearm.txt")
    args = ap.parse_args()
    if args.max_seconds < 600.0:
        print("REFUSE: --max-seconds must be >= 600", file=sys.stderr)
        return 2
    if args.hold_after_atom < 300.0:
        print("REFUSE: --hold-after-atom must be >= 300", file=sys.stderr)
        return 2
    rec = capture_rearm(
        args.port,
        args.baud,
