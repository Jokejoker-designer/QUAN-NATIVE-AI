...
$files = @(
  ...
  (Join-Path $bag "tb_astra_06_r3_multi_slot.sv")
)
$includes = @(
...
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "A06R3_FILE_MISSING $p" }
...
$pre += "# PROVENANCE frozen persist/R2/F3R4/F3R3/F3R2/F3/F2R5/F2R4/F2R3/F2R2 (not compiled; do not edit)"
...
Write-Host "A06R3_SHA_FROZEN $shaPre"
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "A06R3_XVLOG_NOT_FOUND" }

Set-Location $work
& "$bin\xvlog.bat" --sv -i $incq -i $incc -i $inci $files
if ($LASTEXITCODE -ne 0) { throw "A06R3_XVLOG_FAIL" }
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_06_R3_MULTI_SLOT_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "A06R3_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_06_R3_MULTI_SLOT_RUN_OK"
