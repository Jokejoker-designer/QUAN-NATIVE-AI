`timescale 1ns / 1ps
// ASTRA-C4-LM06-BYTE256-CTXCOPY-01. PROGRAM=NO. Bag-local TB.
// QUERY/PROOF bytes are built here; DUT copies from proof ports only.
`include "a7ng_astra_c4_lm06_byte256_ctxcopy.svh"
module tb_astra_c4_lm06_byte256_ctxcopy;
  ...
  logic [7:0] proof0, proof1, proof2, tok, n_out, mat_q0, mat_p0;
  logic [9:0] head10;
  logic [15:0] n_host;
  logic [3:0] vver;
  ...
  function automatic logic [7:0] gold_ch(input int unsigned id, input int unsigned pos);
    begin
      unique case (pos)
        0: gold_ch = 8'h41 + (id % 26);
        1: gold_ch = 8'h61 + 8'((id * 5) % 26);
        2: gold_ch = 8'h30 + (id % 10);
        default: gold_ch = A7NG_C4X_EOS;
      endcase
    end
  endfunction
  ...
  endtask
  task automatic plant_proof(input int obj);
    begin
      proof0 = gold_ch(obj, 0);
      proof1 = gold_ch(obj, 1);
      proof2 = gold_ch(obj, 2);
    end
  endtask
    ...
    begin
      ...
      while (!done && guard<20000) begin
        ...
        if (tok_v) begin
          ...
          ntok_o=ntok_o+1;
          if (eos || tok==A7NG_C4X_EOS) eos_seen=1;
  ...
  initial begin
    ...
    proof0=0; proof1=0; proof2=0; evid_has=0;
    for (q=0;q<20;q=q+1) HOLD[q]=20+q;
    repeat(4) @(posedge clk); rst_n=1; repeat(4) @(posedge clk);
    $display("C4_CTXCOPY_HELDOUT VOCAB_VER=2 N_HOLD=20 N_UNSUP=20 TRAIN=10..19 HOLD=20..39 PROGRAM=NO BOARD_PASS=REJECT LM06_BYTE256=NOT_FROZEN");
    $display("CLASS_not_compose_renderer HIT dut=a7ng_astra_c4_lm06_byte256_ctxcopy");
    $display("CLASS_no_obj_lut HIT dut_ports=proof0_1_2_not_evid_obj");
    $display("CLASS_dict_fpga_visible HIT vocab_ver=%0d law=c4x-proof-ctx-v2", vver);
    chk("VOCAB_VER", vver===A7NG_C4X_VOCAB_VER);
    ...
    for (q=0;q<20;q=q+1) begin
      ...
      if (q==0) begin
        chk("MAT_Q0", mat_q0===A7NG_C4X_MARK_Q);
        chk("MAT_P0", mat_p0===gold_ch(HOLD[0],0));
        if ((mat_q0===A7NG_C4X_MARK_Q) && (mat_p0===gold_ch(HOLD[0],0)))
    ...
    if (name_ok(50) && (seq[0]!=gold_ch(20,0)))
    ...
    if (fail==0) $display("ASTRA_C4_LM06_BYTE256_CTXCOPY_XSIM_PASS");
    else $display("ASTRA_C4_LM06_BYTE256_CTXCOPY_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C4_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO LM06_BYTE256=NOT_FROZEN quality=PROOF_CTX_COPY_NOT_OBJ_LUT_NOT_802K");
    $finish;
  end
endmodule
