...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C4X_FILE_MISSING $p" }
...
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C4X_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C4X_GOLD_HASH_DRIFT GOLDEN.json" }

$dutTxt = [System.IO.File]::ReadAllText($dut)
$svhTxt = [System.IO.File]::ReadAllText($svh)
if ($dutTxt -match "a7ng_c4d_ch") { throw "C4X_DUT_HAS_OBJ_LUT a7ng_c4d_ch" }
if ($svhTxt -match "a7ng_c4d_ch") { throw "C4X_SVH_HAS_OBJ_LUT a7ng_c4d_ch" }
if ($dutTxt -match "evid_obj") { throw "C4X_DUT_HAS_EVID_OBJ" }

$files = @($b256, $dut, $tb)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C4X_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C4X_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c4_lm06_byte256_dict.sv") { throw "C4X_DICT_LUT_DUT" }
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C4X_LEFTOVER_A09" }
}
...
$pre = @(
  ...
  "# LAW=PROOF context-copy BYTE256; obj LUT and TinyGPT not DUT"
) + $hashLines
$pre += "# COMPILED"
...
Write-Host "C4X_SHA_FROZEN"

if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null
Set-Location $work
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C4X_XVLOG_NOT_FOUND" }
& "$bin\xvlog.bat" --sv -i $inci -i $bag $files
if ($LASTEXITCODE -ne 0) { throw "C4X_XVLOG_FAIL" }
...
if ($xvlogTxt -match "tiny_gpt803k_core") { throw "C4X_TINYGPT_COMPILED" }
if ($xvlogTxt -match "a7ng_astra_c4_lm06_byte256_dict") { throw "C4X_DICT_COMPILED" }
if ($xvlogTxt -notmatch "a7ng_astra_c4_lm06_byte256_ctxcopy") { throw "C4X_DUT_NOT_ANALYZED" }
& "$bin\xelab.bat" tb_astra_c4_lm06_byte256_ctxcopy -s c4x -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) {
  if (Test-Path (Join-Path $work "xelab.log")) {
    Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
  }
  throw "C4X_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c4x -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "^ASTRA_C4_LM06_BYTE256_CTXCOPY_XSIM_PASS$" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C4X_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C4_LM06_BYTE256_CTXCOPY_RUN_OK PASS_THIS_GATE_ONLY PROGRAM=NO C4_MASTER=OPEN"
