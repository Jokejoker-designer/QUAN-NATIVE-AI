`timescale 1ps / 100fs
// ASTRA-C3-HELD-OUT-MIG-02. PROGRAM=NO. Bag-local TB.
// Official Digilent AXI MIG + ddr3_model. Plants 2-hop facts via MIG AW.
// Instantiates live a7ng_astra_c3_held_out (CONFLICT law, not edited). Not BOARD.
`include "a7ng_astra_c3_held_out.svh"
`include "tb_oracles.svh"

module tb_astra_c3_held_out_mig_02;
  ...
  int pidmap [0:255];
  int fail, i, q, n_rank, n_ans, npath4_c, host_bad, n_conf;
  string first_div;
  ...
  initial begin
    ...
    n_rank=0; n_ans=0; npath4_c=0; host_bad=0; n_conf=0;
    ...
    $display("C3_HELD_OUT_MIG_02 PROGRAM=NO BOARD_PASS=REJECT C3_MASTER=OPEN MIG_XSIM=1 LIVE_CONFLICT_C3 TO_CYC=%0d", C3_TO_CYC);
    ...
    $display("C3_SUM A=%0d/%0d B=%0d C=%0d Dpid=%0d gain_pp=%0d pair_pos=%0d/%0d host_bad=%0d n_conf=%0d",
      sum_a, 8*C3_N_SEED, sum_b, sum_c, sum_d, gain_pp, pair_pos, C3_N_SEED, host_bad, n_conf);
    ...
    if (fail==0) $display("ASTRA_C3_HELD_OUT_MIG_02_XSIM_PASS");
    else $display("ASTRA_C3_HELD_OUT_MIG_02_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C3_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO quality=MIG_PHY_PLANTED_2HOP_LIVE_CONFLICT_C3");
    $finish;
  end
  ...
  initial begin
    ...
    $display("RESULT=FAIL");
    $display("ASTRA_C3_HELD_OUT_MIG_02_XSIM_PASS ABSENT");
    $finish;
  end
endmodule
