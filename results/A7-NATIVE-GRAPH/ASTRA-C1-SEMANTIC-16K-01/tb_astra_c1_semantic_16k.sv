// tb_astra_c1_semantic_16k.sv — C1 N=16384 qse-v2-intersect-context-02 semantic. PROGRAM=NO.
// Frozen QSE + ctx keys 124be808 + STREAM-02 walker in context wrapper 8255a798.
// NEW lexicon law qse-v2-lex-semantic-16k-01 (bag include, not silent C0 59-word).
// poke_v=0. leftover A09 off. STREAM-02 14f75db7 not compiled as DUT.
// G_BYTES LSB-first: char0 at [7:0]; extract bytes[8*bi +: 8] only.
// PASS marker only if FAIL=0 and SEARCH_INCOMPLETE absent on gold_n>=1 retrieve.
...
module tb_astra_c1_semantic_16k;
  import a7ng_pkg::*;
  ...
  initial begin
    ...
    $display("C1_SEMANTIC_16K_N=%0d N_BUCKETS=%0d CAND_CAP=%0d INDEX_HEAD=%0d LAW=qse-v2-intersect-context-02 LEX=qse-v2-lex-semantic-16k-01 POKE_V=0 PAGE_BUFFER_BEATS=1 RARE_LIST_FIRST=1 CAND_CAP_AFTER_EMIT=1 REDUCTION_X1000=NOT_EMITTED CAND_CAP_FINAL=NOT_FROZEN DDR_QUERY_BOUND_FINAL=NOT_FROZEN MEM_DEPTH=%0d N_SUBJECTS=%0d N_RELS=%0d",
    ...
    if (fail == 0 && dist_gold_ok && dist_excl_ok && dist_no_leak && unrelated_empty && wc_sel && (direct_tp > 0) && direct_ids_ok && late_hit && late_ok && (incomp_retrieve == 0)) begin
      $display("ASTRA_C1_SEMANTIC_16K_XSIM_PASS");
      ...
      $display("ASTRA_C1_SEMANTIC_16K_XSIM_NO_MARKER fail=%0d dist_gold_ok=%0d dist_excl_ok=%0d dist_no_leak=%0d unrelated_empty=%0d wc_sel=%0d direct_tp=%0d direct_ids_ok=%0d late_hit=%0d late_ok=%0d incomp_retrieve=%0d",
