#!/usr/bin/env python3
"""ASTRA-C1-SEMANTIC-16K-01 independent host gold.

N=16384 semantic corpus (NOT a 12-entity HVAC clone).
Law: qse-v2-intersect-context-02 keys (instantiate a7ng_query_role_keys_ctx,
hash-gate MATCH 124be808, do not edit) + STREAM-02 two-pointer walker in
frozen context wrapper a7ng_query_axi_sparse_intersect_context (8255a798;
do not edit). STREAM-02 file 14f75db7 is KEEP, not compiled as DUT, not edited.

C0 59-word lexicon cannot name 100 entities. This bag registers NEW lexicon
law qse-v2-lex-semantic-16k-01 (named file qse_role_lexicon_semantic_16k.svh).
Frozen extract still `include "qse_role_lexicon.svh"`; bag copy of that
include-name points at the named law. xvlog -i $bag FIRST is the NEW law,
NOT a silent C0 59-word claim. C0 FILE at rtl/.../qse_role_lexicon.svh is
unedited (hash-gate MATCH 38189974).

Host dual-indexes plain k0/k1 always and ctx-packed keys when ctx_valid.
Independent gold = label match on {evidence,subj,rel,obj,ctx} BEFORE walker
and BEFORE xvlog. SEARCH_INCOMPLETE on gold_n>=1 retrieve = FAIL.
N_BUCKETS=65536 (16-bit exact bucket) so 8-bit entity ids do not 12-bit alias.
CAND_CAP=16 after emit, not FINAL. poke_v=0. leftover A09 off. PROGRAM=NO.
Do not freeze CAND_CAP_FINAL / DDR_QUERY_BOUND_FINAL / C1 800k / BOARD_PASS.
...
LATE_S, LATE_R, LATE_O = 131, 8, 132  # centrifuge discharges cyclone
...
def write_named_lexicon() -> None:
    ...
    header = [
        ...
        "// semantic entities/relations/ctx so N=16384 can have >=100 subject ids and >=8 rel ids.",
    ...
    (BAG / "qse_role_lexicon_semantic_16k.svh").write_text("\n".join(header), encoding="utf-8")
    shim = [
        f"// NEW lexicon law {LAW_LEX} — include-name required by frozen extract.",
        "// NOT a silent C0 59-word claim. Runtime table is qse_role_lexicon_semantic_16k.svh.",
        "// C0 FILE at rtl/native_graph/query/qse_role_lexicon.svh is unedited.",
        '`include "qse_role_lexicon_semantic_16k.svh"',
        "",
    ]
...
def write_svh(
    ...
    lines = [
        "// generated by host_astra_c1_semantic_16k.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
        f"// Law {LAW} + NEW lexicon {LAW_LEX}; STREAM-02 walker in context wrapper; N=16384",
...
def main() -> int:
    ...
    queries_spec = [
        ("direct", triple_text(DIR_S, DIR_R, DIR_O), pred_direct),
        ("paraphrase", f"the {ENT[DIR_S]} {RELW[DIR_R]} the {ENT[DIR_O]}", pred_direct),
        ("role_reversal", triple_text(REV_S, REV_R, REV_O), lambda d: d["subj_id"] == REV_S and d["rel_id"] == REV_R and d["obj_id"] == REV_O),
        ("wrong_relation", triple_text(WREL_S, WREL_R, WREL_O), lambda d: d["subj_id"] == WREL_S and d["rel_id"] == WREL_R and d["obj_id"] == WREL_O),
        ("wrong_context", triple_text(DIR_S, DIR_R, DIR_O, "glycol"), lambda d: pred_direct(d) and d["ctx_id"] == CTX_GLYCOL),
        ("distractor", triple_text(DIR_S, DIR_R, DIR_O), pred_excl),
        ("unrelated", "payroll tax form", lambda d: False),
        ("high_occupancy", triple_text(HOC_S, HOC_R, HOC_O), lambda d: d["subj_id"] == HOC_S and d["rel_id"] == HOC_R and d["obj_id"] == HOC_O),
        ("overflow_page", triple_text(OVF_S, OVF_R, OVF_O), lambda d: d["subj_id"] == OVF_S and d["rel_id"] == OVF_R and d["obj_id"] == OVF_O),
        ("high_id_sentinel", triple_text(SEN_S, SEN_R, SEN_O), lambda d: d["nid"] == N - 1),
    ...
    chk(len(gold_map["direct"]) >= 1, "direct gold empty")
    chk(all(by_nid[i]["evidence"] == 1 for ids in gold_map.values() for i in ids), "gold evidence")
    chk(set(gold_map["direct"]).isdisjoint(set(gold_map["distractor"])), "excluded contains direct")
    ...
    chk(qd["direct"]["ctx_id"] == 0 and qd["direct"]["ctx_valid"] == 0, "direct ctx unbound")
    chk(qd["direct"]["k0"] == qd["direct"]["k0_plain"], "direct k0 not plain")
    ...
    if fails:
        print("ASTRA_C1_SEMANTIC_16K_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        if not_selective:
    ...
    print("ASTRA_C1_SEMANTIC_16K_HOST_PASS")
    ...
    print("WC_K0", wc["k0"], "DIRECT_K0", dd["k0"], "WC_EMIT", wc["emit"], "DIRECT_EMIT", dd["emit"])
    print("GOLD_SHA", gsha)
    print("SVH_SHA", ssha)
