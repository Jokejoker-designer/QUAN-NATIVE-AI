...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C4BIAS_FILE_MISSING $p" }
...
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C4BIAS_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C4BIAS_GOLD_HASH_DRIFT GOLDEN.json" }

$files = @($b256, $dut, $tb)
foreach ($f in $files) {
  $leaf = Split-Path -Leaf $f
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C4BIAS_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C4BIAS_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "a7ng_astra_c5_prod_top.sv") { throw "C4BIAS_LIVE_PROD_TOP_AS_DUT" }
}

$stamp = (Get-Date).ToString("yyyy-MM-ddTHH:mm:ss.fffffffK")
$pre = @(
  "# SHA freeze BEFORE xvlog $stamp",
  "# PROGRAM=NO BOARD_PASS=NOT_CLAIMED C4_MASTER=OPEN FALSIFIER=BIAS_LOAD_DEAD"
) + $hashLines
$pre += "# COMPILED"
...
Write-Host "C4BIAS_SHA_FROZEN"

if (Test-Path $work) { Remove-Item -Recurse -Force $work }
New-Item -ItemType Directory -Force -Path $work | Out-Null
Set-Location $work
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C4BIAS_XVLOG_NOT_FOUND" }
& "$bin\xvlog.bat" --sv -i $inci -i $bag $files
if ($LASTEXITCODE -ne 0) { throw "C4BIAS_XVLOG_FAIL" }
Copy-Item (Join-Path $work "xvlog.log") (Join-Path $bag "xvlog.log") -Force
& "$bin\xelab.bat" tb_astra_c4_bias_load_dead -s c4bias -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) {
  if (Test-Path (Join-Path $work "xelab.log")) {
    Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
  }
  throw "C4BIAS_XELAB_FAIL"
}
if (Test-Path (Join-Path $work "xelab.log")) {
  Copy-Item (Join-Path $work "xelab.log") (Join-Path $bag "xelab.log") -Force
}
$xsimLog = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" c4bias -R -log $xsimLog
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C4_BIAS_LOAD_DEAD_XSIM_PASS" -Quiet)
  $hasV = [bool](Select-String -Path $xsimLog -Pattern "CLASS_v_slice_is_zero HIT" -Quiet)
  $has1 = [bool](Select-String -Path $xsimLog -Pattern "CLASS_bias_idx1_rejected HIT" -Quiet)
  $has63 = [bool](Select-String -Path $xsimLog -Pattern "CLASS_bias_idx63_rejected HIT" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass) -or (-not $hasV) -or (-not $has1) -or (-not $has63)) {
  if (Test-Path -LiteralPath $xsimLog) {
  ...
  throw "C4BIAS_XSIM_FAIL_OR_MARKER_MISSING pass=$hasPass v0=$hasV idx1=$has1 idx63=$has63"
}
Write-Host "ASTRA_C4_BIAS_LOAD_DEAD_RUN_OK"
