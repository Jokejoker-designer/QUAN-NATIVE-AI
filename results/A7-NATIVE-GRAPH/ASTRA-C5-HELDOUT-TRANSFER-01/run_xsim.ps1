...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C5HT_FILE_MISSING $p" }
...
if ($hashFail) { throw "C5HT_HASH_GATE_FAIL do_not_edit_keep_rtl" }

$goldJson = Join-Path $bag "GOLDEN.json"
$goldPre = Join-Path $bag "GOLD_HASH_PRE_XVLOG.txt"
$tb = Join-Path $bag "tb_astra_c5_heldout_transfer.sv"
$dut = Join-Path $root "rtl\native_graph\integrate\a7ng_astra_c5_prod_top.sv"
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C5HT_GOLD_NOT_HASHED_BEFORE_XVLOG" }
$liveGold = Sha256 $goldJson
$preTxt = [System.IO.File]::ReadAllText($goldPre)
if ($preTxt -notmatch $liveGold) { throw "C5HT_GOLD_HASH_DRIFT GOLDEN.json" }
...
foreach ($f in $files) {
  ...
  if ($leaf -eq "tiny_gpt803k_core.sv") { throw "C5HT_FROZEN_LM06_AS_DUT" }
  if ($leaf -eq "a7ng_evidence_compose.sv") { throw "C5HT_COMPOSE_RENDERER_AS_DUT" }
  if ($leaf -eq "mig_7series_0_mig.v") { throw "C5HT_SYNTH_MIG_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") { throw "C5HT_C0_SPARSE_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") { throw "C5HT_STREAM02_AS_DUT" }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect_context.sv") { throw "C5HT_CTX_8255a798_AS_DUT" }
  if ($leaf -match "a7ng_astra_09_integ_path") { throw "C5HT_LEFTOVER_A09" }
}

$stamp = (Get-Date).ToString("yyyy-MM-ddTHH:mm:ss.fffffffK")
$pre = @(
  "# SHA freeze BEFORE xvlog $stamp",
  "# PROGRAM=NO BOARD_PASS=NOT_CLAIMED C5_MASTER=OPEN DDR_QUERY_BOUND_FINAL=NOT_FROZEN MIG=NO",
  "# LAW=held-out PRE/POST dest change on one production hierarchy; modeled AXI not MIG"
) + $hashLines
$pre += "# COMPILED"
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C5_HELDOUT_TRANSFER_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  ...
  throw "C5HT_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C5_HELDOUT_TRANSFER_RUN_OK"
