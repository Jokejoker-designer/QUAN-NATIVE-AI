`timescale 1ns / 1ps
// ASTRA-C5-DIRECT-FACT-01. PROGRAM=NO. Bag-local TB (test-only plant).
...
module tb_astra_c5_direct_fact;
  ...
  initial begin
    ...
    $display("C5_DIRECT_FACT PROGRAM=NO BOARD_PASS=REJECT DDR_QUERY_BOUND_FINAL=NOT_FROZEN MIG=NO A09=NO");
    guard = 0;
    ...
    uart_str("pump requires valve");
    wait_done("Q1_DONE");
    $display("MEAS Q1 1hop st=%0d ans=%0d p0=%0d p1=%0d", cap_st, cap_ans, cap_p0, cap_p1);
    if ((cap_st == A7NG_C5P_ST_ANSWER) && (cap_ans == VALVE) && (cap_p0 == E2) && (cap_p1 == 20'd0))
      $display("CLASS_direct_fact HIT ans=%0d p0=%0d p1=%0d", cap_ans, cap_p0, cap_p1);
    else $display("CLASS_direct_fact MISS st=%0d ans=%0d p0=%0d p1=%0d", cap_st, cap_ans, cap_p0, cap_p1);
    chk("Q1_1HOP", (cap_st == A7NG_C5P_ST_ANSWER) && (cap_ans == VALVE) && (cap_p0 == E2) && (cap_p1 == 20'd0));

    uart_str("pump requires indirect");
    wait_done("Q2_DONE");
    $display("MEAS Q2 2hop st=%0d ans=%0d p0=%0d p1=%0d", cap_st, cap_ans, cap_p0, cap_p1);
    if ((cap_st == A7NG_C5P_ST_ANSWER) && (cap_ans == DST2) && (cap_p0 == E0) && (cap_p1 == E1))
      $display("CLASS_two_hop_novel HIT ans=%0d p0=%0d p1=%0d", cap_ans, cap_p0, cap_p1);
    else $display("CLASS_two_hop_novel MISS st=%0d ans=%0d p0=%0d p1=%0d", cap_st, cap_ans, cap_p0, cap_p1);
    chk("Q2_2HOP", (cap_st == A7NG_C5P_ST_ANSWER) && (cap_ans == DST2) && (cap_p0 == E0) && (cap_p1 == E1));
    ...
    if (fail == 0) $display("ASTRA_C5_DIRECT_FACT_XSIM_PASS");
    else $display("ASTRA_C5_DIRECT_FACT_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $display("C5_MASTER_CLAIM=NO BOARD_PASS=REJECT PROGRAM=NO DDR_QUERY_BOUND_FINAL=NOT_FROZEN quality=MODELED_AXI_NOT_MIG");
    $finish;
  end
endmodule
