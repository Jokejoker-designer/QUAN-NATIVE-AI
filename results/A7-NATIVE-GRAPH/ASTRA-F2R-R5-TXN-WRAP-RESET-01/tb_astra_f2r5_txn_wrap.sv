`timescale 1ns / 1ps
// ASTRA-F2R-R5-TXN-WRAP-RESET-01. PROGRAM=NO. Bag-local TB.
// TXN_MAX=2 is the documented wrap setup (not a poke of production 255).
module tb_astra_f2r5_txn_wrap;
  ...
  localparam logic [7:0] TXN_MAX=8'd2;
  ...
  logic [15:0] sav_ep, w0_before;
  logic [7:0] sav_txn, sav_gen;
  logic [15:0] nupd0, nstale0, nbad0, nexh0;
  ...
  a7ng_astra_f2r5_txn_wrap #(.TXN_MAX(TXN_MAX)) u_dut (
  ...
  task automatic dump(input string tag);
    $display("%s st=%0d npath=%0d ans=%0d p0=%0d acc=%0d cmt=%0d txn=%0d gen=%0d ep=%0d exh=%0d nexh=%0d nupd=%0d ndup=%0d nbad=%0d nstale=%0d noor=%0d w0=%0d tbl=%0d",
      tag, st, npath, ans, p0, pend_acc, pend_cmt, txn, gen, epoch, txn_exh, nexh, nupd, ndup, nbad, nstale, noor, wdut[0], tbl);
  endtask
  ...
  initial begin
    ...
    live_ep=16'd7; sess_id=16'd7; hard_rst(); q_smoke();
    ...
    chk("DUP", nupd===16'd1 && ndup>=1 && wdut[0]===w0_before);
    @(posedge clk); rew<=-4'sd3; rew_txn<=txn+8'd1; rew_gen<=gen; rew_epoch<=epoch; rew_v<=1;
    @(posedge clk); rew_v<=0; repeat(20) @(posedge clk);
    chk("WRONG_TXN", nbad>=1 && nupd===16'd1 && wdut[0]===w0_before);
    @(posedge clk); rew<=-4'sd3; rew_txn<=txn; rew_gen<=gen-8'd1; rew_epoch<=epoch; rew_v<=1;
    @(posedge clk); rew_v<=0; repeat(20) @(posedge clk);
    chk("STALE_GEN", nstale>=1 && nupd===16'd1 && wdut[0]===w0_before);
    @(posedge clk); rew<=-4'sd3; rew_txn<=txn; rew_gen<=gen; rew_epoch<=epoch-16'd1; rew_v<=1;
    @(posedge clk); rew_v<=0; repeat(20) @(posedge clk);
    chk("STALE_EPOCH", nstale>=2 && nupd===16'd1 && wdut[0]===w0_before);
    @(posedge clk); rew<=-4'sd4; rew_txn<=txn; rew_gen<=gen; rew_epoch<=epoch; rew_v<=1;
    @(posedge clk); rew_v<=0; repeat(20) @(posedge clk);
    chk("OOR_M4", noor>=1 && nupd===16'd1 && wdut[0]===w0_before);
    retire_q();
    ...
    q_smoke();
    dump("B_AFTER_A");
    nupd0=nupd; nbad0=nbad; nstale0=nstale;
    pulse_rew_id(-3, sav_ep, sav_txn, sav_gen);
    repeat(80) @(posedge clk);
    dump("REPLAY_A_ON_B");
    chk("RETIRE_A_NO_UPD_B", nupd===nupd0 && (nbad>nbad0 || nstale>nstale0)
        && wdut[0]===16'sd0 && pend_acc);
    pulse_rew(-3);
    repeat(250) @(posedge clk);
    chk("REWARD_B_ONCE", nupd===16'd1 && pend_cmt && wdut[0]===-16'sd5);
    retire_q();

    live_ep=16'd7; sess_id=16'd20; hard_rst(); q_smoke();
    sav_ep=epoch; sav_txn=txn; sav_gen=gen;
    dump("WRAP_A");
    chk("WRAP_A_KEY", pend_acc && (txn===8'd1) && (gen===8'd1) && (epoch===16'd20));
    retire_q();
    q_smoke();
    dump("WRAP_B");
    chk("WRAP_B_KEY", pend_acc && (txn===8'd2) && (gen===8'd2) && (epoch===16'd20));
    retire_q();
    nexh0=nexh;
    q_smoke();
    dump("WRAP_C_EXH");
    chk("WRAP_EXHAUST", txn_exh && (nexh>nexh0) && !pend_acc && (st===ST_ANSWER)
        && (ans===20'd4) && (p0===20'd17) && !tbl);
    chk("WRAP_NO_REUSE_11", !(pend_acc && (txn===8'd1) && (epoch===16'd20)));
    nupd0=nupd; nbad0=nbad;
    pulse_rew_id(-3, sav_ep, sav_txn, sav_gen);
    repeat(80) @(posedge clk);
    dump("WRAP_REPLAY_A");
    chk("WRAP_REPLAY_NO_UPD", nupd===nupd0 && wdut[0]===16'sd0 && !pend_cmt);
    retire_q();

    live_ep=16'd7; sess_id=16'd21;
    q_smoke();
    dump("EPOCH_RECYCLE");
    chk("EPOCH_RECYCLE_11", pend_acc && (txn===8'd1) && (gen===8'd1) && (epoch===16'd21));
    nupd0=nupd; nstale0=nstale;
    pulse_rew_id(-3, sav_ep, sav_txn, sav_gen);
    repeat(80) @(posedge clk);
    dump("EPOCH_REPLAY_OLD");
    chk("EPOCH_REPLAY_STALE", nupd===nupd0 && nstale>nstale0 && wdut[0]===16'sd0);
    pulse_rew(-3);
    repeat(250) @(posedge clk);
    chk("EPOCH_FRESH_UPD", nupd===nupd0+16'd1 && pend_cmt && wdut[0]===-16'sd5);
    retire_q();

    live_ep=16'd7; sess_id=16'd30; hard_rst(); q_smoke();
    sav_ep=epoch; sav_txn=txn; sav_gen=gen;
    dump("PRE_RST_A");
    chk("PRE_RST_KEY", pend_acc && (txn===8'd1) && (gen===8'd1) && (epoch===16'd30));
    live_ep=16'd7; sess_id=16'd31; hard_rst(); q_smoke();
    dump("POST_RST_B");
    chk("POST_RST_REUSED_NUMERIC", pend_acc && (txn===8'd1) && (gen===8'd1) && (epoch===16'd31));
    nupd0=nupd; nstale0=nstale;
    pulse_rew_id(-3, sav_ep, sav_txn, sav_gen);
    repeat(80) @(posedge clk);
    dump("POST_RST_REPLAY_A");
    chk("POST_RST_REPLAY_NO_UPD", nupd===nupd0 && nstale>nstale0 && wdut[0]===16'sd0 && !pend_cmt);
    pulse_rew(-3);
    repeat(250) @(posedge clk);
    chk("POST_RST_FRESH_UPD", nupd===16'd1 && pend_cmt && wdut[0]===-16'sd5);
    retire_q();

    live_ep=16'd7; sess_id=16'd40; hard_rst(); q_smoke();
    dump("RST_UPD_SETUP");
    pulse_rew(-3);
    repeat(2) @(posedge clk);
    chk("RST_UPD_IN_FLIGHT", busy && !result_v);
    repeat(50) @(posedge clk);
    live_ep=16'd7; sess_id=16'd41; hard_rst();
    dump("RST_UPD_ABORT");
    chk("RST_UPD_CLEARED", !pend_acc && !pend_cmt && (nupd===16'd0) && (wdut[0]===16'sd0)
        && (txn===8'd0) && (gen===8'd0) && (epoch===16'd0));
    q_smoke();
    pulse_rew(-3);
    repeat(250) @(posedge clk);
    dump("RST_UPD_FRESH");
    chk("RST_UPD_FRESH_OK", nupd===16'd1 && pend_cmt && wdut[0]===-16'sd5 && (epoch===16'd41));
    retire_q();

    live_ep=16'd7; sess_id=16'd50; hard_rst(); q_smoke();
    dump("SMOKE_AFTER");
    chk("SMOKE_AFTER", (st===ST_ANSWER) && !tbl && (npath>=5'd2) && (p0===20'd17) && (ans===20'd4));
    retire_q();
    send_text("payroll tax form"); wait_done();
    dump("UNREL");
    chk("UNREL_NO_STALE", st===ST_UNKNOWN && npath===5'd0 && ans===20'd0 && p0===20'd0);

    if (fail==0) $display("ASTRA_F2R5_TXN_WRAP_RESET_XSIM_PASS");
    else $display("ASTRA_F2R5_TXN_WRAP_RESET_XSIM_FAIL n=%0d first=%s", fail, first_div);
    $finish;
  end
endmodule
