...
$files = @(
  ...
  (Join-Path $bag "tb_astra_f2r5_txn_wrap.sv")
)
$includes = @(
...
function ShaLine([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "F2R5_FILE_MISSING $p" }
...
$pre += "# PROVENANCE frozen F2R4/F2R3/F2R2 (not compiled; do not edit)"
$pre += (ShaLine (Join-Path $root "rtl\native_graph\integrate\a7ng_astra_f2r4_axi_drain.sv"))
$pre += (Join-Path $root "rtl\native_graph\integrate\a7ng_astra_f2r4_axi_drain.svh" | ForEach-Object { ShaLine $_ })
...
Write-Host "F2R5_SHA_FROZEN $shaPre"
if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "F2R5_XVLOG_NOT_FOUND" }

Set-Location $work
& "$bin\xvlog.bat" --sv -i $incq -i $incc -i $inci $files
if ($LASTEXITCODE -ne 0) { throw "F2R5_XVLOG_FAIL" }
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_F2R5_TXN_WRAP_RESET_XSIM_PASS" -Quiet)
}
if (($xsimExit -ne 0) -or (-not $hasPass)) {
  if (Test-Path -LiteralPath $xsimLog) {
    Copy-Item $xsimLog (Join-Path $bag "xsim_fail.log") -Force
  }
  throw "F2R5_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_F2R5_TXN_WRAP_RESET_RUN_OK"
