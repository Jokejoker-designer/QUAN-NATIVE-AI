...
function Sha256([string]$p) {
  if (-not (Test-Path -LiteralPath $p)) { throw "C1SEMUNSEEN_FILE_MISSING $p" }
...
if ($hashFail) {
  ...
  throw "C1SEMUNSEEN_HASH_GATE_FAIL do_not_invent do_not_edit_keep_rtl"
}
...
$tb = Join-Path $bag "tb_astra_c1_semantic_unseen_sro.sv"
$hostPy = Join-Path $bag "host_astra_c1_semantic_unseen_sro.py"
...
if (-not (Test-Path -LiteralPath $goldJson)) { throw "C1SEMUNSEEN_GOLD_MISSING GOLDEN.json" }
if (-not (Test-Path -LiteralPath $goldSvh)) { throw "C1SEMUNSEEN_GOLD_MISSING query_gold.svh" }
if (-not (Test-Path -LiteralPath $corpus)) { throw "C1SEMUNSEEN_GOLD_MISSING corpus.json" }
if (-not (Test-Path -LiteralPath $goldPre)) { throw "C1SEMUNSEEN_GOLD_NOT_HASHED_BEFORE_XVLOG" }
if (-not (Test-Path -LiteralPath $lexNamed)) { throw "C1SEMUNSEEN_LEX_MISSING qse_role_lexicon_semantic_16k.svh" }
if (-not (Test-Path -LiteralPath $lexShim)) { throw "C1SEMUNSEEN_LEX_SHIM_MISSING qse_role_lexicon.svh (NEW law include-name, not C0)" }
if (-not (Test-Path -LiteralPath $ctxDut)) { throw "C1SEMUNSEEN_DUT_MISSING a7ng_query_axi_sparse_intersect_context.sv" }
if (-not (Test-Path -LiteralPath $ctxKeys)) { throw "C1SEMUNSEEN_KEYS_MISSING a7ng_query_role_keys_ctx.sv" }
...
if ($preTxt -notmatch $liveGold) { throw "C1SEMUNSEEN_GOLD_HASH_DRIFT GOLDEN.json" }
if ($preTxt -notmatch $liveSvh) { throw "C1SEMUNSEEN_GOLD_HASH_DRIFT query_gold.svh" }
if ($preTxt -notmatch $liveCorpus) { throw "C1SEMUNSEEN_GOLD_HASH_DRIFT corpus.json" }
if ($preTxt -notmatch $liveLex) { throw "C1SEMUNSEEN_GOLD_HASH_DRIFT qse_role_lexicon_semantic_16k.svh" }
...
foreach ($f in $files) {
  ...
  if ($leaf -match "a7ng_astra_09_integ_path") {
    throw "C1SEMUNSEEN_LEFTOVER_A09_ON_XVLOG_LIST"
  }
  if ($leaf -eq "a7ng_query_axi_sparse.sv") {
    throw "C1SEMUNSEEN_FROZEN_AXI_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_relbind.sv") {
    throw "C1SEMUNSEEN_RELBIND_SPARSE_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_intersect.sv") {
    throw "C1SEMUNSEEN_KEY_INTERSECT_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_stream_intersect.sv") {
    throw "C1SEMUNSEEN_STREAM02_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_axi_sparse_page_skip.sv") {
    throw "C1SEMUNSEEN_PAGESKIP_MUST_NOT_BE_DUT"
  }
  if ($leaf -eq "a7ng_query_role_keys_relbind.sv") {
    throw "C1SEMUNSEEN_RELBIND_KEYS_MUST_NOT_BE_COMPILED"
  }
}
...
$pre = @(
  ...
  "# PROGRAM=NO C1_800K=OPEN CAND_CAP=16 N=256 N_BUCKETS=4096 LAW=qse-v2-intersect-context-02 LEX=qse-v2-lex-semantic-16k-01 POKE_V=0",
...
$pre += "$liveGold  results/A7-NATIVE-GRAPH/ASTRA-C1-SEMANTIC-UNSEEN-SRO-01/GOLDEN.json"
$pre += "$liveSvh  results/A7-NATIVE-GRAPH/ASTRA-C1-SEMANTIC-UNSEEN-SRO-01/query_gold.svh"
$pre += "$liveCorpus  results/A7-NATIVE-GRAPH/ASTRA-C1-SEMANTIC-UNSEEN-SRO-01/corpus.json"
$pre += "$liveLex  results/A7-NATIVE-GRAPH/ASTRA-C1-SEMANTIC-UNSEEN-SRO-01/qse_role_lexicon_semantic_16k.svh"
$pre += "# BAG"
...
Write-Host "C1SEMUNSEEN_SHA_FROZEN_BEFORE_XVLOG"

if (-not (Test-Path -LiteralPath "$bin\xvlog.bat")) { throw "C1SEMUNSEEN_XVLOG_NOT_FOUND" }
...
if (Test-Path -LiteralPath $xsimLog) {
  $hasPass = [bool](Select-String -Path $xsimLog -Pattern "ASTRA_C1_SEMANTIC_UNSEEN_SRO_XSIM_PASS" -Quiet)
...
if ($hasRedCap) {
  ...
  throw "C1SEMUNSEEN_REDUCTION_X1000_EMITTED_AS_CAP_N"
}
if (($xsimExit -ne 0) -or (-not $hasPass) -or $hasDiv -or $hasLeak -or $hasFillFail -or $hasUnseenFail -or (-not $hasFillHit) -or (-not $hasUnseenHit) -or (-not $hasUnboundEmpty) -or $hasIncomp) {
  ...
  throw "C1SEMUNSEEN_XSIM_FAIL_OR_MARKER_MISSING"
}
Write-Host "ASTRA_C1_SEMANTIC_UNSEEN_SRO_RUN_OK"
