#!/usr/bin/env python3
"""ASTRA-C1-SEMANTIC-UNSEEN-SRO-01 independent host gold.

ONE UNKNOWN: plant >=1 evidence fact whose (s,r,o) is NOT generated by the
cartesian `{ent} {rel} {ent}` fill used for bulk docs; a query bound to
that SRO must emit that nid (gold_n>=1 retrieve); a query bound to an
(s,r,o) that is NOT indexed must NOT emit fill-grid neighbors {120,121,122}
(empty walk ok). Instantiate ctx keys 124be808 + DUT 8255a798 not edited.
...
N=256 (allowed smaller than 16384; SRO identity, not scale).
SEARCH_INCOMPLETE on gold_n>=1 retrieve = FAIL. C1 800k stays OPEN.
BOARD_PASS not claimed. Do not edit KEEP bags. Do not write independent
audit tree. G_BYTES: LSB-first (char0 at bits[7:0]) matching TB
bytes[8*bi +: 8].
"""
...
GATE = "ASTRA-C1-SEMANTIC-UNSEEN-SRO-01"
...
CTX_GLYCOL = 3
CTX_STEAM = 4
FILL_ENT_IDS = (13, 14, 15)  # boiler, header, coil
...
def cartesian_fill_sros() -> list[tuple[int, int, int]]:
    ...
    for s in FILL_ENT_IDS:
        for r in range(1, 9):
            for o in FILL_ENT_IDS:
                if s == o:
                    continue
                sro = (s, r, o)
                if sro in reserved:
...
def build_corpus() -> list[dict]:
    ...
    while len(docs) < FILL_GRID_NIDS[0]:
        add(pad_src[fi % len(pad_src)], 0, "pad_synth")
        fi += 1
    if len(docs) != 120:
        raise SystemExit(f"fill-grid base {len(docs)} want 120")
    ...
    add(triple_text(UNSEEN_S, UNSEEN_R, UNSEEN_O), 1, "unseen_sro", (UNSEEN_S, UNSEEN_R, UNSEEN_O))
...
def write_svh(
    ...
    lines = [
        "// generated by host_astra_c1_semantic_unseen_sro.py — independent gold BEFORE xvlog. do not hand-edit after FAIL",
        f"// Law {LAW} + named lexicon {LAW_LEX}; STREAM-02 walker in context wrapper; N=256 unseen SRO",
...
def main() -> int:
    ...
    chk(N == 256, "N dropped from registered 256")
    chk(CAND_CAP < N, "cap>=N")
    chk(all(d["subj_id"] < 16 and d["obj_id"] < 16 for d in docs), "entity id >=16 would 12-bit alias N_BUCKETS=4096")
    psc_clone = [
        d
        for d in docs
        if d["evidence"] == 1
    ...
    chk(len(fill_ev1) == len(fill_sros), "fill count")
    ...
    for d in docs:
        if d["kind"] in ("fill", "pad_synth"):
            ...
            if d["kind"] == "fill":
                chk(d["subj_id"] in FILL_ENT_IDS and d["obj_id"] in FILL_ENT_IDS, f"fill ent not FILL_ENT_IDS nid={d['nid']}")
    ...
    if fails:
        print("ASTRA_C1_SEMANTIC_UNSEEN_SRO_HOST_FAIL")
        for f in fails:
            print("FAIL", f)
        return 2
    ...
    corpus_pub = {
        ...
        "fill_ent_ids": list(FILL_ENT_IDS),
    ...
    golden = {
        ...
        "index_fill_template": "{ent} {rel} {ent} over FILL_ENT_IDS={13,14,15}",
    ...
    print("ASTRA_C1_SEMANTIC_UNSEEN_SRO_HOST_PASS")
    ...
    print("FILL_SRO_N", len(fill_sros), "UNSEEN_NID", UNSEEN_NID, "FILL_GRID", list(FILL_GRID_NIDS))
    print("GOLD_SHA", gsha)
    print("SVH_SHA", ssha)
