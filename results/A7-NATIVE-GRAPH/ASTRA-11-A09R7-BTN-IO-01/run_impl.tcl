# ASTRA-11-A09R7-BTN-IO-01. PROGRAM=NO. BIT=NO. write_bitstream forbidden.
...
set files [list \
  ...
  [file join $bag a7ng_astra_11_a09r7_btn_io_plant.sv] \
  [file join $bag a7ng_astra_11_a09r7_btn_io_wrap.sv] \
]
set xdc [file join $bag clk50_btn_io.xdc]
...
proc astra_forbid_bit {args} {
  ...
  puts ASTRA_11_A09R7_BTN_IO_ABORTED
  exit 1
}
...
proc astra_copy_fail_r0 {} {
  ...
  foreach f {timing_route.rpt util_route.rpt route_status.rpt drc.rpt clocks_route.rpt \
             ...
             BTN_IOB.txt BTN_IODELAY.txt BTN_IOBFF.txt timing_btn_in.rpt} {
    set src [file join $bag $f]
    if {[file exists $src]} {
      file copy -force $src [file join $dst $f]
    }
  }
  puts "ASTRA_11_A09R7_BTN_IO_FAIL_R0_PRESERVED $dst"
}
...
puts "BTN_IOBFF_POLICY=IOB=TRUE wrap btn_q"
...
if {[catch {synth_design -top $top -part $part} err]} {
  ...
  puts ASTRA_11_A09R7_BTN_IO_SYNTH_FAIL
  exit 1
}
...
astra_util_extract [file join $bag util_synth.rpt] [file join $bag UTIL_EXTRACT_SYNTH.txt] Synthesized
astra_timing_extract [file join $bag timing_synth.rpt] [file join $bag TIMING_EXTRACT_SYNTH.txt] Synthesized
puts ASTRA_11_A09R7_BTN_IO_SYNTH_DONE
...
if {[catch {opt_design} err]} {
  astra_fail OPT $err
  puts ASTRA_11_A09R7_BTN_IO_OPT_FAIL
  exit 1
}
astra_force_iob_true
astra_apply_hold_policy
puts ASTRA_11_A09R7_BTN_IO_OPT_DONE
...
if {[catch {place_design} err]} {
  ...
  puts ASTRA_11_A09R7_BTN_IO_PLACE_FAIL
  exit 1
}
...
astra_apply_hold_policy
puts ASTRA_11_A09R7_BTN_IO_PLACE_DONE
...
if {[catch {route_design} err]} {
  ...
  puts ASTRA_11_A09R7_BTN_IO_ROUTE_FAIL
  exit 1
}
...
puts "ASTRA_11_A09R7_BTN_IO_ROUTE_DONE WNS=$wns TNS=$tns WHS=$whs THS=$ths BTN_IOB=YES LED_IOB=YES UART_IOB=YES UART_IOBFF=$packed LED_IOBFF=$led_packed BTN_IOBFF=$btn_packed BTN_IN_MAX=2.000 BTN_IN_MIN=0.500 PROGRAM=NO BIT=NOT_BUILT"
...
if {$fail_whs || $fail_wns} {
  ...
  if {[catch {phys_opt_design} e2]} {
    puts "ASTRA_11_A09R7_BTN_IO_EXP_PHYS_OPT_WARN $e2"
  } else {
    puts ASTRA_11_A09R7_BTN_IO_EXP_PHYS_OPT_DONE
  }
  ...
  puts ASTRA_11_A09R7_BTN_IO_EXP_DONE
  ...
  puts "ASTRA_11_A09R7_BTN_IO_DONE WNS=$wns WHS=$whs BTN_IOB=$btn_iob LED_IOB=$led_iob UART_IOB=YES UART_IOBFF=$packed LED_IOBFF=$led_packed BTN_IOBFF=$btn_packed BIT=NOT_BUILT PROGRAM=NO RESULT=$result_tag PRODUCTION_TOP=UNKNOWN"
  exit 0
}

puts "ASTRA_11_A09R7_BTN_IO_DONE WNS=$wns TNS=$tns WHS=$whs BTN_IOB=YES LED_IOB=YES UART_IOB=YES UART_IOBFF=$packed LED_IOBFF=$led_packed BTN_IOBFF=$btn_packed BIT=NOT_BUILT PROGRAM=NO PRODUCTION_TOP=UNKNOWN"
exit 0
