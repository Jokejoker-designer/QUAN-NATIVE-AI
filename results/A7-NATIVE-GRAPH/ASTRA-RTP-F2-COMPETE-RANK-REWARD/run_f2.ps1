...
$files = @(
  ...
  (Join-Path $root "rtl\native_graph\integrate\a7ng_astra_rtp_f2.sv"),
  (Join-Path $bag "tb_astra_f2.sv")
)
& "$bin\xvlog.bat" --sv -i $incq -i $incc $files
if ($LASTEXITCODE -ne 0) { throw "F2_XVLOG_FAIL" }
& "$bin\xelab.bat" tb_astra_f2 -s f2 -timescale 1ns/1ps
if ($LASTEXITCODE -ne 0) { throw "F2_XELAB_FAIL" }
$log = Join-Path $bag "xsim.log"
& "$bin\xsim.bat" f2 -R -log $log
if ($LASTEXITCODE -ne 0) { throw "F2_XSIM_FAIL" }
if (-not (Select-String -Path $log -Pattern "ASTRA_RTP_F2_XSIM_PASS" -Quiet)) { throw "F2_PASS_MARKER_MISSING" }
Write-Host "ASTRA_RTP_F2_RUN_OK"
