"""Conjunctive hardware A7-LM-06 board ladder (802816-byte scale).

Host compares only. Not an 8-class CE close.
...
from python.ref.a7lm06_fixed_ref import (  # noqa: E402
    L,
    LAYER_WORDS,
    OFF_HEAD,
    OFF_L0,
...
BIT_NAME = "arty_a7_lm06.bit"
BIT_MANIFEST = ROOT / "results" / "A7-LM-06" / "build_manifest.json"
OUT_DIR = ROOT / "results" / "A7-LM-06" / "hardware_c1"
...
def read8(port, stream: FrameStream, addr: int):
    L04.send(port, lm06_read_frame(addr))
...
def upload(port, stream: FrameStream, blob: list[int]) -> None:
    if len(blob) != PARAM_COUNT:
        raise ValueError("wrong LM-06 image length")
    layer_boundaries = {OFF_POS, OFF_L0, *(OFF_L0 + li * LAYER_WORDS for li in range(1, L)), OFF_HEAD}
    for addr in range(0, PARAM_COUNT, 8):
        chunk = bytes((v & 0xFF) for v in blob[addr : addr + 8])
        try:
            L04.send(port, lm06_write_frame(addr, chunk))
...
def target_frame(target: int, lr: int = 3) -> bytes:
    if not 0 <= target < 1024:
        raise ValueError("target must be 10-bit")
    return lm04_payload_frame(
        bytes([0x34, target & 0xFF, (lr & 0xF) | (((target >> 8) & 3) << 4)]) + bytes(9)
    )
...
def main() -> int:
    global BIT_MANIFEST, OUT_DIR, OUT_PATH
    port_name = sys.argv[1] if len(sys.argv) > 1 else "COM12"
    tag = sys.argv[2] if len(sys.argv) > 2 else "c1"
    if tag == "c2":
        BIT_MANIFEST = ROOT / "results" / "A7-LM-06" / "build_manifest_c2.json"
        OUT_DIR = ROOT / "results" / "A7-LM-06" / "hardware_c2"
        OUT_PATH = OUT_DIR / "ladder.json"
    if tag == "c3":
        BIT_MANIFEST = ROOT / "results" / "A7-LM-06" / "build_manifest_c3.json"
        OUT_DIR = ROOT / "results" / "A7-LM-06" / "hardware_c3"
        OUT_PATH = OUT_DIR / "ladder.json"
    if OUT_PATH.exists():
        raise RuntimeError(f"hardware candidate already recorded; refuse overwrite: {OUT_PATH}")
    if tag in ("c2", "c3"):
        c1 = ROOT / "build" / "out" / "arty_a7_lm06.bit"
        got = sha256(c1)
        if got != C1_SHA:
            raise RuntimeError(f"C1 bit mutated before {tag.upper()} ladder: {got}")
    if tag == "c3":
        c2 = ROOT / "build" / "out" / "arty_a7_lm06c2.bit"
        got = sha256(c2)
        if got != "EDAAF1204072664DFA5E62CA445B65DAE680C18777AC73DDEFD075C0ED6DD483":
            raise RuntimeError(f"C2 bit mutated before C3 ladder: {got}")
    checks = static_checks()
    if not (checks["bit_match"] and checks["timing_close"] and checks["frozen_ok"]):
        raise RuntimeError(f"static gate failed: {checks}")

    model0 = TinyGPT803k(2)
    ...
    summary = {
        "revision": f"A7-LM-06-HW-{tag.upper()}",
    ...
    try:
        ...
        spot_addrs = [0, OFF_POS, OFF_L0, OFF_L0 + LAYER_WORDS, OFF_L0 + 2 * LAYER_WORDS,
                      OFF_L0 + 3 * LAYER_WORDS, OFF_HEAD, PARAM_COUNT - 8]
