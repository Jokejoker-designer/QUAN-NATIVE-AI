// a7ng_astra_f2r4_axi_drain.sv — ASTRA-F2R-R4-AXI-TIMEOUT-DRAIN-01. PROGRAM=NO.
// New candidate. Does not edit F2R-01 / F2R-R2 / F2R-R3 bags or frozen RTL.
// Instantiates frozen a7ng_shared_rank_sgd_q8_sym_f2r2. F2R3 semantic FSM kept;
// S_AR/S_R abort+drain+reset instead of drop-ARVALID-and-continue.
`timescale 1ns / 1ps
`include "a7ng_astra_f2r4_axi_drain.svh"

module a7ng_astra_f2r4_axi_drain #(
  parameter int unsigned ID_W       = 20,
  parameter int unsigned CAND_CAP   = 16,
  parameter int unsigned TO_CYC     = A7NG_F2R4_TO_CYC,
  parameter int unsigned DRAIN_TO   = A7NG_F2R4_DRAIN_TO,
  ...
  function automatic logic [3:0] next_fact_arid(input logic [15:0] dead);
    integer i;
    begin
      next_fact_arid = A7NG_F2R4_FACT_RID0;
      for (i = 15; i >= 2; i = i - 1)
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      f_arvalid <= 1'b0; f_araddr <= '0; f_arid <= A7NG_F2R4_FACT_RID0;
      axi_ost <= 1'b0; axi_abort <= 1'b0; ost_rid <= A7NG_F2R4_FACT_RID0;
        ...
        S_R: begin
          ...
          if (kill_r) begin
            ...
            if (m_axi_rid == ost_rid) begin
              if (m_axi_rresp != A7NG_F2R4_RRESP_OK) begin
