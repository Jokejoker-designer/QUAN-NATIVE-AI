// a7ng_astra_rtp_pipe_r2.sv — RTP-R2. PROGRAM=NO.
// Keep R1 pipe frozen. Adds AR-hold timeout + late-R still valid; TB high-ID / eid mismatch.
`timescale 1ns / 1ps

module a7ng_astra_rtp_pipe_r2 #(
  ...
  logic [CW:0] nc, fi, ci;
  logic [7:0] nload;
  logic [15:0] nfar, nfok, nferr, nfto, narto, tocnt;
      ...
      unique case (st)
        ...
        S_R: begin
          ...
          if (f_rvalid) begin
            ...
            if (fi + 1'b1 == nc) st <= S_ISSUE;
            else begin fi <= fi + 1'b1; tocnt <= '0; st <= S_AR; end
          end else if (tocnt >= TO_CYC[15:0]) begin
