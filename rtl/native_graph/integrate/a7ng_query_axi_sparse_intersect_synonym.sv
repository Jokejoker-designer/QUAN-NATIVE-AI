// a7ng_query_axi_sparse_intersect_synonym.sv — ASTRA-C1-SYNONYM-LAW-01
// Law: qse-v2-relctx-synonym-01
// Frozen extract (cd7baf49, not edited) → NEW named synonym overlay
// (qse-v2-relctx-synonym-01) → frozen ctx keys (124be808, not edited) →
// frozen route gate. Walker = STREAM-02 two-pointer copy (14f75db7 not
// edited and not compiled as DUT). Context DUT 8255a798 KEEP not compiled
// as DUT and not edited.
// Do NOT patch C0 extract/lexicon FILE. Do NOT rewrite named 187-word lex.
// Do NOT union k0 with k1. Do NOT probe k2/k3. poke_v held 0 by TB. PROGRAM=NO.
`timescale 1ns / 1ps

module a7ng_query_axi_sparse_intersect_synonym #(
  ...
  logic [7:0]  rel_ex, rel_syn;
  logic [15:0] k0_syn, k1_syn;
  logic        syn_hit_w;
  logic        p0, p1, p2, p3;
  ...
  a7ng_query_role_extract u_qse (
    ...
    .rel_id_o(rel_ex), .ctx_id_o(context_id_o),
  ...
  a7ng_query_role_relctx_synonym u_syn (
    ...
    .rel_id_o(rel_syn),
    .k0_o(k0_syn), .k1_o(k1_syn),
    .syn_hit_o(syn_hit_w)
  );

  assign relation_id_o = rel_syn;
