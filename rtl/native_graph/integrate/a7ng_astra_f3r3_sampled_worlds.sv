// a7ng_astra_f3r3_sampled_worlds.sv — ASTRA-F3-R3-SAMPLED-WORLDS-01. PROGRAM=NO.
// New named ranker. Does not edit F2R-01/R2/R3/R4/R5, ASTRA-F3-SHARED-TRANSFER-01, or ASTRA-F3-R2-INDEPENDENT-WORLDS-01 bags or frozen RTL.
...
`timescale 1ns / 1ps
`include "a7ng_astra_f3r3_sampled_worlds.svh"

module a7ng_astra_f3r3_sampled_worlds #(
  parameter int unsigned ID_W       = 20,
  parameter int unsigned CAND_CAP   = 16,
  parameter int unsigned TO_CYC     = A7NG_F3R3_TO_CYC,
  parameter int unsigned MAX_PATH   = A7NG_F3R3_MAX_PATH,
  ...
  logic r_ovf, r_neg, r_amb, r_two, r_obj_v;
  logic [3:0] r_st;
  ...
  assign m_axi_arid = fetch_mode ? A7NG_F3R3_FACT_RID : sp_arid;
    ...
    if (!rst_n) begin
      ...
      unique case (st)
        S_IDLE: begin
          ...
          retire_hold <= 1'b0; f_arvalid <= 1'b0;
          ...
          if (qse_valid) begin
            ...
            r_two <= (qse_ctx == A7NG_F3R3_CTX_INDIRECT);
        ...
        S_WALK: begin
          ...
          if (w_done) begin
            ...
            if (r_amb) begin r_st <= A7NG_F3R3_ST_AMB; st <= S_HOLD; end
            else if (r_neg) begin r_st <= A7NG_F3R3_ST_NEG; st <= S_HOLD; end
        ...
        S_AR: begin
          ...
          if (f_arready && f_arvalid) begin
            ...
            f_arvalid <= 1'b0; r_st <= A7NG_F3R3_ST_INCOMP; st <= S_HOLD;
          end
        end
        S_R: begin
          tocnt <= tocnt + 16'd1;
          if (f_rvalid) begin
            if ((m_axi_rid==A7NG_F3R3_FACT_RID) && (m_axi_rresp==A7NG_F3R3_RRESP_OK)
                && m_axi_rlast && (m_axi_rdata[75:72]==A7NG_F3R3_VER) && m_axi_rdata[70]
            ...
            r_st <= A7NG_F3R3_ST_INCOMP; st <= S_HOLD;
          end
        end
        S_EI: begin
          if (r_ovf) begin r_st <= A7NG_F3R3_ST_INCOMP; st <= S_HOLD; end
          else if (ei >= nf) begin
            if (np == 0) begin r_st <= A7NG_F3R3_ST_UNKNOWN; st <= S_HOLD; end
