// a7ng_astra_06_warm_persist.sv — ASTRA-06-WARM-PERSIST-01. PROGRAM=NO.
// New named DUT. Does not edit F2R-01/R2/R3/R4/R5 or F3 bags or frozen RTL.
// Instantiates frozen a7ng_shared_rank_sgd_q8_sym_f2r2.
// rst_n clears live FSM / AXI ost / in-flight SGD. Persist store is not
// on rst_n: weights + pending {sess,gen,txn,phi,v_pred} + 20-bit eids.
`timescale 1ns / 1ps
`include "a7ng_astra_06_warm_persist.svh"

module a7ng_astra_06_warm_persist #(
  parameter int unsigned ID_W       = 20,
  parameter int unsigned CAND_CAP   = 16,
  parameter int unsigned TO_CYC     = A7NG_A06_TO_CYC,
  parameter int unsigned DRAIN_TO   = A7NG_A06_DRAIN_TO,
  parameter int unsigned MAX_PATH   = 4,
  parameter logic [7:0]  TXN_MAX    = A7NG_A06_TXN_MAX,
  ...
  integer k, kf, pk;
  logic        p_valid, p_acc, p_cmt, need_rl, pick_ok;
  logic [15:0] p_epoch, p_sess_epoch;
  logic [7:0]  p_gen, p_txn;
  logic [1:0]  p_sel;
  logic [4:0]  rl_idx;
  ...
  assign pick_ok = (st==S_PICK) && (sess_id_i != A7NG_A06_EPOCH_INV) &&
  ...
  function automatic logic [3:0] next_fact_arid(input logic [15:0] dead);
    integer i;
    begin
      next_fact_arid = A7NG_A06_FACT_RID0;
      for (i = 15; i >= 2; i = i - 1)
  ...
  always_ff @(posedge clk) begin
    if (persist_clr_i) begin
      ...
      p_gen <= 8'd0; p_txn <= 8'd0; p_sel <= 2'd0;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      f_arvalid <= 1'b0; f_araddr <= '0; f_arid <= A7NG_A06_FACT_RID0;
      axi_ost <= 1'b0; axi_abort <= 1'b0; ost_rid <= A7NG_A06_FACT_RID0;
      ...
      unique case (st)
        S_IDLE: begin
          ...
          r_st <= ST_UNKNOWN;
          if (need_rl && p_valid && restore_en_i && sgd_ready) begin
            need_rl <= 1'b0;
            rl_idx <= 5'd0;
            st <= S_RELOAD;
          end else if (need_rl && !p_valid) begin
            need_rl <= 1'b0;
          end else if (reload_i && p_valid && sgd_ready) begin
            rl_idx <= 5'd0;
            st <= S_RELOAD;
          end else if (rew_v_i) begin
            if (!pend_acc && ((pend_epoch == A7NG_A06_EPOCH_INV) || (rew_epoch_i != pend_epoch)))
              nstale <= nstale + 16'd1;
        ...
        S_R: begin
          ...
          if (kill_r) begin
            ...
            if (m_axi_rid == ost_rid) begin
              if (m_axi_rresp != A7NG_A06_RRESP_OK) begin
        ...
        S_PICK: begin
          ...
          sel_idx <= c_best_idx;
          r_st <= 4'd0;
          if ((sess_id_i == A7NG_A06_EPOCH_INV) ||
