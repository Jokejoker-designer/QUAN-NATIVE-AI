// a7ng_astra_06_r4_sess_reuse.sv — ASTRA-06-R4-SESS-REUSE-01. PROGRAM=NO.
// New named DUT. Does not patch a7ng_astra_06_warm_persist.sv, a7ng_astra_06_r2*.sv,
// a7ng_astra_06_r3_multi_slot.sv, or F2R/F3 RTL.
// Instantiates frozen a7ng_shared_rank_sgd_q8_sym_f2r2.
// Sess lifetime: sess_id is part of pending/persist key. rst without restore
// must not replay pre-rst {sess,txn} as a new live pending.
`timescale 1ns / 1ps
`include "a7ng_astra_06_r4_sess_reuse.svh"

module a7ng_astra_06_r4_sess_reuse #(
  parameter int unsigned ID_W       = 20,
  parameter int unsigned CAND_CAP   = 16,
  parameter int unsigned TO_CYC     = A7NG_A06R4_TO_CYC,
  parameter int unsigned DRAIN_TO   = A7NG_A06R4_DRAIN_TO,
  parameter int unsigned MAX_PATH   = 4,
  parameter logic [7:0]  TXN_MAX    = A7NG_A06R4_TXN_MAX,
  parameter int unsigned CAP_N      = A7NG_A06R4_CAP_N,
  parameter logic [7:0]  SCHEMA_VER = A7NG_A06R4_SCHEMA_VER,
  ...
  output logic [15:0] persist_sess_o,
  output logic [15:0] s0_sess_o,
  output logic [15:0] s1_sess_o,
  output logic        sess_bound_o,
  output logic [15:0] n_sess_reuse_o,
  output logic [15:0] n_sess_rej_o
);
  ...
  logic        sess_match, sess_reuse, sess_bound;
  ...
  logic [15:0] n_sess_reuse, n_sess_rej;
  integer si, kj;
  ...
  assign qse_tok_v = tok_valid_i && (st==S_IDLE) && !(need_rl && restore_en_i && schema_ok && sess_match);
  assign qse_fire = fire_i && (st==S_IDLE) && !(need_rl && restore_en_i && schema_ok && sess_match);
  assign tok_ready_o = qse_tok_r && (st==S_IDLE) && !(need_rl && restore_en_i && schema_ok && sess_match);
  assign cand_ready = (st==S_WALK);
  assign busy_o = (st!=S_IDLE) || (need_rl && restore_en_i && schema_ok && sess_match);
  ...
  assign reload_busy_o = (st==S_RELOAD) ||
                         (need_rl && restore_en_i && schema_ok && sess_match) ||
                         (reload_i && schema_ok && sess_match);
  ...
  assign sess_match = p_valid && (sess_id_i != A7NG_A06R4_EPOCH_INV) &&
                      (sess_id_i == p_sess_epoch);
  assign sess_bound = (sess_epoch != A7NG_A06R4_EPOCH_INV) && (sess_epoch == sess_id_i);
  ...
  assign persist_sess_o = p_sess_epoch;
  assign s0_sess_o = ps_sess_epoch[0];
  assign s1_sess_o = ps_sess_epoch[1];
  assign sess_bound_o = sess_bound;
  assign n_sess_reuse_o = n_sess_reuse;
  assign n_sess_rej_o = n_sess_rej;
  assign pick_ok = (st==S_PICK) && (sess_id_i != A7NG_A06R4_EPOCH_INV) &&
                   !((sess_id_i == sess_epoch) && (txn == TXN_MAX)) &&
                   !(persist_en_i && inst_refuse) &&
                   !(persist_en_i && sess_reuse);
  assign sgd_load_v = (st==S_RELOAD) || (load_v_i && (st==S_IDLE) && !(need_rl && restore_en_i && schema_ok && sess_match));
  ...
  function automatic logic [3:0] next_fact_arid(input logic [15:0] dead);
    integer i;
    begin
      next_fact_arid = A7NG_A06R4_FACT_RID0;
      for (i = 15; i >= 2; i = i - 1)
  ...
  always_comb begin
    ...
    sess_reuse = 1'b0;
    for (kj = 0; kj < CAP_N; kj = kj + 1) begin
      if (ps_valid[kj] && (sess_id_i != A7NG_A06R4_EPOCH_INV) &&
          (ps_sess_epoch[kj] == sess_id_i) && (sess_epoch != sess_id_i))
        sess_reuse = 1'b1;
    end
  ...
  always_ff @(posedge clk) begin
    if (persist_clr_i) begin
      for (si = 0; si < CAP_N; si = si + 1) begin
        ...
        ps_epoch[si] <= A7NG_A06R4_EPOCH_INV;
        ps_sess_epoch[si] <= A7NG_A06R4_EPOCH_INV;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      f_arvalid <= 1'b0; f_araddr <= '0; f_arid <= A7NG_A06R4_FACT_RID0;
      axi_ost <= 1'b0; axi_abort <= 1'b0; ost_rid <= A7NG_A06R4_FACT_RID0;
      ...
      pend_epoch <= A7NG_A06R4_EPOCH_INV; sess_epoch <= A7NG_A06R4_EPOCH_INV;
      nexh <= '0; txn_exh <= 1'b0;
      ...
      unique case (st)
        S_IDLE: begin
          ...
          v_best <= '0; v_second <= '0;
          r_st <= ST_UNKNOWN;
          if (need_rl && restore_en_i && schema_ok && sess_match && sgd_ready) begin
            need_rl <= 1'b0;
            rl_idx <= 5'd0;
            rl_slot <= obs_sel;
            n_rl_auto <= n_rl_auto + 16'd1;
            st <= S_RELOAD;
          end else if (need_rl && restore_en_i && schema_ok && !sess_match) begin
            need_rl <= 1'b0;
            n_sess_rej <= n_sess_rej + 16'd1;
            ...
            need_rl <= 1'b0;
          end else if (reload_i && schema_ok && sess_match && sgd_ready) begin
            need_rl <= 1'b0;
            rl_idx <= 5'd0;
            rl_slot <= obs_sel;
            n_rl_cmd <= n_rl_cmd + 16'd1;
            st <= S_RELOAD;
          end else if (reload_i && p_valid && schema_ok && !sess_match) begin
            n_sess_rej <= n_sess_rej + 16'd1;
          end else if (reload_i && p_valid && !schema_ok) begin
            need_rl <= 1'b0;
            n_schema_rej <= n_schema_rej + 16'd1;
          end else if (rew_v_i) begin
            if (!pend_acc && ((pend_epoch == A7NG_A06R4_EPOCH_INV) || (rew_epoch_i != pend_epoch)))
              nstale <= nstale + 16'd1;
        ...
        S_R: begin
          ...
          if (kill_r) begin
            ...
            if (m_axi_rid == ost_rid) begin
              if (m_axi_rresp != A7NG_A06R4_RRESP_OK) begin
        ...
        S_PICK: begin
          ...
          sel_idx <= c_best_idx;
          r_st <= 4'd0;
          if ((sess_id_i == A7NG_A06R4_EPOCH_INV) ||
            ...
            pend_acc <= 1'b0;
            pend_cmt <= 1'b0;
          end else if (persist_en_i && sess_reuse) begin
            n_sess_reuse <= n_sess_reuse + 16'd1;
            txn_exh <= 1'b0;
            pend_acc <= 1'b0;
            pend_cmt <= 1'b0;
          end else begin
