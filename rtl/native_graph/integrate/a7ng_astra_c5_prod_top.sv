// a7ng_astra_c5_prod_top.sv — ASTRA-C5-PROD-TOP-01. PROGRAM=NO.
// New named production hierarchy. Instantiates UART, C3 held-out (parser +
// sparse DDR index + desc + scorer + typed proof + pending reward), C5
// exclusive DDR arbiter, C2 persist, C4 grounded gen. Does not edit KEEP,
// a7ng_lm_graph_arb, leftover A09, tiny_gpt803k_core, or official mig.prj.
// C5 MIG flush/reload: hold CKPT req across UART 0x12→EOL while C2 is in
// S_AR/S_R (reload_pend || p_busy). Else arbiter drops owner=NONE and
// prod_top drains the real persist R beat. Does not edit KEEP.
// C5 unified bag: ST_DONE returns to ST_UART for the next line; non-ANSWER
// skips persist/gen; UART 0x0C/0x12/0x14/0x15 = flush/reload/freeze/learn.
// Does not edit KEEP. Does not stamp C5_MASTER.
`timescale 1ns / 1ps
      ...
      unique case (pst)
        ...
        ST_WLOAD: begin
          ...
          unique case (wsel)
            ...
            default: begin g_sel <= A7NG_C4G_LD_EOS; g_w <= 16'sd200; end
          endcase
