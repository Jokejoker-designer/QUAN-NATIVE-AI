// a7ng_astra_rtp_f2t.sv — shared CLASS features, transfer world. PROGRAM=NO.
// Does not use timing-fix DSP SGD. Does not one-hot mid==1/8.
`timescale 1ns / 1ps

module a7ng_astra_rtp_f2t #(
  ...
  a7ng_shared_rank_sgd_q8_v1 u_sgd (
  ...
  always_comb begin
    ...
    if (st==S_SC || st==S_SW) begin
      phi[0] = (pcls[pi[1:0]] == 8'd1) ? 8'sd64 : 8'sd0;
      phi[1] = (pcls[pi[1:0]] == 8'd2) ? 8'sd64 : 8'sd0;
      phi[2] = 8'sd0;
    end else begin
      for (k = 0; k < 32; k = k + 1) phi[k] = xsel[k];
    end
    c_best_v = 16'sh8000; c_alt_v = 16'sh8000;
    c_best_a = '0; c_best_p0 = 20'hFFFFF; c_best_p1 = '0; c_best_m = '0; c_best_cls = '0;
    for (k = 0; k < 4; k = k + 1) begin
      if (k < np) begin
        if ((pv[k] > c_best_v) || ((pv[k]==c_best_v) && (pp0[k] < c_best_p0))) begin
          c_alt_v = c_best_v;
          c_best_v = pv[k]; c_best_a = pans[k]; c_best_p0 = pp0[k];
          c_best_p1 = pp1[k]; c_best_m = pmid[k]; c_best_cls = pcls[k];
    ...
    if (!rst_n) begin
      ...
      unique case (st)
        ...
        S_PICK: begin
          ...
          best_cls <= c_best_cls;
          xsel[0] <= (c_best_cls==8'd1) ? 8'sd64 : 8'sd0;
          xsel[1] <= (c_best_cls==8'd2) ? 8'sd64 : 8'sd0;
          xsel[2] <= 8'sd0;
          st <= S_HOLD;
        end
        S_HOLD: begin
