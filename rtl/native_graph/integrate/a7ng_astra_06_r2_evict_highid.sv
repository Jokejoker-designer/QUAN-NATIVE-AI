// a7ng_astra_06_r2_evict_highid.sv — ASTRA-06-R2-EVICT-HIGHID-01. PROGRAM=NO.
// New named DUT. Does not patch a7ng_astra_06_warm_persist.sv or F2R/F3 RTL.
// Instantiates frozen a7ng_shared_rank_sgd_q8_sym_f2r2.
// High-bit 20-bit eids, explicit reload_i, persist capacity N=1
// REFUSE_UNCOMMITTED_EVICT_COMMITTED, schema byte fail-closed restore.
`timescale 1ns / 1ps
`include "a7ng_astra_06_r2_evict_highid.svh"

module a7ng_astra_06_r2_evict_highid #(
  parameter int unsigned ID_W       = 20,
  parameter int unsigned CAND_CAP   = 16,
  parameter int unsigned TO_CYC     = A7NG_A06R2_TO_CYC,
  parameter int unsigned DRAIN_TO   = A7NG_A06R2_DRAIN_TO,
  parameter int unsigned MAX_PATH   = 4,
  parameter logic [7:0]  TXN_MAX    = A7NG_A06R2_TXN_MAX,
  parameter int unsigned CAP_N      = A7NG_A06R2_CAP_N,
  parameter logic [7:0]  SCHEMA_VER = A7NG_A06R2_SCHEMA_VER,
  ...
  integer k, kf, pk;
  logic        p_valid, p_acc, p_cmt, need_rl, pick_ok, cap_full, schema_ok, keep_proof;
  ...
  assign qse_tok_v = tok_valid_i && (st==S_IDLE) && !(need_rl && p_valid && restore_en_i);
  assign qse_fire = fire_i && (st==S_IDLE) && !(need_rl && p_valid && restore_en_i);
  assign tok_ready_o = qse_tok_r && (st==S_IDLE) && !(need_rl && p_valid && restore_en_i);
  assign cand_ready = (st==S_WALK);
  assign busy_o = (st!=S_IDLE) || (need_rl && p_valid && restore_en_i);
  ...
  assign pick_ok = (st==S_PICK) && (sess_id_i != A7NG_A06R2_EPOCH_INV) &&
                   !((sess_id_i == sess_epoch) && (txn == TXN_MAX)) &&
                   !(persist_en_i && cap_full);
  assign sgd_load_v = (st==S_RELOAD) || (load_v_i && (st==S_IDLE) && !(need_rl && p_valid && restore_en_i));
  assign sgd_load_idx = (st==S_RELOAD) ? rl_idx : load_idx_i;
  assign sgd_load_w = (st==S_RELOAD) ? p_w[rl_idx] : load_w_i;

  function automatic logic [3:0] next_fact_arid(input logic [15:0] dead);
    integer i;
    begin
      next_fact_arid = A7NG_A06R2_FACT_RID0;
      for (i = 15; i >= 2; i = i - 1)
  ...
  always_ff @(posedge clk) begin
    if (persist_clr_i) begin
      ...
      p_epoch <= A7NG_A06R2_EPOCH_INV; p_sess_epoch <= A7NG_A06R2_EPOCH_INV;
      p_gen <= 8'd0; p_txn <= 8'd0; p_sel <= 2'd0; p_ver <= SCHEMA_VER;
      p_vpred <= 16'sd0; p_ans <= '0; p_p0 <= '0; p_p1 <= '0;
      ...
      if (pick_ok) begin
        ...
        p_ver <= schema_i;
        p_epoch <= sess_id_i;
        p_sess_epoch <= sess_id_i;
        if (sess_id_i != sess_epoch) begin
          p_gen <= 8'd1; p_txn <= 8'd1;
        end else begin
          p_gen <= gen + 8'd1; p_txn <= txn + 8'd1;
        end
        p_sel <= c_best_idx;
        p_vpred <= c_best_v;
        p_ans <= c_best_a;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      f_arvalid <= 1'b0; f_araddr <= '0; f_arid <= A7NG_A06R2_FACT_RID0;
      axi_ost <= 1'b0; axi_abort <= 1'b0; ost_rid <= A7NG_A06R2_FACT_RID0;
      ...
      pend_epoch <= A7NG_A06R2_EPOCH_INV; sess_epoch <= A7NG_A06R2_EPOCH_INV;
      nexh <= '0; txn_exh <= 1'b0;
      ...
      unique case (st)
        S_IDLE: begin
          ...
          if (need_rl && p_valid && restore_en_i && schema_ok && sgd_ready) begin
            ...
            if (!pend_acc && ((pend_epoch == A7NG_A06R2_EPOCH_INV) || (rew_epoch_i != pend_epoch)))
              nstale <= nstale + 16'd1;
        ...
        S_R: begin
          ...
          if (kill_r) begin
            ...
            if (m_axi_rid == ost_rid) begin
              if (m_axi_rresp != A7NG_A06R2_RRESP_OK) begin
        ...
        S_PICK: begin
          ...
          sel_idx <= c_best_idx;
          r_st <= 4'd0;
          if ((sess_id_i == A7NG_A06R2_EPOCH_INV) ||
              ((sess_id_i == sess_epoch) && (txn == TXN_MAX))) begin
            nexh <= nexh + 16'd1;
            txn_exh <= 1'b1;
            pend_acc <= 1'b0;
            pend_cmt <= 1'b0;
          end else if (persist_en_i && cap_full) begin
        ...
        S_RELOAD: begin
          if (rl_idx == 5'd31) begin
            ...
            best_p1 <= p_p1;
            keep_proof <= 1'b1;
            for (kf = 0; kf < 32; kf = kf + 1) pend_phi[kf] <= p_phi[kf];
            st <= S_IDLE;
