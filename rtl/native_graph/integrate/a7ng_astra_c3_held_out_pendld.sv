...
module a7ng_astra_c3_held_out_pendld #(
  ...
  input  logic        phi_load_v_i,
  input  logic [4:0]  phi_idx_i,
  input  logic signed [7:0]  phi_w_i,
  input  logic        pend_load_v_i,
  input  logic [19:0] pend_p0_i,
  input  logic [19:0] pend_p1_i,
  input  logic [19:0] pend_ans_i,
  input  logic [1:0]  pend_sel_i,
  input  logic [3:0]  pend_st_i,
  input  logic [7:0]  pend_txn_i,
  input  logic [7:0]  pend_gen_i,
  input  logic        pend_acc_i,
  input  logic        pend_cmt_i,
  input  logic        clr_pend_i,
  output logic        busy_o,
      ...
      unique case (st)
        S_IDLE: begin
          ...
          if (clr_pend_i) begin
            ...
            r_st <= A7NG_C3_ST_UNKNOWN;
            proof_ok <= 1'b0;
            pend_id <= 8'd0; pend_gen <= 8'd0;
            pend_acc <= 1'b0; pend_cmt <= 1'b0;
            for (kf = 0; kf < 32; kf = kf + 1) pend_phi[kf] <= '0;
          end else if (pend_load_v_i) begin
            best_p0 <= pend_p0_i;
            best_p1 <= pend_p1_i;
            best_a <= pend_ans_i;
            sel_idx <= pend_sel_i;
            r_st <= pend_st_i;
            proof_ok <= pend_acc_i && (pend_st_i == A7NG_C3_ST_ANSWER);
            pend_id <= pend_txn_i;
            pend_gen <= pend_gen_i;
            pend_acc <= pend_acc_i;
            pend_cmt <= pend_cmt_i;
