// a7ng_astra_f2r_rank.sv — ASTRA-F2R-SHARED-RANK-PENDING-01. PROGRAM=NO.
// New candidate. Does not edit r1/r2/f2/f2t/hop3/sgd_v1 originals.
`timescale 1ns / 1ps

module a7ng_astra_f2r_rank #(
  ...
  input  logic [1:0]  ctrl_i,  // 0 quality+upd, 1 freeze, 2 validity-only
  ...
  localparam logic [3:0] ST_UNKNOWN = 4'd1, ST_INCOMP = 4'd6, ST_AMB = 4'd7, ST_NEG = 4'd8;
  ...
  logic r_ovf, r_neg, r_amb, r_axi;
  logic [3:0] r_st;
  logic [15:0] nfar, nfok, nferr, nfto, narto, tocnt, nupd, ndup, nbad;
  logic signed [7:0] phi [0:31], pend_phi [0:31];
  logic signed [15:0] sgd_v, v_best, v_second;
  logic sgd_ready, sgd_done, sgd_go, sgd_upd, freeze_sgd;
  logic [7:0] txn, pend_id;
  ...
  always_comb begin
    ...
    c_best_a = '0; c_best_p0 = 20'hFFFFF; c_best_p1 = '0;
  ...
  a7ng_shared_rank_sgd_q8_v1_f2r u_sgd (
    .clk(clk), .rst_n(rst_n), .freeze_i(freeze_sgd),
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      nupd <= '0; ndup <= '0; nbad <= '0;
      r_subj <= '0; r_rel <= '0; r_ovf <= 1'b0; r_neg <= 1'b0; r_amb <= 1'b0; r_axi <= 1'b0;
      r_st <= ST_UNKNOWN;
      txn <= 8'd1; pend_id <= 8'd0; pend_acc <= 1'b0; pend_cmt <= 1'b0;
      ...
      unique case (st)
        S_IDLE: begin
          nc <= '0; fi <= '0; nf <= '0; np <= '0; r_axi <= 1'b0; tocnt <= '0;
          for (kf = 0; kf < 16; kf = kf + 1) fv[kf] <= 1'b0;
          best_a <= '0; best_p0 <= '0; best_p1 <= '0; v_best <= '0; v_second <= '0;
          r_st <= ST_UNKNOWN;
          if (qse_valid) begin
            r_subj <= qse_subj; r_rel <= qse_rel;
        ...
        S_AR: begin
          ...
          if (f_arready && f_arvalid) begin
            ...
            f_arvalid <= 1'b0; narto <= narto + 16'd1; r_axi <= 1'b1; tocnt <= '0;
            if (fi + 1'b1 == nc) st <= S_EI;
        ...
        S_R: begin
          ...
          if (f_rvalid) begin
            ...
            nfto <= nfto + 16'd1; r_axi <= 1'b1; tocnt <= '0;
            if (fi + 1'b1 == nc) begin ei <= '0; st <= S_EI; end
            else begin fi <= fi + 1'b1; st <= S_AR; end
          end
        end
        S_EI: begin
          if (r_ovf || r_axi) begin r_st <= ST_INCOMP; st <= S_HOLD; end
            ...
            pi <= '0;
            st <= (np==0) ? S_HOLD : S_SC;
          end else begin ej <= '0; st <= S_EJ; end
        end
        S_EJ: begin
        ...
        S_PICK: begin
          ...
          r_st <= 4'd0;
          pend_id <= txn; txn <= txn + 8'd1;
          pend_acc <= 1'b1; pend_cmt <= 1'b0;
          for (kf = 0; kf < 32; kf = kf + 1) pend_phi[kf] <= 8'sd0;
          if (ctrl_i == 2'd2) pend_phi[5] <= 8'sd64;
          else begin
            // quality of selected: match c_best_p0
            pend_phi[0] <= qphi0(
              (c_best_p0==pp0[0]) ? pc1[0] : ((c_best_p0==pp0[1]) ? pc1[1] : 8'd0),
              (c_best_p0==pp0[0]) ? pc2[0] : ((c_best_p0==pp0[1]) ? pc2[1] : 8'd0));
            pend_phi[1] <= 8'sd64; pend_phi[2] <= 8'sd64;
            pend_phi[3] <= 8'sd64; pend_phi[4] <= 8'sd64;
          end
          st <= S_HOLD;
        end
        S_HOLD: begin
          if (np==0 && r_st==ST_UNKNOWN) begin
            best_a <= '0; best_p0 <= '0; best_p1 <= '0;
          end
          if (rew_v_i) begin
            if (rew_txn_i != pend_id) nbad <= nbad + 16'd1;
            else if (pend_acc && pend_cmt) ndup <= ndup + 16'd1;
            else if (pend_acc && !pend_cmt && sgd_ready && (ctrl_i==2'd0)) begin
              sgd_upd <= 1'b1; st <= S_UW;
            end
          end else if (retire_i) begin
            pend_acc <= 1'b0; st <= S_IDLE;
          end
        end
        S_UW: if (sgd_done) begin
          pend_cmt <= 1'b1; nupd <= nupd + 16'd1; st <= S_HOLD;
        end
        default: st <= S_IDLE;
      endcase
    end
