// a7ng_astra_c5_prod_top_resp.sv — ASTRA-C5-AXI-ERRORS-04B. PROGRAM=NO.
// Named copy of prod_top_stream. Passes RRESP/BRESP/RID/BID/RLAST.
// Does not fabricate OKAY. Does not edit live prod_top / ddr_arb / stream.
`timescale 1ns / 1ps
...
module a7ng_astra_c5_axi1b_resp (
  input  logic        clk,
  input  logic        rst_n,
...
module a7ng_astra_c5_prod_top_resp #(
  ...
  a7ng_astra_c5_axi1b_resp u_adp (
    .clk(clk), .rst_n(rst_n),
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      unique case (pst)
        ...
        ST_PERS: begin
          if (ck_ph == A7NG_C5K_PH_FAILED)
            ck_ret <= 1'b1;
          else if ((owner == A7NG_C5_CKPT) && !ck_busy && (ck_ph == A7NG_C5K_PH_IDLE))
            ck_go_p <= 1'b1;
