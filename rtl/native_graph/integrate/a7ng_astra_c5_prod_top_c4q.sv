// a7ng_astra_c5_prod_top_c4q.sv — ASTRA-C5-PROD-TOP-QPTEXT-01.
// PROGRAM=NO. New named production hierarchy. Instantiates UART, live C3 wrap,
// C5 arb, C2 persist, BYTE256 qptext gen. QUERY/PROOF FPGA dict + weight glue.
...
module a7ng_astra_c5_axi1b_c4q (
  input  logic        clk,
  input  logic        rst_n,
...
module a7ng_astra_c5_prod_top_c4q (
  input  logic        clk,
  input  logic        rst_n,
  ...
  logic [7:0] g_obj, g_rel, g_tok, g_nout, g_qi0, g_ei0, g_o0, g_o1, g_o2, g_tok0, g_tok1;
  logic g_has;
  logic [9:0] g_head;
  logic [15:0] g_nhost;
  ...
  a7ng_astra_c4_lm06_byte256_qptext_gen u_gen (
    ...
    if (!rst_n) begin
      ...
      unique case (pst)
        ...
        ST_WLOAD: begin
          g_ldv <= 1'b1;
          unique case (wsel)
            3'd0: begin g_sel <= A7NG_C4Q_LD_GLUE; g_w <= {8'd0, A7NG_C4Q_GLUE_DEF}; end
            3'd1: begin g_sel <= A7NG_C4Q_LD_COPY; g_w <= 16'sd50; end
            3'd2: begin g_sel <= A7NG_C4Q_LD_SAFE; g_w <= 16'sd80; end
            default: begin g_sel <= A7NG_C4Q_LD_EOS; g_w <= 16'sd200; end
          endcase
          if (wsel == 3'd3) pst <= ST_UART;
          else wsel <= wsel + 3'd1;
        end
        ST_UART: begin
          ...
          if (c3_res || rew_sent) begin
            if ((c3_st == A7NG_C5P_ST_ANSWER) && (c3_ctrl == 2'd0)) begin
              if (!rew_sent) begin
                ...
                g_rel <= 8'd2;
                g_has <= 1'b1;
                g_qi0 <= a7ng_c4q_ch(c3_subj, 0);
                g_ei0 <= a7ng_c4q_ch(c3_p0[7:0], 0);
                g_o0 <= a7ng_c4q_ch(c3_ans[7:0], 0);
                g_o1 <= a7ng_c4q_ch(c3_ans[7:0], 1);
                g_o2 <= a7ng_c4q_ch(c3_ans[7:0], 2);
                p_us <= {12'd0, c3_subj};
                p_urx <= 20'd2;
                p_uo <= {12'd0, c3_obj};
                p_uc <= {12'd0, c3_ctx};
        ...
        ST_GEN: begin
          ...
          if (g_tv) begin
            ...
            else if (g_tok1 == 8'd0) g_tok1 <= g_tok;
          end
          if (g_done) begin
            nl_pend <= 1'b1;
            pst <= ST_TXNL;
