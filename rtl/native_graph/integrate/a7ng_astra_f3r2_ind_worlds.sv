// a7ng_astra_f3r2_ind_worlds.sv — ASTRA-F3-R2-INDEPENDENT-WORLDS-01. PROGRAM=NO.
// New named ranker. Does not edit F2R-01/R2/R3/R4/R5 or ASTRA-F3-SHARED-TRANSFER-01 bags or frozen RTL.
...
`timescale 1ns / 1ps
`include "a7ng_astra_f3r2_ind_worlds.svh"

module a7ng_astra_f3r2_ind_worlds #(
  parameter int unsigned ID_W       = 20,
  parameter int unsigned CAND_CAP   = 16,
  parameter int unsigned TO_CYC     = A7NG_F3R2_TO_CYC,
  parameter int unsigned MAX_PATH   = A7NG_F3R2_MAX_PATH,
  ...
  assign m_axi_arid = fetch_mode ? A7NG_F3R2_FACT_RID : sp_arid;
      ...
      unique case (st)
        S_IDLE: begin
          ...
          if (qse_valid) begin
            ...
            r_two <= (qse_ctx == A7NG_F3R2_CTX_INDIRECT);
        ...
        S_WALK: begin
          ...
          if (w_done) begin
            ...
            if (r_amb) begin r_st <= A7NG_F3R2_ST_AMB; st <= S_HOLD; end
            else if (r_neg) begin r_st <= A7NG_F3R2_ST_NEG; st <= S_HOLD; end
        ...
        S_AR: begin
          ...
          if (f_arready && f_arvalid) begin
            ...
            f_arvalid <= 1'b0; r_st <= A7NG_F3R2_ST_INCOMP; st <= S_HOLD;
          end
        end
        S_R: begin
          tocnt <= tocnt + 16'd1;
          if (f_rvalid) begin
            if ((m_axi_rid==A7NG_F3R2_FACT_RID) && (m_axi_rresp==A7NG_F3R2_RRESP_OK)
                && m_axi_rlast && (m_axi_rdata[75:72]==A7NG_F3R2_VER) && m_axi_rdata[70]
            ...
            r_st <= A7NG_F3R2_ST_INCOMP; st <= S_HOLD;
          end
        end
        S_EI: begin
          if (r_ovf) begin r_st <= A7NG_F3R2_ST_INCOMP; st <= S_HOLD; end
          else if (ei >= nf) begin
            if (np == 0) begin r_st <= A7NG_F3R2_ST_UNKNOWN; st <= S_HOLD; end
