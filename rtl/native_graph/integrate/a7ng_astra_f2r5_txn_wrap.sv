// a7ng_astra_f2r5_txn_wrap.sv — ASTRA-F2R-R5-TXN-WRAP-RESET-01. PROGRAM=NO.
// New candidate. Does not edit F2R-01 / F2R-R2 / F2R-R3 / F2R-R4 bags or frozen RTL.
// Instantiates frozen a7ng_shared_rank_sgd_q8_sym_f2r2. F2R4 drain FSM kept;
// pending identity is {sess_id,gen,txn}; in-session wrap refused; rst_n aborts.
`timescale 1ns / 1ps
`include "a7ng_astra_f2r5_txn_wrap.svh"

module a7ng_astra_f2r5_txn_wrap #(
  parameter int unsigned ID_W       = 20,
  parameter int unsigned CAND_CAP   = 16,
  parameter int unsigned TO_CYC     = A7NG_F2R5_TO_CYC,
  parameter int unsigned DRAIN_TO   = A7NG_F2R5_DRAIN_TO,
  parameter int unsigned MAX_PATH   = 4,
  parameter logic [7:0]  TXN_MAX    = A7NG_F2R5_TXN_MAX,
  ...
  function automatic logic [3:0] next_fact_arid(input logic [15:0] dead);
    integer i;
    begin
      next_fact_arid = A7NG_F2R5_FACT_RID0;
      for (i = 15; i >= 2; i = i - 1)
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      f_arvalid <= 1'b0; f_araddr <= '0; f_arid <= A7NG_F2R5_FACT_RID0;
      axi_ost <= 1'b0; axi_abort <= 1'b0; ost_rid <= A7NG_F2R5_FACT_RID0;
      ...
      pend_epoch <= A7NG_F2R5_EPOCH_INV; sess_epoch <= A7NG_F2R5_EPOCH_INV;
      nexh <= '0; txn_exh <= 1'b0;
        ...
        S_R: begin
          ...
          if (kill_r) begin
            ...
            if (m_axi_rid == ost_rid) begin
              if (m_axi_rresp != A7NG_F2R5_RRESP_OK) begin
