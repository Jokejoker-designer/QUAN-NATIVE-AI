`timescale 1ns/1ps
// Dual-clock persist: BRAM walk on clk_bram (clk50), AXI DMA on clk_dma (ui_clk).
// Handshake holds the 128 B line stable across the CDC. 100352/128 = 784.
import a7lm04_pkg::*;
module lm04_persist (
    ...
    localparam int NCHUNK = NPARAM / CHUNK;
    ...
    logic [6:0]  beat;
    logic [9:0]  ch;
    // One writer per clock: line_wr filled on clk_bram, line_rd filled on clk_dma.
        ...
        if (!rst_dma_n) begin
            ...
            unique case (dst)
                ...
                D_GO: begin
                    dma_go <= 1'b1;
                    dma_wr <= is_flush;
                    dma_addr <= 28'(DDR_WBASE) + {11'd0, base};
                    dma_bytes <= 32'(CHUNK);
                    beat <= 7'd0;
