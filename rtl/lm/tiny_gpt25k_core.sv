...
module tiny_gpt25k_core (
    input  logic               clk,
    input  logic               rst_n,
    input  logic               mem_we,
    input  logic [14:0]        mem_addr,
    input  logic signed [7:0]  mem_wdata,
    output logic signed [7:0]  mem_rdata,
    input  logic               ctx_we,
    input  logic [3:0]         ctx_idx,
    input  logic [3:0]         ctx_n_in,
    ...
    logic [31:0]        var_u;
    logic signed [31:0] score [0:15];
    logic [7:0]         exps [0:15];
    logic [7:0]         e_last [0:1][0:15];
    logic signed [31:0] z_last [0:1];
    ...
    logic [14:0]        waddr, caddr, ck_raddr;
    logic signed [7:0]  wrd, crd, ckd, wwd;
    logic               wwe, cwe;
    logic signed [31:0] ard, ard_b, awd;
    logic [13:0]        aaddr, aaddr_b;
    logic               awe;
    logic signed [31:0] logits [0:127];
    logic [7:0]         smx_e [0:127];
    logic [6:0]         arg_best;
    ...
    // BRAM output latency is then exactly 1 clk (see weight_bram25k).
...
`ifndef SYNTHESIS
    always_ff @(posedge clk) begin
        ...
        if (rst_n && st == ST_LN_V && sub == 4'd4)
            $display("LNV t=%0t acc=%0d var=%0d mu=%0d", $time, acc, acc[36:5], mu);
        ...
        if (rst_n && st == ST_ATT_O && fd_done && dim == 9'd0)
            $display("ATTO t=%0t h=%0d q=%0d z=%0d acc=%0d dest=%0d", $time, hix, fd_q, z_last[hix[0]], acc, aa(ly, 3'd5, tok_i, {1'b0, hix[0], dim[3:0]}));
        ...
        if (!rst_n) begin
            ...
            if (st == ST_IDLE && ctx_we) begin
                ...
                for (ii = 0; ii < 8; ii = ii + 1)
                    if ({12'd0, ctx_idx} + ii < 16)
                        tok[ctx_idx + ii[3:0]] <= ctx_pack[8*ii +: 8];
            end
            unique case (st)
                ST_IDLE: begin
                    if (start_fwd || start_train || start_ce || start_corpus || do_snap || do_restore || do_fold) begin
                        ...
                        ep <= 8'd0;
                        if (do_fold) begin
                            xor32 <= 32'd0; add32 <= 32'd0; caddr <= 15'd0; waddr <= 15'd0; sub <= 4'd0; st <= ST_FOLD;
                        end else if (do_snap) begin
                            caddr <= 15'd0; waddr <= 15'd0; sub <= 4'd0; st <= ST_SNAP;
                        end else if (do_restore) begin
                            caddr <= 15'd0; waddr <= 15'd0; ck_raddr <= 15'd0; sub <= 4'd0; st <= ST_REST;
                            ...
                            ly <= 1'b0; tok_i <= 4'd0; dim <= 9'd0; sub <= 4'd0; acc <= 64'sd0; st <= ST_EMB;
                        end
                    end
                end
                ST_PAIR: begin
                ...
                ST_EMB: begin
                    unique case (sub)
                        4'd0: begin
                            waddr <= 15'(OFF_TOK) + 15'(tok[tok_i]) * 15'(D) + 15'(dim);
                            caddr <= 15'(OFF_POS) + 15'(tok_i) * 15'(D) + 15'(dim);
                            sub <= 4'd1;
                        end
                        4'd1: sub <= 4'd2;
                        default: begin
                            ...
                            if (dim + 9'd1 == 9'(D)) begin
                                dim <= 9'd0;
                                if (tok_i + 4'd1 >= ntok) begin
                                    tok_i <= 4'd0; ly <= 1'b0; ten <= 3'd0; acc <= 64'sd0; st <= ST_LN_S;
                                end else
                                    tok_i <= tok_i + 4'd1;
                            end else
                                dim <= dim + 9'd1;
                ...
                ST_LN_V: begin
                    unique case (sub)
                        ...
                        default: begin
                            var_u <= acc[36:5]; // (sum sq) // 32, unsigned, may set bit31
                            acc <= 64'sd0;
                            sub <= 4'd0;
                            isq_pend <= 1'b0;
                            st <= ST_LN_SQ;
                    ...
                    unique case (sub)
                        ...
                        default: if (fd_done) begin
                            ...
                            if (ten == 3'd0 && tok_i == last_tok(ntok))
                                n1_last[ly][dim[4:0]] <= 32'(sat16(fd_q));
                            sub <= 4'd0;
                            if (dim + 9'd1 == 9'(D)) begin
                                dim <= 9'd0;
                                if (tok_i + 4'd1 >= ntok) begin
                ...
                ST_MV: begin
                    unique case (sub)
                        4'd0: begin
                            waddr <= wbase + 15'(row) * 15'(ncols) + 15'(col);
                            if (relu && ten_o == 3'd0)
                        ...
                        default: begin
                            ...
                            if (row + 9'd1 == nrows) begin
                                row <= 9'd0;
                                if (ten_o == 3'd7) begin
                                    vix <= 7'd0; mx <= logits[0]; st <= ST_SMX;
                                end else if (tok_i + 4'd1 >= ntok) begin
                                    tok_i <= 4'd0;
                                    if (ten_o == 3'd2) begin
                                        wbase <= layer_base(ly) + 15'(LO_WK); ten_o <= 3'd3; st <= ST_MV;
                                    end else if (ten_o == 3'd3) begin
                                        wbase <= layer_base(ly) + 15'(LO_WV); ten_o <= 3'd4; st <= ST_MV;
                                    end else if (ten_o == 3'd4) begin
                                        hix <= 2'd0; tok_i <= 4'd0; tok_j <= 4'd0; dim <= 9'd0; acc <= 64'sd0;
                                        st <= ST_ATT_SC;
                                    end else if (ten_o == 3'd5) begin
                                        tok_i <= 4'd0; dim <= 9'd0; ten <= 3'd0; st <= ST_ADD;
                                    end else if (ten_o == 3'd0 && relu) begin
                                        wbase <= layer_base(ly) + 15'(LO_FF2); ten_o <= 3'd6;
                                        nrows <= 9'(D); ncols <= 9'(FF); relu <= 1'b0; st <= ST_MV;
                                    end else if (ten_o == 3'd6) begin
                                        tok_i <= 4'd0; dim <= 9'd0; ten <= 3'd1; st <= ST_ADD;
                                    end else
                                        st <= ST_DONE;
                                end else
                                    tok_i <= tok_i + 4'd1;
                ...
                ST_ATT_SC: begin
                    unique case (sub)
                        4'd0: begin
                            aaddr <= aa(ly, 3'd2, tok_i, {1'b0, hix[0], dim[3:0]});
                            aaddr_b <= aa(ly, 3'd3, tok_j, {1'b0, hix[0], dim[3:0]});
                            sub <= 4'd1;
                        end
                        4'd1: sub <= 4'd2;
                        4'd2: begin
                ...
                ST_ATT_E: begin
                    ...
                    if (tok_j == 4'd15) begin
                        tok_j <= 4'd0; zsum <= 32'sd0; st <= ST_ATT_Z;
                    end else
                        tok_j <= tok_j + 4'd1;
                end
                ST_ATT_Z: begin
                    ...
                    if (tok_j == tok_i) begin
                        if (zsum + 32'(exps[tok_j]) == 0) z_last[hix[0]] <= 32'sd1;
                        else z_last[hix[0]] <= zsum + 32'(exps[tok_j]);
                        for (ii = 0; ii < 16; ii = ii + 1)
                            e_last[hix[0]][ii] <= exps[ii];
                        tok_j <= 4'd0; dim <= 9'd0; sub <= 4'd0; acc <= 64'sd0;
                        st <= ST_ATT_O;
                    end else
                        tok_j <= tok_j + 4'd1;
                end
                ST_ATT_O: begin
                    unique case (sub)
                        4'd0: begin
                            aaddr <= aa(ly, 3'd4, tok_j, {1'b0, hix[0], dim[3:0]});
                            sub <= 4'd1;
                        end
                        4'd1: sub <= 4'd2;
                        4'd2: begin
                        ...
                        4'd4: begin
                            fd_n <= acc[47:0];
                            fd_d <= denom16(z_last[hix[0]]);
                            fd_go <= 1'b1;
                            sub <= 4'd5;
                        end
                        default: if (fd_done) begin
                            awe <= 1'b1;
                            aaddr <= aa(ly, 3'd5, tok_i, {1'b0, hix[0], dim[3:0]});
                            awd <= 32'(sat16(fd_q));
                            if (tok_i == last_tok(ntok))
                                a_last[ly][{hix[0], dim[3:0]}] <= 32'(sat16(fd_q));
                            acc <= 64'sd0;
                            tok_j <= 4'd0;
                            sub <= 4'd0;
                            if (dim + 9'd1 == 9'(DH)) begin
                                dim <= 9'd0;
                                if (hix[0] == 1'b0) begin
                                    hix <= 2'd1; st <= ST_ATT_SC;
                                end else begin
                                    hix <= 2'd0;
                                    if (tok_i + 4'd1 >= ntok) begin
                                        tok_i <= 4'd0;
                                        wbase <= layer_base(ly) + 15'(LO_WO);
                                        ten_o <= 3'd5; nrows <= 9'(D); ncols <= 9'(D); relu <= 1'b0;
                                        row <= 9'd0; col <= 9'd0; st <= ST_MV;
                                    end else begin
                                        tok_i <= tok_i + 4'd1; st <= ST_ATT_SC;
                                    end
                                end
                            end else
                                dim <= dim + 9'd1;
                ...
                ST_SMX: begin
                    ...
                    if (vix == 7'd127) begin
                        vix <= 7'd0; ssoft <= 32'sd0; sub <= 4'd0; st <= ST_ARG;
                    end else
                        vix <= vix + 7'd1;
                end
                ST_ARG: begin
                        ...
                        4'd1: begin
                            ...
                            if (vix == 7'd127) begin
                                ...
                                if (train && !after_r) begin
                                    vix <= 7'd0; dim <= 9'd0; sub <= 4'd0; acc <= 64'sd0; st <= ST_BH;
                ...
                ST_BH: begin
                    unique case (sub)
                        ...
                        4'd4: begin
                            ...
                            fd_d <= denom16(ssoft);
                            fd_go <= 1'b1;
                            waddr <= 15'(OFF_HEAD) + 15'(vix) * 15'(D) + 15'(dim);
                            sub <= 4'd5;
                        end
                        4'd5: if (fd_done) begin
                            ...
                            op_a <= 32'(wrd);
                            op_b <= dz_r;
                            if (vix == 7'd127 && head_only) begin
                                vix <= 7'd0; acc <= 64'sd0; sub <= 4'd0;
                                if (dim + 9'd1 == 9'(D)) begin
                                    dim <= 9'd0;
                ...
                ST_DHID: begin
                    unique case (sub)
                        4'd0: begin
                            waddr <= layer_base(ly) + 15'(LO_FF2) + 15'(dim) * 15'(FF) + 15'(col);
                            sub <= 4'd1;
                        end
                        4'd1: sub <= 4'd2;
                        4'd2: begin
                        ...
                        4'd3: begin
                            ...
                            if (dim + 9'd1 == 9'(D)) begin
                                dim <= 9'd0;
                                aaddr <= ah(ly, last_tok(ntok), col[5:0]);
                                sub <= 4'd4;
                            end else begin
                                dim <= dim + 9'd1;
                                sub <= 4'd0;
                        ...
                        4'd4: sub <= 4'd5;
                        default: begin
                            dHid[col[5:0]] <= (ard == 32'sd0) ? 32'sd0 : 32'(sat16(acc >>> LIN_SHIFT));
                            acc <= 64'sd0;
                            sub <= 4'd0;
                ...
                ST_DN2: begin
                    unique case (sub)
                        4'd0: begin
                            waddr <= layer_base(ly) + 15'(LO_FF1) + 15'(col) * 15'(D) + 15'(dim);
                            sub <= 4'd1;
                        end
                        4'd1: sub <= 4'd2;
                        4'd2: begin
                        ...
                        default: begin
                            dH[dim[4:0]] <= 32'(sat16(dY[dim[4:0]] + 32'(sat16(sat32(acc >>> LIN_SHIFT)))));
                            acc <= 64'sd0;
                            sub <= 4'd0;
                            if (dim + 9'd1 == 9'(D)) begin
                ...
                ST_ALIN: begin
                    unique case (sub)
                        4'd0: begin
                            ...
                            unique case (ten)
                                3'd0: waddr <= layer_base(ly) + 15'(LO_WO) + 15'(row) * 15'(D) + 15'(col);
                                3'd1: waddr <= layer_base(ly) + 15'(LO_WQ) + 15'(row) * 15'(D) + 15'(col);
                                3'd2: waddr <= layer_base(ly) + 15'(LO_WK) + 15'(row) * 15'(D) + 15'(col);
                                default: waddr <= layer_base(ly) + 15'(LO_WV) + 15'(row) * 15'(D) + 15'(col);
                            endcase
                            sub <= 4'd1;
                        end
                        4'd1: begin
                            x_r <= (ten == 3'd0) ? a_last[ly][col[4:0]] : n1_last[ly][col[4:0]];
                            dy_r <= dH[row[4:0]];
                            sub <= 4'd2;
                        end
                        4'd2: begin
                ...
                ST_BEM: begin
                    unique case (sub)
                        4'd0: begin
                            waddr <= 15'(OFF_TOK) + 15'(tok[last_tok(ntok)]) * 15'(D) + 15'(dim);
                            caddr <= 15'(OFF_POS) + 15'(last_tok(ntok)) * 15'(D) + 15'(dim);
                            sub <= 4'd1;
                        end
                        4'd1: sub <= 4'd2;
                        4'd2: begin
                            if (!after_r) begin
                                wwe <= 1'b1;
                                wwd <= sat8(32'(wrd) - 32'(step_sign(sat16(dY[dim[4:0]]))));
                                wr_n <= wr_n + 32'd1;
                            end
                            sub <= 4'd3;
                        end
                        default: begin
                            if (!after_r) begin
                                wwe <= 1'b1;
                                waddr <= 15'(OFF_POS) + 15'(last_tok(ntok)) * 15'(D) + 15'(dim);
                                wwd <= sat8(32'(crd) - 32'(step_sign(sat16(dY[dim[4:0]]))));
                                wr_n <= wr_n + 32'd1;
                            end
                            sub <= 4'd0;
                            if (dim + 9'd1 == 9'(D)) begin
                ...
                ST_FOLD: begin
                    unique case (sub)
                        ...
                        default: begin
                            ...
                            sub <= 4'd0;
                            if (caddr + 15'd1 >= 15'(NPARAM)) st <= ST_DONE;
                            else caddr <= caddr + 15'd1;
                        end
                    endcase
                end
                ST_SNAP: begin
