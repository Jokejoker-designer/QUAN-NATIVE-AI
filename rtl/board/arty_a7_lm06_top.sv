`timescale 1ns/1ps
// A7-LM-05: 399k 4-layer core + tiled DDR W + official AXI MIG persist + LM-02 K-tests.
// Bitstream arty_a7_lm06.bit only. Does not overwrite frozen 00-05.
import a7lm06_pkg::*;
module arty_a7_lm06_top (
    ...
    // UART + 399k core on clk50 (LM-03 timing class). MIG/tensor stay on ui_clk.
    logic rx_valid;
    logic [7:0] rx_data;
    ...
    logic [19:0] mem_addr, mem_addr_u, mem_addr_p;
    ...
    logic busy, done;
    logic [9:0] pred;
    logic w_stall, wdma_owner, wdma_go, wdma_wr, wdma_w_valid, wdma_w_ready, wdma_r_valid, wdma_r_ready;
    ...
    logic [2:0] dbg_tile_dst, dbg_tile_rg;
    logic       dbg_tile_miss, dbg_tile_dirty, dbg_tile_req;
    logic [3:0] dbg_p_bst;
    logic [2:0] dbg_p_dst;
    logic [12:0] dbg_p_ch;
    logic       dbg_p_is_flush, dbg_p_req;
    logic [1:0] dma_busy_s;
    logic p_busy, p_done, p_go_flush, p_go_reload, p_dma_owner, p_mem_sel;
    logic p_under, p_berr, p_rerr;
    logic [31:0] p_bytes;
    logic dma_busy, dma_done, dma_under, axi_berr, axi_rerr;
    logic dma_w_ready, dma_r_valid;
    logic [127:0] dma_r_data;

    // C3: keep persist on the mem port for the whole op (p_busy), not only
    // FILL/TOUCH/STORE. C2 mem_sel dropped the port in B_REQ and the tile
    // snapped back to the last UART address → a miss + 131k refill per chunk.
    // Flush then never returned A6 (hardware_c2 persist_flush=null).
    ...
    tiny_gpt803k_core #(.SIM_FULL(1'b0)) u_core (
        ...
        .w_stall(w_stall), .clk_dma(ui_clk), .rst_dma_n(ui_rst_n),
        .wdma_owner(wdma_owner),
        .wdma_go(wdma_go), .wdma_wr(wdma_wr),
        ...
        .wdma_r_valid(wdma_r_valid), .wdma_r_ready(wdma_r_ready), .wdma_r_data(dma_r_data),
        .dbg_tile_bst(dbg_tile_bst), .dbg_tile_dst(dbg_tile_dst), .dbg_tile_rg(dbg_tile_rg),
        .dbg_tile_miss(dbg_tile_miss), .dbg_tile_dirty(dbg_tile_dirty), .dbg_tile_req(dbg_tile_req)
    );
    ...
    logic [127:0] t_dma_w_data, p_dma_w_data, dma_w_data, t_dma_r_data, p_dma_r_data;

    // C2: tile refill > persist > tensor. Persist STORE waits a miss;
    // if persist still holds the mux the refill never sees r_valid.
    assign dma_go      = wdma_owner ? wdma_go      : (p_dma_owner ? p_dma_go      : t_dma_go);
    assign dma_wr      = wdma_owner ? wdma_wr      : (p_dma_owner ? p_dma_wr      : t_dma_wr);
    assign dma_addr    = wdma_owner ? wdma_addr    : (p_dma_owner ? p_dma_addr    : t_dma_addr);
    assign dma_bytes   = wdma_owner ? wdma_bytes   : (p_dma_owner ? p_dma_bytes   : t_dma_bytes);
    assign dma_w_valid = wdma_owner ? wdma_w_valid : (p_dma_owner ? p_dma_w_valid : t_dma_w_valid);
    assign dma_w_data  = wdma_owner ? wdma_w_data  : (p_dma_owner ? p_dma_w_data  : t_dma_w_data);
    assign dma_r_ready = wdma_owner ? wdma_r_ready : (p_dma_owner ? p_dma_r_ready : t_dma_r_ready);
    assign t_dma_w_ready = dma_w_ready && !p_dma_owner && !wdma_owner;
    assign p_dma_w_ready = dma_w_ready &&  p_dma_owner && !wdma_owner;
    assign wdma_w_ready  = dma_w_ready &&  wdma_owner;
    assign t_dma_r_valid = dma_r_valid && !p_dma_owner && !wdma_owner;
    assign p_dma_r_valid = dma_r_valid &&  p_dma_owner && !wdma_owner;
    assign wdma_r_valid  = dma_r_valid &&  wdma_owner;
    ...
    tensor_microseq u_seq (
        ...
        .dma_busy(dma_busy && !p_dma_owner && !wdma_owner), .dma_done(dma_done && !p_dma_owner && !wdma_owner), .dma_under(dma_under),
        .axi_berr(axi_berr), .axi_rerr(axi_rerr),
    ...
    lm06_persist u_persist (
        ...
        .dbg_is_flush(dbg_p_is_flush), .dbg_req(dbg_p_req)
    );

    // --- UART A5/0x84 ---
    logic collecting, cmd_stat, cmd_ce, cmd_pred, cmd_persist, cmd_dbg, cmd_tstat, cmd_tfold, cmd_tcnt, cmd_tcnt2, cmd_tovl, cmd_tpsum;
    logic [3:0] idx;
    ...
    always_ff @(posedge clk50) begin
        if (!rst_uart_n) begin
                ...
                if (!collecting) begin
                    ...
                    if (idx == 4'd14) begin
                        ...
                        if ((run_xor ^ rx_data) == 8'd0 && buf_b[1] == 8'h84) begin
                            unique case (buf_b[2])
                                8'h30: begin
                                    if (!wr_go && !rd_go && !busy && !p_busy) begin
                                        wr_base <= {buf_b[5][7:4], buf_b[4], buf_b[3]};
                                        // Preload the address before asserting mem_we.  On a
                                        // layer miss this lets w_stall rise and complete the
                                        // DDR refill before byte 0 is accepted/advanced.
                                        mem_addr_u <= {buf_b[5][7:4], buf_b[4], buf_b[3]};
                                ...
                                8'h31: begin
                                    if (!wr_go && !rd_go && !busy && !p_busy) begin
                                        rd_base <= {buf_b[5][7:4], buf_b[4], buf_b[3]};
                                        mem_addr_u <= {buf_b[5][7:4], buf_b[4], buf_b[3]};
                                        rd_i <= 3'd0;
                                        rd_wait <= 3'd0;
                                        rd_go <= 1'b1;
            ...
            if (wr_go && !busy && !p_busy && !w_stall) begin
                mem_we_u <= 1'b1;
                mem_addr_u <= wr_base + {15'd0, wr_i};
            ...
            if (rd_go && !busy && !p_busy && !w_stall) begin
                mem_addr_u <= rd_base + {16'd0, rd_i};
                if (rd_wait < 3'd3)
            ...
            if (!tx_busy && !tx_start) begin
                if (cmd_stat) begin
                    ...
                    tx_frame[2] <= {after_mode, busy || p_busy || t_busy50 || w_stall || wr_go || rd_go, done, calib50, p_busy, 3'd0};
                    tx_frame[3] <= phase;
                    tx_frame[4] <= wr_n[7:0]; tx_frame[5] <= wr_n[15:8];
                    tx_frame[6] <= wr_n[23:16]; tx_frame[7] <= wr_n[31:24];
                    tx_frame[8] <= pred[7:0]; tx_frame[9] <= last_loss[7:0];
                    tx_frame[10] <= last_loss[15:8];
                    tx_frame[11] <= {6'd0, pred[9:8]}; tx_frame[12] <= 8'd0; tx_frame[13] <= 8'd0;
                    tx_frame[14] <= 8'hA5 ^ 8'hA1
                        ^ {after_mode, busy || p_busy || t_busy50 || w_stall || wr_go || rd_go, done, calib50, p_busy, 3'd0} ^ phase
                        ^ wr_n[7:0] ^ wr_n[15:8] ^ wr_n[23:16] ^ wr_n[31:24]
                        ^ pred[7:0] ^ last_loss[7:0] ^ last_loss[15:8] ^ {6'd0, pred[9:8]};
                    tx_start <= 1'b1;
                    ...
                    tx_frame[12] <= {4'd0, rd_base[19:16]};
                    tx_frame[13] <= 8'd0;
                    ...
                    tx_frame[5] <= {6'd0, pred[9:8]};
                    for (fi = 6; fi < 14; fi = fi + 1) tx_frame[fi] <= 8'd0;
                    tx_frame[14] <= 8'hA5 ^ 8'hA0 ^ pred[7:0] ^ last_loss[7:0] ^ last_loss[15:8]
                        ^ {6'd0, pred[9:8]};
                    tx_start <= 1'b1;
                end else if (cmd_dbg) begin
                    tx_frame[0] <= 8'hA5; tx_frame[1] <= 8'hA8;
                    tx_frame[2] <= dbg_ila[7:0];
                    tx_frame[3] <= dbg_ila[15:8];
                    tx_frame[4] <= dbg_ila[23:16];
                    tx_frame[5] <= dbg_ila[31:24];
                    tx_frame[6] <= dbg_ila[39:32];
                    tx_frame[7] <= dbg_ila[47:40];
                    tx_frame[8] <= dbg_ila[55:48];
                    tx_frame[9] <= dbg_ila[63:56];
                    for (fi = 10; fi < 14; fi = fi + 1) tx_frame[fi] <= 8'd0;
                    tx_frame[14] <= 8'hA5 ^ 8'hA8
                        ^ dbg_ila[7:0] ^ dbg_ila[15:8] ^ dbg_ila[23:16] ^ dbg_ila[31:24]
                        ^ dbg_ila[39:32] ^ dbg_ila[47:40] ^ dbg_ila[55:48] ^ dbg_ila[63:56];
                    tx_start <= 1'b1;
    ...
    assign led[0] = calib;
    assign led[1] = busy || p_busy || t_busy || w_stall || wr_go || rd_go;
    assign led[2] = (ce0 != 0) && (ce1 < ce0);
    assign led[3] = after_mode || sw_s[0];

    // ILA probe (clk50). Kept even when ILA is not inserted so net names stay stable.
    // Bit map: results/A7-LM-06/ILA.md
    always_ff @(posedge clk50) begin
        if (!rst50_n)
            dma_busy_s <= 2'd0;
        else
            dma_busy_s <= {dma_busy_s[0], dma_busy};
    end
    (* mark_debug = "true", dont_touch = "true" *) logic [63:0] dbg_ila;
    assign dbg_ila = {
        mem_addr,          // [63:44]
        p_done,            // [43]
        p_go_flush,        // [42]
