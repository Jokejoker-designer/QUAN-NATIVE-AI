...
// Bitstream arty_a7_lm05.bit only. Does not overwrite 00-04.
import a7lm05_pkg::*;
module arty_a7_lm05_top (
    ...
    logic [18:0] mem_addr, mem_addr_u, mem_addr_p;
    logic signed [7:0] mem_wdata, mem_wdata_u, mem_wdata_p, mem_rdata;
    logic ctx_we;
    logic [5:0] ctx_idx, ctx_n;
    ...
    tiny_gpt399k_core #(.SIM_FULL(1'b0)) u_core (
        ...
        .wdma_busy(dma_busy && wdma_owner), .wdma_done(dma_done && wdma_owner),
    ...
    // persist > tile-W refill > tensor
    assign dma_go      = p_dma_owner ? p_dma_go      : (wdma_owner ? wdma_go      : t_dma_go);
    assign dma_wr      = p_dma_owner ? p_dma_wr      : (wdma_owner ? wdma_wr      : t_dma_wr);
    assign dma_addr    = p_dma_owner ? p_dma_addr    : (wdma_owner ? wdma_addr    : t_dma_addr);
    assign dma_bytes   = p_dma_owner ? p_dma_bytes   : (wdma_owner ? wdma_bytes   : t_dma_bytes);
    assign dma_w_valid = p_dma_owner ? p_dma_w_valid : (wdma_owner ? wdma_w_valid : t_dma_w_valid);
    assign dma_w_data  = p_dma_owner ? p_dma_w_data  : (wdma_owner ? wdma_w_data  : t_dma_w_data);
    assign dma_r_ready = p_dma_owner ? p_dma_r_ready : (wdma_owner ? wdma_r_ready : t_dma_r_ready);
    assign t_dma_w_ready = dma_w_ready && !p_dma_owner && !wdma_owner;
    assign p_dma_w_ready = dma_w_ready &&  p_dma_owner;
    assign wdma_w_ready  = dma_w_ready && !p_dma_owner &&  wdma_owner;
    assign t_dma_r_valid = dma_r_valid && !p_dma_owner && !wdma_owner;
    assign p_dma_r_valid = dma_r_valid &&  p_dma_owner;
    assign wdma_r_valid  = dma_r_valid && !p_dma_owner &&  wdma_owner;
        ...
        if (!rst_uart_n) begin
                            ...
                            unique case (buf_b[2])
                                8'h30: begin
                                    if (!wr_go && !rd_go && !busy && !p_busy) begin
                                        wr_base <= {buf_b[5][6:4], buf_b[4], buf_b[3]};
                                        ...
                                        mem_addr_u <= {buf_b[5][6:4], buf_b[4], buf_b[3]};
                                ...
                                8'h31: begin
                                    if (!wr_go && !rd_go && !busy && !p_busy) begin
                                        rd_base <= {buf_b[5][6:4], buf_b[4], buf_b[3]};
                                        mem_addr_u <= {buf_b[5][6:4], buf_b[4], buf_b[3]};
                                        rd_i <= 3'd0;
                                        rd_wait <= 3'd0;
                                        rd_go <= 1'b1;
            ...
            if (!tx_busy && !tx_start) begin
                if (cmd_stat) begin
                    ...
                    tx_frame[11] <= {7'd0, pred[8]}; tx_frame[12] <= 8'd0; tx_frame[13] <= 8'd0;
                    tx_frame[14] <= 8'hA5 ^ 8'hA1
                        ...
                        ^ pred[7:0] ^ last_loss[7:0] ^ last_loss[15:8] ^ {7'd0, pred[8]};
                    tx_start <= 1'b1;
                    ...
                    tx_frame[12] <= {5'd0, rd_base[18:16]};
                    tx_frame[13] <= 8'd0;
