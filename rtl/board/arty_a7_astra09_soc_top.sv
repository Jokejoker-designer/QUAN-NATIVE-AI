// arty_a7_astra09_soc_top.sv — ASTRA-11 SOC_WRAP. PROGRAM=NO.
// UART bytes → a7ng_astra09_pipe → compact proof/status TX. On-chip AXI BRAM, no MIG.
// Pins from constraints/arty_a7_100.xdc only. Do not overwrite frozen SoC tops.
`timescale 1ns / 1ps

module arty_a7_astra09_soc_top (
  ...
  localparam int unsigned TX_N   = 12;
  localparam logic [7:0]  MAGIC  = 8'hA9;
  ...
  logic [7:0]  tok_b, ans, p0, p1, subj, obj, rel, ctx;
  logic [2:0]  status;
  logic signed [15:0] vq8;
  logic [15:0] ndir, nemit;
  logic        two, skip, trip;
  logic [3:0]  arid;
  logic [27:0] araddr;
  logic [7:0]  arlen;
  ...
  a7ng_astra09_pipe #(
    .N_EDGES(16), .ID_W(8), .WALK_ID_W(20), .CAND_CAP(16),
    .INDEX_BASE(28'h0500_0000)
  ) u_pipe (
    .clk(clk), .rst_n(rst_n),
    .freeze_i(1'b1),
    .live_epoch_i(16'd0),
    ...
    .load_v(1'b0), .load_idx(4'd0),
    .load_s(8'd0), .load_r(8'd0), .load_o(8'd0), .load_eid(8'd0),
    .load_trans(1'b0), .load_pol(1'b0),
    .clr_v(1'b0), .clr_idx(4'd0),
    .busy_o(busy), .result_v_o(result_v), .pkt_valid_o(),
    .subj_id_o(subj), .obj_id_o(obj), .rel_id_o(rel), .ctx_id_o(ctx),
    .direction_o(), .triple_valid_o(trip), .two_hop_o(two),
    .k0_o(), .k1_o(), .k2_o(), .k3_o(),
    .k0_valid_o(), .k1_valid_o(), .k2_valid_o(), .k3_valid_o(),
    .ans_o(ans), .proof0_o(p0), .proof1_o(p1), .status_o(status), .v_q8_o(vq8),
    .out_tok_v_o(), .out_tok_o(), .out_done_o(), .n_out_tok_o(),
    .eng_skip_o(skip),
    .n_dir_ar_o(ndir), .n_post_ar_o(), .n_emit_o(nemit), .probed_mask_o(),
  ...
  a7ng_axi_bram128 #(
    .DEPTH_WORDS(256), .INDEX_BASE(28'h0500_0000)
  ) u_sram (
    .clk(clk), .rst_n(rst_n),
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (result_v && !sent && !tx_active && !do_retire) begin
        tx_bytes[0]  <= MAGIC;
        tx_bytes[1]  <= {1'b0, skip, two, trip, 1'b0, status};
        tx_bytes[2]  <= ans;
        tx_bytes[3]  <= p0;
        tx_bytes[4]  <= p1;
        tx_bytes[5]  <= vq8[7:0];
  ...
  assign led[1] = busy;
  assign led[2] = result_v | (|nemit) | (|ctx);
  assign led[3] = sw[0] | (|sw[3:1]) | btn[1] | (|btn[3:2]);
endmodule
