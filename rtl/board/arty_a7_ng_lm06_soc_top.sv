// arty_a7_ng_lm06_soc_top.sv — lm06_soc_path: SoC + LM-06 weight fabric
// MIG + 16 PE + arb + UART stub + frozen-law weight modules on response path.
// Does NOT overwrite frozen LM-06 / 01R / 02M / A0.3 release bits (new bit only).
// Act scratch u_a (~64 BRAM) intentionally ABSENT — ONE unknown = weight path fit.
`timescale 1ns / 1ps

(* keep_hierarchy = "yes" *)
module arty_a7_ng_lm06_soc_top (
  ...
  always_ff @(posedge ui_clk or negedge ui_rst_n) begin
    if (!ui_rst_n) begin
      ...
      // After smoke, request LM so weight path can assert lm_path (not sw-gated)
      req_lm    <= smoke_pass;
    end
  end
  ...
  logic [7:0] wt_expect;
  logic wt_seen, lm_path_sticky;
  logic compose_busy, tok_v, compose_done, compose_active;
  logic [7:0] tok;
  logic lm_path_active;

  // Stay in TOK region (rg0) so SIM_FULL=0 tile never issues DDR miss refill.
  always_ff @(posedge ui_clk or negedge ui_rst_n) begin
    if (!ui_rst_n) begin
      wt_we     <= 1'b0;
      wt_addr_a <= 20'd0;
      ...
      wt_we     <= tick[0];
      wt_addr_a <= {12'd0, tick};
      wt_addr_b <= {12'd0, tick} ^ 20'd1;
      wt_wd     <= tick ^ 8'hA5;
      wt_expect <= tick ^ 8'hA5;
      // Prove weight BRAM on path: readback after write (1-cycle BRAM latency)
      if (!tick[0] && (wt_ra == wt_expect) && (tick > 8'd8))
        wt_seen <= 1'b1;
      if (wt_seen)
        lm_path_sticky <= 1'b1;
    end else begin
  ...
  // Evidence compose still on LM grant (response tokens); lm_path from weight sticky
  a7ng_evidence_compose u_compose (
    ...
    .entity_i(8'h41), .intent_i(8'h49 ^ pp_keep),
    .busy_o(compose_busy), .tok_valid_o(tok_v), .tok_o(tok),
    .done_o(compose_done), .lm_path_active_o(compose_active)
  );

  // Observable lm_path ≠ 0 only after weight BRAM readback evidence (not a hardwired 1)
  ...
  sync_bits #(.WIDTH(4)) u_led (
    .clk(CLK100MHZ), .rst_n(clk_locked),
    .async_in({|score_or ^ dual_err ^ wt_stall ^ |pp_keep, smoke_pass, (sd == SD_DONE), calib}),
    .sync_out({pe_s, pass_s, seed_s, calib_s})
  );
  assign led = {pe_s, pass_s, seed_s, calib_s} ^ {exam_mode, status_byte[2:0]} ^ {lm_path_100, wt_cached};
endmodule
