# E2R-ATOMIC-SDONE-PROBE-00-EXCL — dest=4∧owner SDONE ATOM pack (one writer)
# Copy of SGO exclusive file list; outdir/build_dir/project name only.
# Do NOT write F1x / DGR / B-FIX / B2 / F1w / F1v / SGO / Grok R6 / frozen LM-06 01R 02M A0.3.
# Do NOT source resume TCLs. Do NOT program. Do NOT reprogram SGO SHA 832E55E2...
# No [\s\S] (Vivado Tcl abort after route on SGO).
...
set build_dir [file join $root build e2r_atomic_sdone_probe_excl]
file mkdir $outdir
file mkdir $build_dir
if {[string match "*E2R-ATOMIC-DGR-PROBE-00*" $outdir] ||
    [string match "*E2R-ATOMIC-SGO-PROBE-00*" $outdir] ||
    [string match "*E2R-WDMA-BFIX-00*" $outdir] ||
    [string match "*E2R-WDMA-SBUSY-CMD-PROBE-00*" $outdir] ||
    [string match "*E2R-UART-MGO-HB-FIX-00*" $outdir] ||
    [string match "*E2R-WDMA-OWNER-GRANT-PROBE-00*" $outdir] ||
    [string match "*HS22-LM06*" $outdir] ||
    [string match "*R6*" $outdir] ||
    [string match "*TINYGPT-SOC*" $outdir] ||
    [string match "*e2r_atomic_dgr_probe*" $build_dir] ||
    [string match "*e2r_atomic_sgo_probe*" $build_dir] ||
    [string match "*e2r_wdma_bfix*" $build_dir] ||
    [string match "*e2r_wdma_sbusy_cmd_probe_00*" $build_dir] ||
    [string match "*e2r_uart_mgo_hb_fix*" $build_dir] ||
    [string match "*e2r_wdma_owner_grant*" $build_dir]} {
  puts stderr "REFUSE: foreign/F1x/SGO/B-FIX/B2/F1w/F1v/R6 path outdir=$outdir build_dir=$build_dir"
  exit 3
}

set this_tcl [file normalize [info script]]
set tcl_copy [file join $outdir build_e2r_atomic_sdone_probe_00_excl.tcl]
if {![string equal $this_tcl $tcl_copy]} {
  file copy -force $this_tcl $tcl_copy
}
...
create_project e2r_atomic_sdone_probe_00_excl $build_dir -part xc7a100tcsg324-1
...
puts "=== SYNTH E2R-ATOMIC-SDONE-PROBE-00-EXCL ==="
...
puts "=== IMPLEMENT E2R-ATOMIC-SDONE-PROBE-00-EXCL ==="
...
# Fallback: first numeric WNS/TNS/WHS/THS after the Design Timing Summary dashes
if {$wns eq "NA"} {
  set tf [open [file join $outdir report_timing_summary.rpt] r]
  set saw_hdr 0
  while {[gets $tf line] >= 0} {
    if {[string match "*Design Timing Summary*" $line]} { set saw_hdr 1 }
    if {$saw_hdr && [regexp {^[[:space:]]+([-+]?[0-9]+[.][0-9]+)[[:space:]]+([-+]?[0-9]+[.][0-9]+)[[:space:]].*[[:space:]]([-+]?[0-9]+[.][0-9]+)[[:space:]]+([-+]?[0-9]+[.][0-9]+)[[:space:]]} $line -> _wns _tns _whs _ths]} {
      set wns $_wns
      set tns $_tns
      set whs $_whs
      set ths $_ths
...
if {[file exists $cdc_rpt]} {
  ...
  puts "CDC unsafe=$unsafe_cdc FINDING_ONLY (1-bit s_done syncs; no dma_st pack)"
}

set gate_pass 1
if {$n36 > 135} { puts stderr "GATE_FAIL BRAM36=$n36"; set gate_pass 0 }
if {$unsafe_cdc != 0} { puts "FINDING unsafe_cdc=$unsafe_cdc (not bitstream skip)" }
...
puts $mf "unsafe_cdc=$unsafe_cdc finding_only=1 gate_pass=$gate_pass"
puts $mf "PROGRAM=NO C_FIX=NONE"
close $mf
...
set frozen_lm06 [file join $root results A7-NATIVE-GRAPH TINYGPT-SOC]
...
if {[string match "*TINYGPT-SOC*" $out_bit] || [string match "*lm06*.bit" $out_bit]} {
  puts stderr "REFUSE: would overwrite frozen LM-06 path $out_bit"
  exit 3
}
if {[string match "*E2R-ATOMIC-DGR-PROBE-00*" $out_bit] || [string equal $out_bit $f1x_bit]} {
  puts stderr "REFUSE: would overwrite F1x bit $out_bit"
  exit 3
}
if {[string match "*E2R-ATOMIC-SGO-PROBE-00*" $out_bit] || [string equal $out_bit $sgo_bit]} {
  puts stderr "REFUSE: would overwrite SGO bit $out_bit"
  exit 3
}
if {$gate_pass} {
  puts "=== WRITE_BITSTREAM E2R-ATOMIC-SDONE-PROBE-00-EXCL ==="
  # Omit e2r_la_pmod_ja.xdc (F1x/SGO exclusive). Waive ja-only NSTD-1/UCIO-1.
  ...
  if {[catch {write_bitstream -force $out_bit} berr]} {
    puts "WARN write_bitstream abort: $berr ; retry from e2r_post_route.dcp"
    ...
    if {[file exists $dcp]} {
      ...
      puts stderr "SKIP_BITSTREAM no DCP after abort"
      exit 5
    }
  }
  ...
  puts "SKIP_BITSTREAM gate_pass=0 (timing/BRAM; DCP archived)"
  exit 5
}
exit 0
