# Resume A7-LM-06 from post-synth DCP in a fresh Vivado process.
# Writes arty_a7_lm06.bit ONLY. Does not touch frozen 00-05 bits.
...
open_checkpoint $dcp
puts "=== IMPLEMENT A7-LM-06 FROM SYNTH (RuntimeOptimized opt) ==="
if {[catch {opt_design -directive RuntimeOptimized} err]} {
    puts "WARN: RuntimeOptimized opt failed ($err); retry propconst+sweep"
    if {[catch {opt_design -propconst -sweep} err2]} {
        puts "WARN: propconst+sweep failed ($err2); skip opt"
    }
}
