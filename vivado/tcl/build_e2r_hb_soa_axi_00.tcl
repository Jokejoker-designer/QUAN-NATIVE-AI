# E2R-HB-SOA-AXI-00  T2-SPI SoC + D1 mid-query SOA/AXI sticky heartbeats
# BOOT..CORE_START + OWNER_RDY/Q_GO/SOA_RUN/AR_BEAT/R_BEAT/R_BUSY/R_IDLE/SOA_Q/TOPK/ACCEPT/PACK/BIND/FWD/LM/PRED
set root [file normalize [file join [file dirname [info script]] ../..]]
set_param general.maxThreads 8
puts "VIVADO maxThreads=[get_param general.maxThreads]"
set outdir [file join $root results A7-NATIVE-GRAPH E2R-HB-SOA-AXI-00]
set build_dir [file join $root build e2r_hb_soa_axi_00]
file mkdir $outdir
...
create_project e2r_hb_soa_axi_00 $build_dir -part xc7a100tcsg324-1 -force
...
puts "=== IMPLEMENT E2R-HB-SOA-AXI-00 ==="
source [file join $root vivado tcl e2r_core_clk_constraints.tcl]
e2r_apply_core_clk_constraints
# opt_design crashed (EXCEPTION_ACCESS_VIOLATION) on this hierarchy  place/route direct
place_design
...
proc e2r_domain_timing {clk_name} {
  ...
  # TNS: sum failing intra paths (0 if WNS>=0)
  set tns 0
  if {$wns < 0} {
...
proc e2r_parse_unsafe_cdc {cdc_text} {
  # Gate metric = user unsafe. MIG infra False-Path rows (c166?div2/pll/mmcm_ps) are BENIGN.
  set unsafe 0
  set mig_benign 0
...
set closeout [file join $outdir CLOSEOUT.md]
set cf [open $closeout w]
if {$gate_pass} {
  puts $cf "# E2R-HB-SOA-AXI-00 CLOSEOUT PASS"
  puts $cf ""
  puts $cf "| Metric | Value | Gate |"
  puts $cf "|--------|-------|------|"
  puts $cf "| core_clk WNS | $core_wns ns | PASS |"
  puts $cf "| core_clk TNS | $core_tns ns | PASS |"
  puts $cf "| clk_pll_i WNS | $ui_wns ns | PASS |"
  puts $cf "| clk_pll_i TNS | $ui_tns ns | PASS |"
  puts $cf "| unsafe_cdc | $unsafe_cdc | PASS |"
  puts $cf "| RAMB36 | $n36 | PASS |"
  puts $cf "| gate_pass | $gate_pass | PASS |"
} else {
  puts $cf "# E2R-HB-SOA-AXI-00 CLOSEOUT FAIL"
  puts $cf ""
  puts $cf "| Metric | Value |"
  puts $cf "|--------|-------|"
  puts $cf "| core_WNS | $core_wns |"
  puts $cf "| core_TNS | $core_tns |"
  puts $cf "| ui_WNS | $ui_wns |"
  puts $cf "| ui_TNS | $ui_tns |"
  puts $cf "| unsafe_cdc | $unsafe_cdc |"
  puts $cf "| RAMB36 | $n36 |"
  puts $cf "| gate_pass | $gate_pass |"
  puts $cf ""
  puts $cf "See report_timing_summary.rpt for cross-domain paths."
}
close $cf


# --- Bitstream (only if timing/CDC/BRAM gates PASS) ---
set bitdir [file join $root build out]
file mkdir $bitdir
set out_bit [file join $bitdir arty_a7_ng_native_v1_hb_soa_axi_00.bit]
if {$gate_pass} {
  puts "=== WRITE_BITSTREAM E2R-HB-SOA-AXI-00 ==="
  write_bitstream -force $out_bit
  file copy -force $out_bit [file join $outdir arty_a7_ng_native_v1_hb_soa_axi_00.bit]
