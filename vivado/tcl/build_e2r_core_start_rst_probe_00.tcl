# E2R-CORE-START-RST-PROBE-00 — sticky CLK_ALIVE/RST_REL/START_SEEN/RST_CAUSE
# UART_SLIM=0 required (CORE_START + probes after it). PROGRAM=NO in this script.
set root [file normalize [file join [file dirname [info script]] ../..]]
set_param general.maxThreads 8
puts "VIVADO maxThreads=[get_param general.maxThreads]"
puts "VIVADO_VERSION=[version]"
set outdir [file join $root results A7-NATIVE-GRAPH E2R-CORE-START-RST-PROBE-00]
set build_dir [file join $root build e2r_core_start_rst_probe_00]
file mkdir $outdir
file mkdir $build_dir
if {[catch {exec git -C $root rev-parse HEAD} git_sha]} { set git_sha UNKNOWN }
puts "GIT_HEAD=$git_sha"
...
create_project e2r_core_start_rst_probe_00 $build_dir -part xc7a100tcsg324-1 -force
...
# Fileset matches live SoC (C9 IO-SAFE BIT-07), plus UART_SLIM=0 for this probe.
set srcs [list \
...
set_property generic {UART_SLIM=0} [get_filesets sources_1]
update_compile_order -fileset sources_1

puts "=== SYNTH E2R-CORE-START-RST-PROBE-00 UART_SLIM=0 ==="
synth_design -top arty_a7_ng_native_v1_ab_soc_top -part xc7a100tcsg324-1 -generic UART_SLIM=0 -generic PHYS=4 -flatten_hierarchy rebuilt
...
puts "=== IMPLEMENT E2R-CORE-START-RST-PROBE-00 ==="
...
set mf2 [open [file join $outdir e2r_metrics.txt] a]
puts $mf2 "git_head=$git_sha"
puts $mf2 "UART_SLIM=0"
close $mf2
if {$gate_pass} {
  puts "=== WRITE_BITSTREAM E2R-CORE-START-RST-PROBE-00 ==="
  ...
  puts $bf $bit_sha
  close $bf
  puts "BIT_OK path=$out_bit sha256=$bit_sha git=$git_sha"
} else {
