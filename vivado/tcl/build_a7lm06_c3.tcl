# A7-LM-06 C3: p_busy mem mux (C2 flush hang). Writes arty_a7_lm06c3.bit ONLY.
# Does not overwrite C1/C2 or frozen 00-05.
set script_dir [file normalize [file dirname [info script]]]
set root_dir [file normalize [file join $script_dir ../..]]
set build_dir [file join $root_dir build vivado_a7lm06_c3]
...
create_project -force a7lm06_c3 $build_dir -part $part_name
...
write_checkpoint -force [file join $out_dir a7lm06_c3_post_synth.dcp]

puts "=== IMPLEMENT A7-LM-06 C3 ==="
...
write_checkpoint -force [file join $out_dir a7lm06_c3_post_route.dcp]
report_timing_summary -delay_type min_max -max_paths 10 -file [file join $out_dir a7lm06_c3_timing_route.rpt]
report_utilization -file [file join $out_dir a7lm06_c3_utilization_route.rpt]

set wns [get_property SLACK [lindex [get_timing_paths -quiet -delay_type max -max_paths 1] 0]]
puts "POST_ROUTE_WNS=$wns"
if {$wns < 0} {
    puts stderr "ERROR: C3 WNS=$wns (need >= 0)"
    exit 4
}
...
set c2_bit_lock [file join $out_dir arty_a7_lm06c2.bit]
set c2_sha EDAAF1204072664DFA5E62CA445B65DAE680C18777AC73DDEFD075C0ED6DD483
foreach pair [list [list $c1_bit $c1_sha] [list $c1_copy $c1_sha] [list $c2_bit_lock $c2_sha]] {
    set p [lindex $pair 0]
    set exp [lindex $pair 1]
    if {![file exists $p]} {
        puts stderr "ERROR: missing lock file $p"
        exit 5
    }
    set got [sha256_file $p]
    if {$got ne $exp} {
        puts stderr "ERROR: SHA changed $p got=$got exp=$exp"
        exit 6
    }
    puts "LOCKED_OK $p"
}
...
set c3_bit [file join $out_dir arty_a7_lm06c3.bit]
write_bitstream -force $c3_bit
set c3_sha [sha256_file $c3_bit]
set still [sha256_file $c1_bit]
if {$still ne $c1_sha} {
    puts stderr "ERROR: C1 bit mutated during C3 write"
    exit 7
}
set still2 [sha256_file $c2_bit_lock]
if {$still2 ne $c2_sha} {
    puts stderr "ERROR: C2 bit mutated during C3 write"
    exit 7
}
set man [file join $root_dir results/A7-LM-06/build_manifest_c3.json]
set fh [open $man w]
puts $fh [format "{\"bit\":\"build/out/arty_a7_lm06c3.bit\",\"sha256\":\"%s\",\"wns_ns\":%s,\"tns_ns\":0.0,\"candidate\":\"c3\"}" $c3_sha $wns]
close $fh
puts "A7_LM06_C3_BUILD_PASS bitstream=$c3_bit SHA=$c3_sha WNS=$wns"
