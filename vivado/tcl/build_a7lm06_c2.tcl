# A7-LM-06 C2 persist-reload fix. Writes arty_a7_lm06c2.bit ONLY.
# Does not overwrite C1 arty_a7_lm06.bit or frozen 00-05.
set script_dir [file normalize [file dirname [info script]]]
set root_dir [file normalize [file join $script_dir ../..]]
set build_dir [file join $root_dir build vivado_a7lm06_c2]
...
create_project -force a7lm06_c2 $build_dir -part $part_name
...
write_checkpoint -force [file join $out_dir a7lm06_c2_post_synth.dcp]

puts "=== IMPLEMENT A7-LM-06 C2 ==="
...
write_checkpoint -force [file join $out_dir a7lm06_c2_post_route.dcp]
report_timing_summary -delay_type min_max -max_paths 10 -file [file join $out_dir a7lm06_c2_timing_route.rpt]
report_utilization -file [file join $out_dir a7lm06_c2_utilization_route.rpt]

set wns [get_property SLACK [lindex [get_timing_paths -quiet -delay_type max -max_paths 1] 0]]
puts "POST_ROUTE_WNS=$wns"
if {$wns < 0} {
    puts stderr "ERROR: C2 WNS=$wns (need >= 0)"
    exit 4
}
...
set c2_bit [file join $out_dir arty_a7_lm06c2.bit]
write_bitstream -force $c2_bit
set c2_sha [sha256_file $c2_bit]
set still [sha256_file $c1_bit]
if {$still ne $c1_sha} {
    puts stderr "ERROR: C1 bit mutated during C2 write"
    exit 7
}
set man [file join $root_dir results/A7-LM-06/build_manifest_c2.json]
set fh [open $man w]
puts $fh [format "{\"bit\":\"build/out/arty_a7_lm06c2.bit\",\"sha256\":\"%s\",\"wns_ns\":%s,\"tns_ns\":0.0,\"candidate\":\"c2\"}" $c2_sha $wns]
close $fh
puts "A7_LM06_C2_BUILD_PASS bitstream=$c2_bit SHA=$c2_sha WNS=$wns"
