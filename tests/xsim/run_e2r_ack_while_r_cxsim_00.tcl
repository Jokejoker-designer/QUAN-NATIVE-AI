...
set outdir [file join $root results A7-NATIVE-GRAPH E2R-ACK-WHILE-R-CXSIM-00]
file mkdir $outdir
cd $outdir
...
set src [list \
  ...
  [file join $xsimdir tb_e2r_ack_while_r_cxsim_00.sv]]
...
if {[catch {exec xelab tb_e2r_ack_while_r_cxsim_00 glbl -s tb_e2r_ack_while_r_cxsim_00_sim -L xpm -timescale 1ns/1ps} elab_out]} {
  puts $elab_out
  puts "XELAB_FAIL"
  exit 3
}
...
if {[catch {exec xsim tb_e2r_ack_while_r_cxsim_00_sim -runall} sim_out]} {
...
if {[string match "*E2R_ACK_WHILE_R_CXSIM_00_XSIM_PASS*" $sim_out]} {
  puts "E2R_ACK_WHILE_R_CXSIM_00_OK"
  exit 0
}
if {[string match "*E2R_ACK_WHILE_R_CXSIM_00_FAIL_NO_DESTWAIT*" $sim_out]} {
  puts "E2R_ACK_WHILE_R_CXSIM_00_NO_DESTWAIT"
  exit 6
}
if {[string match "*E2R_ACK_WHILE_R_CXSIM_00_FAIL_NO_ACK*" $sim_out]} {
  puts "E2R_ACK_WHILE_R_CXSIM_00_NO_ACK"
  exit 7
}
puts "E2R_ACK_WHILE_R_CXSIM_00_NO_PASS"
exit 5
