# run_a7ng_ddr_wavefront.tcl — ddr_wavefront_00: bounded cue working set → 16-wide wave on MIG
# Official Digilent mig.prj unmodified. Archive under DDR-WAVEFRONT-00.
...
set prj [file join $outdir ddr_wavefront_xsim.prj]
set fh [open $prj w]
...
puts $fh "sv work \"[file join $xsimdir tb_a7ng_ddr_wavefront.sv]\""
...
set xvlog_log [file join $outdir xvlog_ddr_wavefront.log]
...
set xelab_log [file join $outdir xelab_ddr_wavefront.log]
# Vivado 2026.1 on this project: default xelab ACCESS_VIOLATION; -mt off -O0 required.
if {[catch {exec $xelab_bin -mt off -O0 tb_a7ng_ddr_wavefront glbl -s tb_a7ng_ddr_wavefront_sim \
...
set log [file join $outdir xsim_ddr_wavefront.log]
if {[catch {exec $xsim_bin tb_a7ng_ddr_wavefront_sim -runall > $log 2>@1} sim_rc]} {
  puts "xsim_rc=$sim_rc"
}
set fp [open $log r]
set sim_out [read $fp]
...
if {[string match "*A7NG_DDR_WAVEFRONT_XSIM_PASS*" $sim_out]} {
  puts A7NG_DDR_WAVEFRONT_XSIM_OK
  exit 0
}
puts A7NG_DDR_WAVEFRONT_NO_PASS
exit 5
