// E2R-SGO-CXSIM-MUX-00 — SGO sticky on MUX leftover occupancy
// Vehicle: copy of E2R-RPATH-IDLE-CXSIM-MUX-00 (SIM_FULL=0 + CDC + B1 + mux
// + shared stub + s_dma_idle=1'b0 + hold busy after 8 R).
// ONE TB CHANGE: wire dbg_s_go_sticky / dbg_m_go_sticky / cmd probes (MUX left open).
...
module tb_e2r_sgo_cxsim_mux_00;
  import a7ng_pkg::*;
  ...
  task automatic dump_row(input string phase_s);
    $display("PROBE t=%0t phase=%s dst=%0d drain=%0b fifo=%0d c_rvalid=%0b tr=%0d idle=%0b own=%0b grant=%0b own_ui=%0b cdc_rv=%0b stub_rv=%0b m_go=%0b s_go=%0b sgo_st=%0b mgo_st=%0b sgo_ever=%0b cmd_st=%0d sbusy_pend=%0b cmd_rd=%0b cmd_empty_mgo=%0b busy=%0b rleft=%0d wst=%0d miss=%0b reqs1=%0b bst=%0d soa_done=%0b fwd=%0b core_busy=%0b phase=%0d gv=%0d",
      ...
      phase, gv_count);
    $fdisplay(pf, "%0t,%s,%0d,%0b,%0d,%0b,%0d,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0b,%0d,%0b,%0b,%0b,%0b,%0b,%0d",
      $time, phase_s, tile_dst, p_drain, p_fifo, p_rvalid, p_tr, r_path_idle,
      wdma_owner, wdma_owner_grant, wdma_owner_ui,
      wdma_go, dma_go, dbg_s_go_sticky, dbg_m_go_sticky, s_go_ever_s,
      dbg_cmd_st, dbg_sbusy_pend, dbg_cmd_rd_sticky, dbg_cmd_empty_mgo,
      wdma_busy, done, phase);
  endtask
  ...
  initial begin
    ...
    if (pf == 0) begin
      $display("E2R_SGO_CXSIM_MUX_00_XSIM_FAIL reason=probe_fopen");
      $fatal(1);
    end
    $fdisplay(pf, "t_ns,phase,tile_dst,r_drain_hold,fifo_cnt,m_axi_rvalid,tr_cnt,r_path_idle,wdma_owner,wdma_owner_grant,wdma_owner_ui,m_go,s_go,sgo_sticky,mgo_sticky,sgo_ever,cmd_st,sbusy_pend,cmd_rd_sticky,cmd_empty_mgo,wdma_busy,soa_done,lm_phase");
    ...
    $display("E2R-SGO-CXSIM-MUX-00 START");
    $display("VEHICLE=a7ng_native_v1_ab_core SIM_FULL=0 do_lm=1 CDC+B1+MUX ungated_cdc_rvalid SHARED_STUB_WDMA s_dma_idle=0");
    $display("PREREG_UNKNOWN: on first dest=4 grant=1 leftover SET, does dbg_s_go_sticky rise");
    $display("PREREG_H_CANDIDATE: sticky=0 whole query SGO_NEVER — silicon SGO=0 can be a true miss");
    $display("PREREG_H_RIVAL: sticky=1 at or before first dest=4 SGO_ROSE — UART SGO=0 is print-time");
    $display("PREREG_FALSIFIER: force dest; C-FIX; A2; LiteScope; soc_top+MIG; retie s_dma_idle");
    $display("PREREG_UNIT: one query; first dest=4 snapshot; sticky may have risen earlier");
    ...
    if (!destwait_seen)
      ...
      verdict_class = "SGO_ROSE";
    else if (!end_sgo_st)
      verdict_class = "SGO_NEVER";
    else
      verdict_class = "SGO_LATE";
    ...
    if (!destwait_seen) begin
      ...
      $display("E2R_SGO_CXSIM_MUX_00_FAIL_NO_DESTWAIT dst=%0d bst=%0d miss=%0b reqs1=%0b m_go=%0b s_go=%0b sgo_st=%0b busy=%0b soa=%0b fwd=%0b grant=%0b",
        tile_dst, tile_bst, tile_miss, tile_req_s1, wdma_go, dma_go, dbg_s_go_sticky,
        wdma_busy, soa_done_seen, start_fwd_seen, wdma_owner_grant);
      $finish;
    end

    if ((verdict_class == "SGO_ROSE") || (verdict_class == "SGO_NEVER")) begin
      $display("XSIM=PASS");
      $display("E2R_SGO_CXSIM_MUX_00_XSIM_PASS class=%s c_fix=NONE dst=%0d grant=%0b idle=%0b fifo=%0d c_rvalid=%0b sgo_st_dest4=%0b sgo_st_end=%0b mgo_st=%0b sgo_ever=%0b",
        verdict_class, snap_dst, snap_grant, snap_idle, snap_fifo, snap_rvalid,
        snap_sgo_st, end_sgo_st, snap_mgo_st, snap_sgo_ever | end_sgo_ever);
    end else begin
      $display("XSIM=INDETERMINATE");
      $display("E2R_SGO_CXSIM_MUX_00_NO_PASS class=%s sgo_st_dest4=%0b sgo_st_end=%0b",
        verdict_class, snap_sgo_st, end_sgo_st);
    end
    $finish;
  end
  ...
  initial begin
    ...
    $display("WALL_LIMIT");
    $display("XSIM=LIMIT");
    $display("E2R_SGO_CXSIM_MUX_00_FAIL_NO_DESTWAIT reason=wall dst=%0d sgo_st=%0b",
      tile_dst, dbg_s_go_sticky);
    $finish;
  end
endmodule
