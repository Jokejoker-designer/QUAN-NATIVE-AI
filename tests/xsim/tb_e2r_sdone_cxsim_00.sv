// E2R-SDONE-CXSIM-00 — completable responder s_done on leftover dest=4
// Vehicle: copy of E2R-SGO-CXSIM-MUX-00 (SIM_FULL=0 + CDC + B1 + mux
// + shared stub + s_dma_idle=1'b0). LATCH CONTROL is the same hold-busy
// responder plus a UART latch replica (unused here).
// ONE TB CHANGE: after 8 shared-stub R beats, pulse s_done and clear s_busy.
// Do NOT hold busy=1 done=0 (that is the FALSIFIER / old MUX vehicle).
// UNKNOWN: dest=4 grant=1 leftover SET s_go fired, completable responder —
// does s_done / dbg_s_done_sticky rise?
...
module tb_e2r_sdone_cxsim_00;
  ...
  // pulse s_done and clear s_busy (completable responder). Do not hold.
  ...
  initial begin
    ...
    if (pf == 0) begin
      $display("E2R_SDONE_CXSIM_00_XSIM_FAIL reason=probe_fopen");
      $fatal(1);
    end
    ...
    $display("E2R-SDONE-CXSIM-00 START");
    $display("VEHICLE=a7ng_native_v1_ab_core SIM_FULL=0 do_lm=1 CDC+B1+MUX ungated_cdc_rvalid SHARED_STUB_WDMA COMPLETABLE_RESPONDER s_dma_idle=0");
    $display("PREREG_UNKNOWN: dest=4 grant=1 leftover SET s_go fired, completable responder — does s_done rise");
    $display("PREREG_H_CANDIDATE: leftover SET keeps DMA in R; done never SDONE_NEVER");
    $display("PREREG_H_RIVAL: done rises after complete beats SDONE_ROSE — silicon SDONE=0 is stubneqboard / still-in-R");
    $display("PREREG_FALSIFIER: hold busy=1 done=0 after 8 R; C-FIX; A2; LiteScope; soc_top+MIG; force dest");
    $display("PREREG_UNIT: one query; first dest=4 snapshot + whether done ever in that query");
    $display("FORBIDDEN: hold busy after 8 R; force TILE_DST/dst; assign r_path_idle=1; C-FIX; soc_top; MIG; board; retie s_dma_idle");
    ...
    else if (snap_sdone_st || end_sdone_st || snap_sdone_ever || end_sdone_ever)
      verdict_class = "SDONE_ROSE";
    else if (snap_sgo_ever || end_sgo_ever)
      verdict_class = "SDONE_NEVER";
    else
      verdict_class = "SDONE_NO_SGO";
    ...
    if (!destwait_seen) begin
      ...
      $display("E2R_SDONE_CXSIM_00_FAIL_NO_DESTWAIT dst=%0d bst=%0d miss=%0b reqs1=%0b m_go=%0b s_go=%0b sgo_st=%0b sdone_st=%0b sdone_ever=%0b busy=%0b soa=%0b fwd=%0b grant=%0b",
    ...
    if ((verdict_class == "SDONE_ROSE") || (verdict_class == "SDONE_NEVER")) begin
      $display("XSIM=PASS");
      $display("E2R_SDONE_CXSIM_00_XSIM_PASS class=%s c_fix=NONE dst=%0d grant=%0b idle=%0b fifo=%0d c_rvalid=%0b sgo_ever=%0b sdone_ever=%0b sdone_st_dest4=%0b sdone_st_end=%0b s_busy_dest4=%0b s_busy_end=%0b",
        verdict_class, snap_dst, snap_grant, snap_idle, snap_fifo, snap_rvalid,
        snap_sgo_ever | end_sgo_ever, snap_sdone_ever | end_sdone_ever,
        snap_sdone_st, end_sdone_st, snap_sbusy_dma, end_sbusy_dma);
    end else begin
      $display("XSIM=INDETERMINATE");
      $display("E2R_SDONE_CXSIM_00_NO_PASS class=%s sdone_st_dest4=%0b sdone_st_end=%0b sdone_ever=%0b sgo_ever=%0b",
        verdict_class, snap_sdone_st, end_sdone_st, snap_sdone_ever | end_sdone_ever,
  ...
  initial begin
    ...
    $display("WALL_LIMIT");
    $display("XSIM=LIMIT");
    $display("E2R_SDONE_CXSIM_00_FAIL_NO_DESTWAIT reason=wall dst=%0d sgo_st=%0b sdone_st=%0b",
