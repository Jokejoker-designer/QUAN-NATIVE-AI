// GO-H4-SIMFULL0-00 — A-FAST vehicle with SIM_FULL=0 + hex DMA stub.
// Stub ≠ MIG ≠ board. PROGRAM=NO. Observe pred (664 vs 733 vs 371).
`timescale 1ns/1ps

module tb_go_h4_simfull0_00;
  import a7ng_pkg::*;
  import a7lm06_pkg::*;
  ...
  logic wdma_owner, wdma_go, wdma_wr, wdma_w_valid, wdma_r_ready;
  logic [27:0] wdma_addr;
  logic [31:0] wdma_bytes;
  logic [127:0] wdma_w_data;
  logic wdma_busy, wdma_done, wdma_w_ready, wdma_r_valid;
  logic [127:0] wdma_r_data;

  localparam int WMEM_N = 802816;
  reg signed [7:0] wmem [0:WMEM_N-1];
  initial $readmemh("a7lm06_wmem.hex", wmem);

  typedef enum logic [1:0] { ST_IDLE, ST_WAIT_DRAIN, ST_BEATS, ST_PULSE } stub_e;
  stub_e stub;
  int unsigned r_beats;
  logic [27:0] stub_base;
  int unsigned go_count, done_count, go_while_busy;

  function automatic [127:0] beat_hex(input logic [27:0] base, input int beat);
    logic [127:0] d;
    int b;
    int wa;
    begin
      ...
      for (b = 0; b < 16; b = b + 1) begin
        wa = int'(base) - int'(DDR_WBASE) + beat * 16 + b;
        if (wa >= 0 && wa < WMEM_N)
          d[8*b +: 8] = wmem[wa];
      end
      return d;
  ...
  a7ng_native_v1_ab_core #(.SIM_FULL(1'b0), .WAVE(16), .MAX_CANDS(TOTAL)) dut (
    ...
    .r_path_idle_o(),
    .clk_dma(clk), .rst_dma_n(dut_rst_n),
  ...
  always_ff @(posedge clk) begin
    if (!dut_rst_n) begin
      go_count <= 0; done_count <= 0; go_while_busy <= 0;
    end else begin
      if (wdma_go) begin
        go_count <= go_count + 1;
        if (wdma_busy) go_while_busy <= go_while_busy + 1;
      end
      if (wdma_done) done_count <= done_count + 1;
    end
  end

  always_ff @(posedge clk) begin
    if (!dut_rst_n) begin
      wdma_busy <= 1'b0; wdma_done <= 1'b0; wdma_r_valid <= 1'b0;
      wdma_r_data <= 128'd0; r_beats <= 0; stub <= ST_IDLE; stub_base <= 28'd0;
    end else begin
      wdma_done <= 1'b0;
      unique case (stub)
        ST_IDLE: begin
          wdma_r_valid <= 1'b0;
          if (wdma_go) begin
            wdma_busy <= 1'b1; r_beats <= 0; stub_base <= wdma_addr; stub <= ST_WAIT_DRAIN;
          end
        end
        ST_WAIT_DRAIN: begin
          if (wdma_r_ready) begin
            wdma_r_valid <= 1'b1; wdma_r_data <= beat_hex(stub_base, 0); r_beats <= 0; stub <= ST_BEATS;
          end
        end
        ST_BEATS: begin
          if (r_beats == 7) begin
            wdma_r_valid <= 1'b0; stub <= ST_PULSE;
          end else begin
            r_beats <= r_beats + 1; wdma_r_data <= beat_hex(stub_base, r_beats + 1);
          end
        end
        ST_PULSE: begin
          wdma_done <= 1'b1; wdma_busy <= 1'b0; stub <= ST_IDLE;
        end
  ...
  initial begin
    ...
    $display("H4_WMEM_HEX_VIA_DMA_STUB n=%0d (no SIM_FULL=1 backdoor)", WMEM_N);
    ...
    $display("H4_PROBE ntok=%0d st_beats=%0d busy=%0b grant_lm=%0b",
             dut.u_core.ntok, st_beats, core_busy, dut.grant_lm_o);
    ...
    begin
      ...
      while (bind_done !== 1'b1 && lt < 200_000_000) begin
        ...
        lt = lt + 1;
        if (lt % 200000 == 0)
          $display("LM_HB cyc=%0d phase=%0d busy=%0b done=%0b pred=%0d bind_done=%0b st_beats=%0d stall=%0b go=%0d",
                   lt, phase, core_busy, core_done, pred, bind_done, st_beats,
                   dut.u_core.w_stall, go_count);
      end
    ...
    $display("H4_GO_COUNT=%0d DONE_COUNT=%0d GO_WHILE_BUSY=%0d", go_count, done_count, go_while_busy);
    $display("H4_PRED=%0d pack=%h dual=%0d st=%0d", pred, ctx_pack, dual_ticks, st_beats);
    if (pred === 10'd664)
      $display("H4_CLASS=MATCH_664");
    else if (pred === 10'd733)
      $display("H4_CLASS=MATCH_733_SILICON");
    else if (pred === 10'd371)
      $display("H4_CLASS=MATCH_371_THREE_GO");
    else
      $display("H4_CLASS=OTHER");
    $display("GO_H4_SIMFULL0_00_DONE PROGRAM=NO");
    $finish;
  end

  initial begin
    #30_000ms;
    $display("GO_H4_SIMFULL0_00_FAIL WALL_TIMEOUT pred=%0d", pred);
    $finish;
  end
endmodule
