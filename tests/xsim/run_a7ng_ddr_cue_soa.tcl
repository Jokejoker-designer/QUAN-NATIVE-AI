# run_a7ng_ddr_cue_soa.tcl — ddr_cue_soa_00 SOA stage-1 fetch on MIG
set root [file normalize [file join [file dirname [info script]] ../..]]
set xsimdir [file join $root tests xsim]
set outdir  [file join $root results A7-NATIVE-GRAPH DDR-CUE-SOA-00]
...
set prj [file join $outdir ddr_cue_soa_xsim.prj]
set fh [open $prj w]
puts $fh "# ddr_cue_soa_00 — Digilent AXI MIG sim"
...
puts $fh "sv work \"[file join $xsimdir tb_a7ng_ddr_cue_soa.sv]\""
...
set xvlog_log [file join $outdir xvlog_ddr_cue_soa.log]
...
set xelab_log [file join $outdir xelab_ddr_cue_soa.log]
if {[catch {exec $xelab_bin -mt off -O0 tb_a7ng_ddr_cue_soa glbl -s tb_a7ng_ddr_cue_soa_v11 \
...
set xsim_log [file join $outdir xsim_ddr_cue_soa.log]
if {[catch {exec $xsim_bin tb_a7ng_ddr_cue_soa_v11 -R -log $xsim_log} xsim_rc]} {
  puts "xsim_rc=$xsim_rc"
}
puts [read [open $xsim_log r]]
if {[string match *A7NG_DDR_CUE_SOA_XSIM_PASS* [read [open $xsim_log r]]]} {
  puts XSIM_PASS
  exit 0
}
puts XSIM_FAIL
