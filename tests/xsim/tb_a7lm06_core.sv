...
module tb_a7lm06_core;
    ...
    initial begin
        ...
        if (pred !== exp_pred[9:0] || last_loss !== exp_loss[15:0]) begin
        ...
        fork
            begin wait (done); end
            begin repeat (16_000_000) @(posedge clk); $display("FAIL fold0 timeout"); $finish; end
        join_any
        disable fork;
        @(posedge clk);
        ...
        @(posedge clk);
        start_train <= 1'b1; tgt_in <= 10'd32; lr_in <= 4'd3;
        @(posedge clk); start_train <= 1'b0;
        fork
            begin wait (done); end
            begin repeat (400_000_000) @(posedge clk); $display("FAIL train timeout phase=%0d", phase); $finish; end
        join_any
        disable fork;
        ...
        fork
            begin wait (done); end
            begin repeat (16_000_000) @(posedge clk); $display("FAIL fold1 timeout"); $finish; end
        join_any
        disable fork;
        @(posedge clk);
        $display("FOLD1 xor=%0d exp=%0d add=%0d exp=%0d wr=%0d", xor32, exp_xor1, add32, exp_add1, wr_n);
        $writememh("a7lm06_after.hex", u_core.u_w.FULL.u_full.mem);
        if (xor32 !== exp_xor1 || add32 !== exp_add1) begin
            $display("FAIL fold1");
            $finish;
        end
        $display("A7LM06_XSIM_PASS pred=%0d loss=%0d wr_n=%0d", pred, last_loss, wr_n);
        $finish;
    end
endmodule
