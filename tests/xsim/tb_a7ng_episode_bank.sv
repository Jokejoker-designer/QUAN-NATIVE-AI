...
module tb_a7ng_episode_bank;
  ...
  // reuse prior-capable mem model — extend idx for episode base
  ...
  initial begin
    ...
    @(negedge clk); wr=1; wid=4'd2; wdata=128'hDEAD_BEEF_0000_0002;
    @(posedge clk); #1; wr=0;
    @(negedge clk); flush=1; @(posedge clk); #1; flush=0; wait(done); @(posedge clk);
    @(negedge clk); forget=1; @(posedge clk); #1; forget=0;
    @(negedge clk); rd=1; rid=4'd2; @(posedge clk); #1; rd=0;
    if (rdata !== 128'd0) begin $display("FAIL forget"); fails=fails+1; end
    @(negedge clk); reload=1; @(posedge clk); #1; reload=0; wait(done); @(posedge clk);
    @(negedge clk); rd=1; rid=4'd2; @(posedge clk); #1; rd=0;
    if (rdata !== 128'hDEAD_BEEF_0000_0002) begin $display("FAIL reload %h", rdata); fails=fails+1; end
    if (fails==0) $display("A7NG_MEM01_EPBANK_XSIM_PASS");
    else $display("A7NG_MEM01_EPBANK_XSIM_FAIL fails=%0d", fails);
    $finish;
  end
endmodule
